...
namespace WBX {
    ...
    void GuiLayerCollector::onDescendantAdded(Instance* instance)
    {
		...
		rebuildGuiVector = true;
		if (GuiBase* gb = wbx::Instance::fastDynamicCast<GuiBase>(instance))
		{
	...
	void GuiLayerCollector::LoadZ(const shared_ptr<wbx::Instance>& instance, GuiLayers guiVectors[])
	{
		if (GuiBase* gb = instance->fastDynamicCast<GuiBase>())
		{
			if (gb->canProcessMeAndDescendants())
			{
				...
				WBXASSERT(queue < GUIQUEUE_COUNT);
				WBXASSERT(z <= GuiBase::maxZIndex());
	...
	void GuiLayerCollector::loadZVectors()
	{
		...
		WBXASSERT(isAllGuiQueuesHaveCorrectNumberOfLayers(mGuiVectors));
    ...
    GuiResponse GuiLayerCollector::doProcessGesture(const boost::shared_ptr<GuiBase>& guiBase, const UserInputService::Gesture& gesture, const shared_ptr<const wbx::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args)
    {
        if (guiBase)
    ...
    GuiResponse GuiLayerCollector::processGesture(const UserInputService::Gesture& gesture, const shared_ptr<const wbx::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args)
    {
