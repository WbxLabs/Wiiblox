...
namespace WBX {
	...
	SolidModelContentProvider::SolidModelContentProvider()
		:DescribedNonCreatable<SolidModelContentProvider, CacheableContentProvider, sSolidModelContentProvider, wbx::Reflection::ClassDescriptor::RUNTIME_LOCAL>(CACHE_ENFORCE_MEMORY_SIZE, 1024 * 1024 * 32)
	{
	...
	TaskScheduler::StepResult SolidModelContentProvider::ProcessTask(const std::string& id, shared_ptr<const std::string> data)
	{
		if(data){
            ...
            if (instances.size() > 0)
            {
                if (shared_ptr<wbx::PartOperationAsset> partOperationAsset = wbx::Instance::fastSharedDynamicCast<wbx::PartOperationAsset>(instances.front()))
                {
            ...
            {
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "SolidModelContentProvider failed to process %s because 'could not fetch'", id.c_str());
	...
	void SolidModelContentProvider::updateContent(const std::string& id, boost::shared_ptr<CacheableContentProvider::CachedItem> solidModel)
	{
		if (solidModel->data)
		{
			boost::shared_ptr<wbx::PartOperationAsset> partOperationAsset = boost::static_pointer_cast<wbx::PartOperationAsset>(solidModel->data);
