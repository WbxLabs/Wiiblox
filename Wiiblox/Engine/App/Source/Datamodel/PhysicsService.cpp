...
PhysicsService::~PhysicsService()
{
	...
	WBXASSERT(!assemblyPhysicsOnConnection.connected());
	WBXASSERT(!assemblyPhysicsOffConnection.connected());
}
...
void PhysicsService::onAssemblyPhysicsOn(Primitive* primitive)
{
	...
	WBXASSERT(primitive->getWorld());		// confirm the Part is in Workspace / primitive is in World
	PartInstance* part = PartInstance::fromPrimitive(primitive);

	WBXASSERT(Assembly::isAssemblyRootPrimitive(part->getPartPrimitive()));
	...
	WBXASSERT(!part->PhysicsServiceHook::is_linked());
	parts.insert(*part);
