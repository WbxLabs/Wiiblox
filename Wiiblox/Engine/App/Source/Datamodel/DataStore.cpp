...
namespace WBX {
	WBX_REGISTER_CLASS(DataStore);
	WBX_REGISTER_CLASS(OrderedDataStore);
	WBX_REGISTER_CLASS(DataStorePages);
	...
	static Reflection::BoundFuncDesc<DataStore, wbx::signals::connection(std::string, Lua::WeakFunctionRef)> func_onUpdate(&DataStore::onUpdate, "OnUpdate", "key", "callback", Security::None);
	...
	void DataStore::updateAsync(std::string key, Lua::WeakFunctionRef transformFunc, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		shared_ptr<Lua::WeakFunctionRef> transform = wbx::make_shared<Lua::WeakFunctionRef>(transformFunc);
	...
	void DataStore::runTransformFunction(std::string key, shared_ptr<Lua::WeakFunctionRef> transform, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		std::stringstream postData;
		postData << "value=";
		postData << wbx::Http::urlEncode(newValue);
		postData << "&expectedValue=";
		postData << wbx::Http::urlEncode(expectedValue);
	...
	bool DataStore::updateCachedKey(const std::string& key, const Reflection::Variant& rawValue)
	{
		...
		else
		{
			...
			WBXASSERT(hasNonJsonType == false);
		}
	...
	std::string DataStore::urlEncodeIfNeeded(const std::string& input)
	{
		...
		{
			return wbx::Http::urlEncode(input);
		}
		return input;
	}
	...
	wbx::signals::connection DataStore::onUpdate(std::string key, Lua::WeakFunctionRef callback)
	{
		if (!checkAccess(key, NULL))
			return wbx::signals::connection();

		FASTLOGS(FLog::DataStore, "Subscribed to key %s", key);
		shared_ptr<wbx::signal<void(Reflection::Variant) > >& signal = onUpdateKeys[key];
		if (!signal)
			signal = wbx::make_shared<wbx::signal<void(Reflection::Variant)> >();

		shared_ptr<EventSlot> slot = wbx::make_shared<EventSlot>(callback);
		wbx::signals::connection conn = signal->connect(
	...
	void DataStore::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider) 
	{
		...
		if (newProvider)
		{
            ...
            if (cp)
            {
                ...
                {
                    //https://api.gametest1.Wiibloxlabs.com//persistence/set?placeId=124921244&key=BF2%5Fds%5Ftest&&type=standard&scope=global&target=BF2%5Fds%5Fkey%5Ftmp&valueLength=31
	...
	void DataStore::lockAcquiredProcessFetchCachedKeys(shared_ptr<std::string> response, shared_ptr<std::exception> exception)
	{
		...
		shared_ptr<const Reflection::ValueArray> keyValueArray = 
			itData->second.cast<shared_ptr<const wbx::Reflection::ValueArray> >();
		...
		for (Reflection::ValueArray::const_iterator it = keyValueArray->begin(); it != keyValueArray->end(); ++it)
		{
			...
			shared_ptr<const Reflection::ValueTable> keyValueEntry = 
				it->cast<shared_ptr<const wbx::Reflection::ValueTable> >();
	...
	void DataStore::sendBatchGet(std::stringstream& keysList)
	{
		...
		http.post(finalKeyList, wbx::Http::kContentTypeUrlEncoded, false, boost::bind(&DataStore::processFetchCachedKeys, shared_from(this), _1, _2));
	}
...
// 			"ExclusiveStartKey": "player_1692004196$4" }}
		...
		shared_ptr<const Reflection::ValueTable> data = 
			itData->second.cast<shared_ptr<const wbx::Reflection::ValueTable> >();
		...
		shared_ptr<Reflection::ValueArray> page = wbx::make_shared<Reflection::ValueArray>();
		shared_ptr<const Reflection::ValueArray> entries = itEntries->second.cast<shared_ptr<const wbx::Reflection::ValueArray> >();
		...
		for(Reflection::ValueArray::const_iterator it = entries->begin(); it != entries->end(); ++it)
		{
			...
			shared_ptr<Reflection::ValueTable> pageEntry = wbx::make_shared<Reflection::ValueTable>();
