...
const char *const wbx::sJointInstance = "JointInstance";
const char *const wbx::sSnap = "Snap";
const char *const wbx::sWeld = "Weld";
const char *const wbx::sManualSurfaceJointInstance = "ManualSurfaceJointInstance";
const char *const wbx::sManualWeld = "ManualWeld";
const char *const wbx::sManualGlue = "ManualGlue";
const char *const wbx::sGlue = "Glue";
const char *const wbx::sRotate = "Rotate";
const char *const wbx::sDynamicRotate = "DynamicRotate";
const char *const wbx::sRotateP = "RotateP";
const char *const wbx::sRotateV = "RotateV";
const char *const wbx::sMotor = "Motor";
const char *const wbx::sMotor6D = "Motor6D";
...
JointInstance::~JointInstance()
{
	...
	WBXASSERT(getParent() == NULL);
	WBXASSERT(joint->findWorld() == NULL);
...
XmlElement* JointInstance::writeXml(const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole)
{
...
bool JointInstance::inEngine()
{
	...
	WBXASSERT(answer == (computeWorld() != NULL));
	return answer;
}
...
void JointInstance::handleWorldChanged()
{
	...
	if (oldWorld != newWorld) {
		...
		if (oldWorld) {
			...
			WBXASSERT_SLOW(!joint->findWorld());
		}
		if (newWorld) {
	...
	:DescribedCreatable<Snap, JointInstance, sSnap>(joint)
{
	...
	WBXASSERT(joint->getJointType() == Joint::SNAP_JOINT);
	//WBXASSERT(joint->getJointCoord(0) == CoordinateFrame());		
	WBXASSERT(Math::isAxisAligned(joint->getJointCoord(0).rotation));		
}
	...
	:DescribedCreatable<Weld, JointInstance, sWeld>(joint)
{
	...
	WBXASSERT(joint->getJointType() == Joint::WELD_JOINT);
//	WBXASSERT(joint->getJointCoord(0) == CoordinateFrame());		
	WBXASSERT(Math::isAxisAligned(joint->getJointCoord(0).rotation));
}
...
{
	return WBX_static_cast<const WeldJoint*>(joint);
}
	...
	:DescribedCreatable<ManualWeld, ManualSurfaceJointInstance, sManualWeld>(joint)
{
	...
	WBXASSERT(joint->getJointType() == Joint::MANUAL_WELD_JOINT);
//	WBXASSERT(joint->getJointCoord(0) == CoordinateFrame());		
	WBXASSERT(Math::isAxisAligned(joint->getJointCoord(0).rotation));
}
...
{
	return WBX_static_cast<const ManualWeldJoint*>(joint);
}
	...
	:DescribedCreatable<ManualGlue, ManualSurfaceJointInstance, sManualGlue>(joint)
{
	...
	WBXASSERT(joint->getJointType() == Joint::MANUAL_GLUE_JOINT);
}
...
{
	return WBX_static_cast<const ManualGlueJoint*>(joint);
}
...
{
	WBXASSERT(joint->getJointType() == Joint::GLUE_JOINT);
}
...
{
	return WBX_static_cast<GlueJoint*>(joint);
}

const GlueJoint* Glue::glueJointConst() const
{
	return WBX_static_cast<const GlueJoint*>(joint);
}
...
{
	WBXASSERT(joint->getJointType() == Joint::ROTATE_JOINT);
}
...
{
	const DynamicRotateJoint* dynamicRotateJoint = WBX_static_cast<const DynamicRotateJoint*>(joint);
...
{
	DynamicRotateJoint* dynamicRotateJoint = WBX_static_cast<DynamicRotateJoint*>(joint);
...
{
	WBXASSERT(joint->getJointType() == Joint::ROTATE_P_JOINT);
}
...
{
	WBXASSERT(joint->getJointType() == Joint::ROTATE_V_JOINT);
}
...
{
	WBXASSERT(joint->getJointType() == Joint::MOTOR_1D_JOINT);
	WBXASSERT(0);	// creating a motor by AutoJoin
	this->setName("Motor");
}
...
{
	return WBX_static_cast<MotorJoint*>(joint);
}
...
{
	WBXASSERT(joint->getJointType() == Joint::MOTOR_6D_JOINT);
	WBXASSERT(0);	// creating a motor by AutoJoin
...
{
	return WBX_static_cast<Motor6DJoint*>(joint);
}
