...
Tool::~Tool()
{
	WBXASSERT(!handleTouched.connected());
	WBXASSERT(!characterChildAdded.connected());
	WBXASSERT(!characterChildRemoved.connected());
	WBXASSERT(!torsoChildAdded.connected());
	WBXASSERT(!torsoChildRemoved.connected());
}
...
void Tool::setBackendToolState(int value)
{
	if (value != backendToolState)
	{
		...
		if ((backendToolState >= EQUIPPED) && (value < EQUIPPED))
		{
			...
			if (currentMouse)
			{
				currentMouse->setIcon(wbx::TextureId());
			}
			onUnequipped();
		}
...
//		WBXASSERT(0);					// only call if >= the HAS_HANDLE state
...
{
#ifdef WBXASSERTENABLED
	int numToolsInCharacter;
#endif
	WBXASSERT((numToolsInCharacter = getNumToolsInCharacter()) || 1);
	...
	else
	{
		...
		else if (desiredState < currentState)
		{
			do 
			{
				...
				case NOTHING:				WBXASSERT(0);	break;
				}
	...
	WBXASSERT(numToolsInCharacter == getNumToolsInCharacter());
}
...
{
	WBXASSERT(ServiceProvider::findServiceProvider(this));
}
...
{
	WBXASSERT(Humanoid::modelIsCharacter(getParent()));
...
void Tool::onEvent_RemovedBackend(shared_ptr<Instance> child)
{
	...
	if (ServiceProvider::findServiceProvider(this))
	{
		if (child.get() != this)	// tool parent changed is handled in ancestor stuff, because this might not be hooked up
		{
			...
			{
				WBXASSERT(backendToolState >= EQUIPPED);
...
void Tool::onAncestorChanged(const AncestorChanged& event)
{
	...
	if (newProvider)
	{
		...
		WBXASSERT(ServiceProvider::findServiceProvider(this) == newProvider);
...
void Tool::setMousePositionForInputType()
{
    if (wbx::UserInputService* inputService = wbx::ServiceProvider::find<wbx::UserInputService>(this))
    {
        ...
        else if (inputService->getTouchEnabled())
        {
            ...
            wbx::StandardOut::singleton()->printf(MESSAGE_INFO, "current num of touches is %lu",currentTouches.size());
            if (currentTouches.size() > 0)
            {
                ...
                shared_ptr<InputObject> fakeMouseActivatePosition = wbx::Creatable<wbx::Instance>::create<wbx::InputObject>(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CHANGE,
                                                                        Vector3(avgTouchPosition.x,avgTouchPosition.y,0), Vector3::zero(), wbx::DataModel::get(this));
...
void Tool::luaActivate()
{
	...
	{
		wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "Tool:Activate() called from script when tool is not equipped. Tool will not be activated.");
	}
}
...
void Tool::activate(bool manuallyActivated)
{
	...
	WBXASSERT(Network::Players::frontendProcessing(this));
...
void Tool::onLocalClicked()
{
	...
	WBXASSERT(this->getNumToolsInCharacter() < 2);
}
...
void Tool::setToolTip(std::string value)
{
	...
	if(!ProfanityFilter::ContainsProfanity(value) || getWiibloxLocked())
	{																									
		if(toolTip != value)
		{																																
			toolTip = value;																														
