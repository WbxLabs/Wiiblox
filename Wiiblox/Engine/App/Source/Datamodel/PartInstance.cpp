...
namespace
{
	bool computeNetworkOwnerIsSomeoneElseImpl(const wbx::SystemAddress& address, const wbx::SystemAddress& localAddress)
	{
		bool networkOwnerAssigned = ((address != wbx::Network::NetworkOwner::Unassigned()) && (address != wbx::Network::NetworkOwner::ServerUnassigned()));
	...
	{
		wbx::Security::Context securityContext = wbx::Security::Context::current();

		if (securityContext.identity == wbx::Security::LocalGUI_ ||
			securityContext.identity == wbx::Security::GameScript_ ||
			securityContext.identity == wbx::Security::GameScriptInWiibloxPlace_ ||
			securityContext.identity == wbx::Security::WiibloxGameScript_ ||
#if defined(WBX_STUDIO_BUILD)
            securityContext.identity == wbx::Security::StudioPlugin ||
#endif
			securityContext.identity == wbx::Security::CmdLine_)
		{
			return true;
		} else {
			return false;
...
static const wbx::Name& namePart = wbx::Name::declare(sPart);
static const wbx::Name& nameCFrame = wbx::Name::declare("CFrame");
static const wbx::Name& namePosition = wbx::Name::declare("Position");
...
const PropDescriptor<PartInstance, wbx::Vector3> prop_Siz("siz", category_Part, NULL, &PartInstance::setPartSizeXml, PropertyDescriptor::LEGACY); // Used to prepare for TA14264
const PropDescriptor<PartInstance, wbx::Vector3> PartInstance::prop_Size("size", category_Part, &PartInstance::getPartSizeXml, &PartInstance::setPartSizeXml, PropertyDescriptor::STREAMING);
static const PropDescriptor<PartInstance, wbx::Vector3> prop_SizeUi("Size", category_Part, &PartInstance::getPartSizeUi, &PartInstance::setPartSizeUi, PropertyDescriptor::UI);
...
const PropDescriptor<PartInstance, wbx::SystemAddress> PartInstance::prop_NetworkOwner("NetworkOwnerV3", category_Data, &PartInstance::getNetworkOwner, &PartInstance::setNetworkOwner, PropertyDescriptor::REPLICATE_ONLY);
static RemoteEventDesc<PartInstance,
    void(wbx::SystemAddress),
    wbx::remote_signal<void(wbx::SystemAddress)>,
    wbx::remote_signal<void(wbx::SystemAddress)>* (PartInstance::*)(bool)
...
EventDesc<PartInstance, 
	...
	wbx::signal<void(shared_ptr<Instance>)>* (PartInstance::*)(bool)> event_LocalSimulationTouched(&PartInstance::getOrCreateLocalSimulationTouchedSignal, "LocalSimulationTouched", "part");
...
EventDesc<PartInstance, 
	void(shared_ptr<Instance>), 
	wbx::signal<void(shared_ptr<Instance>)>,
	wbx::signal<void(shared_ptr<Instance>)>* (PartInstance::*)(bool)> dep_StoppedTouching(&PartInstance::getOrCreateDeprecatedStoppedTouchingSignal, "StoppedTouching", "otherPart", Reflection::Descriptor::Attributes::deprecated(event_TouchEnded));

EventDesc<PartInstance, 
	void(), 
	wbx::signal<void()>,
	wbx::signal<void()>* (PartInstance::*)(bool)> event_OutfitChanged(&PartInstance::getOrCreateOutfitChangedSignal, "OutfitChanged");
...
{
	WBXASSERT(!onDemandRead() || onDemandRead()->touchTransmitter == NULL);
...
{
	WBX_CRASH_ASSERT(!onDemandRead() || onDemandRead()->touchedSlotCount >= 0);
	if (++(onDemandWrite()->touchedSlotCount) == 1) 
		// TODO: This isn't thread-safe, as it might appear
		addTouchTransmitter();
	WBX_CRASH_ASSERT(onDemandRead()->touchedSlotCount >= 0);
}
...
static float toSpecificGravity(PartMaterial material)
{
	...
	switch (material)
	{
		...
		default: WBXASSERT(false); return 1;
	}
}
...
PartInstance::~PartInstance() 
{
	...
	WBXASSERT(getPartPrimitive());
	WBXASSERT(getPartPrimitive()->getClump()==NULL);
	WBXASSERT(getPartPrimitive()->getWorld() == NULL);
}
...
{
	WBXASSERT(!getPartPrimitive()->getWorld());		// not in workspace!
...
bool PartInstance::aligned(bool useGlobalGridSetting) const
{
	...
	if (isAligned) {
		...
		WBXASSERT(delta < (Tolerance::maxOverlapOrGap() * 0.5));
	}
#endif
	return isAligned;
}
...
bool PartInstance::nonNullInWorkspace(const shared_ptr<PartInstance>& part)
{
	...
	WBXASSERT(!answer || part->getPartPrimitive()->inPipeline());
	return answer;
}
...
{
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "PartInstance_ConvertToNewPhysicalProp", 	
				boost::lexical_cast<std::string>(dataModel->getPlaceID()).c_str(), 0, false);
}

void PartInstance::convertToNewPhysicalPropRecursive(wbx::Instance* instance)
{
	if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(instance)) 
	{
...
bool PartInstance::instanceOrChildrenContainsNewCustomPhysics(wbx::Instance* instance)
{
	if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(instance))
	{
...
Vector3 PartInstance::uiToXmlSize(const Vector3& uiSize) const
{
	...
	if (!DFFlag::FormFactorDeprecated) {
		...
		case BRICK:
			...
			newWbxSize = gridUiSize;
			newWbxSize.y = gridUiSize.y * brickHeight();
			break;
		case PLATE:
			gridUiSize = Math::iRoundVector3(uiSize.max(getMinimumUiSize()));
			newWbxSize = gridUiSize;
			newWbxSize.y = gridUiSize.y * plateHeight();
			break;
		case SYMETRIC:
			newWbxSize = Math::iRoundVector3(uiSize.max(getMinimumUiSize()));
			break;
		case CUSTOM:
		default:
			newWbxSize = uiSize.max(getMinimumUiSizeCustom());
			break;
		}
	} else {
		newWbxSize = uiSize.max(getMinimumUiSizeCustom());
...
void PartInstance::onSleepingChanged(bool sleeping)
{
	...
	WBXASSERT(sleeping == getSleeping());
	if(onDemandRead())
...
{
	return p 
		? WBX_static_cast<PartInstance*>(p->getOwner())
		: NULL;
}
...
void PartInstance::updatePrimitiveState()
{
	...
	if (world != oldWorld)
	{
		WBXASSERT(oldWorld || (getPartPrimitive()->getClump()==NULL));
		...
		if (world)
		{
			...
			const wbx::SystemAddress localAddress = wbx::Network::Players::findLocalSimulatorAddress(this);
			...
			if (getNetworkOwner() == wbx::Network::NetworkOwner::Unassigned())
			{
				if (wbx::Network::Player* player = wbx::Network::Players::findLocalPlayer(this))
				{
				...
				{
					getPartPrimitive()->setNetworkOwner(wbx::Network::NetworkOwner::ServerUnassigned());
				}
			}
...
void PartInstance::render3dAdorn(Adorn* adorn) 
{
		...
		if ( body && world  )
		{
			...
			if (simBody->isInKernel())
			{
				if ( simBody->isFreeFallBody() )
					...
					WBXASSERT( simBody->isContactBody() );
		  ...
		  for (int i = 0; i < getPartPrimitive()->getNumContacts(); ++i) {
			  ...
			  for (int j = 0; j < c->numConnectors(); ++j) {
				  ...
				  if (length < 0.0f) {
					  ...
					  adorn->ray( WBXRay::fromOriginAndDirection(Vector3::zero(), normal * length * -4.0f), color * .8f );
    ...
    if (PartInstance::showJointCoordinates) {
        ...
        adorn->ray( WBXRay::fromOriginAndDirection( Vector3::zero(), Vector3::unitX() * 2.0f ), Color3::red() * .5f );
        adorn->ray( WBXRay::fromOriginAndDirection( Vector3::zero(), Vector3::unitY() * 2.0f ), Color3::green() * .5f );
        adorn->ray( WBXRay::fromOriginAndDirection( Vector3::zero(), Vector3::unitZ() * 2.0f ), Color3::blue() * .5f );
	...
	if (Workspace::showEPhysicsOwners) {
		if (this->getPartPrimitive()->getAssembly()) {
			if (Primitive* movingRoot = Mechanism::getRootMovingPrimitive(this->getPartPrimitive())) {
				const wbx::SystemAddress& systemAddress = PartInstance::fromPrimitive(movingRoot)->getNetworkOwner();
				Color3 color = wbx::Network::NetworkOwner::colorFromAddress(systemAddress);
...
bool PartInstance::hitTestImpl(const wbx::WBXRay& worldRay, Vector3& worldHitPoint) 
{
	wbx::WBXRay rayInPartCoords = getCoordinateFrame().toObjectSpace(worldRay);	
...
{
	WBXASSERT(getPartPrimitive());
	WBXASSERT_IF_VALIDATING(!getPartPrimitive()->hasAutoJoints());
...
void PartInstance::setPhysics(const CoordinateFrame& value)
{
	...
	{
		WBXASSERT_IF_VALIDATING(Math::isOrthonormal(value.rotation));
...
{
	WBXASSERT(this->getConstPartPrimitive()->getConstMechanism() == mechanismRootPart->getConstPartPrimitive()->getConstMechanism());
	WBXASSERT(mechanismRootPart->getConstPartPrimitive()->getConstMechanism()->getConstMechanismPrimitive() == mechanismRootPart->getConstPartPrimitive());
...
CoordinateFrame PartInstance::calcRenderingCoordinateFrame(const Time& t)
{
	...
	if (DFFlag::HumanoidFloorPVUpdateSignal && !PartInstance::disableInterpolation && m && (!networkOwnerIsSomeoneElse)) 
	{
		...
		Humanoid* humanoid = wbx::Humanoid::modelIsCharacter(mechRootPart->getParent());
...
wbx::Vector3 PartInstance::getPrincipalMoment() const
{
...
void PartInstance::setPartSizeXml(const Vector3& WBXSize)
{
	if (DFFlag::FixShapeChangeBug)
	{
		...
		if ((!hasThreeDimensionalSize() && WBXSize.xxx() != getPartSizeXml()))
		{
			WBXASSERT_VERY_FAST(!getPartPrimitive()->hasAutoJoints());

			getPartPrimitive()->setSize(WBXSize.xxx());

			raisePropertyChanged(prop_Size);

			refreshPartSizeUi();
		}
		else if (WBXSize != getPartSizeXml())
		{
			WBXASSERT_VERY_FAST(!getPartPrimitive()->hasAutoJoints());

			getPartPrimitive()->setSize(WBXSize);
	...
	{
		if (WBXSize != getPartSizeXml())
		{
			WBXASSERT_VERY_FAST(!getPartPrimitive()->hasAutoJoints());

			Vector3 newWbxSize = hasThreeDimensionalSize() ? WBXSize : WBXSize.xxx();

			// current smallest size is plate height (0.4) and grid x,z (1.0)
			// TODO: ASSERT
			//WBXASSERT(newWbxSize.max(Vector3(1.0, plateHeight(), 1.0)) == newWbxSize);

			getPartPrimitive()->setSize(newWbxSize);
...
{
	WBXASSERT_VERY_FAST(!getPartPrimitive()->hasAutoJoints());
	Vector3 newWbxSize = this->uiToXmlSize(uiSize);
	setPartSizeXml(newWbxSize);
...
void PartInstance::onNetworkIsSleepingChanged(Time now)
{
	...
	{
		WBXASSERT(!now.isZero()); // setNetworkIsSleeping passes 0 for speed when waking up.
		if (DFFlag::CleanUpInterpolationTimestamps)
		{
...
const wbx::SystemAddress PartInstance::getNetworkOwner() const
{
	WBXASSERT(getConstPartPrimitive());
	return getConstPartPrimitive()->getNetworkOwner();
}

void PartInstance::setNetworkOwnerNotifyIfServer(const wbx::SystemAddress value, bool ownershipBecomingManual)
{
	...
	if (wbx::Network::Players::serverIsPresent(this, false))
	{
...
void PartInstance::setNetworkOwner(const wbx::SystemAddress value)
{
    const wbx::SystemAddress currentOwner = getConstPartPrimitive()->getNetworkOwner();
	if (value != currentOwner) {
		WBXASSERT(value != Network::NetworkOwner::Unassigned());
...
void PartInstance::setNetworkOwnerAndNotify(const wbx::SystemAddress value)
{
	const wbx::SystemAddress oldOwner = getConstPartPrimitive()->getNetworkOwner();
	setNetworkOwner(value);

	notifyNetworkOwnerChanged(oldOwner);
}

void PartInstance::notifyNetworkOwnerChanged(const wbx::SystemAddress oldOwner)
{
	const wbx::SystemAddress currentOwner = getConstPartPrimitive()->getNetworkOwner();
	if (oldOwner != currentOwner)
	{
		...
		*/
		...
		else
		{
			...
			WBXASSERT(currentOwner == Network::NetworkOwner::Server());
...
wbx::remote_signal<void(wbx::SystemAddress)>* PartInstance::getOrCreateNetworkOwnerChangedSignal(bool create)
{
    return NULL;
}
...
{
	wbx::SystemAddress consistentAddress;
...
void PartInstance::updateOwnershipSettingsPostAnchor(std::vector<Primitive*>& futureMechRoots, RootPrimitiveOwnershipData& ownershipData)
{
	...
	for (size_t i = 0; i < futureMechRoots.size(); i++)
	{
		if (DFFlag::SetNetworkOwnerFixAnchoring2)
		{
			...
			if (futureMechRootPart)
			{
				...
				else
				{
					...
					if ((wbx::Network::Players::serverIsPresent(this) || DFFlag::NetworkOwnershipRuleReplicates) 
		...
		{
			WBXASSERT(Mechanism::isMovingAssemblyRoot(futureMechRoots[i]->getAssembly()));
...
void PartInstance::updateResetOwnershipSettingsOnUnAnchor()
{
	...
	if (Assembly* currentAssembly = getPartPrimitive()->getAssembly())
	{
		if (!currentAssembly->computeIsGrounded())
		{
			...
			wbx::SystemAddress addressToPass;
...
void PartInstance::setNetworkOwnershipRule(NetworkOwnership value)
{
	if (getNetworkOwnershipRule() != value)
	{
		...
		if (DFFlag::NetworkOwnershipRuleReplicates && wbx::Network::Players::serverIsPresent(this, false))
		{
...
void PartInstance::setNetworkOwnerScript(shared_ptr<Instance> playerInstance)
{
	...
	if (canSetNetworkOwnership(rootPrim, statusMessage))
	{
		...
		{
			static boost::once_flag flag = BOOST_ONCE_INIT;
			boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "PartInstance_SetNetworkOwnerScript", 
				boost::lexical_cast<std::string>(dm->getPlaceID()).c_str(), 0, false));

			if (!player)
			{
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "PartInstance_SetNetworkOwnerScript_TOSERVER", 
					boost::lexical_cast<std::string>(dm->getPlaceID()).c_str(), 0, false));
			}
		}

		wbx::SystemAddress ownerAddress;
		if (player)
		{
			ownerAddress = player->getRemoteAddressAsWbxAddress();
		}
		else
		{
			ownerAddress = wbx::Network::NetworkOwner::Server();
		}
...
shared_ptr<Instance> PartInstance::getNetworkOwnerScript()
{
	...
	if (canSetNetworkOwnership(rootPrim, statusMessage))
	{
		if(Network::Players::serverIsPresent(this))
		{
			...
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "PartInstance_GetNetworkOwnerScript", 
					boost::lexical_cast<std::string>(dm->getPlaceID()).c_str(), 0, false));
			}

			wbx::SystemAddress networkOwner = rootPrim->getNetworkOwner();
...
{
	shared_ptr<Reflection::Tuple> args = wbx::make_shared<Reflection::Tuple>();
...
void PartInstance::checkConsistentOwnerAndRuleResetRoots(wbx::SystemAddress& consistentAddress, bool& hasConsistentOwner, bool& hasConsistentManualOwnershipRule, std::vector<Primitive*>& primRoots)
{
...
bool PartInstance::resizeImpl(NormalId localNormalId, int amount)
{
	...
	if (amount == 0) {
		...
		if (hasThreeDimensionalSize()) {
			...
			if (!DFFlag::FormFactorDeprecated) 
			{
				WBXASSERT(getFormFactor() == PartInstance::SYMETRIC);
			}
...
Surface PartInstance::getSurface(const wbx::WBXRay& gridRay, int& surfaceId)
{
	const CoordinateFrame& currentCoord = getCoordinateFrame();

	wbx::WBXRay localRay = currentCoord.toObjectSpace(gridRay);
...
bool PartInstance::containedByFrustum(const wbx::Frustum& frustum) const
{
	return frustum.containsAABB(getConstPartPrimitive()->getExtentsLocal(), getCoordinateFrame());
}

bool PartInstance::intersectFrustum(const wbx::Frustum& frustum) const
{
...
namespace Reflection {
template<>
void wbx::TypedPropertyDescriptor<CoordinateFrame>::readValue(DescribedBase* instance, const XmlElement* element, IReferenceBinder& binder) const
{
	if (!element->isXsiNil())
	{
		CoordinateFrame c;
...
template<>
void wbx::TypedPropertyDescriptor<CoordinateFrame>::writeValue(const DescribedBase* instance, XmlElement* element) const
{
...
template<>
bool wbx::TypedPropertyDescriptor<CoordinateFrame>::hasStringValue() const {
	return false;
}

template<>
std::string wbx::TypedPropertyDescriptor<CoordinateFrame>::getStringValue(const DescribedBase* instance) const{
	return Super::getStringValue(instance);
}

template<>
bool wbx::TypedPropertyDescriptor<CoordinateFrame>::setStringValue(DescribedBase* instance, const std::string& text) const {
