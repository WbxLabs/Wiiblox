...
namespace WBX
{
	...
	void TextBox::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
	{
        ...
        if(newProvider)
		{
            ...
            if( wbx::UserInputService* userInputService = wbx::ServiceProvider::create<wbx::UserInputService>(newProvider) )
	...
	int TextBox::getCursorPos(wbx::Vector2 mousePos)
	{
	...
	void TextBox::gainFocus(const shared_ptr<InputObject>& event)
	{
        ...
        if( wbx::UserInputService* userInputService = wbx::ServiceProvider::create<wbx::UserInputService>(this) )
	...
	void TextBox::captureFocus()
	{
		...
		gainFocus(shared_ptr<wbx::InputObject>());
	}
	...
	void TextBox::keyUp(RepeatKeyState::KeyType keyType, wbx::KeyCode keyCode, char key)
	{
	...
	void TextBox::keyDown(RepeatKeyState::KeyType keyType, wbx::KeyCode keyCode, char key)
	{
	...
	{
		WBXASSERT(event->isTextInputEvent());
	...
	GuiResponse TextBox::processKeyEvent(const shared_ptr<InputObject>& event)
	{
		...
		if (iAmFocus) 
		{
			if (event->isKeyDownEvent())
			{
                ...
                {
                    wbx::UserInputService* inputService = wbx::ServiceProvider::find<wbx::UserInputService>(this);
                    WBXASSERT(inputService);
    ...
    {
        if(event->getKeyCode() == wbx::SDLK_v)
        {
            if(wbx::UserInputService* inputService = wbx::ServiceProvider::find<wbx::UserInputService>(this))
            {
                std::vector<wbx::ModCode> modCodes = inputService->getCommandModCodes();
                for(std::vector<wbx::ModCode>::iterator it = modCodes.begin(); it != modCodes.end(); ++it)
                {
