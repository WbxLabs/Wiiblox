...
template<>
bool wbx::StringConverter<Controller::Button>::convertToValue(const std::string& text, Controller::Button& value)
{
	if(text.find("Jump")){
...
const UserInputBase* Controller::getHardwareDevice() const
{
	...
	WBXASSERT(controllerService == ServiceProvider::find<ControllerService>(this));
...
WBX_REGISTER_CLASS(ButtonBindingWidget);
...
void HumanoidController::updateCamera( const Stepped& event, const NavKeys& nav )
{
	...
	int rotation = wbx::GameBasicSettings::singleton().camLockedInCamLockMode() ?
