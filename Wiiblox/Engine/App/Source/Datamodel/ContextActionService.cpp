...
static Reflection::BoundFuncDesc<ContextActionService, void(std::string, Lua::WeakFunctionRef, bool, shared_ptr<const Reflection::Tuple>)> func_bindCore(&ContextActionService::bindCoreActionForInputTypes, "BindCoreAction","actionName","functionToBind","createTouchButton","inputTypes", Security::WiibloxScript);
...
// unbinding
static Reflection::BoundFuncDesc<ContextActionService, void(std::string)> func_unbindCoreAction(&ContextActionService::unbindCoreAction, "UnbindCoreAction", "actionName", Security::WiibloxScript);
...
static Reflection::EventDesc<ContextActionService, void(std::string, std::string, shared_ptr<const Reflection::ValueTable>)> event_boundFunctionUpdated(&ContextActionService::boundActionChangedSignal, "BoundActionChanged", "actionChanged", "changeName" ,"changeTable", Security::WiibloxScript);
static Reflection::EventDesc<ContextActionService, void(std::string, bool, shared_ptr<const Reflection::ValueTable>)> event_addedBoundFunction(&ContextActionService::boundActionAddedSignal, "BoundActionAdded", "actionAdded","createTouchButton","functionInfoTable", Security::WiibloxScript);
static Reflection::EventDesc<ContextActionService, void(std::string, shared_ptr<const Reflection::ValueTable>)> event_removedBoundFunction(&ContextActionService::boundActionRemovedSignal, "BoundActionRemoved", "actionRemoved","functionInfoTable", Security::WiibloxScript);
    
static Reflection::EventDesc<ContextActionService, void(std::string)> event_getActionButton(&ContextActionService::getActionButtonSignal, "GetActionButtonEvent", "actionName", Security::WiibloxScript);
static Reflection::BoundFuncDesc<ContextActionService, void(std::string, shared_ptr<Instance>)> func_actionButtonFound(&ContextActionService::fireActionButtonFoundSignal, "FireActionButtonFoundSignal", "actionName","actionButton", Security::WiibloxScript);
    
static Reflection::BoundFuncDesc<ContextActionService, void(std::string, InputObject::UserInputState, shared_ptr<Instance>)> func_callFunction(&ContextActionService::callFunction, "CallFunction", "actionName", "state", "inputObject", Security::WiibloxScript);
REFLECTION_END();
...
ContextActionService::ContextActionService() : Super()
{
	...
	wbx::Guid::generateWBXGUID(activateGuid);
}
...
Tool* ContextActionService::getCurrentLocalTool()
{
    ...
    if (wbx::Network::Player* player = players->getLocalPlayer())
        if(ModelInstance* character = player->getCharacter())
            if( wbx::Tool* activeTool = character->findFirstChildOfType<wbx::Tool>() )
                return activeTool;
...
void ContextActionService::checkForLocalPlayer(shared_ptr<Instance> newPlayer)
{
    ...
    {
        if ( wbx::Network::Player* player = Instance::fastDynamicCast<wbx::Network::Player>(newPlayer.get()) )
        {
...
void ContextActionService::setupLocalPlayerConnections(wbx::Network::Player* localPlayer)
{
...
void ContextActionService::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if(newProvider)
    {
        wbx::Network::Players* players = ServiceProvider::create<wbx::Network::Players>(newProvider);
        if (wbx::Network::Player* player = players->getLocalPlayer())
            setupLocalPlayerConnections(player);
        else
				...
				else if (iter->first.compare(activateGuid) == 0)
				{
					...
					if (canActivate && !menuIsOpen)
					{
						if (UserInputService* inputService = wbx::ServiceProvider::find<UserInputService>(this))
						{
							...
							shared_ptr<InputObject> simulateInputObject = wbx::Creatable<wbx::Instance>::create<wbx::InputObject>(InputObject::TYPE_MOUSEBUTTON1, isDown ? InputObject::INPUT_STATE_BEGIN : InputObject::INPUT_STATE_END,
								Vector3(currentPos.x,currentPos.y,0), Vector3::zero(), wbx::DataModel::get(this));
							...
							if (inputObject->getKeyCode() == SDLK_GAMEPAD_BUTTONR2 || inputObject->getKeyCode() == SDLK_GAMEPAD_BUTTONL2)
							{
								...
								if (zPos >= 0.5 && lastZPos < 0.5)
								{
									...
									wbx::DataModel::get(this)->processInputObject(simulateInputObject);
								}
...
{
	shared_ptr<Reflection::ValueTable> functionTable = wbx::make_shared<Reflection::ValueTable>();
	...
	{
		shared_ptr<Reflection::ValueArray> inputArray = wbx::make_shared<Reflection::ValueArray>(functionData.inputTypes->values);
...
void unbindActionInternal(const std::string actionName, FunctionMap& funcMap, FunctionVector& funcVector, wbx::signal<void(std::string, shared_ptr<const Reflection::ValueTable>)>& removedSignal)
{
		...
		if (iter->second.inputTypes && iter->second.inputTypes->values.size() > 0)
		{
			...
			for (Reflection::ValueArray::const_iterator tupleIter = inputBindingsValues.begin(); tupleIter != inputBindingsValues.end(); ++tupleIter)
			{
				...
				if (didOverride)
				{
					...
					{
						cancelInputObject = wbx::Creatable<wbx::Instance>::create<wbx::InputObject>(wbx::InputObject::TYPE_NONE,
							wbx::InputObject::INPUT_STATE_CANCEL,
							wbx::Vector3::zero(),
							wbx::Vector3::zero(),
...
void ContextActionService::bindActionInternal(const std::string actionName, Lua::WeakFunctionRef functionToBind, bool createTouchButton, shared_ptr<const Reflection::Tuple> hotkeys, FunctionMap& funcMap, FunctionVector& funcVector)
{
	...
	{
		throw wbx::runtime_error("ContextActionService:BindAction can only be called from a local script");
		return;
	}

	if (actionName.empty())
	{
		throw wbx::runtime_error("ContextActionService:BindAction called with an empty actionName");
		return;
	}
	...
	if (hotkeys && !hotkeys->values.empty())
	{
		for(Reflection::ValueArray::const_iterator iter = hotkeys->values.begin(); iter != hotkeys->values.end(); ++iter)
		{
			...
			{
				throw wbx::runtime_error("ContextActionService:BindAction called with an invalid hotkey (should be either Enum.KeyCode, Enum.UserInputType or string)");
				return;
			}

			//Now check to see if this bind is overriding another bind action (so we can signal the action it is no longer active)

			// core Wiiblox overrides only happen when its another core bind
	...
	(*functionTable)["title"] = wbx::Reflection::Variant();
	(*functionTable)["image"] = wbx::Reflection::Variant();
	(*functionTable)["description"] = wbx::Reflection::Variant();
	(*functionTable)["createTouchButton"] = wbx::Reflection::Variant(createTouchButton);
	if (hotkeys && !hotkeys->values.empty())
	{
		shared_ptr<Reflection::ValueArray> hotkeyArray = wbx::make_shared<Reflection::ValueArray>(hotkeys->values);
		(*functionTable)["inputTypes"] = shared_ptr<const wbx::Reflection::ValueArray>(hotkeyArray);
	}
...
void ContextActionService::bindActivate(InputObject::UserInputType inputType, KeyCode keyCode)
{
	if (!Network::Players::frontendProcessing(this))
	{
		throw wbx::runtime_error("ContextActionService:BindActivate can only be called from a local script");
	}

	if (inputType == InputObject::TYPE_NONE)
	{
		throw wbx::runtime_error("ContextActionService:BindActivate called with Enum.UserInputType.None, must be some other input type.");
	}

	unbindActivate(inputType, keyCode);

	shared_ptr<Reflection::Tuple> activateTuple = wbx::make_shared<Reflection::Tuple>();
...
void ContextActionService::unbindActivate(InputObject::UserInputType inputType, KeyCode keyCode)
{
	if (!Network::Players::frontendProcessing(this))
	{
		throw wbx::runtime_error("ContextActionService:UnbindActivate can only be called from a local script");
	}

	if (inputType == InputObject::TYPE_NONE)
	{
		throw wbx::runtime_error("ContextActionService:UnbindActivate called with Enum.UserInputType.None, must be some other input type.");
	}
...
{
    shared_ptr<Reflection::ValueTable> reflectedFuncMap = wbx::make_shared<Reflection::ValueTable>();
    ...
    for(FunctionMap::iterator iter = functionMap.begin(); iter != functionMap.end(); ++ iter)
    {
        if (iter != functionMap.end())
        {
            ...
            Reflection::Variant variantValue = shared_ptr<const wbx::Reflection::ValueTable>(functionInfo);
...
shared_ptr<const Reflection::ValueTable> ContextActionService::getBoundCoreActionData(const std::string actionName)
{
    ...
    return wbx::make_shared<Reflection::ValueTable>();
}
