...

{
	...
	WBXASSERT(Edge::getPrimitive(0) == NULL);
	WBXASSERT(Edge::getPrimitive(1) == NULL);
...
void SkateboardPlatform::onLocalPlatformStanding(Humanoid* humanoid)
{
	...
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Error mounting skateboard platform. Both Humanoid and SkateboardPlatform must be direct children of a Model");
	}
}
...
void SkateboardPlatform::countGroundedWheels()
{
	...
	if(humanoid && geom)
	{
		...
		{
			wbx::WBXRay caster( wheels[i].joint->getJointWorldCoord(1).translation, downforceDir* 2.0f); // search 2 stud
...
bool SkateboardPlatform::stepUi(double distributedGameTime)
{
	...
	if (motor6D) {
		WBXASSERT(humanoidFromMotor6D(motor6D) == this->humanoid);
		WBXASSERT(humanoidFromMotor6D(motor6D) == this->humanoid);
	}
...
void SkateboardPlatform::doLoadWheels(Primitive* primitive)
{
	for (int i = 0; i < primitive->numChildren(); ++i) {
		...
		if (Joint::getJointType(joint) == Joint::ROTATE_JOINT) {
			Wheel w;
			w.joint = WBX_static_cast<RotateJoint*>(joint);
			wheels.append(w);
		}
	}
}
...
void SkateboardPlatform::renderGuange(
	...
	adorn->rect2d(speedo.toRect2D(), Color3::blue());	// note WBX color
