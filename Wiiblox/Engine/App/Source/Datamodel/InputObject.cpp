...
namespace WBX {    
	...
	InputObject::InputObject(UserInputType inputType, UserInputState inputState, G3D::Vector3 newPosition, G3D::Vector3 newDelta, DataModel* dm)
		...
		, inputText("")
		, scanCode(SDL_SCANCODE_UNKNOWN)
		, weakWorkspace(weak_from(wbx::ServiceProvider::find<Workspace>(dm)))
	...
	Vector4 InputObject::getGuiInset() const
	{
		...
		if (shared_ptr<wbx::Workspace> workspace = weakWorkspace.lock())
		{
			if (wbx::GuiService* guiService = wbx::ServiceProvider::find<wbx::GuiService>(workspace.get()))
			{
