...
namespace WBX {
	...
	void AssetService::processServiceResults(std::string *result, const std::exception* httpException, boost::function<void(shared_ptr<const Reflection::ValueTable>)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		{
			errorFunction(wbx::format("Http exception occurred %s", httpException->what())); 
		}
	}
	...
	namespace {
		...
		{
			WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "CreatePlace");
		}
		static void sendCreatePlaceInPlayerInventoryStats()
		{
			WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "CreatePlaceInPlayerInventory");
		}
	} // namespace
	...
	static void CreatePlaceSuccessHelper(weak_ptr<DataModel> dm, const std::string& response, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		if (shared_ptr<DataModel> sharedDM = dm.lock())
		{
			if (!response.empty())
			{
				...
				{
					errorFunction( wbx::format("Game:CreatePlace response was not a valid placeId because %s",e.what()) );
					return;
				}
	...
	static void CreatePlaceErrorHelper(weak_ptr<DataModel> dm, const std::string& error, boost::function<void(std::string)> errorFunction)
	{
		if (shared_ptr<DataModel> sharedDM = dm.lock())
		{
			if (!error.empty())
			{
				errorFunction( wbx::format("Game:CreatePlace received and error: %s.",error.c_str()) );
			}
			else
			{
		...
		shared_ptr<Instance> playerInstance, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		std::string parameters = wbx::format("?currentPlaceId=%d&placeName=%s&templatePlaceId=%d",
			dm->getPlaceID(),wbx::Http::urlEncode(placeName).c_str(),templatePlaceId);
		std::string baseUrl = ServiceProvider::create<ContentProvider>(this)->getApiBaseUrl();

		std::string playerParameter = "";
		if(player)
			playerParameter = wbx::format("&playerId=%i", player->getUserID());
		...
		try
		{
			if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
			{
				...
				apiService->postAsync(createHttp, postString, wbx::PRIORITY_SERVER_ELEVATED, HttpService::TEXT_PLAIN,
		...
		{
			errorFunction( wbx::format("Game:CreatePlace failed because %s",e.what()) );
		}
	}
	...
	{
		Http http(wbx::format(assetVersionsUrl.c_str(), assetId, pageNum)); 
	...
	{
		Http http(wbx::format(placeAccessUrl.c_str(), placeId));		
		http.get(boost::bind(&AssetService::processServiceResults, this, _1, _2, resumeFunction, errorFunction)); 	
	}
	...
	{		
		Http http(wbx::format(assetRevertUrl.c_str(), assetId, versionNumber));
		std::string in; 
	...
	{			
		...
		url = wbx::format(url.append("/update?").c_str(), placeId); 
		url = wbx::format(url.append("access=%s").c_str(), enumToString(type).c_str()); 		
	...
	{	
		...
		{
			errorFunction( wbx::format("AssetService:GetCreatorAssetID response could not convert to number because %s",e.what()) );
			return;
		}
	...
	void AssetService::getCreatorAssetIDErrorHelper(std::string error, boost::function<void(std::string)> errorFunction)
	{
		...
		errorFunction(wbx::format("AssetService:GetCreatorAssetID error: %s", error.c_str()));
	}
	...
	void AssetService::getCreatorAssetID(int creationID, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		std::string baseUrl = wbx::ServiceProvider::create<ContentProvider>(this)->getApiBaseUrl();
		std::string parameters =  wbx::format("?creationID=%d",creationID);

		if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
		{
			apiService->getAsync("GetCreatorAssetID" + parameters, true, wbx::PRIORITY_DEFAULT,
