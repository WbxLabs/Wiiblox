...
/* Copyright 2003-2014 Wiiblox Corporation, All Rights Reserved  */
...
StarterPlayerService::StarterPlayerService()
	: cameraMode(wbx::Camera::CAMERAMODE_CLASSIC)
	, nameDisplayDistance(defaultDisplayDistance)
	, healthDisplayDistance(defaultDisplayDistance)
	, cameraMaxZoomDistance(wbx::Camera::distanceMaxCharacter())
	, cameraMinZoomDistance(wbx::Camera::distanceMin())
...
void StarterPlayerService::setCameraMode(wbx::Camera::CameraMode value)
{
	if(cameraMode != value)
	{
		cameraMode = value;
...
void StarterPlayerService::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
	...
	if (newProvider && wbx::Network::Players::backendProcessing(newProvider)) 
	{
...
{
	wbx::StarterPlayerScripts* currPlayerScripts= findFirstChildOfType<wbx::StarterPlayerScripts>();
	if (!currPlayerScripts)
	{
		shared_ptr<wbx::StarterPlayerScripts> playerScripts = Creatable<Instance>::create<wbx::StarterPlayerScripts>();
	...
	if (DFFlag::UseStarterPlayerCharacterScripts)
	{
		wbx::StarterPlayerScripts* pStarterCharacterScripts = findFirstChildOfType<wbx::StarterCharacterScripts>();
		if (!pStarterCharacterScripts)
		{
			shared_ptr<wbx::StarterCharacterScripts> characterScripts = Creatable<Instance>::create<wbx::StarterCharacterScripts>();
...
void StarterPlayerService::recordSettingsInGA() const
{
	...
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevTouchCameraMove", CameraMovement);
	...
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevTouchMovement", CharacterMovement);
	...
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevComputerCameraMove", CameraMovement);
	...
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevComputerMovement", CharacterMovement);

	if (enableMouseLockOption) {
		WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevComputerEnableMouseLock", "True");
	} else {
		WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevComputerEnableMouseLock", "False");
	}
	...
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DevCameraOcclusion", CameraOcclusion);
}
