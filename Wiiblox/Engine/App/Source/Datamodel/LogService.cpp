...
//    "WBXcdn%2ecom%2f", // url-encoded case
...
static const std::string kStartWords[kNumWords] = {
    ...
    "WBXcdn.com/" // avoids asset/place stealing
    };
}
...
namespace WBX {
    ...
    static Reflection::BoundFuncDesc<LogService, void()> func_RequestServerOutput(
        &LogService::requestServerOutput, "RequestServerOutput", Security::WiibloxScript);
	static Reflection::BoundFuncDesc<LogService, void(std::string)> func_executeScript(
		&LogService::executeScript, "ExecuteScript", "source", Security::WiibloxScript);
    ...
    static Reflection::RemoteEventDesc<LogService, void(std::string, MessageType, int)> event_ServerOutputMessage(
        ...
        Security::WiibloxScript, Reflection::RemoteEventCommon::SCRIPTING, Reflection::RemoteEventCommon::CLIENT_SERVER);
    static Reflection::RemoteEventDesc<LogService, void(shared_ptr<Instance>)> event_RequestServerOutputSignal(
        ...
        Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
    ...
    {
        WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "GetLogHistory", 
        ...
        {
            shared_ptr<Reflection::ValueTable> logTable = wbx::make_shared<Reflection::ValueTable>();
    ...
    int LogService::filterSensitiveKeys(std::string& text)
    {
        ...
        // prefilter the string and exit if http and (Wiiblox or WBXcdn) is not found.
        ...
        offsetSaved = offset;
        offset = lower.find("Wiiblox", offset);
        if (offset == std::string::npos)
        {
            offset = lower.find("WBXcdn", offsetSaved);
            if (offset == std::string::npos)
                return changeMask;
        }
    ...
    void LogService::doFireEvent(StandardOutMessage message, weak_ptr<LogService> logService)
    {
        if (shared_ptr<LogService> sharedLogService = logService.lock())
        {
            ...
            if (filterResults & DFInt::BadLogMask)
            {
                ...
                wbx::Analytics::InfluxDb::Points analyticsPoints;
				...
				itr != playerList.end(); ++itr)
			{
				if (shared_ptr<Network::Player> player = itr->lock())
				{
					raiseServerEventFromMessage(sharedLogService, message,
						player->getRemoteAddressAsWbxAddress());
				}
			}
		}
    }
		...
		weak_ptr<Network::Player> weakPlayer)
	{
		if (shared_ptr<LogService> logService = weakLogService.lock())
		{
            ...
            {
                WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "SendingServerLogHistory", 
                    format("%d", DataModel::get(logService.get())->getPlaceID()).c_str());
                WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "CreatorReadingServerLogs",
                ...
                // send backlog
                const SystemAddress& target = p->getRemoteAddressAsWbxAddress();
	...
	{
		WBXASSERT(Network::Players::serverIsPresent(this));
