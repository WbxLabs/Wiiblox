...
namespace {
	...
	{
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "HttpService", wbx::StringConverter<int>::convertToString(placeId).c_str());
	}
}
...
namespace WBX {
	...
	{
		DataModel* dm = DataModel::get(this);
		request.additionalHeaders["Wiiblox-Id"] = wbx::StringConverter<int>::convertToString(dm->getPlaceID());
	}
