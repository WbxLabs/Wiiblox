...
{
	wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "SurfaceGui");
}

static void sendSurfaceGUINonTextUsage()
{
	wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME,"SurfaceGuiNonText");
}
...
namespace WBX {
	...
	{ 
		return !!Network::Players::findLocalPlayer(this) && this->isDescendantOf( wbx::ServiceProvider::find<wbx::StarterGuiService>(this) ); 
	}

	void SurfaceGui::unProcess()
	{
		shared_ptr<InputObject> mouseMoveEvent = Creatable<Instance>::create<InputObject>(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CANCEL,wbx::DataModel::get(this));
		mouseMoveEvent->setPosition(Vector3(-1e4,-1e4,-1e4));
		Super::process(mouseMoveEvent);

		shared_ptr<InputObject> focusEndEvent = Creatable<Instance>::create<InputObject>(InputObject::TYPE_FOCUS, InputObject::INPUT_STATE_CANCEL,wbx::DataModel::get(this));
	...
	GuiResponse SurfaceGui::process3d(const shared_ptr<InputObject>& event, Vector3 point3d, bool ignoreMaxDistance)
	{
        ...
        if( wbx::ModelInstance* character = Network::Players::findLocalCharacter(this) )
        {
