...
namespace WBX {
	...
	static Reflection::RemoteEventDesc<BadgeService, void(std::string, int, int)> event_BadgeAwarded(&BadgeService::badgeAwardedSignal, "BadgeAwarded", "message", "userId", "badgeId", Security::WiibloxScript, Reflection::RemoteEventCommon::SCRIPTING, Reflection::RemoteEventCommon::BROADCAST);
    REFLECTION_END();
	...
	BadgeService::HotUserHasBadge::HotUserHasBadge(int userId, int badgeId, int cooldownTime)
		:userId(userId)
		,badgeId(badgeId)
		,expiration(wbx::Time::now<Time::Fast>() + Time::Interval(cooldownTime))
	{
	}

	bool BadgeService::HotUserHasBadge::expired() const
	{
		return wbx::Time::now<Time::Fast>() >= expiration;
	}
	...
	void BadgeService::userHasBadge(int userId, int badgeId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		if(!Workspace::serverIsPresent(this)){
			...
			StandardOut::singleton()->printf(MESSAGE_WARNING, "Sorry, badges can only be queried by the Wiiblox game servers");
			resumeFunction(false);
			return;
		}
		...
		Http http(wbx::format(hasBadgeUrl.c_str(), userId, badgeId));
	...
	void BadgeService::isDisabled(int badgeId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		if(!Workspace::serverIsPresent(this)){
			//Throw a friendly exception to the users that they can't get badges during user games
			StandardOut::singleton()->printf(MESSAGE_WARNING, "Sorry, badges can only be tested if they are disabled on Wiiblox game servers");
			resumeFunction(true);
			return;
		}
		...
		//Make the request and send it out
		Http http(wbx::format(isBadgeDisabledUrl.c_str(), badgeId, placeId));
	...
	void BadgeService::isLegal(int badgeId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		//Make the request and send it out
		Http http(wbx::format(isBadgeLegalUrl.c_str(), badgeId, placeId));
										...
										boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		if(*response == ""){
			...
			wbx::DataModel* dataModel = (wbx::DataModel*)this->getParent();
			if (dataModel)
			{
	...
	void BadgeService::awardBadge(int userId, int badgeId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		if(!Workspace::serverIsPresent(this)){
			//Throw a friendly exception to the users that they can't get badges during user games
			StandardOut::singleton()->printf(MESSAGE_WARNING, "Sorry, badges can only be awarded by Wiiblox game servers");
			resumeFunction(false);
			return;
		}
		...
		//Make the request and send it out
		Http http(wbx::format(awardBadgeUrl.c_str(), userId, badgeId, placeId));
