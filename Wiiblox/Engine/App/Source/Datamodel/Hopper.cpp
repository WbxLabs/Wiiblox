...
static Reflection::EventDesc<HopperBin, void(shared_ptr<Instance>), wbx::remote_signal<void(shared_ptr<Instance>)> > desc_Selected(&HopperBin::selectedSignal, "Selected", "mouse");
...
static const Reflection::BoundFuncDesc<HopperBin,void()> func_toggleSelect(&HopperBin::onLocalClicked,"ToggleSelect", Security::WiibloxScript);
static const Reflection::BoundFuncDesc<HopperBin,void()> func_disable(&HopperBin::disable,"Disable", Security::WiibloxScript);
...
{
	return false /*(Instance::fastDynamicCast<wbx::Network::Player>(instance) != NULL)*/;
}
	...
	if(newProvider)
	{
		if(Workspace::serverIsPresent(this)){
			if(!replicationInitialized){
				...
				WBXASSERT(!selectedConnectionShim.connected());
...
{
	WBXASSERT(binType != SCRIPT_BIN);
	
	DataModel* dataModel = WBX_static_cast<DataModel*>(getRootAncestor());
