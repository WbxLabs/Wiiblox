...
namespace {
	...
	{
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "RemoteFunction", function);
	}

	static void sendRemoteEventStatsOnServer(const char* function)
	{
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "RemoteEvent", function);
	}
...
namespace WBX {
	...
	static Reflection::RemoteEventDesc<RemoteEvent,
        ...
        LatchedSignal<wbx::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)>,
        LatchedSignal<wbx::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)>* (RemoteEvent::*)(bool)
	...
	static Reflection::RemoteEventDesc<RemoteEvent,
        ...
        LatchedSignal<wbx::remote_signal, void(shared_ptr<const Reflection::Tuple>)>,
        LatchedSignal<wbx::remote_signal, void(shared_ptr<const Reflection::Tuple>)>* (RemoteEvent::*)(bool)
    ...
    bool isPlayerValid(Instance* instance, const SystemAddress& address)
    {
        ...
        return player && player->getRemoteAddressAsWbxAddress() == address;
    }
    ...
    void DelayedInvocationQueue::process()
    {
        ...
        for (size_t i = 0; i < temp.size(); ++i)
        {
            ...
            {
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_OUTPUT, "DelayedInvocationQueue::process encountered an error %s", e.what());
                WBXCRASH();
            }
        }
    }
    ...
    void RemoteFunction::invokeClient(shared_ptr<Instance> player, shared_ptr<const Reflection::Tuple> arguments, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
    {
        ...
        else if (Network::Players::serverIsPresent(this))
        {
            ...
            const wbx::SystemAddress& target = p->getRemoteAddressAsWbxAddress();
    ...
    int RemoteFunction::createRemoteInvocation(const weak_ptr<Instance>& player, const boost::function<void(shared_ptr<const Reflection::Tuple>)>& resumeFunction, const boost::function<void(std::string)>& errorFunction)
    {
        ...
        WBXASSERT(remoteInvocations.count(id) == 0);
    ...
    void RemoteFunction::processRemoteEvent(const Reflection::EventDescriptor& descriptor, const Reflection::EventArguments& args, const SystemAddress& source)
    {
        if (&descriptor == &event_RemoteOnInvokeServer)
        {
            ...
            {
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_SENSITIVE, "RemoteFunction::processRemoteEvent: ignore a remote call from %x:%d", source.getAddress(), source.getPort());
            }
        }
        else if (&descriptor == &event_RemoteOnInvokeClient)
        {
            WBXASSERT(args.size() == 2);
    ...
    LatchedSignal<wbx::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)>* RemoteEvent::getOrCreateOnServerEvent(bool create)
    {
    ...
    LatchedSignal<wbx::remote_signal, void(shared_ptr<const Reflection::Tuple>)>* RemoteEvent::getOrCreateOnClientEvent(bool create)
    {
    ...
    void RemoteEvent::processRemoteEvent(const Reflection::EventDescriptor& descriptor, const Reflection::EventArguments& args, const SystemAddress& source)
    {
        if (&descriptor == &event_OnServerEvent)
        {
            ...
            {
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_SENSITIVE, "RemoteEvent::processRemoteEvent: ignore a remote call from %x:%d", source.getAddress(), source.getPort());
            }
        }
        else
        {
