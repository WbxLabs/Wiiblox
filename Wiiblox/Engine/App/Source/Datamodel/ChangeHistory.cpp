/**
 * ChangeHistory.cpp
 * Copyright (c) 2013 Wiiblox Corp. All Rights Reserved.
 */
#include "stdafx.h"
...
const char* const wbx::sChangeHistoryService = "ChangeHistoryService";
...
namespace WBX
{
	namespace Reflection
	{
		...
		{
			addPair(wbx::ChangeHistoryService::Aggregate, "Aggregate");
			addPair(wbx::ChangeHistoryService::Snapshot, "Snapshot");
			addPair(wbx::ChangeHistoryService::Hybrid, "Hybrid");
		}
	}
}
...
wbx::ChangeHistoryService::RuntimeUndoBehavior wbx::ChangeHistoryService::runtimeUndoBehavior = wbx::ChangeHistoryService::Aggregate;

#define WBX_GET_CELL_DETAIL(cellData)                  (cellData & 0xff)
#define WBX_GET_CELL_MATERIAL(cellData)                (Voxel::CellMaterial)((cellData>>8) & 0xff)
#define WBX_GET_CELL_INDEX(cellData)                   (cellData>>16)
#define WBX_SET_CELL_DATA(cellIndex, detail, material) ((cellIndex<<16) | (material<<8)| (detail))

#define WBX_MAX_CLUSTER_CELLS (Voxel::kXZ_CHUNK_SIZE * Voxel::kXZ_CHUNK_SIZE * Voxel::kY_CHUNK_SIZE)
...
{	return (WBX_GET_CELL_INDEX(lhs) < WBX_GET_CELL_INDEX(rhs)); }
...
{
	return wbx::Security::Context::current().identity == wbx::Security::Replicator_;
}
...
private:
	...
	void applyClusterData(const std::pair<SpatialRegion::Id, ChunkCells> &chunkCellData) const
	{
		...
		for (ChunkCells::const_iterator chunkCellsIter = chunkCells.begin(); chunkCellsIter != chunkCells.end(); ++chunkCellsIter)
		{
			...
			changeHistory->setCell(chunkCellData.first, 
					cellIndexToVector3(WBX_GET_CELL_INDEX(*chunkCellsIter)),
					Voxel::Cell::readUnsignedCharFromDeprecatedUse(WBX_GET_CELL_DETAIL(*chunkCellsIter)),
					WBX_GET_CELL_MATERIAL(*chunkCellsIter));
		}
	}
	...
	void addValue(const PropertyDescriptor& propertyDescriptor)
	{
		...
		//	wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService::recording %s.%s\n", instance->getName().c_str(), propertyDescriptor.name.c_str());
	...
	void absorbClusterData(const std::pair<SpatialRegion::Id, ChunkCells> &chunkCellData)
	{
		...
		ChunkCells chunkCellsToCopy = chunkCellData.second, &chunkCellsLocal = clusterCellsIter->second, tempVector(WBX_MAX_CLUSTER_CELLS, 0);
		...
		for (ChunkCells::const_iterator chunkCellsLocalIter = chunkCellsLocal.begin(); chunkCellsLocalIter != chunkCellsLocal.end(); ++chunkCellsLocalIter)
			tempVector[WBX_GET_CELL_INDEX(*chunkCellsLocalIter)] = *chunkCellsLocalIter;
		...
		for (ChunkCells::const_iterator chunkCellsToCopyIter = chunkCellsToCopy.begin(); chunkCellsToCopyIter != chunkCellsToCopy.end(); ++chunkCellsToCopyIter)
			tempVector[WBX_GET_CELL_INDEX(*chunkCellsToCopyIter)] = *chunkCellsToCopyIter;
		
		//clear local data
		ChunkCells().swap(chunkCellsLocal);
		chunkCellsLocal.reserve(WBX_MAX_CLUSTER_CELLS);
...
public:
	...
	{
		WBXASSERT(action==Create || action == Change);
		WBXASSERT(next.action!=Delete);
		if (next.parent)
	...
	void onSetWaypoint()
	{
		...
		for (ClusterCells::iterator iter = clusterCells.begin(); iter != clusterCells.end(); ++iter)
		{
			...
			tempVector.reserve(WBX_MAX_CLUSTER_CELLS);
	...
	void recordCreate()
	{
		WBXASSERT(action==Delete || action==None);
	...
	ChunkCells& recordClusterDataGetChunk(const SpatialRegion::Id& chunkIndex)
	{
		...
		if (clusterCellsIter == clusterCells.end())
		{
			ChunkCells cells(WBX_MAX_CLUSTER_CELLS, 0);
	...
	void recordClusterDataSetCell(ChunkCells& cells, const Vector3int16 &cellPos, const Voxel::Cell &cellDetail, const Voxel::CellMaterial &material)
	{
		...
		unsigned int cellData = WBX_SET_CELL_DATA(cellIndex, Voxel::Cell::asUnsignedCharForDeprecatedUses(cellDetail), material);
	...
	void updateClusterData(const SpatialRegion::Id &chunkIndex, const Vector3int16 &cellPos, const Voxel::Cell &cellDetail, const Voxel::CellMaterial &material)
	{
		...
		for (auto& cell: cells)
		{
			if (WBX_GET_CELL_INDEX(cell) == cellIndex)
			{
				cell = cellData;
				break;
			}
	...
	void play() const
	{
		...
		{
			case Delete:
				//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService Delete %s 0x%X\n", instance->getName().c_str(), instance.get());
				instance->setParent(NULL);
				break;
			case Change:
			{
				//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService Change %s 0x%X\n", instance->getName().c_str(), instance.get());
			...
			case Create:
			case Replicate:
				//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService Create %s 0x%X\n", instance->getName().c_str(), instance.get());
				std::for_each(properties.begin(), properties.end(), boost::bind(&Item::apply, this, _1));
	...
	void unplay() const
	{
		switch (action)
		{
			case Delete:
				...
				break;
			case Change:
				//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService Un-Change %s 0x%X\n", instance->getName().c_str(), instance.get());
				unplayChange();
				break;
			case Create:
				//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService Un-Create %s 0x%X\n", instance->getName().c_str(), instance.get());
				instance->setParent(NULL);
				break;
            default:
                break;
	...
	bool getCellData(const SpatialRegion::Id& chunkIndex, boost::uint32_t cellIndex, unsigned int &cellData)
	{
		...
		unsigned int compareData = WBX_SET_CELL_DATA(cellIndex, 0, 0);
...
public:
	...
	{
		WBXASSERT_SLOW(!findItem(item.instance.get()));
	...
	void absorb(const Waypoint* next)
	{
		...
		for (size_t i = 0; i < itemsOrdered.size(); ++i)
		{
			...
			{
				//				WBXASSERT(src.action==Item::Create);	// TODO  - this assert is going off
				addItem(src);
			}
		}
	}
...
void ChangeHistoryService::Item::unplayClusterData(const std::pair<SpatialRegion::Id, ChunkCells> &chunkCellData) const
{
	...
	for (ChunkCells::const_iterator chunkCellsIter = chunkCells.begin(); chunkCellsIter != chunkCells.end(); ++chunkCellsIter)
	{
		cellIndexInChunk = WBX_GET_CELL_INDEX(*chunkCellsIter);
			...
			if (item)
			{
				if (item->getCellData(chunkIndex, cellIndexInChunk, cellData))
				{
					changeHistory->setCell(chunkIndex, cellPosInChunk,
						Voxel::Cell::readUnsignedCharFromDeprecatedUse(WBX_GET_CELL_DETAIL(cellData)),
						WBX_GET_CELL_MATERIAL(cellData));
					cellFound = true;
					break;
				}
			}
...
void ChangeHistoryService::mergeFirstTwoWaypoints()
{
	...
	//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService::mergeFirstTwoWaypoints %s\n", (*next)->name.c_str());
...
void ChangeHistoryService::trimWaypoints()
{
	...
	if (playWaypoint != waypoints.end())
	{
		...
		{
			WBXASSERT(runStartWaypoint != waypoints.end());
			runStartWaypoint = waypoints.end();
		}
...
{
	WBXASSERT(recording==NULL);
	WBXASSERT(waypoints.size()==0);
	recording = new Waypoint(this);
	playWaypoint = waypoints.end();
	unplayWaypoint = waypoints.begin();
...
void ChangeHistoryService::onItemAdded(shared_ptr<Instance> instance)
{
	...
	//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "ChangeHistoryService::onItemAdded %s\n", instance->getDescriptor().name.c_str());
	
	Item& item = isReplicatedChange() ? (*waypoints.begin())->getItem(instance) : recording->getItem(instance);
	item.recordCreate();

	shared_ptr<wbx::MegaClusterInstance> cluster = Instance::fastSharedDynamicCast<wbx::MegaClusterInstance>(instance);
	if(cluster)
	{
		WBXASSERT(megaClusterInstance == NULL);
...
void ChangeHistoryService::onItemRemoved(shared_ptr<Instance> instance)
{
	...
	{
		WBXASSERT(megaClusterInstance != NULL);
		WBXASSERT(megaClusterInstance == cluster);
...
void ChangeHistoryService::onItemChanged(shared_ptr<Instance> instance, const PropertyDescriptor* descriptor)
{
	...
	if ( instance->fastDynamicCast<Camera>() && (*descriptor != wbx::Instance::desc_Name) && (*descriptor != Instance::propParent) )
		return;
...
void ChangeHistoryService::Waypoint::play()
{
	...
	for (size_t i = 0; i < itemsOrdered.size(); ++i)
    {
        ...
        {
            wbx::StandardOut::singleton()->print(wbx::MESSAGE_WARNING, ex.what());
        }
    }
...
void ChangeHistoryService::Waypoint::selectModifiedParts(bool isPlay)
{
	...
	if (pWorkspace)
	{
		wbx::Selection* pSelection = wbx::ServiceProvider::create<wbx::Selection>(pWorkspace);
		if (pSelection)
		{
			...
			boost::unordered_set<shared_ptr<wbx::Instance> > instancesToSelect;
			boost::unordered_set<wbx::Instance*> instancesParent;
			...
			for (boost::unordered_set<shared_ptr<wbx::Instance> >::iterator iter = instancesToSelect.begin(); iter != instancesToSelect.end(); ++iter)
...
void ChangeHistoryService::playLua()
{
	...
	WBXASSERT(playWaypoint!=waypoints.begin());
...
void wbx::ChangeHistoryService::reportMissedPhysicsChanges(shared_ptr<wbx::Instance> instance)
{
	WBXASSERT(enabled);
	WBXASSERT(!playing);
	WBXASSERT(recording);
	...
	{
		WBXASSERT(isRecordable(instance.get()));

		Waypoints::iterator waypoint = unplayWaypoint;
		WBXASSERT(waypoint != waypoints.end());
		...
		while (true) 
		{
			if (Item* item = (*waypoint)->findItem(instance.get()))
			{
				if (!foundCFrame)
				{
					...
					{
						const wbx::CoordinateFrame& v = iter2->second.cast<wbx::CoordinateFrame>();
				...
				if (!foundLinearVelocity)
				{
					...
					if (iter2 != item->properties.end())
					{
						const wbx::Vector3& v = iter2->second.cast<wbx::Vector3>();
