...
{
	WBXASSERT(Network::Players::backendProcessing(serviceProvider));
...
void Accoutrement::onEvent_RemovedBackend(shared_ptr<Instance> child)
{
	if (child.get() != this)	// tool parent changed is handled in ancestor stuff, because this might not be hooked up
	{
		...
		{
			WBXASSERT(backendAccoutrementState >= EQUIPPED);
...
void Accoutrement::onEvent_AttachmentAdjusted(const wbx::Reflection::PropertyDescriptor* propertyDescriptor)
{
	WBXASSERT(Network::Players::backendProcessing(this));

	if (propertyDescriptor == &wbx::Attachment::prop_Frame)
	{
		updateWeld();
	}
}
