...
namespace {
	...
	{
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DataStorePersistence");
	}

	static inline void sendDataStoreOldNamingSchemeStats()
	{
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DataStoreOldNamingScheme");
	}
}
...
namespace WBX {
	...
	void DataStoreService::onRequestFinishReport(DataStoreService::HttpRequest* request, bool isError, std::string errorMessage)
	{
		...
		if (isError)
		{
			...
			WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "DataStoreError", errorMessage.c_str());
		}
		else
		{
	...
	void DataStoreService::HttpRequest::execute(DataStoreService* dataStoreService)
	{
		...
		// todo: use HttpWbxApiService to throttle datastores instead of doing in this class
		if (DFFlag::UseNewDataStoreLogging)
		{
