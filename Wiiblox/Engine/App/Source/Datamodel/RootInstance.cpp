...
void RootInstance::movePartsToCameraFocus(PartArray& partArray)
{
	...
	wbx::Plugin *activePlugin = PluginManager::singleton()->getActivePlugin(DataModel::get(this));
...
{
	WBXASSERT(requestedParent && this->contains(requestedParent));
...
void RootInstance::publicInsertRaw(const Instances& instances, Instance* requestedParent, PartArray& partArray, bool joinPartsInInstancesOnly, bool suppressPartMove)
{
	...
	if( wbx::GameBasicSettings::singleton().inStudioMode() ) // only works in studio (InsertService::Insert calls this function)
...
void RootInstance::insertDecal(Decal *d, wbx::InsertMode insertMode)
{
										...
										bool lerpCameraInStudio)
{
	...
	if (requestedParent && !this->contains(requestedParent))
	{
        ...
        for (size_t i = 0; i < instances.size(); ++i)
        {
            ...
            if (!pInstance)
                continue;
            if(dynamic_cast<wbx::Service*>(pInstance))
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Do Menu Insert->Service, to insert a Service");
            else
	...
	else
	{
		...
		for (size_t i = 0; i < instances.size(); ++i)
		{
			...
			else if(dynamic_cast<wbx::Service*>(instance))// Do not allow insertion of services from Toolbox or any other place. Insert Service Dialog does not call this fn so it is fine.
			{
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Do Menu Insert->Service, to insert a Service");
			}
			else
			{
				...
				if (decal)
				{
					...
					if (insertMode != INSERT_TO_3D_VIEW)
					{
						...
						if (Instance::fastDynamicCast<wbx::PartInstance>(requestedParent))
