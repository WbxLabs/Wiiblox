...
void SurfaceTool::render3dAdorn(Adorn* adorn)
{
	...
	if (partInstance) {
		WBXASSERT(surface.getPartInstance());
...
DecalTool::DecalTool(Workspace* workspace, Decal *decal, wbx::InsertMode insertMode)
: Named<SurfaceTool, sDecalTool>(workspace)	
{
	this->decal = shared_from(decal);
	this->insertMode = insertMode;
	parentIsPart = Instance::fastDynamicCast<wbx::PartInstance>(decal->getParent()) != NULL;
}
...
void DecalTool::onMouseHover(const shared_ptr<InputObject>& inputObject)
{
	...
	if ((insertMode == wbx::INSERT_TO_3D_VIEW) && !parentIsPart)
	{
		if (partInstance)
		{
...
shared_ptr<MouseCommand> DecalTool::onMouseUp(const shared_ptr<InputObject>& inputObject)
{
	...
	if (Instance::fastDynamicCast<wbx::PartInstance>(decal->getParent()) != NULL)
	{
		parentIsPart = true;
