...
namespace WBX
{
	...
	void CSGDictionaryService::reparentAllChildData()
	{
		...
		std::vector<shared_ptr<wbx::Instance> > children;

		children.insert(children.begin(), getChildren()->begin(), getChildren()->end());

		for (std::vector<shared_ptr<wbx::Instance> >::const_iterator iter = children.begin(); iter != children.end(); ++iter)
			reparentChildData(*iter);
	}

	void CSGDictionaryService::reparentChildData(shared_ptr<wbx::Instance> childInstance)
	{
	...
	void CSGDictionaryService::refreshRefCountUnderInstance(wbx::Instance* instance)
	{
    ...
    boost::shared_ptr<CSGMesh> CSGDictionaryService::insertMesh(const std::string key, const wbx::BinaryString& meshData)
    {
    ...
    boost::shared_ptr<CSGMesh> CSGDictionaryService::getMesh(PartOperation& partOperation)
    {
   		...
   		if (keyIn.value().size() > minKeySize)
		{
			...
			if (strncmp(keyIn.value().c_str(), localKeyTag.c_str(), minKeySize) == 0)
			{
				...
				{
					if (shared_ptr<wbx::BinaryStringValue> meshData = instanceMap.at(key).ref.lock())
