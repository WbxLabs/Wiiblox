...
namespace WBX {
    ...
    void ScreenGui::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
    {
        ...
        if (newProvider)
        {
            ...
            {
                if (shared_ptr<wbx::ScreenGui> coreScreenGui = coreGui->getWiibloxScreenGui())
                {
    ...
    GuiResponse ScreenGui::processGesture(const UserInputService::Gesture gesture, shared_ptr<const wbx::Reflection::ValueArray> touchPositions, shared_ptr<const Reflection::Tuple> args)
    {
        return Super::processGesture(gesture,touchPositions,args);
    }

	bool ScreenGui::removeModalButton(wbx::GuiButton* guiButton)
	{
	...
	bool ScreenGui::insertModalButton(wbx::GuiButton* guiButton)
	{
		for(std::vector<GuiButton*>::iterator iter = modalGuiObjects.begin(); iter != modalGuiObjects.end(); ++iter)
			if( (*iter) == guiButton )
				return false;
	...
	void ScreenGui::onModalButtonChanged(const wbx::Reflection::PropertyDescriptor* desc, wbx::GuiButton* guiButton)
	{
	...
	void ScreenGui::onDescendantAdded(Instance* instance)
	{
		...
		if(wbx::GuiButton* guiButton = Instance::fastDynamicCast<wbx::GuiButton>(instance))
		{
	...
	void ScreenGui::onDescendantRemoving(const shared_ptr<Instance>& instance)
	{
		...
		if(wbx::GuiButton* guiButton = Instance::fastDynamicCast<wbx::GuiButton>(instance.get()))
		{
			removeModalButton(guiButton);
