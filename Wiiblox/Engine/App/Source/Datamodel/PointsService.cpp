...
namespace WBX
{
	...
	void PointsService::startAwardPointsBatching(weak_ptr<wbx::DataModel> weakDm)
	{
		if (shared_ptr<wbx::DataModel> dm = weakDm.lock())
		{
	...
	static void GetHelperSuccess(weak_ptr<DataModel> weakDm, shared_ptr<std::string> methodName, shared_ptr<std::string> keyToCheck, std::string response, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		{
			wbx::PointsService* pointsService = ServiceProvider::find<wbx::PointsService>(dm.get());
			if (!pointsService)
			{
				errorFunction( wbx::format("%s failed because PointsService is unavailable.",methodName->c_str()) );
				return;
			}

			if (!response.empty())
			{
				shared_ptr<const Reflection::ValueTable> responseTable(wbx::make_shared<const Reflection::ValueTable>());
				if(!WebParser::parseJSONTable(response,responseTable))
				{
					errorFunction( wbx::format("%s failed because could not parse JSON response",methodName->c_str()) );
					return;
				}
				...
				if (responseTable && responseTable->size() > 0)
				{
					...
					{
						errorFunction( wbx::format("%s failed because %s",methodName->c_str(),e.what()) );
						return;
					}
				...
				{
					errorFunction( wbx::format("%s failed because JSON table is empty or null.",methodName->c_str()) );
					return;
				}
			}
			else
			{
				errorFunction( wbx::format("%s failed because no valid response or exception.", methodName->c_str()) );
				return;
			}
		}
		else
		{
			errorFunction( wbx::format("%s failed because could not lock datamodel in response.", methodName->c_str()) );
			return;
		}
	}
	static void GetHelperError(weak_ptr<DataModel> weakDm, shared_ptr<std::string> methodName, std::string error, boost::function<void(std::string)> errorFunction)
	{
		if (!error.empty())
		{
			errorFunction( wbx::format("%s failed because %s", methodName->c_str(), error.c_str()) );
		}
		else
		{
			errorFunction( wbx::format("%s failed because no valid response or exception.", methodName->c_str()) );
	...
	void PointsService::internalGetPointBalance(int userId, int placeId, shared_ptr<std::string> methodName, shared_ptr<std::string> keyToReturn, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		{
			errorFunction( wbx::format("%s failed because not called from server script.",methodName->c_str()) );
			return;
		}

		if (userId <= 0 )
		{
			errorFunction( wbx::format("%s failed because userId <= 0, please supply a positive userId.",methodName->c_str()) );
			return;
		}
		...
		if (!dm)
		{
			errorFunction( wbx::format("%s failed because of null datamodel.",methodName->c_str()) );
			return;
		}

		std::string parameters = wbx::format("?userId=%d",userId);
		std::string apiBaseUrl = ServiceProvider::create<ContentProvider>(this)->getUnsecureApiBaseUrl();

		if (placeId > 0)
		{
			parameters += wbx::format("&placeId=%d",placeId);
		}
		...
		try
		{
			...
			{
				apiService->getAsync( getPointBalanceHttp, wbx::PRIORITY_DEFAULT,
	...
	{
		WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "HasAwardedAPoint", format("%d", placeId).c_str());
	}
	...
	static void processAwardPointsResponseBatchSuccess(weak_ptr<DataModel> weakDm, int userId, PointsService::AwardPointYieldFunctions yieldingFunctions, std::string response)
	{
		if (shared_ptr<DataModel> dm = weakDm.lock())
		{
			wbx::PointsService* pointsService = NULL;
			{
				pointsService = ServiceProvider::find<wbx::PointsService>(dm.get());
				if (!pointsService)
				{
					wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Processing PointsService:AwardPoints response and could not find PointsService, userId = %i",userId);
					return;
				}
			}
			...
			if (!response.empty())
			{
				shared_ptr<const Reflection::ValueTable> awardPointsTable(wbx::make_shared<const Reflection::ValueTable>());
				...
				if (awardPointsTable && awardPointsTable->size() > 0)
				{
					...
					if (successVariant.isType<bool>())
					{
						...
						if (success)
						{
							...
							{
								fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints response, but failed because %s. userId = %i",e.what(), userId).c_str());
								return;
							}
						...
						{
							fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints response, but not successful award. userId = %i", userId).c_str());
							return;
						}
					}
					else
					{
						fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints response, but JSON table is empty. userId = %i",userId).c_str());
						return;
					}
				}
				else
				{
					fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints response, but JSON table is null. userId = %i",userId).c_str());
					return;
				}
			}
			else
			{
				fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints response, but response has length of 0. userId = %i",userId).c_str());
				return;
			}
		}
		else
		{
			fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints but could not get lock on datamodel. userId = %i",userId).c_str());
			return;
		}
	}
	...
	{
		fireErrorFunctions(yieldingFunctions, wbx::format("Processing PointsService:AwardPoints error: %s. userId = %i", error.c_str(), userId).c_str());
	}
		...
		if (DataModel* dm = DataModel::get(this))
		{
			...
			{	
				if (isAtAwardPointsLimit())
				{
					...
					fireErrorFunctions(iter->second.second, wbx::format("%s failed because the max number of calls per minute has been exceeded. Don't call this more than %i times per minute per user.", AWARD_POINTS_METHOD_NAME, DFInt::MaxAwardPointsHttpCallsPerMinute).c_str());
					continue;
				}
				...
				try
				{
					...
					if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
					{
						const std::string formattedPath = wbx::format(("points/award-points" + parameters).c_str(),dm->getPlaceID(),userId,amount);

						apiService->postAsync( formattedPath.c_str(), postData, false, wbx::PRIORITY_DEFAULT, HttpService::TEXT_PLAIN,
				...
				{
					fireErrorFunctions(iter->second.second, wbx::format("%s failed because %s",e.what(), AWARD_POINTS_METHOD_NAME).c_str());
	...
	void PointsService::awardPoints(int userId, int amount, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		if (!Network::Players::backendProcessing(this))
		{
			errorFunction( wbx::format("%s failed because not called from server script.", AWARD_POINTS_METHOD_NAME) );
			return;
		}

		if (userId <= 0 )
		{
			errorFunction( wbx::format("%s failed because userId <= 0, please supply a positive userId.", AWARD_POINTS_METHOD_NAME) );
			return;
		}

		if (amount == 0)
		{
			errorFunction( wbx::format("%s failed because trying to award zero points, please supply a non-zero amount.", AWARD_POINTS_METHOD_NAME) );
			return;
		}
		...
		if (!dm)
		{
			errorFunction( wbx::format("%s failed because of null datamodel.", AWARD_POINTS_METHOD_NAME) );
			return;
		}
