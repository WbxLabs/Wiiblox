...
const char* const wbx::sTeleportService	= "TeleportService";
...
static Reflection::BoundCallbackDesc<bool(std::string,int,std::string)> callback_ConfirmationCallback("ConfirmationCallback", &TeleportService::confirmationCallback, "message", "placeId", "spawnName", Security::WiibloxScript);
static Reflection::BoundCallbackDesc<void(std::string)> errorCallback("ErrorCallback", &TeleportService::showErrorCallback, "message", Security::WiibloxScript);
static Reflection::BoundFuncDesc<TeleportService, void()> func_TeleportCancel(&TeleportService::TeleportCancel, "TeleportCancel", Security::WiibloxScript);
...
{
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "Teleport", label);
}
}
...
static std::string ReadStringValue(shared_ptr<const Reflection::ValueTable> jsonResult, std::string name) 
{
    ...
    {
        throw std::runtime_error(wbx::format("Unexpected string result for %s", name.c_str()));
    }
}
...
static int ReadIntValue(shared_ptr<const Reflection::ValueTable> jsonResult, std::string name)
{
    ...
    {
        throw std::runtime_error(wbx::format("Unexpected int result for %s", name.c_str()));
    }
}
...
{
    WBXASSERT(!Workspace::serverIsPresent(this));
	...
	try
	{
		...
		if (FFlag::UseBuildGenericGameUrl)
		{
			...
			else if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToReservedServer)
			{
				...
				_spawnName = teleportInfo->at("spawnName").get<std::string>();
				char urlBuf[2048] = {0};
				sprintf_s(urlBuf, 2048, "Game/PlaceLauncher.ashx?request=RequestPrivateGame&placeId=%d&accessCode=%s&linkCode=&privateGameMode=ReservedServer%s", teleportInfo->at("placeId").get<int>(), wbx::Http::urlEncode(teleportInfo->at("reservedServerAccessCode").get<std::string>()).c_str(), extraStuff);
				url = BuildGenericGameUrl(_baseUrl, urlBuf);
			}
		}
		else
		{
			...
			else if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToReservedServer)
			{
				// to-reserved server teleport
				char urlBuf[2048] = {0};
				sprintf_s(urlBuf, 2048, "%s/Game/PlaceLauncher.ashx?request=RequestPrivateGame&placeId=%d&accessCode=%s&linkCode=&privateGameMode=ReservedServer%s", _baseUrl.c_str(), teleportInfo->at("placeId").get<int>(), wbx::Http::urlEncode(teleportInfo->at("reservedServerAccessCode").get<std::string>()).c_str(), extraStuff);
				url = urlBuf;
				_spawnName = teleportInfo->at("spawnName").get<std::string>();
			}
		}
        ...
        retryTimer.reset();
        teleportThread.reset(new boost::thread(wbx::thread_wrapper(boost::bind(&TeleportService::TeleportThreadImpl, shared_from(this), teleportInfo), "Teleport Thread")));
    }
        ...
        else
        {
            try
            {
                ...
                std::istringstream input("");
                    wbx::Http(url).post(input, wbx::Http::kContentTypeDefaultUnspecified, false, response);
                }
                else
                {
                ...
                jsonStream << response;
                shared_ptr<const Reflection::ValueTable> jsonResult(wbx::make_shared<const Reflection::ValueTable>());
                ...
                else if (retries<0)
                {
					...
					try
					{
						...
						if (Network::Player* player = Network::Players::findLocalPlayer(this))
						{
							...
							WBXASSERT(teleportInfo->at("placeId").get<int>() >= -1);
...
void TeleportService::GetPlayerPlaceInstanceAsync(int playerId, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
	{
		...
		apiService->getAsync(url, true, wbx::PRIORITY_SERVER_ELEVATED,
...
void TeleportService::TeleportToPrivateServer(int placeId, std::string reservedServerAccessCode, shared_ptr<const Instances> players, std::string spawnName, Reflection::Variant teleportData, shared_ptr<Instance> customLoadingGUI)
{
	if (Workspace::serverIsPresent(this))
	{
		...
		char urlBuf[2048] = {0};
		sprintf_s(urlBuf, 2048, "reservedservers/grantaccess?reservedServerAccessCode=%s", wbx::Http::urlEncode(reservedServerAccessCode).c_str());
		url = urlBuf;
		...
		if (playerFound)
		{
			if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
			{
				...
				apiService->postAsync(url, playerIdsStream.str(), true, wbx::PRIORITY_SERVER_ELEVATED, HttpService::APPLICATION_URLENCODED,
...
void TeleportService::ReserveServer(int placeId, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	if (Workspace::serverIsPresent(this))
	{
		if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
		{
			...
			apiService->postAsync(url, "", true, wbx::PRIORITY_SERVER_ELEVATED, HttpService::APPLICATION_URLENCODED,
