...
, minDistance(0.0f)
, maxDistance(wbx::Camera::distanceMaxCharacter())
{
}

int ICharacterSubject::getControlMode() const
{
	return wbx::GameBasicSettings::singleton().getControlMode();
}

int ICharacterSubject::getCustomCameraMode() const
{
	return wbx::GameBasicSettings::singleton().getCameraModeWithDefault();
}
...
{
	if (wbx::GameBasicSettings::singleton().getRotationType() == wbx::GameBasicSettings::ROTATION_TYPE_CAMERA_RELATIVE)
	{
...
void ICharacterSubject::onCameraHeartbeat(const Vector3& cameraLocation, const Vector3& focusPoint)
{
	...
	wbx::GameBasicSettings& settings(wbx::GameBasicSettings::singleton());
...
void ICharacterSubject::setCameraMode(wbx::Camera::CameraMode value)
{
	if(value != cameraMode)
		if(value == wbx::Camera::CAMERAMODE_CLASSIC)
