...
const char* const wbx::sReplicatedFirst = "ReplicatedFirst";
...
static Reflection::EventDesc<ReplicatedFirst, void()> event_finishedReplicating(&ReplicatedFirst::finishedReplicatingSignal, "FinishedReplicating", Security::WiibloxScript);
static Reflection::BoundFuncDesc<ReplicatedFirst, bool()> func_getFinishedReplicating(&ReplicatedFirst::getIsFinishedReplicating, "IsFinishedReplicating", Security::WiibloxScript);

static Reflection::EventDesc<ReplicatedFirst, void()> event_loadingFinished(&ReplicatedFirst::removeDefaultLoadingGuiSignal, "RemoveDefaultLoadingGuiSignal", Security::WiibloxScript);
static Reflection::BoundFuncDesc<ReplicatedFirst, bool()> func_getLoadingFinished(&ReplicatedFirst::getIsDefaultLoadingGuiRemoved, "IsDefaultLoadingGuiRemoved", Security::WiibloxScript);
REFLECTION_END();
...
void ReplicatedFirst::gameIsLoaded()
{
	...
	if (wbx::Network::Players::frontendProcessing(this) && wbx::Network::Players::backendProcessing(this))
	{
...
{
	WBXASSERT(allInstancesHaveReplicated == false);
...
{	
	...
	if (wbx::Network::Players::frontendProcessing(script) && wbx::Network::Players::backendProcessing(script))
	{
		// in play solo, this is a valid configuration, do nothing
	}
	else
	{
		if (!wbx::Network::Players::frontendProcessing(script))
		{
			return false;
		}

		if (wbx::Network::Players::backendProcessing(script))
		{
			return false;
		}
	}
