...
void Attachment::setSecondaryAxisInPartInternal( const Vector3& _axis2 )
{
    ...
    axis2.unitize();
    WBXASSERT( fabsf( axis2.dot(axis) ) < 0.00001f );
...
float Attachment::intersectAdornWithRay( const WBXRay& r )
{
    Sphere s;
...
void Attachment::verifySetParent(const Instance* instance) const
{
    ...
    {
        throw wbx::runtime_error("Attachments can only be parented to parts or the workspace");
    }
}
...
void Attachment::verifyAddChild(const Instance* newChild) const
{
    ...
    {
        throw wbx::runtime_error("Attachments can't have children.");
    }
}
