...
namespace WBX
{
	...
	void FlyweightService::onChildAdded(shared_ptr<wbx::Instance> childInstance)
	{
		static boost::once_flag flag = BOOST_ONCE_INIT;
		boost::call_once(boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "UsingCSG", "none", 0, false), flag);

		if (shared_ptr<wbx::BinaryStringValue> strValue = wbx::Instance::fastSharedDynamicCast<wbx::BinaryStringValue>(childInstance))
		{
	...
	{
  	    boost::scoped_ptr<wbx::MD5Hasher> hasher(wbx::MD5Hasher::create());
        hasher->addData(data);
        return hasher->c_str();
	}
	...
	{
		WBXASSERT(str.size() > localKeyTag.size());
	...
	const BinaryString FlyweightService::peekAtData(const BinaryString& str)
	{
        ...
        if (hasHash)
		{
            ...
            if (instanceMap.count(key))
                if (shared_ptr<wbx::BinaryStringValue> strChild = instanceMap.at(key).ref.lock())
	...
	void FlyweightService::storeStringData(BinaryString& str, bool forceIncrement, const std::string& name)
	{
        ...
        if (!hasHash)
		{
            ...
            if (instanceMap.count(key))
            {
                shared_ptr<wbx::BinaryStringValue> strChild = instanceMap.at(key).ref.lock();
    ...
    void FlyweightService::removeStringData(const BinaryString& str)
    {
        ...
        if (isHashKey(str.value()))
        {
            ...
            {
                if (shared_ptr<wbx::BinaryStringValue> strChild = iter->second.ref.lock())
                {
	...
	void FlyweightService::cleanChildren()
	{
		...
		if (numChildren())
		{
			...
			for (wbx::Instances::const_iterator iter = getChildren()->begin(); iter != getChildren()->end(); ++iter)
			{
				if (shared_ptr<BinaryStringValue> stringValue = wbx::Instance::fastSharedDynamicCast<BinaryStringValue>(*iter))
				{
	...
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_OUTPUT, "Instance map size:%d ,Child count:%d", (int)instanceMap.size(), (int)numChildren());
		for(FlyweightInstanceMap::const_iterator iter = instanceMap.begin(); iter != instanceMap.end(); ++iter)
		{
			if (iter->first.c_str())
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_OUTPUT, "Count: %d, %s Value: %s", iter->second.count, dataType(iter->first).c_str(), iter->first.c_str());
		}
	}

	bool FlyweightService::isChildData(shared_ptr<wbx::Instance> childData)
	{
		if (shared_ptr<wbx::BinaryStringValue> bStrValue = wbx::Instance::fastSharedDynamicCast<wbx::BinaryStringValue>(childData))
			if (strncmp(bStrValue->getValue().value().c_str(), "<Wiiblox", 6) == 0)
				return true;
		return false;
	}
