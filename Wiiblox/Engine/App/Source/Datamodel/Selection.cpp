...
const char* const wbx::sSelection			= "Selection";
const char* const wbx::sFilteredSelection	= NULL;
...
{
		WBXASSERT_SLOW(connections.find(instance)==connections.end());
...
{
		WBXASSERT(connections.find(instance)!=connections.end());
		Connections::iterator iter = connections.find(instance);
		iter->second.disconnect();
		connections.erase(iter);
}

void Selection::propagateChangeSignalToLua(const wbx::SelectionChanged& event) {
	luaSelectionChanged();
}
...
{
	WBXASSERT(DataModel::get(this)->write_requested);
...
void Selection::addToSelection(Instance* instance)
{
	...
	WBXASSERT(!DataModel::get(this) || DataModel::get(this)->write_requested);
	shared_ptr<Instance> shared = shared_from(instance);
