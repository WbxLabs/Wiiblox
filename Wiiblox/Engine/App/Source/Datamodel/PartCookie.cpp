...
unsigned int PartCookie::compute(wbx::PartInstance* part)
{
	...
	if (const copy_on_write_ptr<Instances>& children = part->getChildren())
	{
		for (size_t i = 0; i < children->size(); ++i)
		{
			...
			else if (wbx::Decal* decal = Instance::fastDynamicCast<wbx::Decal>(instance))
			{
				if (!decal->getTexture().isNull())
				{
					cookie |= HAS_DECALS;

					if (decal->getFace() == wbx::NORM_Z_NEG)
					{
