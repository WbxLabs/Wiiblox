...
void renderClickDetector(shared_ptr<wbx::Instance> descendant, Adorn* adorn, int cycle)
{
...
void ClickDetector::render3dAdorn(Adorn* adorn) 
{
	...
	WBXASSERT(!dynamic_cast<Workspace*>(parent));
...
void ClickDetector::fireMouseClick(float distance, wbx::Network::Player* player)
{
	if (distance < maxActivationDistance) 
		desc_MouseClick.fireAndReplicateEvent(this, shared_from(player));
}

bool ClickDetector::isClickable(shared_ptr<PartInstance> part, float distanceToCharacter, bool raiseClickedEvent, wbx::Network::Player* player)
{
...
bool ClickDetector::updateLastHoverPart(shared_ptr<Instance> newHover, wbx::Network::Player* player)
{
...
void ClickDetector::fireMouseHover(wbx::Network::Player* player)
{
	desc_MouseHoverEnter.fireAndReplicateEvent(this, shared_from(player));
}

void ClickDetector::fireMouseHoverLeave(wbx::Network::Player* player)
{
	desc_MouseHoverLeave.fireAndReplicateEvent(this, shared_from(player));
	lastHoverPart = shared_ptr<PartInstance>();
}

void ClickDetector::stopHover(shared_ptr<PartInstance> part, wbx::Network::Player* player)
{
	if(part && part.get())
	{
...
bool ClickDetector::isHovered(PartInstance *part, float distanceToCharacter, bool raiseHoveredEvent, wbx::Network::Player* player)
{
	if (part)
	{
