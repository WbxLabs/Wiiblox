...
void PhysicsInstructions::changeSimulationRadius(wbx::Network::Player* dPhysPlayer, float change)
{
	timeSinceLastRadiusChange = 0.0;

	WBXASSERT(dPhysPlayer);
...
void PhysicsInstructions::changeMaxSimulationRadius(wbx::Network::Player* dPhysPlayer, float change)
{
	WBXASSERT(dPhysPlayer);
...
void PhysicsInstructions::setThrottlesBase(wbx::Network::Player* dPhysPlayer, Workspace* workspace, bool realTimePerfOK, bool dutyPerfOK, double avgDutyPercent, double dt)
{
	if (realTimePerfOK)
	{
		if (!dutyPerfOK)
...
void PhysicsInstructions::setCyclicThrottles(wbx::Network::Player* dPhysPlayer, Workspace* workspace, double cyclicDt, double dt, double dutyDt)
{
...
void PhysicsInstructions::setThrottles(wbx::Network::Player* dPhysPlayer, Workspace* workspace, double dt, double dutyDt)
{
	averageDt.sample(dt);
	averageDutyDt.sample(dutyDt);
