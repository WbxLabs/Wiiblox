...
BaseRenderJob::BaseRenderJob(double minFps, double maxFps, boost::shared_ptr<DataModel> dataModel)
	: wbx::DataModelJob("Render", wbx::DataModelJob::Render, false, dataModel, wbx::Time::Interval(.02))
	, isAwake(true)
	, minFrameRate(minFps)
	, maxFrameRate(maxFps)
	{
...
{
	return wbx::TaskScheduler::singleton().isCyclicExecutive() && cyclicExecutive;
}
...
{
	return Time::now<wbx::Time::Fast>() - lastRenderTime;
}
