...
const char* const wbx::sGlobalAdvancedSettings		= "GlobalSettings";
const char* const wbx::sGlobalBasicSettings			= "UserSettings";
const char* const wbx::sSettings					= "GenericSettings";
const char* const wbx::sSettingsItem				= NULL;
...
static Reflection::BoundFuncDesc<GlobalAdvancedSettings, shared_ptr<const wbx::Reflection::ValueTable>()> fun_getFastVariables(&GlobalAdvancedSettings::getFVariables, "GetFVariables", Security::WiibloxScript);
...
{
	settingsErased = true;
#if defined(_WIN32) && !defined(WBX_PLATFORM_UWP)
...
void Settings::loadState(const std::string& optGlobalSettingsFile)
{
	...
	{
		StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Exception thrown in loadState %s", e.what());
	}
}
...
void Settings::verifyAddDescendant(const Instance* newParent, const Instance* instanceGettingNewParent) const {
	...
	if (invalid) {
		throw wbx::runtime_error("Not allowed to add that under settings");
	}
...
{
	boost::filesystem::path file = wbx::FileSystem::getUserDirectory(true, wbx::DirAppData);
...
{
	if (FFlag::DisableGlobalSettingsParentChange && (wbx::Security::Context::current().identity != wbx::Security::Anonymous))
	{
		try {
			wbx::Security::Context::current().requirePermission(wbx::Security::WiibloxScript, "set settings parent");
		}
		catch (wbx::base_exception& e)
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Insufficient permissions to set settings parent");
			throw e;
		}
	}
...
bool wbx::GlobalBasicSettings::isUserFeatureEnabled( std::string name )
{
	std::string user("User");

	if (name.compare(0, user.length(), user) != 0)
	{
		throw std::runtime_error(wbx::format("Flag %s does not exist", name.c_str()));
	}
...
/*override*/ void GlobalBasicSettings::verifySetParent(const Instance* instance) const
{
	...
	{
		try {
			wbx::Security::Context::current().requirePermission(wbx::Security::WiibloxScript, "set UserSettings parent");
		} 
		catch (wbx::base_exception& e) 
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Insufficient permissions to set UserSettings parent");
			throw e;
		}
	}
	Super::verifySetParent(instance);
...
{
	wbx::Reflection::ValueTable* table = reinterpret_cast<wbx::Reflection::ValueTable*>(context);

	(*table)[name] = value;
}

shared_ptr<const wbx::Reflection::ValueTable> wbx::GlobalAdvancedSettings::getFVariables()
{
...
bool wbx::GlobalAdvancedSettings::getFFlag( std::string name )
{
	std::string result;
	if (FLog::GetValue(name.c_str(), result, true))
	{
...
std::string wbx::GlobalAdvancedSettings::getFVariable( std::string name )
{
	std::string result;
