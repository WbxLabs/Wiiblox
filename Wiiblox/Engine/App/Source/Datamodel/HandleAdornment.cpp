/**
 * HandleAdornment.cpp
 * Copyright (c) 2015 Wiiblox Corp. All Rights Reserved.
...
CoordinateFrame HandleAdornment::getWorldCoordinateFrame() const
{
	...
	{
		if (shared_ptr<PartInstance> partInstance = wbx::Instance::fastSharedDynamicCast<PartInstance>(pvInstance))
		{
		...
		else if (shared_ptr<ModelInstance> modelInstance = wbx::Instance::fastSharedDynamicCast<ModelInstance>(pvInstance))
		{
			wbx::Part pvPart = modelInstance->computePart();
...
bool BoxHandleAdornment::isCollidingWithHandle(const shared_ptr<InputObject>& inputObject)
{
	...
	{
		WBXRay gridRay(MouseCommand::getUnitMouseRay(inputObject, workspace));

		CoordinateFrame worldFrame = getWorldCoordinateFrame();
		WBXRay localRay = worldFrame.toObjectSpace(gridRay);
...
bool LineHandleAdornment::isCollidingWithHandle(const shared_ptr<InputObject>& inputObject)
{
	if (Workspace * workspace = Workspace::findWorkspace(this))
	{
		...
		wbx::Camera* camera = workspace->getCamera();
...
ImageHandleAdornment::ImageHandleAdornment()
	...
	, image("WBXasset://textures/SurfacesDefault.png")
{
