...
: m_data(r.m_data)
{
    WBXASSERT(validate(m_data, false));
...
void NumberSequence::resample(Vector2* result, int numPoints, float minV, float maxV) const
{
    ...
    WBXASSERT( m_data.size() >= 2 );
    WBXASSERT( m_data.front().time == 0.0f ); // ==
    WBXASSERT( m_data.back().time == 1.0f );  // 
...
bool NumberSequence::validate(const std::vector<Key>& keys, bool exc)
{
    if (keys.size()<2)
    {
        if (exc) throw wbx::runtime_error("NumberSequence: requires at least 2 keypoints"); else return false;
    }
    
    if (keys.size()>kMaxSize)
    {
        if (exc) throw wbx::runtime_error("NumberSequence: max. number of keypoints exceeded."); else return false;
    }
    ...
    for (unsigned j=0; j<keys.size(); ++j)
    {
        ...
        {
            if (exc) throw wbx::runtime_error("NumberSequence: all keypoints must be ordered by time"); else return false;
        }
//        if (keys[j].value < 0)
//            if (exc) throw wbx::runtime_error("NumberSequence: value must be non-negative"); else return false;

        if (keys[j].envelope < 0 )
        {
            if (exc) throw wbx::runtime_error("NumberSequence: envelope must be non-negative"); else return false;
        }
        
        if (keys[j].value - keys[j].envelope < 0)
        {
            if (exc) throw wbx::runtime_error("NumberSequence: envelope must not exceed the value"); else return false;
        }
    }

    if (fabsf(keys.front().time) > 1e-4f)
    {
        if (exc) throw wbx::runtime_error("NumberSequence must start at time=0.0"); else return false;
    }
    
    if (fabsf(keys.back().time - 1.0f) > 1e-4f)
    {
        if (exc) throw wbx::runtime_error("NumberSequence must end at time=1.0"); else return false;
    }
