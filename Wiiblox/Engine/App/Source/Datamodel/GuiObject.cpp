...
static Reflection::EventDesc<GuiObject, void(shared_ptr<const wbx::Reflection::ValueArray>)> event_TapGesture(&GuiObject::tapGestureEvent, "TouchTap", "touchPositions", Security::None);
static Reflection::EventDesc<GuiObject, void(shared_ptr<const wbx::Reflection::ValueArray>,float,float,InputObject::UserInputState)> event_PinchGesture(&GuiObject::pinchGestureEvent, "TouchPinch", "touchPositions", "scale", "velocity", "state", Security::None);
...
static Reflection::EventDesc<GuiObject, void(shared_ptr<const wbx::Reflection::ValueArray>,InputObject::UserInputState)> event_LongPressGesture(&GuiObject::longPressGestureEvent, "TouchLongPress", "touchPositions", "state", Security::None);
static Reflection::EventDesc<GuiObject, void(shared_ptr<const wbx::Reflection::ValueArray>,float,float,InputObject::UserInputState)> event_RotateGesture(&GuiObject::rotateGestureEvent, "TouchRotate", "touchPositions","rotation", "velocity", "state", Security::None);
static Reflection::EventDesc<GuiObject, void(shared_ptr<const wbx::Reflection::ValueArray>,Vector2,Vector2,InputObject::UserInputState)> event_PanGesture(&GuiObject::panGestureEvent, "TouchPan", "touchPositions", "totalTranslation", "velocity", "state", Security::None);
...
{
template<>
EnumDesc<wbx::GuiObject::TweenEasingDirection>::EnumDesc()
:EnumDescriptor("EasingDirection")
{
	addPair(wbx::GuiObject::EASING_DIRECTION_IN, "In");
	addPair(wbx::GuiObject::EASING_DIRECTION_OUT, "Out");
	addPair(wbx::GuiObject::EASING_DIRECTION_IN_OUT, "InOut");
}
template<>
wbx::GuiObject::TweenEasingDirection& Variant::convert<wbx::GuiObject::TweenEasingDirection>(void)
{
	return genericConvert<wbx::GuiObject::TweenEasingDirection>();
}

template<>
EnumDesc<wbx::GuiObject::TweenEasingStyle>::EnumDesc()
:EnumDescriptor("EasingStyle")
{
	addPair(wbx::GuiObject::EASING_STYLE_LINEAR,	"Linear");
	addPair(wbx::GuiObject::EASING_STYLE_SINE,		"Sine");
	addPair(wbx::GuiObject::EASING_STYLE_BACK,		"Back");
	addPair(wbx::GuiObject::EASING_STYLE_QUAD,		"Quad");
	addPair(wbx::GuiObject::EASING_STYLE_QUART,	"Quart");
	addPair(wbx::GuiObject::EASING_STYLE_QUINT,	"Quint");
	addPair(wbx::GuiObject::EASING_STYLE_BOUNCE,	"Bounce");
	addPair(wbx::GuiObject::EASING_STYLE_ELASTIC,	"Elastic");
}
template<>
wbx::GuiObject::TweenEasingStyle& Variant::convert<wbx::GuiObject::TweenEasingStyle>(void)
{
	return genericConvert<wbx::GuiObject::TweenEasingStyle>();
}

template<>
EnumDesc<wbx::GuiObject::TweenStatus>::EnumDesc()
:EnumDescriptor("TweenStatus")
{
	addPair(wbx::GuiObject::TWEEN_CANCELED,		"Canceled");
	addPair(wbx::GuiObject::TWEEN_COMPLETED,	"Completed");
}
template<>
wbx::GuiObject::TweenStatus& Variant::convert<wbx::GuiObject::TweenStatus>(void)
{
	return genericConvert<wbx::GuiObject::TweenStatus>();
}
}
template<>
...
template<>
bool wbx::StringConverter<GuiObject::TweenEasingDirection>::convertToValue(const std::string& text, GuiObject::TweenEasingDirection& value)
{
...
template<>
bool wbx::StringConverter<GuiObject::TweenStatus>::convertToValue(const std::string& text, GuiObject::TweenStatus& value)
{
...
void GuiObject::UpdateTween(Tween& tween, GuiObject* obj, boost::function<void(GuiObject*, UDim2)> updateFunc, float timeStep)
{
	...
	if(tween.elapsedTime >= tween.totalTime)
	{
		...
		if(tween.callback)
		{
			...
			{
				if (TweenService* tweenService = wbx::ServiceProvider::create<TweenService>(obj))
				{
...
bool GuiObject::tweenPositionDelay(UDim2 startPosition, UDim2 endPosition, float time, TweenEasingDirection style, TweenEasingStyle variance, float delayTime, bool overwrite, boost::function<void(TweenStatus)> callback, TweenService* tweenService)
{
	...
	WBXASSERT(!tweens->positionTween);
...
void GuiObject::handleDrag(wbx::Vector2 mousePosition)
{	
	if(mousePosition != lastMousePosition)
	{
		Vector2 mouseDelta = mousePosition - lastMousePosition;
		setPosition(wbx::UDim2(position.x.scale,position.x.offset + mouseDelta.x, position.y.scale,position.y.offset + mouseDelta.y));
...
{
    WBXASSERT(isFinite(value.x.scale) || isFinite(value.y.scale));
...
{
	if(wbx::Security::Context::current().hasPermission(wbx::Security::WiibloxScript)){
...
void GuiObject::renderSelectionFrame(Adorn* adorn, GuiObject* baseSelectionObject)
{
	...
	if (baseSelectionObject && baseSelectionObject->getVisible())
	{
		...
		if (GuiService* guiService = wbx::ServiceProvider::find<wbx::GuiService>(this))
		{
...
void GuiObject::renderStudioSelectionBox(Adorn* adorn)
{
	if(selectionBox)
	{
		...
		adorn->outlineRect2d(outlineRect,3.0f,wbx::Color4(wbx::Color3(0.1f, 0.6f, 1.0f)), absoluteRotation);
	}
}
									...
									TextService::YAlignment _yalignment)
{
	...
	if (clippingObject == NULL || !absoluteRotation.empty())
	{
		return adorn->drawFont2D(	_textName,
									...
									wh,
									getClipping() ? rect : wbx::Rect2D::xyxy(-1,-1,-1,-1),
									absoluteRotation);
	}
	else
	{
		wbx::Rect2D clippedRect = clippingObject->getClippedRect();
		if(getClipping())
...
void GuiObject::fireGenericInputEvent(const shared_ptr<InputObject>& event)
{
	...
	WBXASSERT(event->getUserInputState() != InputObject::INPUT_STATE_NONE && event->getUserInputType() != InputObject::TYPE_NONE);
    ...
    if(wbx::UserInputService* inputService = wbx::ServiceProvider::find<wbx::UserInputService>(this))
...
void GuiObject::fireGestureEvent(const UserInputService::Gesture& gesture, const shared_ptr<const wbx::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args)
{
    switch (gesture)
    {
        ...
        case UserInputService::GESTURE_PAN:
            WBXASSERT(args->values.size() == 3);
            panGestureEvent(touchPositions,args->values[0].cast<wbx::Vector2>(),args->values[1].cast<wbx::Vector2>(),args->values[2].cast<InputObject::UserInputState>());
            break;
...
GuiResponse GuiObject::processGesture(const UserInputService::Gesture& gesture, const shared_ptr<const wbx::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args)
{
    if (!visible)
        return GuiResponse::notSunk();
    ...
    {
        if( !mouseIsOver(iter->get<wbx::Vector2>()) )
        {
            gestureIsOver = false;
            break;
        }
...
bool GuiObject::mouseIsOver(const Vector2& mousePosition)
{
    ...
    {
        wbx::Rect2D clippedRect = getRect2D().intersect(clippingObject->getClippedRect());
...
bool GuiObject::isCurrentlyVisible()
{
	...
	if (Instance::fastDynamicCast<wbx::ScreenGui>(getParent()))
		return true;
...
REFLECTION_BEGIN();
static const Reflection::BoundFuncDesc<GuiButton, void(std::string)> func_setVerb(&GuiButton::setVerb,"SetVerb", "verb", Security::WiibloxScript);
	...
	:EnumDescriptor("ButtonStyle")
{
	...
	addPair(GuiButton::Wiiblox_RED_STYLE, "WiibloxButtonDefault");
	addPair(GuiButton::Wiiblox_GREY_STYLE, "WiibloxButton");
	addPair(GuiButton::Wiiblox_BUTTON_ROUND_STYLE, "WiibloxRoundButton");
	addPair(GuiButton::Wiiblox_BUTTON_ROUND_DEFAULT_STYLE, "WiibloxRoundDefaultButton");
	addPair(GuiButton::Wiiblox_BUTTON_ROUND_DROPDOWN_STYLE, "WiibloxRoundDropdownButton");
}
}//namespace Reflection
...
{	
	switch(style)
	{
		case GuiButton::CUSTOM_STYLE:
			return getRect2D();
		case GuiButton::Wiiblox_RED_STYLE:
		case GuiButton::Wiiblox_GREY_STYLE:
			return Scale9Rect2D(getRect2D(), 12, 36);
		case GuiButton::Wiiblox_BUTTON_ROUND_STYLE:
		case GuiButton::Wiiblox_BUTTON_ROUND_DEFAULT_STYLE:
		case GuiButton::Wiiblox_BUTTON_ROUND_DROPDOWN_STYLE:
...
void GuiButton::setVerb(std::string verbString)
{
	...
	if(Workspace* workspace = ServiceProvider::find<Workspace>(this))
	{
        ...
        if (verb && verb->getVerbSecurity())
        {
			...
			wbx::Security::setHackFlagVmp<LINE_RAND1>(wbx::Security::hackFlag10, HATE_VERB_SNATCH);
			#else
            wbx::Security::setHackFlagVmp<LINE_RAND4>(wbx::Security::hackFlag10, HATE_VERB_SNATCH);
			#endif
            verb = NULL;
        }
#endif
...
{
	static const TextureId sGreyTransp =	TextureId("WBXasset://textures/ui/btn_greyTransp.png");
	static const TextureId sGrey =			TextureId("WBXasset://textures/ui/btn_grey.png");
	static const TextureId sWhite =			TextureId("WBXasset://textures/ui/btn_white.png");
	static const TextureId sRed	=			TextureId("WBXasset://textures/ui/btn_red.png");
	static const TextureId sRedGlow =		TextureId("WBXasset://textures/ui/btn_redGlow.png");
	static const TextureId sNewGrey	=		TextureId("WBXasset://textures/ui/btn_newGrey.png");
	static const TextureId sNewGreyGlow =	TextureId("WBXasset://textures/ui/btn_newGreyGlow.png");
	static const TextureId sNewBlue	=		TextureId("WBXasset://textures/ui/btn_newBlue.png");
	static const TextureId sNewBlueGlow =	TextureId("WBXasset://textures/ui/btn_newBlueGlow.png");
	static const TextureId sNewWhite	=	TextureId("WBXasset://textures/ui/btn_newWhite.png");
	static const TextureId sNewWhiteGlow =	TextureId("WBXasset://textures/ui/btn_newWhiteGlow.png");
	...
	switch(style)
	{
		...
		case Wiiblox_BUTTON_ROUND_STYLE:
		case Wiiblox_BUTTON_ROUND_DEFAULT_STYLE:
		case Wiiblox_BUTTON_ROUND_DROPDOWN_STYLE:
		{
...
GuiResponse GuiButton::processTouchEvent(const shared_ptr<InputObject>& event)
{
	...
	{
		if( wbx::ScrollingFrame* scrollFrame = findFirstAncestorOfType<wbx::ScrollingFrame>())
		{
                ...
                if (event->getUserInputState() == InputObject::INPUT_STATE_END)
                {
                    ...
                    if (event == lastSharedObj)
                    {
						...
						if (verb != NULL)
						{
							...
							if (verb->isEnabled())
							{
								Verb::doItWithChecks(verb, wbx::DataModel::get(this));
							}
						}
						#if defined(WIN32) && defined(I_AM_GOY_THAT_LOVES_VMPROTECT)
						VMProtectEnd();
