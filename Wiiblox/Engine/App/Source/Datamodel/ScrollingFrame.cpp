...
namespace WBX
{
    ...
    ScrollingFrame::ScrollingFrame()
        ...
        , topImage(TextureId("WBXasset://textures/ui/Scroll/scroll-top.png"))
        , midImage(TextureId("WBXasset://textures/ui/Scroll/scroll-middle.png"))
        , bottomImage(TextureId("WBXasset://textures/ui/Scroll/scroll-bottom.png"))
	...
	void ScrollingFrame::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
	{
        ...
        if (newProvider)
        {
			...
			if(GuiService* guiService = wbx::ServiceProvider::find<GuiService>(newProvider))
			{
	...
	void ScrollingFrame::selectedGuiObjectChanged(const wbx::Reflection::PropertyDescriptor* desc)
	{
		...
		wbx::GuiObject* selectedObject = NULL;
    ...
    Vector2 ScrollingFrame::calculateCanvasPositionClamped(Vector2 desiredValue) const
    {
        ...
        if (wbx::Math::isNanInf(desiredValue.x))
        {
            desiredValue.x = canvasPosition.x;
        }
        if (wbx::Math::isNanInf(desiredValue.y))
        {
	...
	void ScrollingFrame::setCanvasPosition(Vector2 value, bool printWarnings)
	{
		...
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "ScrollingFrame.CanvasPosition set to invalid position (%f, %f), clamping to position (%f, %f).", value.x,
																																											value.y,
																																											newValue.x,
																																											newValue.y);
		}
	...
	void ScrollingFrame::setScrollBarThickness(int value)
	{
		...
		if (newValue != value)
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "ScrollingFrame.ScrollBarThickness set to negative value, clamping value to 0.");
		}
    ...
    {
        if (wbx::UserInputService* userInputService = wbx::ServiceProvider::find<wbx::UserInputService>(this))
	...
	bool ScrollingFrame::doProcessGamepadEvent(const shared_ptr<InputObject>& event, const bool inputOver)
	{
		...
		GamepadService* gamepadService = wbx::ServiceProvider::find<GamepadService>(this);
	...
	void ScrollingFrame::renderScrollbarLines(Adorn* adorn, const bool scrollVertical, const bool scrollHorizontal)
	{
		...
		if( GuiObject* clippingObject = firstAncestorClipping())
		{
			const wbx::Rect2D clipRect = clippingObject->getClippedRect();
	...
	void ScrollingFrame::renderScrollbarSection(Adorn* adorn, GuiDrawImage& drawImage, const TextureId& image, const char* context, const Rect2D& imageRect, const Rotation2D& rotation)
	{
		if (drawImage.setImage(adorn, image, GuiDrawImage::NORMAL, NULL, this, context))
		{
			...
			{
				const wbx::Rect2D clipRect = clippingObject->getClippedRect();
				wbx::Rect2D clippedRect = clipRect.intersect(imageRect);
				...
				if (rotation != Rotation2D())
				{
					if (DFFlag::FixRotatedHorizontalScrollBar)
					{
						...
						wbx::Rect2D clipRectRotated = Rect2D::xyxy(clippedOriginRotated, clippedx1y1Rotated);
