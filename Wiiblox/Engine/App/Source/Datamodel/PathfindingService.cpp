...
WBX_REGISTER_ENUM(Path::PathStatus);
...
namespace WBX {
	...
	namespace Reflection
	{
		template<> Reflection::EnumDesc<Path::PathStatus>::EnumDesc()
			:wbx::Reflection::EnumDescriptor("PathStatus")
		{
	...
	void PathfindingService::PathfindingState::addOpenNode(const Node& node)
	{
		...
		WBXASSERT_SLOW(checkOpenNodesCounts());
	...
	{
        WBXPROFILER_SCOPE("Voxel", "countOrComputeDirtyChunks");
	...
	{
        WBXPROFILER_SCOPE("Voxel", "computePath");
		...
		while(parentCell != startCell)
		{
			...
			WBXASSERT(itOpen != state.nodesByPos.end());
	...
	{
		{
			...
			boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent,GA_CATEGORY_GAME, "PathfindingService", 
				wbx::StringConverter<int>::convertToString(dm->getPlaceID()).c_str(), 0,false));
		}
	...
	void PathfindingService::executeThrottledRequests()
	{
		...
		do 
		{
            WBXPROFILER_SCOPE("Voxel", "executeRequest");
            ...
            WBXPROFILER_LABELF("Voxel", "(%d %d %d) -> (%d %d %d)", request->startCell.x, request->startCell.y, request->startCell.z, request->finishCell.x, request->finishCell.y, request->finishCell.z);
	...
	ExtentsInt32 hashToVoxelChunk(const ExtentsInt32& extents, int level)
	{
		...
		WBXASSERT((1 << voxelShift.x) == kVoxelChunkSizeXZ);
		WBXASSERT((1 << voxelShift.y) == kVoxelChunkSizeY);
		WBXASSERT((1 << voxelShift.z) == kVoxelChunkSizeXZ);
