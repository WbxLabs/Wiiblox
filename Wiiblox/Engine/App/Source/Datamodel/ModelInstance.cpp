...
static const wbx::Name& partName = wbx::Name::declare(sModel);
...
{
	if(lockedName){
		wbx::Security::Context::current().requirePermission(wbx::Security::WritePlayer, "set a Character's name");
	}
	Super::setName(value);
}
...
void ModelInstance::onChildAdded(Instance* child)
{
	...
	{
		WBXASSERT_IF_VALIDATING(std::find(modelModifiers.begin(), modelModifiers.end(), child) == modelModifiers.end());
...
void ModelInstance::onChildRemoving(Instance* child)
{
	if (dynamic_cast<IModelModifier*>(child))
	{
		...
		WBXASSERT(it != modelModifiers.end());
...
PartInstance* ModelInstance::getPrimaryPart() 
{
	...
	if (primaryPartSetByUser.lock().get()) {
		if (!primaryPartSetByUser.lock()->isDescendantOf(this)) {
			WBXASSERT(0);					// just want to see this happening
	...
	if (primaryPartSetByUser.lock().get()) {
		...
		WBXASSERT(computedPrimaryPart->isDescendantOf(this));
...
void VisitModelDescendants(shared_ptr<wbx::Instance> instance, PartInstance** biggestPart, float* biggestSize)
{
...
{
	WBXASSERT(computedPrimaryPart == NULL);
	computedPrimaryPart = NULL;
	float biggest = -1.0;
...
void ModelInstance::setFrontAndTop(const Vector3& front)
{
	...
	if (primaryPartSetByUser.lock().get()) {
		WBXASSERT(primaryPartSetByUser.lock()->isDescendantOf(this));
...
bool ModelInstance::hitTestImpl(const class wbx::WBXRay& worldRay, Vector3& worldHitPoint)
{
...
static void testFrustumParts(shared_ptr<Instance> descendant, const wbx::Frustum& frustum, bool& isContained)
{
...
bool ModelInstance::containedByFrustum(const wbx::Frustum& frustum) const
{
...
void ModelInstance::render3dSelect(Adorn* adorn, SelectState selectState)
{
	...
	if (selectState == wbx::SELECT_NORMAL && getPrimaryPartSetByUser())
        wbx::Draw::selectionBox(getPrimaryPartSetByUser()->getPart(), adorn, primaryPartSelectColor(), primaryPartLineThickness());
