...
const Reflection::PropDescriptor<PartOperation, BinaryString> PartOperation::desc_ChildData("ChildData", category_Data, &PartOperation::getChildData, &PartOperation::setChildData, Reflection::PropertyDescriptor::CLUSTER, Security::Wiiblox);
const Reflection::PropDescriptor<PartOperation, BinaryString> PartOperation::desc_MeshData("MeshData", category_Data, &PartOperation::getMeshData, &PartOperation::setMeshData, Reflection::PropertyDescriptor::STREAMING, Security::Wiiblox);
const Reflection::PropDescriptor<PartOperation, BinaryString> PartOperation::desc_PhysicsData("PhysicsData", category_Data, &PartOperation::getPhysicsData, &PartOperation::setPhysicsData, Reflection::PropertyDescriptor::STREAMING, Security::Wiiblox);
const Reflection::PropDescriptor<PartOperation, Vector3> PartOperation::desc_InitialSize("InitialSize", category_Data, &PartOperation::getInitialSize, &PartOperation::setInitialSize, Reflection::PropertyDescriptor::STREAMING, Security::Wiiblox);
...
const Reflection::EnumPropDescriptor<PartOperation, PartOperation::FormFactor> PartOperation::desc_FormFactor("FormFactor", category_Data, &PartOperation::getFormFactor, &PartOperation::setFormFactor, Reflection::PropertyDescriptor::STREAMING, Security::Wiiblox);
const Reflection::PropDescriptor<PartOperation, ContentId> PartOperation::desc_AssetId("AssetId", category_Data, &PartOperation::getAssetId, &PartOperation::setAssetId, Reflection::PropertyDescriptor::STREAMING, Security::Wiiblox);
...
const BinaryString PartOperation::peekChildData(wbx::Instance* context)
{
    ...
    wbx::CSGDictionaryService* dictionaryService = ServiceProvider::find<CSGDictionaryService>(context ? context : this);
	wbx::NonReplicatedCSGDictionaryService* nrDictionaryService = ServiceProvider::create<NonReplicatedCSGDictionaryService>(context ? context : this);
...
const BinaryString& PartOperation::getChildDataBlocking(wbx::Instance* context) const
{
  if (FFlag::StudioCSGAssets)
  {
    if (hasAssetId)
...
shared_ptr<CSGMesh> PartOperation::getMesh()
{
	if (!cachedMesh)
	{
		...
		CSGDictionaryService* dictionaryService = ServiceProvider::create<CSGDictionaryService>(wbx::DataModel::get(this));
...
void PartOperation::setCollisionFidelity(CollisionFidelity value)
{
	if (value != getCollisionFidelity())
	{
		...
		{
			wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "Cannot change SolidModel CollisionFidelity during Run-Time");
		}
	}
}

void PartOperation::setPartSizeXml(const Vector3& WBXSize)
{
	if (WBXSize != getPartSizeXml())
	{
		//Has to happen before Primitive->setSize
		setBulletObjectsScale(WBXSize);
...
Vector3 PartOperation::uiToXmlSize(const Vector3& uiSize) const
{
	...
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "Cannot set SolidModel size to Vector3(0, 0, 0)");
...
bool PartOperation::createPhysicsData(const CSGMesh* mData)
{
	...
	else
	{
		...
		TriangleMesh* primitiveMesh = WBX_static_cast<TriangleMesh*>(getPrimitive(this)->getGeometry());
...
{
	CSGDictionaryService* dictionaryService = ServiceProvider::create<CSGDictionaryService>(wbx::DataModel::get(sourceOperation));
	if(PartOperation* iteratedPartOp = wbx::Instance::fastDynamicCast<PartOperation>(descendant.get()))
	{
...
{
	if(wbx::DataModel::get(this)->isStudio())
	{
		...
		{
			wbx::DataModel::get(this)->visitDescendants(boost::bind(&visitDescendentsSetPhysics, _1, this));
		}
		else
		{
