...
namespace WBX {
    ...
    REFLECTION_BEGIN();
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_ClickButton1(&VirtualUser::clickButton1, "ClickButton1", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_Button1Down(&VirtualUser::button1Down, "Button1Down", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_Button1Up(&VirtualUser::button1Up, "Button1Up", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_ClickButton2(&VirtualUser::clickButton2, "ClickButton2", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_Button2Down(&VirtualUser::button2Down, "Button2Down", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_Button2Up(&VirtualUser::button2Up, "Button2Up", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	static Reflection::BoundFuncDesc<VirtualUser, void(wbx::Vector2, wbx::CoordinateFrame)> func_MoveMouse(&VirtualUser::moveMouse, "MoveMouse", "position", "camera", wbx::CoordinateFrame(), Security::TestLocalUser);
	...
	public:
        ...
        virtual void setKeyState(wbx::KeyCode code, wbx::ModCode modCode, char modifiedKey, bool isDown)
        {
			if (code<0)
				return;
	...
	void VirtualUser::sendMouseEvent(InputObject::UserInputType eventType, InputObject::UserInputState eventState, wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		captureInputDevice();
		...
		const shared_ptr<InputObject>& downEvent = Creatable<Instance>::create<InputObject>(eventType, eventState, Vector3(mousePosition,0), Vector3::zero(), wbx::DataModel::get(this));

		Camera* c = getDataModel()->getWorkspace()->getCamera();
		if (c && camera != CoordinateFrame())
		{
			//wbx::CoordinateFrame oldCamera = c->getCameraCoordinateFrame();
	...
	void VirtualUser::moveMouse(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		sendMouseEvent(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CHANGE, position, camera);
	}

	void VirtualUser::clickButton1(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		button1Down(position, camera);
		button1Up(position, camera);
	}
	void VirtualUser::button1Down(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		sendMouseEvent(InputObject::TYPE_MOUSEBUTTON1, InputObject::INPUT_STATE_BEGIN, position, camera);
	}
	void VirtualUser::button1Up(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		sendMouseEvent(InputObject::TYPE_MOUSEBUTTON1, InputObject::INPUT_STATE_END, position, camera);
	}

	void VirtualUser::clickButton2(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		button2Down(position, camera);
		button2Up(position, camera);
	}
	void VirtualUser::button2Down(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
		sendMouseEvent(InputObject::TYPE_MOUSEBUTTON2, InputObject::INPUT_STATE_BEGIN, position, camera);
	}
	void VirtualUser::button2Up(wbx::Vector2 position, wbx::CoordinateFrame camera)
	{
	...
	KeyCode VirtualUser::convert(const std::string& key)
	{
		...
		if (key.length()!=1)
			throw wbx::runtime_error("Unsupported key %s", key.c_str());
	...
	void VirtualUser::setKeyDown(std::string key)
	{
        ...
        virtualHardwareDevice->setKeyState(keyCode, wbx::KMOD_NONE, 0, true);

		const shared_ptr<InputObject>& downEvent = Creatable<Instance>::create<InputObject>(InputObject::TYPE_KEYBOARD, InputObject::INPUT_STATE_BEGIN, keyCode, (ModCode)0, (char)0, wbx::DataModel::get(this));
		getDataModel()->processInputObject(downEvent);
	}
	...
	void VirtualUser::setKeyUp(std::string key)
	{
        ...
        virtualHardwareDevice->setKeyState(keyCode, wbx::KMOD_NONE, 0, false);

		const shared_ptr<InputObject>& upEvent = Creatable<Instance>::create<InputObject>(InputObject::TYPE_KEYBOARD, InputObject::INPUT_STATE_END, keyCode, (ModCode)0,(char)0, wbx::DataModel::get(this));
	...
	static wbx::Vector2 toNormalized(Vector2int16 windowSize, const Vector2int16 mousePosition)
	{
		Vector2 position;
	...
	{
		writeWait();
		recording << "virtualUser:" << func << "('" << wbx::format("0x%02x", event->getKeyCode()) << "')";
