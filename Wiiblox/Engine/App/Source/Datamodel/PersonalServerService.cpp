...
static Reflection::BoundFuncDesc<PersonalServerService, void(shared_ptr<Instance>)> func_Promote(&PersonalServerService::promote, "Promote", "player", Security::WiibloxScript);
static Reflection::BoundFuncDesc<PersonalServerService, void(shared_ptr<Instance>)> func_Demote(&PersonalServerService::demote, "Demote", "player", Security::WiibloxScript);

static Reflection::BoundYieldFuncDesc<PersonalServerService, std::string(int)>	func_GetRoleSets(&PersonalServerService::getWebRoleSets, "GetRoleSets", "placeId", Security::WiibloxScript);

static Reflection::PropDescriptor<PersonalServerService, std::string> prop_roleSets("RoleSets", category_Behavior, &PersonalServerService::getRoleSets, &PersonalServerService::setRoleSets, Reflection::PropertyDescriptor::SCRIPTING, Security::WiibloxScript);
REFLECTION_END();
...
void PersonalServerService::getWebRoleSets(int placeId, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	{
		std::string url(wbx::format(personalServerRoleSetsUrl.c_str(), placeId));
...
void PersonalServerService::getRank(Network::Player* player, int placeId, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	{
		std::string url(wbx::format(personalServerGetRankUrl.c_str(), placeId, player->getUserID()));
	    dispatchRequest(url, resumeFunction, errorFunction);
	}
}
...
void PersonalServerService::setRank(Network::Player* player, int placeId, int newRank, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	{
		std::string url(wbx::format(personalServerSetRankUrl.c_str(), placeId, player->getUserID(), newRank));
	    dispatchRequest(url, resumeFunction, errorFunction);
	}
}
