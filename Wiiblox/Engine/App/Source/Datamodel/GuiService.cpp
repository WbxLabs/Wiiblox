...
namespace WBX
{
	...
	static Reflection::EventDesc<GuiService, void(std::string, std::string)> event_keypressed(&GuiService::keyPressed, "KeyPressed", "key", "modifiers", Security::WiibloxScript);
	static Reflection::EventDesc<GuiService, void(GuiService::SpecialKey, std::string)> event_specialKeypressed(&GuiService::specialKeyPressed, "SpecialKeyPressed", "key", "modifiers", Security::WiibloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(std::string)> func_addKey(&GuiService::addKey, "AddKey", "key", Security::WiibloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(std::string)> func_removeKey(&GuiService::removeKey, "RemoveKey", "key", Security::WiibloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(GuiService::SpecialKey)> func_addSpecialKey(&GuiService::addSpecialKey, "AddSpecialKey", "key", Security::WiibloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(GuiService::SpecialKey)> func_removeSpecialKey(&GuiService::removeSpecialKey, "RemoveSpecialKey", "key", Security::WiibloxScript);

	static Reflection::EventDesc<GuiService, void()> event_escapeKeypressed(&GuiService::escapeKeyPressed, "EscapeKeyPressed", Security::WiibloxScript);

	static Reflection::BoundFuncDesc<GuiService, void(shared_ptr<Instance>, GuiService::CenterDialogType, Lua::WeakFunctionRef, Lua::WeakFunctionRef)> func_showCenterDialog(&GuiService::addCenterDialog, "AddCenterDialog", "dialog", "centerDialogType", "showFunction", "hideFunction", Security::WiibloxScript);
	static Reflection::BoundFuncDesc<GuiService, void(shared_ptr<Instance>)> func_hideCenterDialog(&GuiService::removeCenterDialog, "RemoveCenterDialog", "dialog", Security::WiibloxScript);

	static Reflection::BoundFuncDesc<GuiService, void(int, int, int, int)> func_globalGuiInset(&GuiService::setGlobalGuiInset, "SetGlobalGuiInset", "x1", "y1", "x2", "y2", Security::WiibloxScript);
    
    static Reflection::BoundYieldFuncDesc<GuiService, Vector2(void)> func_getScreenResolution(&GuiService::getScreenResolutionLua, "GetScreenResolution", Security::WiibloxScript);

	static Reflection::BoundFuncDesc<GuiService, void(std::string)> func_openBrowserWindow(&GuiService::openBrowserWindow, "OpenBrowserWindow", "url", Security::WiibloxScript);
	static Reflection::EventDesc<GuiService, void()> event_urlWindowClosed(&GuiService::urlWindowClosed, "BrowserWindowClosed", Security::WiibloxScript);

	static Reflection::BoundFuncDesc<GuiService, shared_ptr<Instance>(Vector3)> func_GetClosestDialogToPosition(&GuiService::getClosestDialogToPosition, "GetClosestDialogToPosition", "position", Security::WiibloxScript);
	
	static Reflection::BoundFuncDesc<GuiService, int()> func_GetBrickCount(&GuiService::getBrickCount, "GetBrickCount", Security::WiibloxScript);
	...
	static Reflection::BoundFuncDesc<GuiService, std::string()> func_getUiMessage(&GuiService::getUiMessage, "GetUiMessage", Security::WiibloxScript);
	static Reflection::EventDesc<GuiService, void(GuiService::UiMessageType, std::string)> event_newUiMessage(&GuiService::newUiMessageSignal, "UiMessageChanged", "msgType", "newUiMessage", Security::WiibloxScript);
	...
	static Reflection::BoundFuncDesc<GuiService, std::string()> func_getErrorMessage(&GuiService::getErrorMessage, "GetErrorMessage", Security::WiibloxScript, Reflection::Descriptor::Attributes::deprecated(func_getUiMessage));
	static Reflection::EventDesc<GuiService, void(std::string)> event_newError(&GuiService::newErrorSignal, "ErrorMessageChanged", "newErrorMessage", Security::WiibloxScript, Reflection::Descriptor::Attributes::deprecated(event_newUiMessage));

	static Reflection::EventDesc<GuiService, void()> event_showLeaveConfirmation(&GuiService::showLeaveConfirmationSignal, "ShowLeaveConfirmation", Security::WiibloxScript);

	static Reflection::BoundFuncDesc<GuiService, void()> func_toggleFullscreen(&GuiService::toggleFullscreen, "ToggleFullscreen", Security::WiibloxScript);

	static Reflection::BoundFuncDesc<GuiService, bool()> func_isTenFootInterface(&GuiService::isTenFootInterface, "IsTenFootInterface", Security::WiibloxScript);
    ...
    Reflection::RefPropDescriptor<GuiService, GuiObject> GuiService::prop_selectedCoreGuiObject("SelectedCoreObject", category_Data, &GuiService::getSelectedCoreGuiObjectLua, &GuiService::setSelectedCoreGuiObjectLua, Reflection::PropertyDescriptor::UI, Security::WiibloxScript);
	...
	static Reflection::BoundAsyncCallbackDesc<GuiService, void(std::string, std::string)> callback_notificationCallback("SendCoreUiNotification", &GuiService::notificationCallback, "title", "text", Security::WiibloxScript);
	...
	static const Reflection::BoundFuncDesc<GuiService, void(bool)> func_setMenuOpen(&GuiService::setMenuOpen, "SetMenuIsOpen", "open", Security::WiibloxScript);
	...
	static Reflection::BoundFuncDesc<GuiService, bool(std::string)> func_ShowStatsBasedOnInputString(&GuiService::showStatsBasedOnInputString, "ShowStatsBasedOnInputString","input", Security::WiibloxScript);
    REFLECTION_END();
	...
	bool GuiService::shouldPreemptCurrentDialog(DialogWrapper* newDialog) const
	{
		...
		if(newDialog->dialogType < currentDialog->dialogType)
			return false;
		WBXASSERT(newDialog->dialogType == currentDialog->dialogType);
	...
	void GuiService::addCenterDialog(shared_ptr<Instance> dialogInstance, GuiService::CenterDialogType type, Lua::WeakFunctionRef show, Lua::WeakFunctionRef hide)
	{
		...
		if(!dialog)
			throw wbx::runtime_error("dialog must be a GuiObject instance");
		...
		if(iter != dialogWrapperMap.end()){
			if( DialogWrapper* wrapper = iter->second){
				if(wrapper->dialogType != type){
					throw wbx::runtime_error("A dialogInstance should not change CenterDialogTypes");
				}
		...
		WBXASSERT(dialogWrapperMap.find(weakDialog) == dialogWrapperMap.end());
	...
	bool GuiService::processKeyDown(const shared_ptr<wbx::InputObject>& event)
	{
		// ensure we are dealing with a key down
		WBXASSERT(event->isKeyDownEvent());

		if(event->isTextCharacterKey() || (UserInputService::IsUsingNewKeyboardEvents() && event->getUserInputType() == wbx::InputObject::TYPE_KEYBOARD))
		{
	...
	{
		if(key.size() != 1)
			throw wbx::runtime_error("GuiService:AddKey requires a string with a single character");
	...
	{
		if(key.size() != 1)
			throw wbx::runtime_error("GuiService:RemoveKey requires a string with a single character");
		char character = tolower(key[0]);
	...
	{
		if( !wbx::Http::isWiibloxSite(url.c_str()) )
		{
			StandardOut::singleton()->print(MESSAGE_WARNING, "GuiService::OpenBrowserWindow() was called on non-Wiiblox url.");
			return;
		}
	...
	void GuiService::toggleFullscreen()
	{
		...
		if(Workspace* workspace = ServiceProvider::find<Workspace>(this))
		{
			...
			{
				wbx::Tokens::simpleToken |= HATE_VERB_SNATCH;
				verb = NULL;
			}
#endif
		}
	...
	void GuiService::addSelectionGroup(std::string selectionGroupName, shared_ptr<Instance> selectionParent)
	{
		...
		if (!guiObjectParent)
		{
			wbx::StandardOut::singleton()->printf(MESSAGE_ERROR, "GuiService:AddSelectionParent for group name %s: parent is not a GuiObject.", selectionGroupName.c_str());
			return;
		}

		if (selectionMap.find(selectionGroupName) != selectionMap.end())
		{
			wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "GuiService:AddSelectionParent already has selection group with name %s, overwriting selection group.", selectionGroupName.c_str());
	...
	void GuiService::addSelectionGroup(std::string selectionGroupName, shared_ptr<const Reflection::Tuple> selectionTuple)
	{
		if (selectionMap.find(selectionGroupName) != selectionMap.end())
		{
			wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "GuiService:AddSelectionTuple already has selection group with name %s, overwriting selection group.", selectionGroupName.c_str());
			removeSelectionGroup(selectionGroupName);
		}
