...
ToggleViewMode::ToggleViewMode(wbx::DataModel* dm)
...
void ToggleViewMode::doIt(wbx::IDataState* dataState)
{
...
bool RunCommand::isEnabled() const
{
	...
	bool clientIsPresent = wbx::Network::Players::clientIsPresent(dataModel);
...
bool ResetCommand::isEnabled() const
{
	...
	return		(		(runState == RS_RUNNING ||	runState == RS_PAUSED)
					&&	!wbx::Network::Players::findLocalPlayer(dataModel)	);
}
...
void SelectChildrenVerb::doIt(IDataState *dataState)
{
	...
	shared_ptr<wbx::Instances> childItems(new Instances);
...
void AddChildToRoot(XmlElement* root, shared_ptr<Instance> wsi, const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole)
{
...
void AddSelectionToRoot(XmlElement* root, Selection* selection, wbx::CreatorRole creatorRole)
{
			...
			for (Instances::const_iterator it = selectionRead->begin(); it != selectionRead->end(); ++it)
			{
				...
				if (iPtr)
				{
					if (!iPtr->fastDynamicCast<Workspace>() && !iPtr->fastDynamicCast<Network::Players>() && !dynamic_cast<Service*>(iPtr)
						...
						if (iPtr->fastDynamicCast<wbx::Camera>())
...
bool CameraZoomExtentsCommand::isEnabled() const
{
	...
	if (ServiceClient< FilteredSelection<PVInstance> >(workspace)->size() >= 1)
	{
		...
		wbx::Network::Players* players = ServiceProvider::create<Network::Players>(worldCamera);
...
bool applyColor(Instance* instance, wbx::BrickColor color)
{
	bool applied = false;
...
wbx::BrickColor ColorVerb::m_currentColor = wbx::BrickColor::brick_194;
...
PartMaterial MaterialVerb::parseMaterial(const std::string materialString)
{
    ...
    {
        return wbx::WOODPLANKS_MATERIAL;
    }
    else if(materialString == "Cobblestone")
    {
        return wbx::COBBLESTONE_MATERIAL;
    }
    else if(materialString == "Metal")
    {
        return wbx::METAL_MATERIAL;
    }
	else 
		return wbx::PLASTIC_MATERIAL;
}
...
bool AnchorVerb::isChecked() const
{
	...
	for (std::vector<Instance*>::const_iterator iter = m_selection->begin(); iter != m_selection->end(); ++iter)
	{
		if (!wbx::AnchorTool::allChildrenAnchored(*iter))
		{
	...
	: EditSelectionVerb("AnchorVerb", dataModel)
{
	...
	m_selection = ServiceProvider::create< FilteredSelection<wbx::Instance> >(dataModel);
}
} // namespace
