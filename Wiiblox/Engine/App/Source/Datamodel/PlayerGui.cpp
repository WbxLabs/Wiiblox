...
/* Copyright 2003-2009 Wiiblox Corporation, All Rights Reserved  */
...
{
	defaultSelectionImage = wbx::Creatable<Instance>::create<ImageLabel>();
	defaultSelectionImage->setImage( TextureId("WBXasset://textures/ui/SelectionBox.png") );
									  ...
									  float& minIntersectDist, float& minProjection, const GuiObject* nextSelectionGuiObject)
{
	...
	{
		const float maxAngle =  wbx::Math::piHalff()/4.0f;
	...
	{
		{
			...
			wbx::Network::Player* localPlayer = Network::Players::findLocalPlayer(this);
...
GuiResponse BasePlayerGui::processGestureOnChild(GuiBase2d* guiBase, const UserInputService::Gesture gesture, shared_ptr<const wbx::Reflection::ValueArray> touchPositions, shared_ptr<const Reflection::Tuple> args)
{
    return guiBase->processGesture(gesture, touchPositions, args);
}
    
GuiResponse BasePlayerGui::processGesture(const UserInputService::Gesture gesture, shared_ptr<const wbx::Reflection::ValueArray> touchPositions, shared_ptr<const Reflection::Tuple> args)
{
...
{
	return Instance::fastDynamicCast<wbx::Network::Player>(parent)!=NULL;
}
...
void PlayerGui::setTopbarTransparency(float transparency)
{
    if(wbx::Network::Players::frontendProcessing(this))
    {
        if (G3D::isFinite(transparency))
        {
            ...
            {
                wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "PlayerGui:SetTopbarTransparency - Attempted to set to %f, but only 0 or 0.5 are valid; setting to %f.", transparency, newTransparency);
            }
...
static Reflection::EventDesc<StarterGuiService, void(StarterGuiService::CoreGuiType,bool)> event_coreGuiChangedSignal(&StarterGuiService::coreGuiChangedSignal, "CoreGuiChangedSignal", "coreGuiType", "enabled", Security::WiibloxScript);
static const Reflection::BoundFuncDesc<StarterGuiService, void(std::string, Lua::WeakFunctionRef)> func_registerSetCore(&StarterGuiService::registerSetCore,"RegisterSetCore", "parameterName", "setFunction", Security::WiibloxScript);
static const Reflection::BoundFuncDesc<StarterGuiService, void(std::string, Lua::WeakFunctionRef)> func_registerGetCore(&StarterGuiService::registerGetCore,"RegisterGetCore", "parameterName", "getFunction", Security::WiibloxScript);
...
void StarterGuiService::setCore(std::string parameterName, Reflection::Variant value)
{
	...
	{
		throw wbx::runtime_error("SetCore: %s has not been registered by the CoreScripts", parameterName.c_str());
	}
}
...
void StarterGuiService::getCore(std::string parameterName,  boost::function<void(Reflection::Variant)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	if (iter != getCoreFunctions.end())
	{
		...
		if (Lua::ThreadRef threadRef = getFunction.lock())
		{
			if (ScriptContext* sc = ServiceProvider::create<ScriptContext>(this))
			{
				...
				{
					errorFunction(wbx::format("GetCore: Unexpected error while invoking callback: %s", e.what()));
				}
				...
				{
					errorFunction(wbx::format("GetCore: CoreScript function %s did not return a result", parameterName.c_str()));
				}
			}
		}
	}
	...
	{
		errorFunction(wbx::format("GetCore: %s has not been registered by the CoreScripts", parameterName.c_str()));
	}
}
...
REFLECTION_BEGIN();
static Reflection::RefPropDescriptor<CoreGuiService, GuiObject> prop_CoreSelectionImageObject("SelectionImageObject", category_Appearance, &CoreGuiService::getSelectionImageObject, &CoreGuiService::setSelectionImageObject, Reflection::PropertyDescriptor::STANDARD, Security::WiibloxScript);
...
void CoreGuiService::createWiibloxScreenGui()
{
	screenGui = Creatable<wbx::Instance>::create<ScreenGui>();
	screenGui->setName("WiibloxGui");
	screenGui->setWiibloxLocked(true);
	screenGui->setParent(this);
}

shared_ptr<wbx::ScreenGui> CoreGuiService::getWiibloxScreenGui()
{
    if (!screenGui)
    {
        createWiibloxScreenGui();
    }
    
    if ( wbx::ScreenGui* theScreenGui = Instance::fastDynamicCast<wbx::ScreenGui>(screenGui.get()) )
    {
        return shared_from(theScreenGui);
    }
    
    return shared_ptr<wbx::ScreenGui>();
}
...
void CoreGuiService::onDescendantAdded(Instance* instance)
{
	if(instance)
    {
		instance->setWiibloxLocked(true);
    }

	Super::onDescendantAdded(instance);
}

void CoreGuiService::addChild(wbx::Instance* instance)
{
	if(instance)
		instance->setParent(screenGui.get());
}

void CoreGuiService::removeChild(wbx::Instance* instance)
{
...
void CoreGuiService::setGuiVisibility(bool visible)
{
	if(screenGui->getChildren())
	{
		Instances::const_iterator end = screenGui->getChildren()->end();
		for (Instances::const_iterator iter = screenGui->getChildren()->begin(); iter != end; ++iter)
			if(wbx::GuiObject* obj = Instance::fastDynamicCast<GuiObject>(iter->get()))
...
void CoreGuiService::displayOnScreenMessage(int slot, const std::string &message, double duration)
{
	...
	Rect2D rect = screenGui->fastDynamicCast<wbx::GuiBase2d>()->getRect2D();

	onScreenMessages[slot] = Creatable<wbx::Instance>::create<TextLabel>();

	wbx::Frame* bottomRightControl = Instance::fastDynamicCast<wbx::Frame>(screenGui->findFirstChildByName2("BottomRightControl",true).get());
	if(bottomRightControl)
		onScreenMessages[slot]->setParent(bottomRightControl);
	else
		onScreenMessages[slot]->setParent(screenGui.get());

	if (wbx::TextLabel *textLabel = Instance::fastDynamicCast<wbx::TextLabel>(onScreenMessages[slot].get()))
	{
		...
		{
			onScreenMessages[slot]->setParent(bottomRightControl);
			textLabel->setSize(wbx::UDim2(0,textLabel->getTextBounds().x,1,0));
			textLabel->setPosition(wbx::UDim2(1, -textLabel->getTextBounds().x, -1.05 + (-slot - 0.5), 0));
		}
		else
		{
			onScreenMessages[slot]->setParent(screenGui.get());
			textLabel->setSize(wbx::UDim2(0,300,0,30));
			textLabel->setPosition(wbx::UDim2(1, (int)rect.x1() - ON_SCREEN_MESSAGE_DISTANCE_TO_RIGHT,
	...
	if (duration != 0)
	{
		...
		wbx::DebrisService *ds = wbx::ServiceProvider::find<wbx::DebrisService>(this);
