...
bool PlayerScripts::askAddChild(const Instance* instance) const
{
	...
	return Instance::fastDynamicCast<wbx::LocalScript>(instance) != NULL;
}
...
void PlayerScripts::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
	...
	if (newProvider && wbx::Network::Players::frontendProcessing(newProvider))
	{
		// Check to see if we need to load the default scripts into this object - Client side from StarterPlayerService
		wbx::Network::Player* player = findFirstAncestorOfType<wbx::Network::Player>();
		wbx::Network::Player* localPlayer = Network::Players::findLocalPlayer(newProvider);
		if (player && player == localPlayer)
		{
			if (StarterPlayerService* starterPlayer = ServiceProvider::find<StarterPlayerService>(newProvider)) 
			{
				...
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "PlayerScripts %s didn't find StarterPlayerService", getName().c_str());
			}
		}
	}
}
...
{
	return Instance::fastDynamicCast<wbx::StarterPlayerService>(parent) != NULL;
}
...
void StarterPlayerScripts::InitializeDefaultScripts()
{
	...
	if (!controlScript) {
		ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets("fonts/characterControlScript.WBXmx"), 
	...
	if (!cameraScript) {
		ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets("fonts/characterCameraScript.WBXmx"), 
			ContentProvider::PRIORITY_SCRIPT, boost::bind(&PlayerScriptsLoadHelper, weak_from(this), _1, _2), AsyncHttpQueue::AsyncWrite);
	}
...
{
	wbx::Network::Player* pPlayer = Instance::fastDynamicCast<wbx::Network::Player>(player.get());
	if (pPlayer)
	{		    
...
void StarterPlayerScripts::defaultScriptsSend(weak_ptr<wbx::Network::Player> p)
{

	shared_ptr<wbx::Network::Player> player = p.lock();
	if (player) 
	{
		wbx::Network::Player* pPlayer = player.get();
		if (pPlayer) {

			if (Network::Players::serverIsPresent(this)) // check for play solo
			{
				const wbx::SystemAddress& target = pPlayer->getRemoteAddressAsWbxAddress();
