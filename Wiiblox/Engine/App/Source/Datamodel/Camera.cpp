...
static Reflection::BoundFuncDesc<Camera, wbx::WBXRay(float,float,float)> func_viewportToWorldRay(&Camera::worldRayViewportLua, "ViewportPointToRay", "x","y","depth",0, Security::None);
static Reflection::BoundFuncDesc<Camera, wbx::WBXRay(float,float,float)> func_screenToWorldRay(&Camera::worldRayLua, "ScreenPointToRay", "x","y","depth",0, Security::None);
...
static Reflection::BoundFuncDesc<Camera, bool(float)> func_zoom(&Camera::zoom, "Zoom", "distance", Security::WiibloxScript);
...
static Reflection::EventDesc<Camera, void(bool)> event_firstPersonTransition(&Camera::firstPersonTransitionSignal, "FirstPersonTransition", "entering", Security::WiibloxPlace);
...
template<>
EnumDesc<wbx::Camera::CameraPanMode>::EnumDesc()
:EnumDescriptor("CameraPanMode")
{
	addPair(wbx::Camera::CAMERAPANMODE_CLASSIC,	"Classic");
	addPair(wbx::Camera::CAMERAPANMODE_EDGEBUMP, "EdgeBump");
}

template<>
wbx::Camera::CameraPanMode& Variant::convert<wbx::Camera::CameraPanMode>(void)
{
	return genericConvert<wbx::Camera::CameraPanMode>();
}
}//namespace Reflection

template<>
bool wbx::StringConverter<wbx::Camera::CameraPanMode>::convertToValue(const std::string& text, wbx::Camera::CameraPanMode& value)
{
	if(text.find("Classic") != std::string::npos){
		value = wbx::Camera::CAMERAPANMODE_CLASSIC;
		return true;
	}
	if(text.find("EdgeBump") != std::string::npos){
		value = wbx::Camera::CAMERAPANMODE_EDGEBUMP;
		return true;
	}
	return false;
}
...
bool Camera::isPartVisibleFast(const PartInstance& part, const ContactManager& contactManager, const HitTestFilter* filter) const
{
	...
	WBXRay ray = WBXRay::fromOriginAndDirection(cframe.translation, direction);
...
void Camera::onHeartbeat(const Heartbeat& event)
{
	...
	else if(wbx::GameBasicSettings::singleton().inStudioMode()) // only do camera interpolation if we are currently using studio
    {
...
{
	if(wbx::Time::nowFastSec() - lastHistoryPushTime < 0.5f) // don't update history too fast
		return;
	...
	lastHistoryPushTime = wbx::Time::nowFastSec();
...
{
	WBXASSERT(camInterpolation == CAM_INTERPOLATION_CONSTANT_SPEED);
...
void Camera::beginCameraInterpolation(CoordinateFrame endPos, CoordinateFrame endFocus, float duration)
{
    ...
    {
        WBXASSERT(duration >= 0.f);
        WBXASSERT(cameraType == Camera::LOCKED_CAMERA ||
    ...
    {
        WBXASSERT(duration > 0.f);
        WBXASSERT(cameraType == Camera::LOCKED_CAMERA);	// camera must be scriptable
...
void Camera::lerpToExtents(const Extents& extents)
{
	...
	if ( wbx::ServiceProvider::findServiceProvider(this) != NULL &&
		 ( wbx::Network::Players::getGameMode(this) == wbx::Network::EDIT || 
		   wbx::Network::Players::getGameMode(this) == wbx::Network::DPHYS_GAME_SERVER ||
		   wbx::Network::Players::getGameMode(this) == wbx::Network::GAME_SERVER ) )
	{
...
void Camera::zoomExtents(const Extents& extents, ZoomType zoomType)
{
	...
	WBXASSERT(G3D::isFinite(current));
...
void Camera::setCameraSubject(Instance* newSubject)
{
	if ( (newSubject != cameraSubject.get()) &&	dynamic_cast<CameraSubject*>(newSubject) )
	{
		...
		shared_ptr<wbx::Instance> potentialHuman = shared_from(cameraSubject->findFirstChildByName("Humanoid"));
		if(potentialHuman &&  Instance::fastDynamicCast<wbx::Humanoid>(potentialHuman.get()))
...
{
	return wbx::ServiceProvider::findServiceProvider(this) != NULL && 
		(wbx::Network::Players::getGameMode(this) == wbx::Network::EDIT || wbx::Network::Players::isCloudEdit(this));
}

bool Camera::hasClientPlayer() const
{
	wbx::Network::Players* players = ServiceProvider::create<Network::Players>(this);
...
bool Camera::tiltRadians(float tilt)
{
	if (tilt!=0.0f)
	{
		...
		{
			if(wbx::GameBasicSettings::singleton().inHybridMode() && !isFirstPersonCamera() && isCharacterCamera()) // don't allow hybrid mode to tilt so much
...
{
	WBXASSERT(angle > -100.0f);
	WBXASSERT(angle < 100.0f);	// catch weird numbers here
...
WBXRay Camera::worldRayLua(float x, float y, float depth)
{
	if (GuiService* guiService = wbx::ServiceProvider::find<GuiService>(this))
	{
		Vector4 guiInset = guiService->getGlobalGuiInset();
...
WBXRay Camera::worldRayViewportLua(float x, float y, float depth)
{
	return worldRay(x, y, depth);
}

wbx::WBXRay Camera::worldRay(float x, float y, float depth) const
{
	int screenWidth  = viewport.x;
	int screenHeight = viewport.y;
	...
	return wbx::WBXRay::fromOriginAndDirection(origin + (direction * depthToNearClipPlane) + (direction * depth), direction);
}
...
wbx::Frustum Camera::frustum() const {
    wbx::Frustum f;
	frustum(farPlaneZ(), f);
    return f;
}

void Camera::frustum(const float farPlaneZ, wbx::Frustum& fr) const 
{
