...
shared_ptr<Instance> Animator::loadAnimation(shared_ptr<Instance> instance)
{
	...
	{
		throw wbx::runtime_error("Argument error: must be an Animation object");
	}
}
...
{
    WBXPROFILER_SCOPE("Physics", "Animator::onStepped");
...
/*override*/ void Animator::verifySetParent(const Instance* instance) const
{
    ...
    {
        throw wbx::runtime_error("Animator has to be placed under Humanoid or AnimationController!");
    }
}
...
{
	WBXASSERT(animatableJoints.size() > 0);
...
{
	shared_ptr<Reflection::ValueArray> playingAnimationTracks(wbx::make_shared<Reflection::ValueArray>());
