...
namespace WBX
{
    ...
    REFLECTION_BEGIN();
	static Reflection::RemoteEventDesc<AdService, void(int, UserInputService::Platform, bool)> event_sendServerRecordImpression(&AdService::sendServerRecordImpression, "SendServerRecordImpression", "userId", "platform", "wasSuccessful", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);

	static Reflection::RemoteEventDesc<AdService, void(bool, int, std::string)> event_sendClientAdVerificationResults(&AdService::sendClientVideoAdVerificationResults, "ClientAdVerificationResults", "canShowAd", "userId", "errorMessage", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);
    static Reflection::RemoteEventDesc<AdService, void(int, UserInputService::Platform)> event_serverAdVerification(&AdService::sendServerVideoAdVerification, "ServerAdVerification", "userId", "platform", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
    ...
    void AdService::showVideoAd()
    {
		...
		{
			if (wbx::ServiceProvider::find<wbx::UserInputService>(this))
			{
	...
	{
		wbx::DataModel::LegacyLock dmLock( wbx::DataModel::get(this), wbx::DataModelJob::Write);
	...
	{
		if (!wbx::Network::Players::serverIsPresent(this))
		{
			return;
		}
		...
		{
            std::string data = wbx::format("userId=%d&placeId=%d&deviceOSType=%s", userId, dataModel->getPlaceID(), platformToWebString(userPlatform).c_str());

			if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
			{
				apiService->postAsync(wbx::format("adimpression/validate-request?%s", data.c_str()),
					std::string("ServerCanPlayAd"),
					true, wbx::PRIORITY_DEFAULT, HttpService::TEXT_PLAIN,
	...
	void AdService::sendAdImpression(int userId, UserInputService::Platform platform, bool didPlay)
	{
		if(wbx::DataModel* dataModel = wbx::DataModel::get(this))
		{
			if (Network::Players::serverIsPresent(dataModel))
			{
                ...
                std::string data = wbx::format("userId=%i&placeId=%d&deviceOSType=%s&wasSuccessful=%s", userId, dataModel->getPlaceID(), platformToWebString(platform).c_str(), didPlay ? "true" : "false");

				if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
				{
					apiService->postAsync( wbx::format("adimpression/record-impression?%s", data.c_str()),
