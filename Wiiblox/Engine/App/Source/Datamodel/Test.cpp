...
template<>
bool wbx::StringConverter<FunctionalTest::Result>::convertToValue(const std::string& text, FunctionalTest::Result& value)
{
...
,scriptCount(0)
{
	...
	wbx::NetworkSettings &ns = wbx::NetworkSettings::singleton();
...
void TestService::message( std::string text, shared_ptr<Instance> source, int line )
{
	...
	if (onMessage)
		...
		output(wbx::MESSAGE_INFO, source, line, text);
}
...
void TestService::checkpoint( std::string text, shared_ptr<Instance> source, int line )
{
	if (onCheckpoint)
		...
		output(wbx::MESSAGE_INFO, source, line, "checkpoint " + text);
}
...
void TestService::warn( bool condition, std::string description, shared_ptr<Instance> source, int line )
{
	...
	if (onWarn)
		...
		output(wbx::MESSAGE_WARNING, source, line, description);
}
...
void TestService::check( bool condition, std::string description, shared_ptr<Instance> source, int line )
{
	...
	if (onCheck)
		...
		output(wbx::MESSAGE_ERROR, source, line, description);
}
...
void TestService::require2( bool condition, std::string description, shared_ptr<Instance> source, int line )
{
	...
	if (onCheck)
		...
		output(wbx::MESSAGE_ERROR, source, line, "fatal " + description);
...
void TestService::done()
{
	...
	if (onDone)
		...
		wbx::StandardOut::singleton()->print(wbx::MESSAGE_INFO, "TestService: Doesn't include any assertions");
	else
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "TestService: Run completed, tests: %u, warns: %u, errors: %u", testCount, warnCount, errorCount);
...
void TestService::stillWaiting(int runCount, double timeout)
{
	...
	if (onStillWaiting)
		...
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "TestService: Waiting a total of %g seconds for the test to end", timeout);
}
...
void TestService::onTimeout(int runCount, double timeout)
{
	...
	if (onCheck)
		onCheck(false, wbx::format("Tests failed to complete in %g seconds", timeout), shared_ptr<Instance>(), 0);
	else
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "TestService: Tests failed to complete in %g seconds", timeout);
...
void TestService::startScript( shared_ptr<Instance> instance )
{
	...
	{
		wbx::ScriptContext* scriptContext = wbx::ServiceProvider::create<wbx::ScriptContext>(this);
		if (!scriptContext)
		...
		options.identity = wbx::Security::Context::current().identity;
...
{
	wbx::PhysicsSettings &ps = wbx::PhysicsSettings::singleton();
...
private:
	...
	{
		if (doRBX_Test_Equality(lineNumber, line, "WBX_CHECK_EQUAL", "Check", "==", "!="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_CHECK_NE", "Check", "~=", "=="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_CHECK_GE", "Check", ">=", "<"))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_CHECK_LE", "Check", "<=", ">"))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_CHECK_GT", "Check", ">", "<="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_CHECK_LT", "Check", "<", ">="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_WARN_EQUAL", "Warn", "==", "!="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_WARN_NE", "Warn", "!=", "=="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_WARN_GE", "Warn", ">=", "<"))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_WARN_LE", "Warn", "<=", ">"))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_WARN_GT", "Warn", ">", "<="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_WARN_LT", "Warn", "<", ">="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_REQUIRE_EQUAL", "Require", "==", "!="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_REQUIRE_NE", "Require", "!=", "=="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_REQUIRE_GE", "Require", ">=", "<"))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_REQUIRE_LE", "Require", "<=", ">"))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_REQUIRE_GT", "Require", ">", "<="))
			return;
		if (doRBX_Test_Equality(lineNumber, line, "WBX_REQUIRE_LT", "Require", "<", ">="))
			return;

		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_WARN_MESSAGE", "Warn"))
			return;
		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_CHECK_MESSAGE", "Check"))
			return;
		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_REQUIRE_MESSAGE", "Require"))
			return;

		if (doRBX_Test_Throw(lineNumber, line, "WBX_WARN_THROW", "Warn"))
			return;
		if (doRBX_Test_Throw(lineNumber, line, "WBX_CHECK_THROW", "Check"))
			return;
		if (doRBX_Test_Throw(lineNumber, line, "WBX_REQUIRE_THROW", "Require"))
			return;

		if (doRBX_Test_NoThrow(lineNumber, line, "WBX_WARN_NO_THROW", "Warn"))
			return;
		if (doRBX_Test_NoThrow(lineNumber, line, "WBX_CHECK_NO_THROW", "Check"))
			return;
		if (doRBX_Test_NoThrow(lineNumber, line, "WBX_REQUIRE_NO_THROW", "Require"))
			return;

		if (doRBX_Test(lineNumber, line, "WBX_WARN", "Warn"))
			return;
		if (doRBX_Test(lineNumber, line, "WBX_CHECK", "Check"))
			return;
		if (doRBX_Test(lineNumber, line, "WBX_REQUIRE", "Require"))
			return;

		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_ERROR", "Error"))
			return;
		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_FAIL", "Fail"))
			return;
		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_MESSAGE", "Message"))
			return;
		if (doRBX_SimpleSubstitution(lineNumber, line, "WBX_CHECKPOINT", "Checkpoint"))
			return;
	...
	bool doRBX_Test_Equality(int lineNumber, const std::string& line, const char* macro, const char* f, const char* equality, const char* notEquality)
	{
		...
		// WBX_CHECK_EQUAL(a, b) ===> if a == b then Game:GetService('TestService'):Check(true, [==[a == b]==]) else Game:GetService('TestService'):Check(false,[==[test a == b failed [1 != 2]]==]) end
		...
		if (args.size() != 2)
			throw wbx::runtime_error("Expected 2 arguments but got %d", (int)args.size());
		...
		// First write everything before WBX_CHECK
	...
	bool doRBX_Test(int lineNumber, const std::string& line, const char* macro, const char* f)
	{
		...
		// WBX_CHECK(a) ===> Game:GetService('TestService'):Check(a,[==[test failed: a]==])
	...
	bool doRBX_Test_Throw(int lineNumber, const std::string& line, const char* macro, const char* f)
	{
		...
		// WBX_CHECK_THROW(a) ===> Game:GetService('TestService'):Check(ypcall(function() a end) == false, 'Exception expected in ' .. [==[a]==])
		...
		// First write everything before WBX_CHECK_THROW
		result << line.substr(0, i);
	...
	bool doRBX_Test_NoThrow(int lineNumber, const std::string& line, const char* macro, const char* f)
	{
		...
		// WBX_CHECK_NO_THROW(a) ===> Game:GetService('TestService'):Check(ypcall(function() a end) == true, 'Exception thrown by ' .. [==[a]==])
	...
	bool doRBX_SimpleSubstitution(int lineNumber, const std::string& line, const char* macro, const char* f)
	{
		...
		// WBX_CHECK(a) ===> Game:GetService('TestService'):Check(a,[==[a]==])
...
{
	if (source)
		wbx::StandardOut::singleton()->printf(type, "TestService.%s(%d): %s", source->getName().c_str(), line, message.c_str());
	else
		wbx::StandardOut::singleton()->printf(type, "TestService: %s", message.c_str());
}
...
Verb* TestService::getVerb( std::string name )
{
	...
	if (!verb)
		throw wbx::runtime_error("Verb %s does not exist", name.c_str());
	return verb;
}
#endif
