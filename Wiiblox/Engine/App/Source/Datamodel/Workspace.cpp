...
// static			
bool Workspace::serverIsPresent(const Instance* context)				// shortcut for wbx::Network::Players::serverIsPresent
{
	return wbx::Network::Players::serverIsPresent(context, false);		// if not in the datamodel, will go false
}

// static
bool Workspace::clientIsPresent(const Instance* context)				// shortcut for wbx::Network::Players::clientIsPresent
{
	return wbx::Network::Players::clientIsPresent(context, false);		// if not in the datamodel, will go false
}
...
{
	WBXASSERT(getWorldIfInWorkspace(context));
	Workspace* workspace = Workspace::getWorkspaceIfInWorkspace(context);
	WBXASSERT(workspace);
	WBXASSERT(workspace != context);
...
bool Workspace::contextInWorkspace(const Instance* context)
{
	...
	WBXASSERT(!workspace || (workspace != context));
...
static Reflection::BoundFuncDesc<Workspace, shared_ptr<const Reflection::Tuple>(WBXRay, shared_ptr<Instance>, bool, bool)> workspace_FindPartOnRay(&Workspace::getRayHit<Instance>, "FindPartOnRay", "ray", "ignoreDescendentsInstance", shared_ptr<Instance>(), "terrainCellsAreCubes", false, "ignoreWater", false, Security::None);
static Reflection::BoundFuncDesc<Workspace, shared_ptr<const Reflection::Tuple>(WBXRay, shared_ptr<Instance>, bool, bool)> dep_FindPartOnRay(&Workspace::getRayHit<Instance>, "findPartOnRay", "ray", "ignoreDescendentsInstance", shared_ptr<Instance>(), "terrainCellsAreCubes", false, "ignoreWater", false, Security::None, Reflection::Descriptor::Attributes::deprecated(workspace_FindPartOnRay));
...
static Reflection::BoundFuncDesc<Workspace, shared_ptr<const Reflection::Tuple>(WBXRay, shared_ptr<const Instances>, bool, bool)> workspace_FindPartOnRay2(&Workspace::getRayHit<const Instances>, "FindPartOnRayWithIgnoreList", "ray", "ignoreDescendentsTable", "terrainCellsAreCubes", false, "ignoreWater", false, Security::None);
...
shared_ptr<const Reflection::Tuple> Workspace::getRayHit(WBXRay ray, shared_ptr<IgnoreType> ignoreInstance, bool terrainCellsAreCubes, bool ignoreWaterCells)
{
...
{
	WBXASSERT(currentCommand.get() != NULL);
...
namespace {
	...
	{
		WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "NetworkFilteringEnabled");
	}
} // namespace
...
{
	return dynamic_cast<const IAdornable*>(instance)!=NULL;		// TODO: Hmmm. Is this a good choice? What about wbx::Message?
}
...
bool Workspace::startDecalDrag(Decal *decal, wbx::InsertMode insertMode)
{
...
void Workspace::createTerrain()
{
	if(!terrain)
	{
		...
		Vector3 WBXSize = part->getPartSizeXml();
		CoordinateFrame clusterFrame(Vector3(-2,WBXSize.y/2, -2));
...
void Workspace::start()
{
	...
	WBXASSERT(!getCurrentMouseCommand()->captured());

	// makes sure building tools are not greyed out
	shared_ptr<wbx::CoreGuiService> coreGuiService = shared_from(ServiceProvider::find<wbx::CoreGuiService>(this));
	if(coreGuiService)
		if(wbx::ImageLabel* frame = Instance::fastDynamicCast<wbx::ImageLabel>(coreGuiService->findFirstChildByName2("Frame",true).get()))
			frame->setZIndex(1);
}
...
{
	WBXASSERT(fallenParts.size() == 0);
	WBXASSERT(fallenPrimitives.size() == 0);
	...
	if (Network::Players::getGameMode(this) == Network::DPHYS_CLIENT)
	{
		for (size_t i = 0; i < fallenParts.size(); ++i)
		{
			...
			part->setNetworkOwnerAndNotify(wbx::Network::NetworkOwner::Server());
		}
	}
...
{
	WBXASSERT(world->isAssembled());				// testing - assemble in 
    ...
    {
        wbx::Network::Player* localPlayer = wbx::Network::Players::findLocalPlayer(DataModel::get( this ));
        int id = 0;
        if( localPlayer ) id = localPlayer->getUserID();
        getWorld()->setUserId( id );
    }

    world->setPhysicsAnalyzerEnabled( wbx::PhysicsSettings::singleton().getPhysicsAnalyzerState() );
...
void Workspace::setMouseCommand(shared_ptr<MouseCommand> newMouseCommand, bool allowPluginOverride)
{
	...
	wbx::DataModel *dataModel = (DataModel*)ServiceProvider::findServiceProvider(this);
	wbx::Plugin *activePlugin = PluginManager::singleton()->getActivePlugin(dataModel);
	...
	WBXASSERT(newMouseCommand.get());

	if (newMouseCommand != currentCommand) 
	{
		WBXASSERT((currentCommand.get() == NULL) || !currentCommand.get()->captured());
...
double Workspace::getRealPhysicsFPS(void)
{
	...
	if (DFFlag::ReportElevatedPhysicsFPSToGA && realPhysicsFPS > ((double) DFInt::ElevatedPhysicsFPSReportThresholdTenths / 10.0))
	{
		...
		if (dm)
		{
			...
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "ElevatedPhysicsFPSDetected_100", 
					boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
			}
			else if (realPhysicsFPS > 90.0)
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "ElevatedPhysicsFPSDetected_90", 
					boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
			}
			else if (realPhysicsFPS > 80.0)
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "ElevatedPhysicsFPSDetected_80", 
					boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
			}
			else if (realPhysicsFPS > 70.0)
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "ElevatedPhysicsFPSDetected_70", 
					boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
			}
			else if (realPhysicsFPS > 65.0)
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "ElevatedPhysicsFPSDetected_65", 
					boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
			}
			else
			{
				static boost::once_flag flag = BOOST_ONCE_INIT;
				boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "ElevatedPhysicsFPSDetected_SetMinimum", 
					boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
			}
		}
	}
...
void Workspace::render3dAdorn(Adorn* adorn)
	...
	if(show3DGrid && (!localPlayer || Network::Players::isCloudEdit(this)))
				wbx::DrawAdorn::zeroPlaneGrid(adorn, *getCamera(), gridSizeModifier, 0.05, Color3(0.3f,0.3f,0.3f), Color3(0.4f,0.4f,0.4f));

		if(showAxisWidget)
			wbx::DrawAdorn::axisWidget(adorn, *getCamera());
	}
...
void Workspace::requestFirstPersonCamera(bool firstPersonOn, bool cameraTransitioning, int controlMode)	
{
	...
	else if( ( (firstPersonOn && !cameraTransitioning) || wbx::GameBasicSettings::singleton().mouseLockedInMouseLockMode() ) && !hasModalGuiObjects() )
	{
		userInput->centerCursor();
		userInputService->setMouseWrapMode(UserInputService::WRAP_CENTER);
	}
	else if(wbx::GameBasicSettings::singleton().inHybridMode() && !inRightMousePan && !inMiddleMouseTrack)
		userInputService->setMouseWrapMode(UserInputService::WRAP_HYBRID);
	else 
	{
		if (!wbx::GameBasicSettings::singleton().inMousepanMode() && !inRightMousePan && !inMiddleMouseTrack)
			userInputService->setMouseWrapMode(UserInputService::WRAP_AUTO);
	}
}
...
GuiResponse Workspace::handleSurfaceGui(const shared_ptr<InputObject>& event)
{
	...
	{
		wbx::Network::Player* player = Network::Players::findLocalPlayer(this);
...
GuiResponse Workspace::process(const shared_ptr<InputObject>& event)
{
	...
	case InputObject::TYPE_MOUSEBUTTON2:
		{
			WBXASSERT(event->isRightMouseDownEvent() || event->isRightMouseUpEvent());
	...
	case InputObject::TYPE_MOUSEBUTTON1:
		{
			WBXASSERT(event->isLeftMouseDownEvent() || event->isLeftMouseUpEvent());
	...
	case InputObject::TYPE_MOUSEWHEEL:
		{
			WBXASSERT(event->isMouseWheelBackward() || event->isMouseWheelForward());
	...
	WBXASSERT(processingCommand.get() != NULL);
	if (processingCommand->captured()) {
...
{ 
	...
	if (expSolverEnabled_Replicate && FFlag::UsePGSSolver)
	{
		...
		if (dm)
		{
			int placeID = dm->getPlaceID();
			static boost::once_flag flag = BOOST_ONCE_INIT;
			boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent, GA_CATEGORY_GAME, "PGSSolverActivated", 
				boost::lexical_cast<std::string>(placeID).c_str(), 0, false));
		}
	}
}
...
void callGAForPhysicalProperties(const DataModel* dm, PhysicalPropertiesMode mode)
{
	...
	WiibloxGoogleAnalytics::trackEvent( GA_CATEGORY_GAME, gaMessage.c_str(), boost::lexical_cast<std::string>(placeID).c_str());
}
...
{	
	...
	if (getWorld()->getPhysicalPropertiesMode() != mode)
	{
		...
		{
			wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "Cannot change PhysicalPropertiesMode during Runtime");
		}
	}
}
	...
	static shared_ptr<WorkspaceStatsItem> create(const Workspace* workspace, const World* world, const RunService* runService)
	{
		...
		Stats::Item* pDataModelStep = result->createBoundChildItem(*workspace->profileDataModelStep);
			Stats::Item* pWorkspaceStep = pDataModelStep->createBoundChildItem(*workspace->profileWorkspaceStep);
				...
				std::vector<wbx::Profiling::CodeProfiler*> worldProfilers;
...
{
	std::size_t result = boost::hash<const wbx::PartInstance*>()(p.p1.get());
