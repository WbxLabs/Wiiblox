...
{
	errorFunction( wbx::format("InsertService getLatestAssetVersion : Request Failed because %s", error.c_str()) );
}
...
void InsertService::getLatestAssetVersion(int id, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
	{
		...
		{
			apiService->getAsync(wbx::format(DFString::AssetVersionsUrl.c_str(), id, dm->getPlaceID()), true, wbx::PRIORITY_DEFAULT,
...
void InsertService::privateLoadAsset(int id, bool isAssetVersion, boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	int userId = -1;
	if (wbx::Network::Player* localPlayer = Network::Players::findLocalPlayer(this))
	{
...
void InsertService::insert(shared_ptr<Instance> instance)
{
	if(DataModel* dataModel = (DataModel*)getParent())
	{
		...
		// make sure we aren't trying to manipulate a Wiibloxlocked object under the wrong permission
		if (instance->getWiibloxLocked())
			wbx::Security::Context::current().requirePermission(wbx::Security::Plugin, "Wiiblox locked in InsertService:Insert()");
...
{
    WBXASSERT(instance);
    WBXASSERT(instance->isDescendantOf(this));
...
void InsertService::backendInsertReady(std::string key, shared_ptr<Instance> pseudoRoot, AsyncHttpQueue::RequestResult requestResult, shared_ptr<std::exception> requestError)
{
	...
	else
	{
		if (DFFlag::GASendInsertRequestFail)
		{
			...
			if (dm != NULL)
			{
				wbx::Analytics::GoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "InsertServiceException", "PlaceID", dm->getPlaceID());
			}
		}
		if (DFFlag::InfluxSendInsertRequestFail)
		{
			...
			if (dm != NULL)
			{
				boost::unordered_set<wbx::Analytics::InfluxDb::Point> points;
				rapidjson::Value placeIDNode;
				placeIDNode.SetInt(dm->getPlaceID());
				wbx::Analytics::InfluxDb::Point point("PlaceID",placeIDNode);
				points.insert(point);
				wbx::Analytics::InfluxDb::reportPointsV2("InsertServiceFailure", points, 0);
			}
		}
	...
	if (DFFlag::MaterialPropertiesEnabled)
	{
		...
		{		
			for (size_t i = 0; i < instances->size(); ++i)
				wbx::PartInstance::convertToNewPhysicalPropRecursive((*instances)[i].get());
		}
	}
...
void InsertService::populateExtraInsertUrlParams(std::stringstream& url, bool clientInsert)
{
    ...
    if (!FFlag::AllowInsertFreeModels || !allowInsertFreeModels || clientInsert) {
        wbx::DataModel* dataModel = DataModel::get(this);
        url << "&serverplaceid=";
