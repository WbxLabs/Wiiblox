...
static const ContentId kSmoothTerrainMaterials("WBXasset://terrain/materials.json");

namespace Reflection
{
	template<> Reflection::EnumDesc<CellMaterial>::EnumDesc()
		:wbx::Reflection::EnumDescriptor("CellMaterial")
	{
	...
	template<> Reflection::EnumDesc<CellBlock>::EnumDesc()
		:wbx::Reflection::EnumDescriptor("CellBlock")
	{
	...
	template<> Reflection::EnumDesc<CellOrientation>::EnumDesc()
		:wbx::Reflection::EnumDescriptor("CellOrientation")
	{
	...
	template<> Reflection::EnumDesc<WaterCellForce>::EnumDesc()
		:wbx::Reflection::EnumDescriptor("WaterForce")
	{
	...
	template<> Reflection::EnumDesc<WaterCellDirection>::EnumDesc()
		:wbx::Reflection::EnumDescriptor("WaterDirection")
	{
...
template<>
bool wbx::StringConverter<CellMaterial>::convertToValue(const std::string& text, CellMaterial& value)
{
	return Reflection::EnumDesc<CellMaterial>::singleton().convertToValue(text.c_str(),value);
}
template<>
bool wbx::StringConverter<CellBlock>::convertToValue(const std::string& text, CellBlock& value)
{
	return Reflection::EnumDesc<CellBlock>::singleton().convertToValue(text.c_str(),value);
}
template<>
bool wbx::StringConverter<CellOrientation>::convertToValue(const std::string& text, CellOrientation& value)
{
	return Reflection::EnumDesc<CellOrientation>::singleton().convertToValue(text.c_str(),value);
}
template<>
bool wbx::StringConverter<WaterCellForce>::convertToValue(const std::string& text, WaterCellForce& value)
{
	return Reflection::EnumDesc<WaterCellForce>::singleton().convertToValue(text.c_str(),value);
}
template<>
bool wbx::StringConverter<WaterCellDirection>::convertToValue(const std::string& text, WaterCellDirection& value)
{
...
{
	throw wbx::runtime_error("GetTouchingParts is not a valid member of Terrain");
}
...
void MegaClusterInstance::setPartSizeXml(const Vector3& WBXSize)  
{
	// do nothing. this property should be essentially read only
}

void MegaClusterInstance::setPartSizeUi(const Vector3& WBXSize)
{
	// do nothing. this property should be essentially read only
}
...
void MegaClusterInstance::verifySetParent(const Instance* instance) const
{
	...
	if( instance != NULL && ((workspace == NULL) || (workspace->getTerrain() != NULL)) ){
		throw wbx::runtime_error("Unable to change Terrain's parent. Workspace already has Terrain");
	}
...
Surface MegaClusterInstance::getSurface(const WBXRay& gridRay, int& surfaceId)
{
...
{
    WBXASSERT(!getParent());
    WBXASSERT(!voxelGrid && !smoothGrid);
...
int MegaClusterInstance::writeVoxels(lua_State* L)
{
	...
	if (!isArray(L, 4, box.getSizeX()))
		throw wbx::runtime_error("Bad argument materials to 'WriteVoxels' (%dx%dx%d array expected)", box.getSizeX(), box.getSizeY(), box.getSizeZ());

	if (!isArray(L, 5, box.getSizeX()))
		throw wbx::runtime_error("Bad argument occupancy to 'WriteVoxels' (%dx%dx%d array expected)", box.getSizeX(), box.getSizeY(), box.getSizeZ());
	...
	for (int x = 0; x < box.getSizeX(); ++x)
	{
		...
		if (!isArray(L, -2, box.getSizeY()))
			throw wbx::runtime_error("Bad argument materials[%d] to 'WriteVoxels' (%dx%d array expected)", x + 1, box.getSizeY(), box.getSizeZ());

		if (!isArray(L, -1, box.getSizeY()))
			throw wbx::runtime_error("Bad argument occupancy[%d] to 'WriteVoxels' (%dx%d array expected)", x + 1, box.getSizeY(), box.getSizeZ());
		...
		for (int y = 0; y < box.getSizeY(); ++y)
		{
			...
			if (!isArray(L, -2, box.getSizeZ()))
				throw wbx::runtime_error("Bad argument materials[%d][%d] to 'WriteVoxels' (%d-element array expected)", x + 1, y + 1, box.getSizeZ());

			if (!isArray(L, -1, box.getSizeZ()))
				throw wbx::runtime_error("Bad argument occupancy[%d][%d] to 'WriteVoxels' (%d-element array expected)", x + 1, y + 1, box.getSizeZ());
