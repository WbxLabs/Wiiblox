...
template<>
bool wbx::StringConverter<ChatService::ChatColor>::convertToValue(const std::string& text, ChatService::ChatColor& value)
{
...
void ChatService::gotFilteredStringSuccess(std::string response, Network::Player* player, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	if (!response.empty())
	{
		...
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "ChatService:FilterString", "Filter failed because could not parse response");

		errorFunction("ChatService:FilterString could not parse response");
	}
	else
	{
		wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "ChatService:FilterString", "Filter failed because response was empty");
...
{
	wbx::WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "ChatService:FilterStringForPlayerAsync", wbx::format("placeId: %i", placeId).c_str());
}
...
void ChatService::filterStringForPlayer(std::string stringToFilter, shared_ptr<Instance> playerToFilterFor, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	if(wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
	{
		if (DFFlag::UseComSiftUpdatedWebChatFilterParamsAndHeader)
		{
			apiService->postAsyncWithAdditionalHeaders("moderation/filtertext", params.str(), true, wbx::PRIORITY_DEFAULT, wbx::HttpService::APPLICATION_URLENCODED, headers,
		...
		{
			apiService->postAsync("moderation/filtertext", params.str(), true, wbx::PRIORITY_DEFAULT, wbx::HttpService::APPLICATION_URLENCODED,
				boost::bind(&ChatService::gotFilteredStringSuccess, this, _1, playerFilter, resumeFunction, errorFunction),
				boost::bind(&ChatService::gotFilterStringError, this, _1, errorFunction) );
		}
	}
