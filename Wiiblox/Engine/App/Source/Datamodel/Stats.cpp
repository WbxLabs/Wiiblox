...
namespace JNI
{
extern std::string WiibloxVersion; // JNIMain.cpp
}
}
#endif
...
DYNAMIC_FASTSTRINGVARIABLE(HttpInfluxUser, "Wiiblox")
...
class TotalCountTimeIntervalItem : public wbx::DescribedNonCreatable<TotalCountTimeIntervalItem, wbx::Stats::Item, sTotalCountTimeIntervalItem> 
{
	const wbx::TotalCountTimeInterval<>& tcti;
public:
	TotalCountTimeIntervalItem(const wbx::TotalCountTimeInterval<>* tcti) : tcti(*tcti) {}
	virtual void update() 
	{
		int v = tcti.getCount();
		if (val != v)
		{
			val = v;
			sValue = wbx::format("%d", v);
		}
	}
};

template<class T>
class RunningAverageItem : public wbx::Stats::Item 
{
	const wbx::RunningAverage<T, double>& ra;
public:
	RunningAverageItem(const wbx::RunningAverage<T, double>* ra):ra(*ra) {}
	...
	virtual void update() {
		...
		{
			val = v;
			sValue = wbx::format("%g (%.0f%%CV)", v, 100.0 * ra.coefficient_of_variation());
		}
	}
};
...
class RunningAverageItemInt : public wbx::DescribedNonCreatable<RunningAverageItemInt, RunningAverageItem<int>, sRunningAverageItemInt> 
{
public:
	RunningAverageItemInt(const wbx::RunningAverage<int, double>* ra)
		:wbx::DescribedNonCreatable<RunningAverageItemInt, RunningAverageItem<int>, sRunningAverageItemInt>(ra) 
	{}
};
...
class RunningAverageItemDouble : public wbx::DescribedNonCreatable<RunningAverageItemDouble, RunningAverageItem<double>, sRunningAverageItemDouble> 
{
public:
	RunningAverageItemDouble(const wbx::RunningAverage<double, double>* ra)
		:wbx::DescribedNonCreatable<RunningAverageItemDouble, RunningAverageItem<double>, sRunningAverageItemDouble>(ra) 
	{}
};

template<wbx::Time::SampleMethod sampleMethod = wbx::Time::Benchmark>
class RunningAverageTimeIntervalItem : public wbx::Stats::Item 
{
    const wbx::RunningAverageTimeInterval<sampleMethod>& ra;
public:
    RunningAverageTimeIntervalItem(const wbx::RunningAverageTimeInterval<sampleMethod>* ra):ra(*ra) {}
    ...
    virtual void update() {
        ...
        {
            val = v;
            sValue = wbx::format("%g (%.2f%%CV)", v, 100.0 * ra.coefficient_of_variation());
        }
    }
};
...
class RunningAverageTimeIntervalItemTimeBenchmark : public wbx::DescribedNonCreatable<RunningAverageTimeIntervalItemTimeBenchmark, RunningAverageTimeIntervalItem<wbx::Time::Benchmark>, sRunningAverageTimeInterval>
{
public:
    RunningAverageTimeIntervalItemTimeBenchmark(const wbx::RunningAverageTimeInterval<wbx::Time::Benchmark>* ra)
        :wbx::DescribedNonCreatable<RunningAverageTimeIntervalItemTimeBenchmark, RunningAverageTimeIntervalItem<wbx::Time::Benchmark>, sRunningAverageTimeInterval>(ra)
    {}
};

extern const char *const sProfilingItem = "ProfilingItem";

class ProfilingItem : public wbx::DescribedNonCreatable<ProfilingItem, wbx::Stats::Item, sProfilingItem> 
{
	const wbx::Profiling::Profiler& p;
public:
	ProfilingItem(const wbx::Profiling::Profiler* p):p(*p) {}

	// Returns a Tuple of 4 numbers
	shared_ptr<const wbx::Reflection::Tuple> getTimes(double window)
	{
		wbx::Profiling::Bucket b = p.getWindow(window);
		shared_ptr<wbx::Reflection::Tuple> result(new wbx::Reflection::Tuple(3));
	...
	shared_ptr<const wbx::Reflection::Tuple> getTimesForFrames(int frames)
	{
		wbx::Profiling::Bucket b = p.getFrames(frames);
		shared_ptr<wbx::Reflection::Tuple> result(new wbx::Reflection::Tuple(3));
		result->values[0] = b.getWallTime();
		result->values[1] = b.getSampleTime();
		result->values[2] = b.frames;
	...
	virtual void update() {
		const double window = wbx::Profiling::Profiler::profilingWindow;
		wbx::Profiling::Bucket b = p.getWindow(window);
		if (b.sampleTimeElapsed>0)
		{
			...
			if (b.frames>0)
			{
				std::string t = wbx::Log::formatTime(b.getNominalFramePeriod());
...
REFLECTION_BEGIN();
static wbx::Reflection::BoundFuncDesc<ProfilingItem, shared_ptr<const wbx::Reflection::Tuple>(double)> func_GetTimes(&ProfilingItem::getTimes, "GetTimes", "window", 0.0, wbx::Security::Plugin);
static wbx::Reflection::BoundFuncDesc<ProfilingItem, shared_ptr<const wbx::Reflection::Tuple>(int)> func_GetTimesForFrames(&ProfilingItem::getTimesForFrames, "GetTimesForFrames", "frames", 1, wbx::Security::Plugin);
REFLECTION_END();
...
{		
	namespace Stats
	{
		...
		static void HttpHelper(std::string* response, std::exception* exception, const std::string& url)
		{
			if (exception)
			{
#if !defined(WBX_PLATFORM_DURANGO) // remove this define in future?
		...
		{
			userId = wbx::format("%d", id);
			wbx::Analytics::setUserId(id);
		}
		...
		void reportGameStatus(const std::string& status, bool blocking)
		{
			...
			httpPost(wbx::format("%sclient-status/set?browserTrackerId=%s&status=%s", baseUrl.c_str(), Http::urlEncode(browserTrackerId).c_str(), Http::urlEncode(status).c_str()), "", blocking, "*/*");
		}

		const char* const  sStats = "Stats";

        REFLECTION_BEGIN();
		static Reflection::BoundFuncDesc<StatsService, void(std::string, shared_ptr<const Reflection::ValueTable>)> func_report(&StatsService::report, "Report", "category", "data", wbx::Security::WiibloxScript);
		static Reflection::BoundFuncDesc<StatsService, void(bool)> func_reportTaskScheduler(&StatsService::reportTaskScheduler, "ReportTaskScheduler", "includeJobs", false, wbx::Security::WiibloxScript);
		static Reflection::BoundFuncDesc<StatsService, void()> func_reportJobsStepWindow(&StatsService::reportJobsStepWindow, "ReportJobsStepWindow", wbx::Security::WiibloxScript);
		static Reflection::BoundFuncDesc<StatsService, void(std::string)> prop_reportUrl(&StatsService::setReportUrl, "SetReportUrl", "url", Security::WiibloxScript);
		static Reflection::BoundProp<std::string> prop_reporterType("ReporterType", "Reporting", &StatsService::reporterType, Reflection::PropertyDescriptor::UI, Security::WiibloxScript);
		static Reflection::BoundProp<double> prop_minReportInterval("MinReportInterval", "Reporting", &StatsService::minReportInterval, Reflection::PropertyDescriptor::UI, Security::WiibloxScript);
		
		const char* const  sStatsItem = "StatsItem";

		static Reflection::BoundFuncDesc<Item, std::string()> func_values(&Item::getStringValue2, "GetValueString", wbx::Security::Plugin);
		static Reflection::BoundFuncDesc<Item, double()> func_value(&Item::getValue, "GetValue", wbx::Security::Plugin);
        REFLECTION_END();

		static void reportResult(std::string * message, std::exception * exception)
		{
			if (exception)
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "StatsService Report failed: %s", exception->what());
			//if (message)
			//	wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "StatsService Report result: %s", message->c_str());
		}
		...
		void JsonWriter::writeValue(const Reflection::Variant& value)
		{
			...
			else if (value.type() == wbx::Reflection::Type::singleton<bool>())
			{
				s << value.get<std::string>();	// v = "true" | "false"
			}
			else if (value.type() == wbx::Reflection::Type::singleton< shared_ptr<const Reflection::ValueTable> >())
			{
				s << "{ "; 
			...
			else if (value.type() == wbx::Reflection::Type::singleton< shared_ptr<const Reflection::ValueArray> >())
			{
				s << "[ "; 
				bool needComma = false;
				JsonWriter writer(s, needComma);
		...
		bool StatsService::addHeader(shared_ptr<std::stringstream> stream)
		{
			*stream << "{";
			...
			if (wbx::DataModel* dataModel = DataModel::get(this))
			{
...
static const bool jobsAsArray = true;
		...
		std::string StatsService::getDefaultReportUrl(const std::string& baseUrlInput, const std::string& shard) {
			...
			WBXASSERT(baseUrl.substr(0, 7) == "http://");
		...
		void StatsService::reportJobsStepWindow()
		{
			...
			*stream << ", \"ThreadPoolSize\": "     << wbx::TaskScheduler::singleton().threadPoolSize();
		...
		void StatsService::reportTaskScheduler(bool includeJobs)
		{
			...
			*stream << ", \"ThreadAffinity\": "     << wbx::TaskScheduler::singleton().threadAffinity();
			*stream << ", \"NumSleepingJobs\": "    << wbx::TaskScheduler::singleton().numSleepingJobs();
			*stream << ", \"NumWaitingJobs\": "     << wbx::TaskScheduler::singleton().numWaitingJobs();
			*stream << ", \"NumRunningJobs\": "     << wbx::TaskScheduler::singleton().numRunningJobs();
			*stream << ", \"ThreadPoolSize\": "     << wbx::TaskScheduler::singleton().threadPoolSize();
			*stream << ", \"SchedulerRate\": "      << wbx::TaskScheduler::singleton().schedulerRate();
			*stream << ", \"SchedulerDutyCyclePerThread\": " << wbx::TaskScheduler::singleton().getSchedulerDutyCyclePerThread();
			*stream << ", \"PriorityMethod\": "     << wbx::TaskScheduler::priorityMethod;
		...
		bool StatsService::checkLastReport(const std::string& category)
		{
			...
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "The report '%s' was throttled. Adhere to MinReportInterval", category.c_str());
			return false;
		}
		...
		static void runScript(shared_ptr<DataModel> d, std::string script)
		{
			try
			{
				...
				if (sc)
				{
					...
					sc->executeInNewThread(Security::WiibloxGameScript_, verifiedSource, "StatsReporting");
					#if defined(WIN32) && defined(I_AM_GOY_THAT_LOVES_VMPROTECT)
					VMProtectEnd();
					#endif
				}
		...
		{
			if (error)
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "StatsService script url retrieval failed: %s", error->what());
			else if (json)
			{
				if (shared_ptr<DataModel> d = dm.lock())
				{
					...
					{
						wbx::StandardOut::singleton()->printf(wbx::MESSAGE_SENSITIVE, "StatsService script %s retrieval failed: %s", url.c_str(), e.what());
						wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "StatsService script retrieval failed: %s", e.what());
						return;
					}
					...
					{
						wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "StatsService failed to run script: %s", e.what());
					}
...
#include <stdint.h>
        ...
        template<>
        Item* Item::createBoundChildItem(const char* name, const RunningAverageTimeInterval<wbx::Time::Benchmark>& ra)
        {
...
WBX_REGISTER_CLASS(wbx::Stats::Item);
WBX_REGISTER_CLASS(RunningAverageItemInt);
WBX_REGISTER_CLASS(RunningAverageItemDouble);
WBX_REGISTER_CLASS(ProfilingItem);
WBX_REGISTER_CLASS(wbx::Stats::StatsService);
WBX_REGISTER_CLASS(TotalCountTimeIntervalItem);
WBX_REGISTER_CLASS(RunningAverageTimeIntervalItemTimeBenchmark);

void wbx::registerStatsClasses()
{
	// Prevent optimizer from stripping code
	wbx::Stats::StatsService::classDescriptor();
}
