...
#include <fstream>
#include "wbx/WbxDbgInfo.h"
#include <boost/format.hpp> 

#include "util/WbxStringTable.h"
...
static void dummyLoader(wbx::DataModel*) {}
...
boost::function<void(wbx::DataModel*)> DataModel::loaderFunc = boost::function<void(wbx::DataModel*)>(dummyLoader);

REFLECTION_BEGIN();
static Reflection::BoundFuncDesc<DataModel, void()> func_loadPlugins(&DataModel::loadPlugins, "LoadPlugins", Security::Wiiblox);
...
#if defined(WBX_STUDIO_BUILD) || defined (WBX_RCC_SECURITY) || defined (WBX_TEST_BUILD)
...
static Reflection::BoundFuncDesc<DataModel, void(ContentId)>  saveFunction(&DataModel::save, "Save", "url", Security::Wiiblox);
...
static const Security::Permissions kHttpPermission = Security::WiibloxScript;
...
static Reflection::BoundFuncDesc<DataModel, void(std::string, std::string, std::string, std::string, std::string)>  func_reportMeasurement(&DataModel::reportMeasurement, "ReportMeasurement", "id", "key1", "value1", "key2", "value2", Security::WiibloxScript);
static Reflection::BoundFuncDesc<DataModel, void(std::string, std::string, std::string, int)> googleAnalyticsFunction(&DataModel::luaReportGoogleAnalytics, "ReportInGoogleAnalytics", "category", "action", "custom", "label", "none", "value", 0, Security::WiibloxScript);
...
static Reflection::PropDescriptor<DataModel, bool> prop_isPersonalServer("IsPersonalServer", category_State, &DataModel::getIsPersonalServer, &DataModel::setIsPersonalServer, Reflection::PropertyDescriptor::SCRIPTING, Security::WiibloxScript);
static Reflection::PropDescriptor<DataModel, bool> prop_canSaveLocal("LocalSaveEnabled", category_State, &DataModel::canSaveLocal, NULL, Reflection::PropertyDescriptor::UI, Security::WiibloxScript);

static Reflection::BoundYieldFuncDesc<DataModel, bool()> saveToWiibloxFunction(&DataModel::saveToWiiblox, "SaveToWiiblox", Security::WiibloxScript);
static Reflection::BoundCallbackDesc<bool()> requestShutdownCallback("RequestShutdown", &DataModel::requestShutdownCallback, Security::WiibloxScript);
static Reflection::BoundFuncDesc<DataModel, void(bool)> finishShutdownFunction(&DataModel::completeShutdown, "FinishShutdown", "localSave", Security::WiibloxScript);
...
static Reflection::BoundFuncDesc<DataModel, void(int, bool)>  setPlaceId(&DataModel::setPlaceID, "SetPlaceId", "placeId", "WiibloxPlace", false, Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, void(int, bool)>  setPlaceIdDeprecated(&DataModel::setPlaceID, "SetPlaceID", "placeID", "WiibloxPlace", false, Security::Plugin);
...
static Reflection::PropDescriptor<DataModel, bool> prop_forceR15("ForceR15", category_State, &DataModel::getForceR15, &DataModel::setForceR15, Reflection::PropertyDescriptor::REPLICATE_ONLY, Security::Wiiblox);
...
template<>
bool wbx::StringConverter<DataModel::CreatorType>::convertToValue(const std::string& text, DataModel::CreatorType& value)
{
	return Reflection::EnumDesc<DataModel::CreatorType>::singleton().convertToValue(text.c_str(),value);
}

template<>
bool wbx::StringConverter<DataModel::Genre>::convertToValue(const std::string& text, DataModel::Genre& value)
{
	return Reflection::EnumDesc<DataModel::Genre>::singleton().convertToValue(text.c_str(),value);
}

template<>
bool wbx::StringConverter<DataModel::GearGenreSetting>::convertToValue(const std::string& text, DataModel::GearGenreSetting& value)
{
	return Reflection::EnumDesc<DataModel::GearGenreSetting>::singleton().convertToValue(text.c_str(),value);
}

template<>
bool wbx::StringConverter<DataModel::GearType>::convertToValue(const std::string& text, DataModel::GearType& value)
{
	return Reflection::EnumDesc<DataModel::GearType>::singleton().convertToValue(text.c_str(),value);
}

template<>
bool wbx::StringConverter<DataModel::SaveFilter>::convertToValue(const std::string& text, DataModel::SaveFilter& value)
{
...
public:
	...
	wbx::timestamped_safe_queue<Task> tasks;
...
private:
	...
	void processTasks()
	{
		...
		while ((count < size) && (tasks.pop_if_present(task))) // need to keep count because task can add more tasks
		{
			...
			wbx::Time timeNow = wbx::Time::now<wbx::Time::Fast>();
...
{
    static wbx::atomic<int> count = 0;
	return wbx::format("DataModel-%d", (int)++count);
}

bool DataModel::canSave(const wbx::Instance* instance)
{
...
namespace {
	...
	{
		WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "SavePlace");
	}
} // namespace
...
DataModel::RequestShutdownResult DataModel::requestShutdown(bool useLuaShutdownForSave)
{
	...
	Visit* visit = wbx::ServiceProvider::find<Visit>(this);
...
void DataModel::doDataModelSetup(shared_ptr<DataModel> dataModel, bool startHeartbeat, bool shouldShowLoadingScreen)
{
	...
	wbx::DataModel::LegacyLock::mainThreadId = GetCurrentThreadId();
	...
	if (shouldShowLoadingScreen && !(TeleportService::didTeleport() && TeleportService::getCustomTeleportLoadingGui()))
	{
		try
		{
			...
			{
				if (ScriptContext* sc = wbx::ServiceProvider::create<ScriptContext>(dataModel.get()))
				{
					sc->executeInNewThread(wbx::Security::WiibloxGameScript_, *source, "LoadingScript");
				}
			}
		}
...
shared_ptr<DataModel> DataModel::createDataModel(bool startHeartbeat, wbx::Verb* lockVerb, bool shouldShowLoadingScreen)
{
	FASTLOG1(FLog::CloseDataModel, "Create DataModel - heartbeat(%d)", startHeartbeat);
		
	shared_ptr<DataModel> dataModel = Creatable<Instance>::create<wbx::DataModel>(lockVerb);
...
void DataModel::doCloseDataModel(shared_ptr<DataModel> dataModel)
{
	...
	wbx::Security::Impersonator impersonate(wbx::Security::LocalGUI_);
	...
	WBXASSERT(dataModel->isInitialized);
	...
	WBXASSERT(dataModel->onlyJobsLeftForThisArbiterAreGenericJobs());

	FASTLOG(FLog::CloseDataModel, "Removing GenericJobs...");
	{
		bool blockingRemove = DataModel::BlockingDataModelShutdown && wbx::DebugSettings::singleton().blockingRemove;
...
void DataModel::closeDataModel(shared_ptr<DataModel> dataModel)
{
	...
	if(dataModel->onCloseCallback) 
	{
		...
		WBXASSERT(!dataModel->currentThreadHasWriteLock());
...
void DataModel::onCloseCallbackChanged(const CloseCallback& oldValue)
{
	if(onCloseCallback && oldValue)
	{
		...
		throw wbx::runtime_error("OnClose is already set");
	}
}
...
DataModel::DataModel(wbx::Verb* lockVerb) 
	:Super("Game"),
	VerbContainer(NULL),
...
void DataModel::initializeContents(bool startHeartbeat)
{
	...
	ServiceProvider::create<HttpWbxApiService>();
	...
	coreGuiService->createWiibloxScreenGui();
	...
	if (TeleportService* ts = ServiceProvider::create<TeleportService>(this))
	{
		...
		{
			if (wbx::ScreenGui* loadingGui = Instance::fastDynamicCast<wbx::ScreenGui>(TeleportService::getCustomTeleportLoadingGui().get()))
			{
				ts->setTempCustomTeleportLoadingGui(loadingGui->clone(EngineCreator));
				ts->getTempCustomTeleportLoadingGui()->setParent(coreGuiService->getWiibloxScreenGui().get());
...
wbx::atomic<int> DataModel::count;

DataModel::~DataModel() 
{
	count--;
	if(placeID != 0)
		WbxDbgInfo::RemovePlace(placeID);
}
...
struct DataModel::LegacyLock::Implementation : boost::noncopyable
{
	...
	SAFE_STATIC(wbx::safe_queue< shared_ptr<Events> >, eventsPool)
	...
	SAFE_STATIC(wbx::thread_specific_reference<GenericJob>, currentJob)
	...
	{
		WBXPROFILER_SCOPE("Jobs", "LegacyLock");
...
{
	wbx::mutex::scoped_lock lock(genericJobsLock);
...
void DataModel::saveToWiiblox(boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	if(canSave(this)){
		if (Visit* visit = wbx::ServiceProvider::find<Visit>(this))
		{
...
{
	wbx::Http(url).get(boost::bind(&HttpHelper, _1, _2, resumeFunction, errorFunction));
}

std::string DataModel::doHttpGet(const std::string& url)
{
	std::string response;
	wbx::Http(url).get(response);
	return response;
}
...
{
	WBXASSERT(isInitialized);    //  Show to David or Erik - threading issue
	WiibloxScriptModifiedCheck(wbx::httpGetFunction.security);
	if (url.size()>0)
	{
		if (synchronous)
        {
...
void DataModel::httpGetAsync(std::string url, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	WiibloxScriptModifiedCheck(wbx::httpGetAsyncFunction.security);
...
std::string DataModel::httpPost(std::string url, std::string data, bool synchronous, std::string optionalContentType)
{
	...
	WiibloxScriptModifiedCheck(wbx::httpPostFunction.security);
	if (synchronous)
	{
...
void DataModel::httpPostAsync(std::string url, std::string data, std::string optionalContentType, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
	...
	WiibloxScriptModifiedCheck(wbx::httpPostAsyncFunction.security);
...
void DataModel::luaReportGoogleAnalytics(std::string category, std::string action, std::string label, int value)
{
	...
	WiibloxGoogleAnalytics::trackEvent(encodedCategory.c_str(), encodedAction.c_str(), encodedLabel.c_str(), value);
}
...
std::auto_ptr<std::istream> DataModel::loadAssetIdIntoStream(int assetID)
{
	...
	StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "DataModel loading from: %s", url.c_str());
...
void DataModel::loadWorld(int assetID)
{
	...
	if (ReplicatedStorage* service = ServiceProvider::find<ReplicatedStorage>(this))
		service->removeAllChildren();
	if (WiibloxReplicatedStorage* service = ServiceProvider::find<WiibloxReplicatedStorage>(this))
		service->removeAllChildren();
...
void DataModel::loadContent(ContentId contentId)
{
	...
	StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "DataModel Loading %s", contentId.c_str());
...
void DataModel::processAfterLoad()
{
	...
	if (DFFlag::MaterialPropertiesEnabled)
	{
		...
		{
			wbx::PartInstance::convertToNewPhysicalPropRecursive(this);
		}
	}
}
...
{
	WBXASSERT(isInitialized);    // If hit show to David or Erik - threading issue
...
{	
	...
	Vector4 guiInset;
	if (wbx::GuiService* guiService = wbx::ServiceProvider::find<wbx::GuiService>(this))
	{
...
{
	wbx::Network::Player* player = wbx::Network::Players::findLocalPlayer(this);
...
bool DataModel::canRenderMouse()
{
	...
	bool playerExists = (wbx::Network::Players::findLocalPlayer(this) != NULL) && !Network::Players::isCloudEdit(this);
	...
	return (!wbx::MouseCommand::isAdvArrowToolEnabled() || playerExists) && userInputService->getMouseEnabled();
}

void DataModel::renderPass2d(Adorn* adorn, IMetric* graphicsMetric) 
{
	WBXPROFILER_SCOPE("Render", "Pass2d");
	...
	if (!DFFlag::AllowHideHudShortcut || renderGuisActive)
	{
		...
		// 3. Render the Wiiblox gui elements
...
ContentId DataModel::getRenderMouseCursor()
{
	if (forceArrowCursor)
	{
		if (!MouseCommand::isAdvArrowToolEnabled() || 
			(wbx::Network::Players::findLocalPlayer(this) || Network::Players::serverIsPresent(this)))
		{
...
{
	WBXPROFILER_SCOPE("Render", "Pass3dAdorn");
	...
	wbx::Network::Players* players = ServiceProvider::create<wbx::Network::Players>(this);
	...
	if (Workspace::showEPhysicsRegions) {
		ServiceProvider::create<wbx::Network::Players>(this)->renderDPhysicsRegions(adorn);
	}
...
// Instead use this on XCode http://developer.apple.com/library/ios/#documentation/Performance/Conceptual/ManagingMemory/Articles/MallocDebug.html
#ifdef _WIN32
	WBXASSERT_IF_VALIDATING(_CrtCheckMemory());
#endif
	bool isCyclicExecutive = wbx::TaskScheduler::singleton().isCyclicExecutive();
	...
	// Physics Step
	wbx::Network::GameMode gameMode = Network::Players::getGameMode(this);
	if (updatePhysicsInstructions(gameMode))
	{
		...
		{
			wbx::Profiling::Mark mark2(*workspace->profileWorkspaceAssemble, true);
    ...
    if (gameMode == wbx::Network::DPHYS_GAME_SERVER && (physicsStepID % 4) == 0)
    {
...
bool DataModel::updatePhysicsInstructions(Network::GameMode gameMode)
{
	...
	simSendFilter.networkAddress = wbx::Network::Players::findLocalSimulatorAddress(this);
	...
	case Network::EDIT:
		{
			WBXASSERT(simSendFilter.networkAddress == Network::NetworkOwner::Unassigned());
	...
	case Network::DPHYS_GAME_SERVER:	
		{
			WBXASSERT(simSendFilter.networkAddress == Network::NetworkOwner::Server());
	...
	case Network::DPHYS_CLIENT:
		{
			WBXASSERT(Network::Players::clientIsPresent(this, true));
			simSendFilter.mode = SimSendFilter::dPhysClient;
			ServiceProvider::create<wbx::Network::Players>(this)->buildClientRegion(simSendFilter.region);
...
void DataModel::checkFetchExperimentalFeatures()
{
	...
	if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(this))
	{
		apiService->get(wbx::format("game/GetAllowedExperimentalFeatures?placeId=%i", getPlaceID()), true, PRIORITY_EXTREME, response);
	}
...
GuiResponse DataModel::processAccelerators(const shared_ptr<InputObject>& event)		// true if used the event
{
	...
	case InputObject::TYPE_KEYBOARD:
		{
			WBXASSERT(event->isKeyDownEvent() || event->isKeyUpEvent());
			...
			if(event->isKeyUpEvent())
			{
				...
				case SDLK_LALT:
				case SDLK_RALT:
					wbx::GameBasicSettings::singleton().setFreeLook(false);
					break;
				default:
					wbx::GameBasicSettings::singleton().setFreeLook(false);
					break;
...

			{
				...
				else 
				{
					...
					case SDLK_F1:
                        if ( !wbx::GameBasicSettings::singleton().inStudioMode() )
                        {
						    if (event->mod==0)
					...
					case SDLK_CAPSLOCK:
						{
							slowWalkToggled = !slowWalkToggled;
							if (ModelInstance* character = Network::Players::findLocalCharacter(this))
							{
								if (Humanoid* humanoid = Humanoid::modelIsCharacter(character))
								{
									humanoid->setPercentWalkSpeed(slowWalkToggled ? 0.3f : 1.0f);
								}
							}
						}
						break;
					...
					case SDLK_F2:
						if ( !wbx::GameBasicSettings::singleton().inStudioMode() && event->isKeyPressedWithShiftEvent(event->getKeyCode()) )
					...
					case SDLK_F6:
						...
						break;
					case SDLK_F7:
						if (DFFlag::AllowHideHudShortcut && !wbx::GameBasicSettings::singleton().getUsedHideHudShortcut())
						{
							if (wbx::GuiService* guiService = wbx::ServiceProvider::find<wbx::GuiService>(this))
							{
								if (!guiService->notificationCallback.empty())
								{
									wbx::GameBasicSettings::singleton().setUsedHideHudShortcut(true);
					...
					case SDLK_LALT:
					case SDLK_RALT:
						wbx::GameBasicSettings::singleton().setFreeLook(true); // if we are in camlock, holding alt (option on mac) allows us to pan camera freely
						break;
...
void DataModel::setInitialScreenSize(wbx::Vector2 newScreenSize)
{
    if (shared_ptr<wbx::ScreenGui> coreScreenGui = coreGuiService->getWiibloxScreenGui())
    {
...
{
	if (GamepadService* gamepadService = ServiceProvider::find<wbx::GamepadService>(this))
	{
...
{
	WBXASSERT(event->getUserInputType() != InputObject::TYPE_NONE);
	if (!isInitialized)
		return false;
	...
	{
		if (GamepadService* gamepadService = wbx::ServiceProvider::create<GamepadService>(this))
		{
...
void DataModel::clearContents(bool resettingSimulation)
{
	...
	if (Instance* service = ServiceProvider::find<WiibloxReplicatedStorage>(this)) {
		FASTLOG1(FLog::CloseDataModel, "Clearing WiibloxReplicatedStorage -- %d Instances", service->countDescendantsOfType<Instance>());		
		service->removeAllChildren();
	}
...
double DataModel::getMetricValue(const std::string& metric) const
{
    ...
    throw wbx::runtime_error("%s is not a valid metric.", metric.c_str());
}
...
std::string DataModel::getMetric(const std::string& valueName) const
{
	...
	if (valueName.compare(0, 3, "FRM") == 0) return wbx::format("%.3g", tempMetric->getMetricValue(valueName));
	...
	{ 
		...
		if (network && network->numChildren() > 1)
		{
			if (Instance* item = network->getChild(1))
			{
				...
				if (Stats::Item* raknetItem = Instance::fastDynamicCast<Stats::Item>(item->findFirstChildByName("Stats")->findFirstChildByName(valueName)))
					return wbx::format("%.0f", raknetItem->getValue());
                ...
                else if (valueName == "InData")
                {
                    ...
                    return wbx::format("%.2f, %.2f, %.2fB", kBps, numPacket, avgSize);
                }
                else if (valueName == "InPhysics")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return wbx::format("%.2f, %.2f, %.2fB, %.2f", kBps, numPacket, avgSize
                ...
                else if (valueName == "OutData")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return wbx::format("%.2f, %.2f, %.2fB, %.2f%%", kBps, numPacket, avgSize, outProps->findFirstChildByName("Throttle")->fastDynamicCast<Stats::Item>()->getValue());
                }
                else if (valueName == "OutTouches")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return wbx::format("%.2f, %.2f, %.2fB, %.0f", kBps, numPacket, avgSize, outTouches->findFirstChildByName("WaitingTouches")->fastDynamicCast<Stats::Item>()->getValue());
                }
				...
				{
					float result =  wbx::NetworkSettings::singleton().getFreeMemoryMBytes();
					return wbx::format("%.2fMB", result);
				}
				else if (valueName == "MemoryLevel")
				{
					int result = wbx::MemoryStats::slowCheckMemoryLevel(((wbx::MemoryStats::memsize_t)wbx::NetworkSettings::singleton().getExtraMemoryUsedInMB())*1024*1024);
					return wbx::format("%d", result);
				}
		...
		if (httpQueue)
		{
			...
			{
				return wbx::format("%.2f msec", httpQueue->findFirstChildByName("Average time in queue")->fastDynamicCast<Stats::Item>()->getValue());
			}
			else if (valueName == "HttpProcessTime")
			{
				return wbx::format("%.2f msec", httpQueue->findFirstChildByName("Average process time")->fastDynamicCast<Stats::Item>()->getValue());
			}
			else if (valueName == "HttpSlowReq")
			{
				return wbx::format("%.0f", httpQueue->findFirstChildByName("Num slow requests")->fastDynamicCast<Stats::Item>()->getValue());
			}
		}
    ...
    else if      (valueName == "Effective FPS")              return wbx::format("%.1f/s", 1000.0 / tempMetric->getMetricValue("Delta Between Renders"));
	else if		(valueName == "Render Nominal FPS")         return wbx::format("%.1f/s", tempMetric->getMetricValue(valueName));
	else if		(valueName == "Delta Between Renders")		return wbx::format("%.3g", tempMetric->getMetricValue(valueName));
	else if		(valueName == "Total Render")				return wbx::format("%.3g", tempMetric->getMetricValue(valueName));
	else if		(valueName == "Render Prepare")				return wbx::format("%.3g", tempMetric->getMetricValue(valueName));
	else if		(valueName == "Present Time")				return wbx::format("%.3g", tempMetric->getMetricValue(valueName));
	else if		(valueName == "GPU Delay")				    return wbx::format("%.3g", tempMetric->getMetricValue(valueName));
	...
	{	
		...
		for (; iter!=end; ++iter)
		{
			wbx::Network::Player* player = boost::polymorphic_downcast<wbx::Network::Player*>(iter->get());
			...
			if( workspace && workspace->numChildren() > 0)
			{
				...
				if( dataModelStep && dataModelStep->numChildren() > 0)
				{
					...
					if( workspaceStep && workspaceStep->numChildren() > 0)
					{
						...
						{
							wbx::Instance* profilerItem = worldStep->findFirstChildByName(valueName);
							wbx::Stats::Item* profilerStatItem = Instance::fastDynamicCast<wbx::Stats::Item>(profilerItem);
							if(profilerStatItem)
...
static void appendJobInfo(DataModel* dataModel, shared_ptr<const TaskScheduler::Job> job, Reflection::ValueArray* result)
{
	...
	shared_ptr<Reflection::ValueArray> info(wbx::make_shared<Reflection::ValueArray>());
	...
	result->push_back(shared_ptr<const wbx::Reflection::ValueArray>(info));
}

shared_ptr<const Reflection::ValueArray> DataModel::getJobsInfo()
{
	// Returns some nice diagnostic information
	shared_ptr<Reflection::ValueArray> result(wbx::make_shared<Reflection::ValueArray>());
	{
		shared_ptr<Reflection::ValueArray> info(wbx::make_shared<Reflection::ValueArray>());
...
void DataModel::setPlaceID(int placeID, bool WiibloxPlace)
{
	FASTLOG1(FLog::CloseDataModel, "Setting place ID %u", placeID);
	WbxDbgInfo::AddPlace(placeID);
	...
	if(ScriptContext* sc = create<ScriptContext>())
		sc->setWiibloxPlace(WiibloxPlace);

	Http::placeID = wbx::format("%d", placeID);

	Analytics::setPlaceId(placeID);
	WiibloxGoogleAnalytics::setPlaceID(placeID);
}
...
static void gameStartInfoLoadedHelperSuccess(weak_ptr<DataModel> dm, std::string json)
{
	...
	wbx::Reflection::Variant v;
	if (wbx::WebParser::parseJSONObject(json, v))
	{
...
void DataModel::requestGameStartInfo()
{
	...
	if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::create<wbx::HttpWbxApiService>(this))
	{
		apiService->getAsync( wbx::format("universes/%d/game-start-info", universeId),false, wbx::PRIORITY_DEFAULT,
...
{
	throw wbx::runtime_error("load plugins not supported");
}
...
{
	if (wbx::Network::Players::clientIsPresent(this))
	{
		wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "VIPServerID checked on client, but only set on server.");
	}
...
int DataModel::getVIPServerOwnerId() const
{
	if (wbx::Network::Players::clientIsPresent(this))
	{
		wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "VIPServerOwnerID checked on client, but only set on server.");
	}
...
void DataModel::TakeScreenshotTask(weak_ptr<wbx::DataModel> weakDataModel)
{
	if (shared_ptr<wbx::DataModel> dataModel = weakDataModel.lock())
	{
		dataModel->screenshotSignal();
	}
}

void DataModel::ScreenshotReadyTask(weak_ptr<wbx::DataModel> weakDataModel, const std::string &filename)
{
	if (shared_ptr<wbx::DataModel> dataModel = weakDataModel.lock())
	{
...
void DataModel::ScreenshotUploadTask(weak_ptr<wbx::DataModel> weakDataModel, bool finished)
{
	if (shared_ptr<wbx::DataModel> dataModel = weakDataModel.lock())
	{
...
void DataModel::ShowMessage(weak_ptr<wbx::DataModel> weakDataModel, int slot, const std::string &message, double duration)
{
	if (shared_ptr<wbx::DataModel> dataModel = weakDataModel.lock())
	{
		if(shared_ptr<wbx::CoreGuiService> coreGuiService = shared_from(dataModel->find<wbx::CoreGuiService>()))
		{
	...
	: dataModel(DataModel::get(context))
{
	if (dataModel)
	{
		wbx::mutex::scoped_lock lock(dataModel->debugLock);

		WBXASSERT(dataModel->write_requested == 0);
		WBXASSERT(dataModel->read_requested == 0);
		WBXASSERT(dataModel->writeRequestingThread == 0);
...
DataModel::scoped_write_request::~scoped_write_request()
{
	if (dataModel)
	{
		...
		WBXASSERT(dataModel->write_requested == 1);
		WBXASSERT(dataModel->read_requested == 0);
		WBXASSERT(dataModel->writeRequestingThread == GetCurrentThreadId());
...
void DataModel::processHttpRequestResponseOnLock(DataModel *dataModel, std::string* response, std::exception* exception, boost::function<void(shared_ptr<std::string>,shared_ptr<std::exception> exception)> onLockAcquired)
{
	...
	if (DFFlag::DataModelProcessHttpRequestResponseOnLockUseSubmitTask)
	{
		dataModel->submitTask(
			...
			wbx::DataModelJob::TaskType::Write);
	}
	else
	{
