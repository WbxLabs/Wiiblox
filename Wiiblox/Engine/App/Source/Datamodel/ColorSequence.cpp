...
{
    WBXASSERT(validate(r.m_data, false));
...
void ColorSequence::resample( G3D::Vector3* min, G3D::Vector3* max, int numPoints) const
{
    ...
    WBXASSERT( m_data.front().time == 0.0f ); // yup, straight equal 
    WBXASSERT( m_data.back().time == 1.0f ); // ditto
    ...
    {
        t += dt;
        WBXASSERT( data[src].time <= data[src+1].time ); // invariant violation
...
bool ColorSequence::validate(const std::vector<Key>& keys, bool exc)
{
    if (keys.size()<2)
    {
        if(exc) throw wbx::runtime_error("ColorSequence: requires at least 2 keypoints"); else return false;
    }
    ...
    for (unsigned j=0; j<keys.size(); ++j)
    {
        ...
        {
            if(exc) throw wbx::runtime_error("ColorSequence: all keypoints must be ordered by time"); else return false;
        }
        
        if (keys[j].value.min()<0 || keys[j].value.max()>1)
        {
            if(exc) throw wbx::runtime_error("ColorSequence: color value out of range"); else return false;
        }
    }

    if (fabsf(keys.front().time) > 1e-4f)
    {
        if(exc) throw wbx::runtime_error("ColorSequence must start at time=0.0"); else return false;
    }
    
    if (fabsf(keys.back().time - 1.0f) > 1e-4f)
    {
        if(exc) throw wbx::runtime_error("ColorSequence must end at time=1.0"); else return false;
    }
    return true;
}
////////////////////////////////////////////////////////////////////////////
