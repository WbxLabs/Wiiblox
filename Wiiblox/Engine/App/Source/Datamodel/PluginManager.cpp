...
const char* const wbx::sPluginManager = "PluginManager";
const char* const wbx::sPlugin = "Plugin";
const char *const wbx::sToolbar = "Toolbar";
const char *const wbx::sButton = "Button";
...
static Reflection::BoundFuncDesc<Plugin, void(void)> func_saveSelectedToWiiblox(&Plugin::saveSelectedToWiiblox, "SaveSelectedToWiiblox", Security::Plugin);
...
static Reflection::BoundFuncDesc<Plugin, void(shared_ptr<wbx::Instance>, int)>	func_openScript(&Plugin::openScriptDoc, "OpenScript", "script", "lineNumber", 0, Security::Plugin);
...
void Plugin::saveSelectedToWiiblox()
{
    pluginManager->getStudioHost()->uiAction("saveToWiibloxAction");
}
...
{
	return wbx::AdvArrowTool::getJointCreationMode();
}
...
void Plugin::openScriptDoc(shared_ptr<wbx::Instance> script, int lineNumber)
{
...
{
	DataModel* dm = DataModel::get(WiibloxExtraSpace::get(L)->context());
	WBXASSERT(dm);
...
{
	wbx::Plugin *activePlugin = getActivePlugin(dataModel);
	if (activePlugin) 
	{
...
void PluginManager::buttonClick(DataModel *dataModel, void *id)
{
	...
	if (iter != state.end()) 
	{
		wbx::DataModel::LegacyLock lock(dataModel, DataModelJob::Write);
...
{
	getStudioHost()->exportPlace(filePath, wbx::ExporterSaveType_Everything);
}

void PluginManager::exportSelection(std::string filePath)
{
	getStudioHost()->exportPlace(filePath, wbx::ExporterSaveType_Selection);
}
...
void PluginManager::fireDragEnterEvent(DataModel* dataModel, shared_ptr<const wbx::Instances> instances, Vector2 location)
{
	if (!dataModel)
		return;
	
	if (wbx::Plugin* activePlugin = getActivePlugin(dataModel))
	{
		wbx::DataModel::LegacyLock lock(dataModel, DataModelJob::Write);
