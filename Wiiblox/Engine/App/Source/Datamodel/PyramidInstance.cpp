...
void PyramidInstance::setPartSizeXml(const Vector3& WBXSize)
{
	Vector3 newSize = WBXSize;
	Vector3 currentSize = getPartSizeXml();

	if( WBXSize.x != currentSize.x )
		newSize.x = newSize.z = WBXSize.x;
	else if( WBXSize.z != currentSize.z )
		newSize.z = newSize.x = WBXSize.z;
