...
{
	return wbx::FileSystem::getUserDirectory(false, wbx::DirExe, "ClientSettings") / "StatDisplaySettings.json";
}
...
void GuiBuilder::togglePhysicsStats()
{
	...
	{
		statsMenu->setVisible(!statsMenu->isVisible());
		wbx::Profiling::setEnabled(statsMenu->isVisible());
	}
}
	...
	bool buildInGameGui)
{
    ...
    if (gDebugDisplay == DISPLAY_PHYSICS || gDebugDisplay == DISPLAY_PHYSICS_AND_OWNER)
	{
		...
		WBXASSERT(physicsStats2 != NULL);
		if (physicsStats2)
		{
			physicsStats2->setVisible(true);
			wbx::Profiling::setEnabled(physicsStats2->isVisible());
		}
	}
...
void GuiBuilder::buildLuaGui()
{
	...
	controls->setWiibloxLocked(true);
	...
	bottomLeftControl->setWiibloxLocked(true);
	...
	bottomRightControl->setWiibloxLocked(true);
	...
	Instance::propWiibloxLocked.set(topLeftControl.get(),true);
...
void GuiBuilder::updateSummaryStats(wbx::TopMenuBar* hudArray)
{
	...
	WBXASSERT(NetworkStat_End == hudArray->numChildren());
