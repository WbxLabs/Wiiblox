...
namespace WBX {
    ...
    static bool getBillboardHit(Workspace* workspace, const Vector2& pos, const WBXRay& ray, const Vector2& viewport, const CoordinateFrame& projectionFrame, bool alwaysOnTop, Vector2& result)
    {
        ...
        WBXRay searchRay = WBXRay::fromOriginAndDirection(ray.origin(), ray.direction() * 2048);
	...
	void BillboardGui::render3dSortedAdorn(Adorn* adorn)
	{
        if (FFlag::BillboardGuiVR)
        {
    		...
    		if (wbx::Network::Player* player = Network::Players::findLocalPlayer(DataModel::get(part.get())))
    			if (shared_ptr<Instance> noPlayerRender = playerToHideFrom.lock())
    				if (player == fastDynamicCast<wbx::Network::Player>(noPlayerRender.get()))
    					return;
        ...
        else
        {
    		...
    		if (wbx::Network::Player* player = Network::Players::findLocalPlayer(DataModel::get(part.get())))
    			if (Instance* noPlayerRender = playerToHideFrom.lock().get())
    				if (player == fastDynamicCast<wbx::Network::Player>(noPlayerRender))
    					return;
        ...
        if (FFlag::BillboardGuiVR)
        {
    		...
    		if (event->isMouseEvent())
            {
    			if (Workspace* workspace = ServiceProvider::find<Workspace>(this))
    			{
                    ...
                    WBXRay ray = workspace->getCamera()->worldRay(pos.x, pos.y);
                    
                    Vector2 hit;
                    if (getBillboardHit(workspace, pos, ray, viewport.wh(), projectionFrame, alwaysOnTop, hit))
                    {
    					shared_ptr<InputObject> transformedEvent = wbx::Creatable<Instance>::create<InputObject>(*event);
	...
	{
		if (value && !fastDynamicCast<wbx::Network::Player>(value))
