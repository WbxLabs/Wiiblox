...
bool FloorWire::incrementalBuildSegments(const Workspace* workspace,
	...
	WBXRay hitRay(source, Vector3(moveInX ? deltaGoal.x : 0, 0, !moveInX ? deltaGoal.z : 0));
...
void FloorWire::buildTrailSegments(const Workspace* workspace,
	...
	computeSurfacePosition(from, WBXRay(toPosition, fromPosition - toPosition),
			&fromPosition);
	computeSurfacePosition(to, WBXRay(fromPosition, toPosition - fromPosition),
			&toPosition);
	...
	WBXRay downAtFromPosition(fromPosition,
