...
namespace WBX {
	...
	public:
		...
		bool checkTouch(const shared_ptr<PartInstance>& other, TouchPair::Type type)
		{
			...
			static const wbx::Time::Interval expiration = wbx::Time::Interval(5);

			const wbx::Time now = wbx::Time::nowFast();
			...
			for (size_t i = 0; i < items.size(); )
			{
				...
				if (item.other.lock() == other)
				{
					...
					{
						item.lastType = type;
						//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Touch accepted");
						return true;
					}
					else
					{
						//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Touch rejected");
						// Rejected!
						return false;
					}
				}
				...
				{
					//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Touch expired");
