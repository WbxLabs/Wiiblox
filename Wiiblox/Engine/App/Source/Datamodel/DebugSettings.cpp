...
#include "util/http.h"
#include "wbx/ProcessPerfCounter.h"
...
const char *const wbx::sDebugSettings = "DebugSettings";
const char *const wbx::sTaskSchedulerSettings = "TaskScheduler";
...
std::string DebugSettings::WiibloxVersion = "?";
std::string DebugSettings::WiibloxProductName = "?";

REFLECTION_BEGIN();
static Reflection::PropDescriptor<DebugSettings, std::string> prop_WiibloxVersion("WiibloxVersion", "Profile", &DebugSettings::getWiibloxVersion, NULL);
static Reflection::PropDescriptor<DebugSettings, std::string> prop_WiibloxProductName("WiibloxProductName", "Profile", &DebugSettings::getWiibloxProductName, NULL);
...
static Reflection::PropDescriptor<DebugSettings, int> prop_WiibloxSuccesses("WiibloxSuccessCount", "Performance", &DebugSettings::getWiibloxSuccessCount, NULL);
static Reflection::PropDescriptor<DebugSettings, int> prop_WiibloxFailures("WiibloxFailureCount", "Performance", &DebugSettings::getWiibloxFalureCount, NULL);
static Reflection::PropDescriptor<DebugSettings, double> prop_WiibloxResponce("WiibloxRespoceTime", "Performance", &DebugSettings::getWiibloxResponce, NULL);
...
{
	return wbx::Block::BlockMeshPool::getSize();
}

int DebugSettings::getLuaRamLimit() const
{
	return wbx::LuaAllocator::heapLimit;
}
void DebugSettings::setLuaRamLimit(int value)
{
	wbx::LuaAllocator::heapLimit = value;
}
...
public:
	...
	DummyJob(bool exclusive, double fps)
		:Job(wbx::format("DummyJob%s %gfps", exclusive ? "Exclusive" : "", fps).c_str(), dummyArbiter)
		,exclusive(exclusive)
		,fps(fps)
	  {}
...
long DebugSettings::getWiibloxSuccessCount() const
{
	return Http::WiibloxSuccessCount;
}
long DebugSettings::getWiibloxFalureCount() const
{
	return Http::WiibloxFailureCount;
}
double DebugSettings::getWiibloxResponce() const
{
	if (!Http::getWiibloxResponceLock())
	{
		return 0;
	}
	wbx::mutex::scoped_lock lock(*Http::getWiibloxResponceLock());
	return Http::WiibloxResponce.getStats().average;
}
...
double DebugSettings::getCdnRespoce() const
{
	...
	wbx::mutex::scoped_lock lock(*Http::getCdnResponceLock());
...
shared_ptr<const Reflection::Tuple> DebugSettings::resetCdnFailureCounts()
{
	...
	result->values[4] = Http::WiibloxSuccessCount.swap(0);
	result->values[5] = Http::WiibloxFailureCount.swap(0);

	{
		if (Http::getWiibloxResponceLock())
		{
			wbx::mutex::scoped_lock lock(*Http::getWiibloxResponceLock());
			result->values[6] = Http::WiibloxResponce.getStats().average;
			Http::WiibloxResponce.clear();
		}
		else
		{
			result->values[6] = 0;
...
std::string DebugSettings::systemProductName() const
{
 	...
 	std::string name;
	bool b = wbx::RegistryUtil::readString("HKEY_LOCAL_MACHINE\\HARDWARE\\DESCRIPTION\\System\\BIOS\\SystemProductName", name);
	if (b)
		return name;
	else
		return "?";
...
//TODO
	...
	return -1;
#elif defined(_WIN32) && !defined(WBX_PLATFORM_UWP)
