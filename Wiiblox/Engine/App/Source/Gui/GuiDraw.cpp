...
void GuiDrawImage::tryCreateTextureProxy(Adorn *adorn, const std::string& contentString, const std::string& context, wbx::TextureProxyBaseRef& textureRef, bool& isWaiting)
{
	ContentId content(contentString);

	if ( (content.isAsset() && !wbx::ContentProvider::findAsset(content).empty()) || !content.isAsset())
	{
...
Vector2 GuiDrawImage::getImageSize() const
{
	...
	{
		wbx::Vector2 sz = normal->getOriginalSize();
		size.x = sz.x;
		size.y = sz.y;
	}
	return size;
...
void GuiDrawImage::draw(Adorn* adorn, const wbx::TextureProxyBaseRef& texture, const Rect& rect, const Vector2& texul, const Vector2& texbr, const Color4& color,
    const Rect& clipRect, const Color4& behind, const Color4& inFront)
{
	...
	if( clipRect2D.intersects(rect2D) )
	{
		if (texture) {
			...
			wbx::Rect2D intersectRect = clipRect2D.intersect(rect2D);
