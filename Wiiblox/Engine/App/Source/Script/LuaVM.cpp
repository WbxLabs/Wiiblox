...
void lua_vmhooked_handler(lua_State* L) {
    ...
    #endif
    wbx::Tokens::sendStatsToken.addFlagFast(HATE_LUA_VM_HOOKED);
    wbx::DataModel* dm = wbx::DataModel::get(WiibloxExtraSpace::get(L)->context());
    if (dm)
    {
...
#ifdef WBX_SECURE_DOUBLE
WBX_ALIGN(16) int LuaSecureDouble::luaXorMask[4];
...
namespace WBX { namespace Security {
    // All of the .text
    volatile const uintptr_t WBXTextBase = 0x00400000;
    volatile const size_t WBXTextSize = 0xFFFFFFFF;
}
}
#endif
