...
void wbx::Lua::newweaktable(lua_State *L, const char *mode) {
	lua_newtable(L);
...
int ObjectBridge::newInstance(lua_State *thread)
{
	...
	const wbx::Name& className = wbx::Name::lookup(name);

	shared_ptr<Instance> instance = Creatable<Instance>::createByName(className, wbx::ScriptingCreator);
	if (!instance)
		throw wbx::runtime_error("Unable to create an Instance of type \"%s\"", name);
	...
	if (lua_gettop(thread)>=2)
	{
		...
		if (parent)
		{
			// only allow parenting to Wiiblox locked objects if we are in the right permission level 
			if (parent->getWiibloxLocked())
				wbx::Security::Context::current().requirePermission(wbx::Security::Plugin, "Parent in Instance.new()");
...
static void pushLuaValue(wbx::Reflection::ConstProperty p, lua_State *L, wbx::Security::Context& securityContext)
{
	const wbx::Reflection::PropertyDescriptor& desc(p.getDescriptor());

	if (desc.security!=wbx::Security::None)
		securityContext.requirePermission(desc.security, desc.name.c_str());

	if (!desc.isScriptable())
		throw wbx::runtime_error("Unable to query property %s. It is not scriptable", p.getName().c_str());
	...
	if (desc.type==Type::singleton<wbx::WBXRay>())
	{
		WBXRayBridge::pushNewObject(L, p.getValue<wbx::WBXRay>());
		return;
	}
	...
	if (p.getDescriptor().bIsEnum)
	{
		...
		if (enumDesc!=NULL)
		{
			...
			throw wbx::runtime_error("Invalid value for enum %s", enumDesc->name.c_str());
		}
	}
	...
	throw wbx::runtime_error("Unable to get property %s, type %s", p.getName().c_str(), desc.type.name.c_str());
}
...
static const char* PropertyNameCorrection(const shared_ptr<Instance>& object, const char* name, lua_State *L)
{
	if ( name[0] == 's' ) {
		if(strcmp(name,"size") == 0){
			ScriptContext::getContext(L).camelCaseViolation(object, name, WiibloxExtraSpace::get(L)->script.lock());
			return "Size";
		}
	}
	return name;
...
namespace WBX { namespace Lua {
	...
	int Bridge< shared_ptr<Instance>, false >::on_index(const shared_ptr<Instance>& object, const char* name, lua_State *L)
	{
        ...
        WBXPROFILER_SCOPE("LuaBridge", "$index");
        WBXPROFILER_LABELF("LuaBridge", "%s.%s", object->getDescriptor().name.c_str(), name);
		...
		if (Reflection::YieldFunctionDescriptor* func = object->findYieldFunctionDescriptor(name))
		{
			...
			WBXASSERT(lua_isfunction(L, -1));          // Stack:   closure
			return 1;
		}
		...
		{
			object->securityCheck(securityContext);
			if (object->getWiibloxLocked())
				securityContext.requirePermission(wbx::Security::Plugin, name);
		...
		{
			wbx::Instance* child = object->findFirstChildByName(name);
			if (child!=NULL)
			{
				object->securityCheck(securityContext);
				if (object->getWiibloxLocked())
        ...
        if (wbx::Name::lookup(name).empty())
        {
            ...
            if (name2[0]!=name[0] && !wbx::Name::lookup(name2).empty())
            {
        ...
        {
            throw wbx::runtime_error("%s is a callback member of %s; you can only set the callback value, get is not available", name, object->getDescriptor().name.c_str());
        }

		// Failure
		throw wbx::runtime_error("%s is not a valid member of %s", name, object->getDescriptor().name.c_str());
	}

}} //namespace


static void assignLuaValue(wbx::Reflection::Property p, lua_State *L, int index, wbx::Security::Context& securityContext)
{
	const wbx::Reflection::PropertyDescriptor& desc(p.getDescriptor());
	...
	if (!desc.isScriptable())
		throw wbx::runtime_error("Unable to assign property %s. It is not scriptable", p.getName().c_str());
	...
	if (desc.type==Type::singleton<wbx::WBXRay>())
	{
		p.setValue<wbx::WBXRay>(WBXRayBridge::getObject(L, index));
		return;
	}
	...
	const TypedPropertyDescriptor<wbx::Soundscape::SoundId>* soundProp = dynamic_cast<const TypedPropertyDescriptor<wbx::Soundscape::SoundId>*>(&p.getDescriptor());
	if (soundProp!=NULL)
	{
		p.setValue<wbx::Soundscape::SoundId>(ContentId(throwable_lua_tostring(L, index)));
		return;
	}

	const TypedPropertyDescriptor<wbx::TextureId>* textureProp = dynamic_cast<const TypedPropertyDescriptor<wbx::TextureId>*>(&p.getDescriptor());
	if (textureProp!=NULL)
	{
	...
	const TypedPropertyDescriptor<wbx::MeshId>* meshProp = dynamic_cast<const TypedPropertyDescriptor<wbx::MeshId>*>(&p.getDescriptor());
	if (meshProp!=NULL)
	{
	...
	const TypedPropertyDescriptor<wbx::AnimationId>* animationProp = dynamic_cast<const TypedPropertyDescriptor<wbx::AnimationId>*>(&p.getDescriptor());
	...
	if (enumDesc!=NULL)
	{
		...
		if (EnumItem::getItem(L, index, item))
		{
			if (!enumDesc->setEnumItem(p.getInstance(), *item))
				throw wbx::runtime_error("Invalid type %s for enum %s", item->owner.name.c_str(), enumDesc->name.c_str());
		}
		else if (lua_isnumber(L, index))
		{
			if (!enumDesc->setEnumValue(p.getInstance(), lua_tointeger(L, index) ))
				throw wbx::runtime_error("Invalid value %d for enum %s", static_cast<int>(lua_tointeger(L, index)), enumDesc->name.c_str());
		}
		else if (lua_isstring(L, index))
		{
			if (!enumDesc->setStringValue(p.getInstance(), lua_tostring(L, index)))
				throw wbx::runtime_error("Invalid value \"%s\" for enum %s", lua_tostring(L, index), enumDesc->name.c_str());
		} 
		else
			throw wbx::runtime_error("Invalid value for enum %s", enumDesc->name.c_str());
		return;
	...
	throw wbx::runtime_error("Unable to set property %s, type %s", p.getName().c_str(), p.getDescriptor().type.name.c_str());
}
...
shared_ptr<Tuple> callCallback(Lua::WeakFunctionRef function, shared_ptr<const Tuple> args, boost::intrusive_ptr<WeakThreadRef> cachedCallbackThread)
{
	...
	WBXASSERT_BALLANCED_LUA_STACK(functionThread);
	...
	{	
		...
		WBXASSERT(lua_isthread(functionThread, -1));
		...
		WBXASSERT(lua_isthread(functionThread, -1));
		WBXASSERT(top+1 == lua_gettop(functionThread));			//oldTop, slotThread

		createdThread = true;
	}

	{
		WBXASSERT_BALLANCED_LUA_STACK(callbackThread);
		...
		wbx::Scripting::ScriptDebugger* pDebugger = NULL;
		if (::FFlag::LuaDebugger)
			pDebugger = wbx::Scripting::DebuggerManager::singleton().findDebugger(callbackThread.get());
...
void callAsyncCallback(Lua::WeakFunctionRef function, shared_ptr<const Tuple> args, AsyncCallbackDescriptor::ResumeFunction resumeFunction, AsyncCallbackDescriptor::ErrorFunction errorFunction, boost::intrusive_ptr<WeakThreadRef> cachedCallbackThread)
{
    ...
    // Push the function onto the stack
    WBXASSERT(lua_gettop(callbackThread) == 0);
    ...
    case ScriptContext::Yield:
        {
            ...
            WBXASSERT(WiibloxExtraSpace::get(callbackThread.get())->continuations.get() == NULL);
            WiibloxExtraSpace::get(callbackThread.get())->continuations.reset(new Lua::Continuations(continuations));
...
static void assignLuaCallback(wbx::Reflection::Callback callback, lua_State *L, int index)
{
	const wbx::Reflection::CallbackDescriptor& desc(callback.getDescriptor());

	if (desc.security!=wbx::Security::None)
		wbx::Security::Context::current().requirePermission(desc.security, desc.name.c_str());

    if (desc.isAsync())
    {
        const wbx::Reflection::AsyncCallbackDescriptor& adesc = static_cast<const wbx::Reflection::AsyncCallbackDescriptor&>(desc);
    ...
    {
        const wbx::Reflection::SyncCallbackDescriptor& sdesc = static_cast<const wbx::Reflection::SyncCallbackDescriptor&>(desc);
...
void Bridge< shared_ptr<Instance>, false >::on_newindex(shared_ptr<Instance>& object, const char* name, lua_State *L)
{
    ...
    WBXPROFILER_SCOPE("LuaBridge", "$newindex");
    WBXPROFILER_LABELF("LuaBridge", "%s.%s", object->getDescriptor().name.c_str(), name);
	...
	if (PropertyDescriptor* prop = object->findPropertyDescriptor(name))
	{
		...
		{
			if(instance->getWiibloxLocked())
				securityContext.requirePermission(wbx::Security::Plugin, desc.name.c_str());

			if(!object->getDescriptor().isScriptCreatable())
				throw wbx::runtime_error("Cannot change Parent of type %s", object->getDescriptor().name.c_str());

			shared_ptr<Instance> parent = ObjectBridge::getInstance(L, 3);
			if (instance->getParent() != parent.get())
			{
				if((parent && parent->getWiibloxLocked()) || (instance->getParent() && instance->getParent()->getWiibloxLocked())){
	...
	{
		if(instance->getWiibloxLocked())
			securityContext.requirePermission(wbx::Security::Plugin, callback->name.c_str());
...
{
    WBXPROFILER_SCOPE("LuaBridge", "$call");

	// FunctionDescriptor* is at lua_upvalueindex(1)
	WBXASSERT(lua_islightuserdata(L, lua_upvalueindex(1)));
    ...
    WBXPROFILER_LABELF("LuaBridge", "%s.%s", desc->owner.name.c_str(), desc->name.c_str());

	// Instance (self) is at arg index 1
	shared_ptr<Instance> instance;
	if (!getPtr(L, 1, instance) || !instance)
		throw wbx::runtime_error("Expected ':' not '.' calling member function %s", desc->name.c_str());

	wbx::Security::Context& securityContext = wbx::Security::Context::current();

	instance->securityCheck(securityContext);

	// only don't call Wiiblox locked objects if they are under core gui (this is a service we control, and Wiiblox locked was designed for this class)
	if ( instance->getWiibloxLocked() && instance->isDescendantOf(ServiceProvider::find<CoreGuiService>(instance.get())) )
		securityContext.requirePermission(wbx::Security::Plugin, desc->name.c_str());

	// Make sure the function is truly a member of the object
	if (!desc->isMemberOf(instance.get()))
		throw wbx::runtime_error("The function %s is not a member of \"%s\"", desc->name.c_str(), instance->getDescriptor().name.c_str());
...
protected:
	static void resumeThreadWithException(boost::intrusive_ptr<WeakThreadRef> threadRef, std::string message)
	{
		...
		{
			if (Continuations* continuations = WiibloxExtraSpace::get(thread)->continuations.get()) {
            ...
            wbx::ScriptContext* context = WiibloxExtraSpace::get(thread)->context();
...
int ObjectBridge::callMemberYieldFunction(lua_State *L) {
	...
	shared_ptr<Instance> instance;
	if (!getPtr(L, 1, instance) || !instance)
		throw wbx::runtime_error("Did you forget a colon? The first argument of member function %s must be an Object", desc->name.c_str());
