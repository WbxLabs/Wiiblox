...
using namespace WBX;
using namespace wbx::Lua;
using namespace boost::posix_time;

REFLECTION_BEGIN();
static Reflection::EventDesc<ScriptContext, void(shared_ptr<Instance>, std::string, shared_ptr<Instance>)> event_camelCaseViolation(&ScriptContext::camelCaseViolation, "CamelCaseViolation", "object", "member", "script", wbx::Security::WiibloxScript);
REFLECTION_END();
...
* protected all metatables associated with Wiiblox userdata objects
...
{
	WBXASSERT_BALLANCED_LUA_STACK(thread);
...
struct LuaProfiler
{
    static std::string getResumePosition(lua_State* L, int nargs)
    {
        ...
        if (L->status == 0)
        {
            ...
            return wbx::format("%s(%d)", string ? "<string>" : file, line);
        }
        else
        {
            // resume from yield
    ...
    struct StringCache
    {
        ...
        const char* getCall(const char* name, const char* namewhat, const char* file, int line)
        {
            ...
            if (res.empty())
            {
                ...
                res = wbx::format("LUACALL%s(%d): %s%s", file ? file : "?", line, type, name ? name : "?");
            }
...
static wbx::atomic<int> contextCount;
...
//WiibloxScript permissions
Reflection::BoundFuncDesc<ScriptContext, void(int, shared_ptr<Instance>, std::string name)> func_AddCoreScript(&ScriptContext::addCoreScript, "AddCoreScript", "assetId","parent","name", Security::WiibloxScript);
Reflection::BoundFuncDesc<ScriptContext, void(std::string name, shared_ptr<Instance>)> func_AddCoreScriptLocal(&ScriptContext::addCoreScriptLocal, "AddCoreScriptLocal", "name", "parent", Security::WiibloxScript);
...
Reflection::BoundFuncDesc<ScriptContext, shared_ptr<const Reflection::Tuple>(bool)> func_GetHeapStats(&ScriptContext::getHeapStats, "GetHeapStats", "clearHighwaterMark", true, Security::WiibloxScript);
Reflection::BoundFuncDesc<ScriptContext, shared_ptr<const Reflection::ValueArray>()> func_GetScriptStats(&ScriptContext::getScriptStatsNew, "GetScriptStats", Security::WiibloxScript);
Reflection::BoundFuncDesc<ScriptContext, void(bool)> func_CollectScriptStats(&ScriptContext::setCollectScriptStats, "SetCollectScriptStats", "enable", false, Security::WiibloxScript);
...
static int panic(lua_State *L) 
{
	...
	if (shared_ptr<BaseScript> script = WiibloxExtraSpace::get(L)->script.lock())
	{
...
void ScriptContext::onHook(lua_State *L, lua_Debug *ar)
{
	if (timedout)
		if (Security::Context::current().identity==Security::GameScript_ || 
			Security::Context::current().identity==Security::WiibloxGameScript_)
			luaL_error(L, "Game script timeout");
}


void wbx::registerScriptDescriptors()
{
	wbx::Reflection::Type::singleton<int>();
	wbx::Reflection::Type::singleton<bool>();
	wbx::Reflection::Type::singleton<float>();
	wbx::Reflection::Type::singleton<double>();
	wbx::Reflection::Type::singleton<std::string>();
	wbx::Reflection::Type::singleton<Lua::WeakFunctionRef>();
	wbx::Reflection::Type::singleton<shared_ptr<GenericFunction> >();
	wbx::Reflection::Type::singleton<shared_ptr<GenericAsyncFunction> >();
	wbx::Reflection::Type::singleton< shared_ptr<Instance> >();
	wbx::Reflection::Type::singleton<G3D::Vector3>();
	wbx::Reflection::Type::singleton<G3D::Vector2>();
    wbx::Reflection::Type::singleton<G3D::Rect2D>();
	wbx::Reflection::Type::singleton<PhysicalProperties>();
	wbx::Reflection::Type::singleton<wbx::WBXRay>();
	wbx::Reflection::Type::singleton<G3D::CoordinateFrame>();
	wbx::Reflection::Type::singleton<G3D::Color3>();
	wbx::Reflection::Type::singleton<BrickColor>();
}


int ScriptContext::crash(lua_State *L)
{
	wbx::Security::Context::current().requirePermission(wbx::Security::LocalUser, "crash");

	WBXCRASH("User-generated crash");

	return 0;
}

int getGlobalStateIndex(wbx::Security::Identities identity)
{
    // transform identity into a particular VM. this can improve security by not mixing user scripts with Wiiblox scripts (we've
    ...
    if (wbx::Security::Context::isInRole(identity, wbx::Security::WiibloxScript))
        vm_index = Security::VM_WiibloxScriptPlus;
...
lua_State* ScriptContext::getGlobalState( wbx::Security::Identities identity )
{
	int vm_index = getGlobalStateIndex(identity);
	WBXASSERT(globalStates[vm_index].state);
	return globalStates[vm_index].state;
}

static bool preventLinkerStrip = false;

static int luaopen_os_WBX(lua_State* L)
{
    luaL_register(L, "os", LuaOsExtension::registry);
    return 1;
}

static int luaopen_math_WBX(lua_State* L)
{
	luaopen_math(L);
...
static int luaopen_debug_WBX(lua_State* L)
{
...
bool ScriptContext::openState(size_t idx)
{
	...
	if (!allocator)
	{
		allocator.reset(new wbx::LuaAllocator(FLog::UseLuaMemoryPool != 0));
	}
	...
	lua_gc(globalState, LUA_GCSETSTEPMUL, wbx::LuaSettings::singleton().gcStepMul);
	lua_gc(globalState, LUA_GCSETPAUSE, wbx::LuaSettings::singleton().gcPause);

	WiibloxExtraSpace::get(globalState)->setContext(this);

	{
		WBXASSERT_BALLANCED_LUA_STACK(globalState);
        ...
        loadLibraryProtected(globalState, luaopen_math_WBX);
        loadLibraryProtected(globalState, luaopen_table);
		loadLibraryProtected(globalState, luaopen_debug_WBX);
...

	{
		...
		WBXRayBridge::registerClass(globalState);
...

	{
		...
		WBXRayBridge::registerClassLibrary(globalState);
...

	{
        ...
        lua_pushcfunction(globalState, WbxTime);
        ...
        loadLibraryProtected(globalState, luaopen_os_WBX);
...
void ScriptContext::setWiibloxPlace(bool WiibloxPlace)
{
	this->WiibloxPlace = WiibloxPlace;

	// slightly obfuscated message info, since don't want users trying to change this bool

	FASTLOG1(FLog::CoreScripts, "srp: %u", WiibloxPlace);
}
void ScriptContext::closeStates(bool resettingSimulation)
{
	...
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Exception thrown while cleaning up Lua: %s", e.what());
		WBXASSERT(false);	// we should NEVER get an exception in here
	}
...
void ScriptContext::closeState(lua_State* globalState)
{
	...
	{
		dumpThreadRefCounts();
		WiibloxExtraSpace::get(globalState)->eraseRefsFromAllNodes();
		dumpThreadRefCounts();
	}
	catch (std::exception& e)
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Exception thrown while erasing Lua refs: %s", e.what());
		WBXASSERT(false);	// we should NEVER get an exception in here
	}
...
,preventNewConnection(false)
,WiibloxPlace(false)
,nextPendingScripts(wbx::Time::now<Time::Fast>())
,timedout(false)
...
shared_ptr<const Reflection::ValueArray> ScriptContext::getScriptStatsNew()
{
	...
	const ScriptStats::ScriptActivityMeterMap& stats = scriptStats->getScriptActivityMap();
	shared_ptr<Reflection::ValueArray> result(wbx::make_shared<Reflection::ValueArray>(stats.size()));
	int pos = 0;
	for(ScriptStats::ScriptActivityMeterMap::const_iterator iter = stats.begin(); iter != stats.end(); ++iter)
	{
		std::string hash = iter->first;
...
void ScriptContext::setTimeout(double seconds)
{
	...
	else
	{
		...
		if (!timeoutThread)
			timeoutThread.reset(new boost::thread(wbx::thread_wrapper(boost::bind(&ScriptContext::onCheckTimeout, shared_from(this)), "ScriptContext Timeout")));
		else
			checkTimeout.Set();
	}
}
...
void ScriptContext::setThreadIdentityAndSandbox(lua_State* thread, wbx::Security::Identities identity, shared_ptr<BaseScript> script)
{
	sandboxThread(thread);

	WiibloxExtraSpace::get(thread)->identity = identity;
	WiibloxExtraSpace::get(thread)->script = script;

	WBXASSERT(getThreadIdentity(thread)==identity);
}
...
size_t ScriptContext::getThreadCount() const {
	...
	for (GlobalStates::const_iterator iter = globalStates.begin(); iter != globalStates.end(); ++iter)
		if (iter->state)
			count += WiibloxExtraSpace::get(iter->state)->getThreadCount();
	return count;
}

wbx::Security::Identities ScriptContext::getThreadIdentity(lua_State* thread)
{
	return thread ? WiibloxExtraSpace::get(thread)->identity : wbx::Security::Anonymous;
}

ScriptContext& ScriptContext::getContext(lua_State* thread)
{
	return *WiibloxExtraSpace::get(thread)->context();
}
...
void ScriptContext::executeInNewThread(wbx::Security::Identities identity, const ProtectedString& script, const char* name)
{
	executeInNewThread(
		identity, 
		script,
...
std::auto_ptr<Reflection::Tuple> ScriptContext::executeInNewThread(
		wbx::Security::Identities identity, const ProtectedString& script,
    ...
    unsigned int modkey)
{
...
#if defined(_WIN32) && !defined(WBX_STUDIO_BUILD) && defined(I_AM_GOY_THAT_LOVES_VMPROTECT) 
	...
	//WBXASSERT_SLOW(strlen(kXHIntData) % 16 != 0);
    ...
    const bool isCmdLine = (identity==wbx::Security::CmdLine_);
	...
	if (isCmdLine)
	{
		if (clSandboxEmpty)
		{
			lua_State* globalState = getGlobalState(wbx::Security::CmdLine_);
			WBXASSERT_BALLANCED_LUA_STACK(globalState);
		    initializeLuaStateSandbox(commandLineSandbox, globalState, wbx::Security::CmdLine_);
			lua_pop(globalState, 1);
		}
		ThreadRef safeCommandlineSandbox = commandLineSandbox.lock();
		WBXASSERT(safeCommandlineSandbox);
	...
	if (ThreadRef safeThread = thread.lock())
	{
		...
		WBXASSERT(continuations.empty());
		WiibloxExtraSpace::get(safeThread)->continuations.reset(new Lua::Continuations(continuations));
...
int ScriptContext::spawn(lua_State *thread)
{
	...
	if (DFFlag::BadTypeOnSpawnErrorEnabled)
	{
		...
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Attempt to call spawn failed: Passed value is not a function");
...
{
	if (timeout < wbx::LuaSettings::singleton().defaultWaitTime)
		timeout = wbx::LuaSettings::singleton().defaultWaitTime;
...
int ScriptContext::delay(lua_State *thread)
{
	...
	if (DFFlag::BadTypeOnDelayErrorEnabled)
	{
		...
		if (!varResult.isType<Lua::WeakFunctionRef>())
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Attempt to call delay failed: Passed value is not a function");
			ScriptContext::printCallStack(thread);
		}
	}
...
int ScriptContext::ypcall(lua_State *thread)
{
	...
	case ScriptContext::Success:
		{
			WBXASSERT(top == lua_gettop(thread));
	...
	case ScriptContext::Yield:
		{
			...
			WBXASSERT(WiibloxExtraSpace::get(safeFunctor)->continuations.get() == NULL);
			WiibloxExtraSpace::get(safeFunctor)->continuations.reset(new Lua::Continuations(continuations));

			//Capture the yield
			WiibloxExtraSpace::get(thread)->yieldCaptured = true;
...
{
	wbx::Security::Context::current().requirePermission(wbx::Security::Plugin, "settings()");
...
{
	wbx::Security::Context::current().requirePermission(wbx::Security::None, "UserSettings()");
...
{
	wbx::Security::Context::current().requirePermission(wbx::Security::Plugin, "PluginManager()");
...
{
	wbx::Security::Context::current().requirePermission(wbx::Security::Plugin, "DebuggerManager()");

	ObjectBridge::push(thread, shared_from(&wbx::Scripting::DebuggerManager::singleton()));
	return 1;
}
...
int ScriptContext::loadLibrary(lua_State *L)
{
    ...
    unsigned int modkey =
        (getGlobalState(L) == context.globalStates[Security::VM_WiibloxScriptPlus].state)
        ? LUAVM_MODKEY_DUMMY
...
static void endThreadsWithError(std::vector<WeakThreadRef>& threads, const char* message)
{
	for (std::vector<WeakThreadRef>::iterator itr = threads.begin(); itr != threads.end(); ++itr)
	{
		if (ThreadRef thread = itr->lock())
		{
			...
			WiibloxExtraSpace* space = WiibloxExtraSpace::get(thread);
...
void ScriptContext::reloadModuleScriptInternal(lua_State* globalState, shared_ptr<ModuleScript> moduleScript)
{
    ...
    if (!loaded)
    {
        ...
        throw wbx::runtime_error("syntax error: %s", err.c_str());
    }
    ...
    if (reloadResult == LUA_YIELD)
    {
        ...
        // yield capturing.
        WBXASSERT(WiibloxExtraSpace::get(reloadThread)->continuations == NULL);
        Lua::Continuations continuations;
        ...
        continuations.error = boost::bind(&ScriptContext::reloadModuleScriptErrorContinuation,
                                          moduleScript, _1);
        WiibloxExtraSpace::get(reloadThread)->continuations.reset(new Lua::Continuations(continuations));
        return;
    }
    else if (reloadResult != 0)
    {
        const char* message = lua_tostring(reloadThread, -1);
        throw wbx::runtime_error("runtime error: %s", message);
    }
   
    // Unrefs the new state created by the reloadThread.
    vmState.reassignResultRegistryIndex(oldResultIndex);

    WBXASSERT(moduleScript->getReloadRequested() == false);
    moduleScript->setReloadRequested(false);
}
...
{
	WBXASSERT(result != AsyncHttpQueue::Waiting);
	WBXASSERT(info->state == AssetModuleInfo::Fetching);
...
void ScriptContext::startRunningModuleScript(Security::Identities identity, lua_State* rootGlobalState, shared_ptr<ModuleScript> moduleScript)
{
	...
	{
		WBXASSERT(WiibloxExtraSpace::get(thread)->continuations == NULL);
		Lua::Continuations continuations;
		...
		continuations.error = boost::bind(&ScriptContext::requireModuleScriptErrorContinuation,
			moduleScript, _1);
		WiibloxExtraSpace::get(thread)->continuations.reset(new Lua::Continuations(continuations));
	}
	else
	{
		vmState.setCompletedError();
...
int ScriptContext::requireModuleScriptFromInstance(lua_State* L, shared_ptr<ModuleScript> moduleScript)
{
	...
	WBXASSERT(state != ModuleScript::NotRunYet);
	...
	else if (state == ModuleScript::Running)
	{
		...
		WBXASSERT(WiibloxExtraSpace::get(L)->yieldCaptured == false);
		WiibloxExtraSpace::get(L)->yieldCaptured = true;
...
int ScriptContext::requireModuleScriptFromAssetId(lua_State* L, int assetId)
{
	...
	if (info.state == AssetModuleInfo::NotFetchedYet)
	{
		...
		WBXASSERT(info.yieldedImporters.empty());
		...
		if (DFFlag::LogPrivateModuleRequires)
		{
			...
			contentId = ContentId(wbx::format("%s/asset/?id=%d&modulePlaceId=%d", ::trim_trailing_slashes(contentProvider->getBaseUrl()).c_str(), assetId, dm->getPlaceID()));
		}
		else
		{
			contentId = ContentId(wbx::format("wbxassetid://%d", assetId));
		}
...
{
	wbx::Security::Context::current().requirePermission(wbx::Security::LocalUser, "loadfile");

	ContentId contentId(throwable_lua_tostring(L, -1)); /* get file name */
	
	if (contentId.isNull())
		throw wbx::runtime_error("Unable to load %s", contentId.c_str());

	if(!wbx::Network::isTrustedContent(contentId.c_str()))
...
{
	WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "loadstring", format("%d", placeId).c_str());
}
...
{
	lua_pushstring(L, DebugSettings::WiibloxVersion);
	return 1;
}
...
{
	ObjectBridge::push(L, shared_from(wbx::ServiceProvider::create<wbx::Stats::StatsService>(&ScriptContext::getContext(L))));
	return 1;
}
...
int ScriptContext::WbxTime(lua_State *L)
{
...
int ScriptContext::doPrint(lua_State *L, const wbx::MessageType& messageType)
{
	std::string message;
...
void ScriptContext::onHeartbeat(const Heartbeat& heartbeat)
{
	...
	if (wbx::Time::now<Time::Fast>() >= nextPendingScripts)
	{
		WBXASSERT(!kCLuaResumeStackSize.get() || (*kCLuaResumeStackSize) == 0);
		// Try starting pending scripts, because some of them might be postponed for the ScriptId to download...
		startPendingScripts();
		nextPendingScripts = wbx::Time::now<Time::Fast>() + wbx::Time::Interval(1);
		WBXASSERT(!kCLuaResumeStackSize.get() || (*kCLuaResumeStackSize) == 0);
	}
...
void ScriptContext::resumeWaitingScripts(const Time expirationTime)
{
...

	{
		...
		while (count-- > 0 && waitingThreads.pop_if_present(wt))
		{
			...
			if (wbx::Time::nowFast() > expirationTime)
			{
				throttling = true;
				break;
			}
...
void ScriptContext::stepGc()
{
	...
	for (GlobalStates::iterator iter = globalStates.begin(); iter != globalStates.end(); ++iter)
		if (iter->state)
		{
			...
			WBXPROFILER_LABELF("Lua", "Allocated %d Kb (total %d Kb)", gcAlloc, gcCountPre);
			WBXPROFILER_LABELF("Lua", "Freed %d Kb (requested %d Kb)", gcCountPre - gcCountPost, gcRequest);
		}
...
void ScriptContext::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
	...
	if (newProvider)
	{
        ...
        {
            globalStates[Security::VM_WiibloxScriptPlus].state->l_G->ckey = LuaVM::getKeyCore();
        }
...
{
    wbx::BaseScript::adminScriptsPath = newPath;
}
...
void ScriptContext::addStarterScript(int assetId)
{
    ...
    throw wbx::runtime_error("AddStarterScript disabled (asset %d)", assetId);
}
...
void ScriptContext::addCoreScript(int assetId, shared_ptr<Instance> parent, std::string name)
{
    ...
    throw wbx::runtime_error("AddCoreScript disabled (asset %d)", assetId);
}
...
{
	line = -1;
	source = WiibloxExtraSpace::get(thread)->script.lock();
...
{
	if (!wbx::DebugSettings::singleton().getStackTracingEnabled())
		return;
	...
	if (lua_getstack(thread, 0, &ar)) 
	{
		...
		while (level<maxLevel && lua_getstack(thread, level++, &ar)){
			if (lua_getinfo(thread, "nSlu", &ar))
			{
				if (ar.currentline>=0)
				{
					...
					{
						if (!dontPrint)
							wbx::StandardOut::singleton()->printf(MESSAGE_INFO, "Stack Begin");

						if(output)
							stream << "Stack Begin";
					}
					if (ar.name)
						messageLine = wbx::format("Script '%s', Line %d - %s %s", source, ar.currentline, ar.namewhat, ar.name);
					else
						messageLine = wbx::format("Script '%s', Line %d", source, ar.currentline);

					if (!dontPrint)
						wbx::StandardOut::singleton()->printf(MESSAGE_INFO, "%s", messageLine.c_str());
		...
		{
			if (!dontPrint)
				wbx::StandardOut::singleton()->printf(MESSAGE_INFO, "Stack End");
...
void ScriptContext::reportError(lua_State* thread)
{
	...
	shared_ptr<BaseScript> scriptPtr = WiibloxExtraSpace::get(thread)->script.lock();
...
:wbx::Security::Impersonator(getThreadIdentity(thread))
{
}
...
ScriptContext::Result ScriptContext::resume(ThreadRef thread, int narg)
{
	...
	int result;
	{
		boost::shared_ptr<BaseScript> script = WiibloxExtraSpace::get(thread)->script.lock();

		ScriptImpersonator impersonate(thread);

		if (timedout && (Security::Context::current().identity==Security::GameScript_ || Security::Context::current().identity==Security::WiibloxGameScript_))
		{
			if (script)
		...
		WBXPROFILER_SCOPE("Lua", "$Script");
		WBXPROFILER_LABEL("Lua", script ? script->getName().c_str() : "Unknown");
		...
		Continuations* continuations = WiibloxExtraSpace::get(thread)->continuations.get();
		if (continuations)
        {
			if (result == Error)
			{
			...
			else if (result == Success)
			{
				if (continuations->success)
				{
					...
					catch (const wbx::base_exception& e)
					{
	...
	case LUA_YIELD:
		// Add to yield queue
		if (!WiibloxExtraSpace::get(thread)->yieldCaptured && yieldEvent.get())
...
void ScriptContext::startScript(ScriptStart scriptStart)
{
	...
	WBXASSERT(script->getRootAncestor() == getRootAncestor());
	WBXASSERT(scripts.find(script.get()) != scripts.end());
	...
	if (wbx::LuaSettings::singleton().areScriptStartsReported)
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Running Script \"%s\"", script->getFullName().c_str());

	try
	{
		wbx::Security::Identities identity;

		if (script->isA<CoreScript>())
			identity = wbx::Security::WiibloxGameScript_;
		else if (scriptStart.options.identity != wbx::Security::GameScript_)
			identity = scriptStart.options.identity;
		else if (WiibloxPlace)
			identity = wbx::Security::GameScriptInWiibloxPlace_;
		else
			identity = wbx::Security::GameScript_;
		...
		if (thread==NULL)
			throw wbx::runtime_error("Unable to create a new thread for %s", script->getName().c_str());
...

		{
			...
			if (LuaVM::canCompileScripts() && scriptStart.options.filter)
			{
				...
				catch (const ScriptStartOptions::LuaSyntaxError& e)
				{
					...
					std::string message = wbx::format("%s(%d) - %s", script->getFullName().c_str(), e.lineNumber, e.what());
					throw std::runtime_error(message);
				}
			}

			if (!scriptStart.options.continuations.empty())
				WiibloxExtraSpace::get(thread)->continuations.reset(new Lua::Continuations(scriptStart.options.continuations));
...
Reflection::Variant ScriptContext::evaluateStudioCommandItem(const char* itemToEvaluate, shared_ptr<LuaSourceContainer> script)
{
    if (commandLineSandbox.empty())
        initializeLuaStateSandbox(commandLineSandbox, getGlobalState(wbx::Security::CmdLine_), wbx::Security::CmdLine_);
	...
	wbx::Reflection::Variant result = Reflection::Variant();

	try
	{
		result = wbx::Scripting::DebuggerManager::readWatchValue(itemToEvaluate, 0, L);
	}
	catch (std::runtime_error const& e)
	{
		wbx::Log::current()->writeEntry(wbx::Log::Information, wbx::format("IntelleSenseException: %s", e.what()).c_str());
	}
...
{
	WBXASSERT(runningScripts.find(weak_from(script)) == runningScripts.end());

	ScriptContext* scriptContext = ServiceProvider::create<ScriptContext>(this);

	WBXASSERT(!scriptContext->hasScript(script));
...
void RuntimeScriptService::releaseScript(BaseScript* script)
{
	...
	if (script->fastDynamicCast<CoreScript>())
	{
		WBXASSERT(pendingScripts.find(weakPtr) == pendingScripts.end());
		WBXASSERT(runningScripts.find(weakPtr) == runningScripts.end());
		WBXASSERT(!scriptContext->hasScript(script));
		return;
	}
...
{
	ScriptContext* context = WiibloxExtraSpace::get(L)->context();
    ...
    {
        WBXASSERT(DataModel::currentThreadHasWriteLock(context));
    }
}
