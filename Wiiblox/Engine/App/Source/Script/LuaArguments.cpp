...
bool LuaArguments::getVector3int16(int index, Vector3int16& value) const
{
	...
	return Bridge<wbx::Vector3int16>::getValue(L, index, value);
}
...
bool LuaArguments::getRegion3int16(int index, Region3int16& value) const
{
	...
	return Bridge<wbx::Region3int16>::getValue(L, index, value);
}
...
bool LuaArguments::getVector3(int index, Vector3& value) const
{
	...
	return Bridge<wbx::Vector3>::getValue(L, index, value);
}
...
bool LuaArguments::getRegion3(int index, Region3& value) const
{
	...
	return Bridge<wbx::Region3>::getValue(L, index, value);
}
...
bool LuaArguments::getRect(int index, Rect2D& value) const
{
	...
	return Bridge<wbx::Rect2D>::getValue(L, index, value);
}
	...
	case LUA_TTABLE:
		{
			...
			if (n > 0)
			{
				...
				shared_ptr<Reflection::ValueArray> values(wbx::make_shared<Reflection::ValueArray>(n));
				...
				value = shared_ptr<const wbx::Reflection::ValueArray>(values);
			}
			else
			{
				// Create a new collection so that we can populate it
				...
				while (lua_next(L, tableIndex) != 0)
				{
					...
					if (!values)
						values = wbx::make_shared<Reflection::ValueTable>();
				...
				if (values)
					value = shared_ptr<const wbx::Reflection::ValueTable>(values);
				else
					// Assume an empty Lua table is an empty ValueArray (they can auto-convert later)
					value = wbx::make_shared<const Reflection::ValueArray>();
	...
	case LUA_TUSERDATA:
		{
			...
			if (Vector2Bridge::getValue(L, luaIndex, value))
				return true;
			if (WBXRayBridge::getValue(L, luaIndex, value))
				return true;
...
public:
	...
	int operator()(const wbx::ProtectedString& value)
	{
