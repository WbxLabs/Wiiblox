...
void YieldingThreads::queueWaiter(lua_State *L, LUA_NUMBER delay)
{
	...
	waitingThreads.push(WaitingThread(L, wbx::Time::Interval(delay)));
}
...
void YieldingThreads::resume(double wallTime, Time expirationTime, bool& throttling)
{
	...
	{
		wbx::Time now = wbx::Time::now<wbx::Time::Precise>();
		...
		wbx::Time::Interval elapsedTime = now - w.waitTime;
...
namespace WBX
{
	namespace Lua
	{
		...
		template<>
		int Bridge<wbx::signals::connection>::on_tostring(const wbx::signals::connection& object, lua_State *L)
		{
