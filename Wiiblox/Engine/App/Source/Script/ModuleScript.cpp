...
const char* const wbx::sModuleScript = "ModuleScript";
...
{
	WBXASSERT(scriptLoadingState == NotRunYet);
...
{
	WBXASSERT(scriptLoadingState == Running);
...
void ModuleScript::PerVMState::releaseReferenceIfCompletedSuccessfully()
{
	...
	{
		WBXASSERT(resultRegistryIndex != LUA_NOREF);
		WBXASSERT(globalStateContainingResult);
...
void ModuleScript::PerVMState::releaseScriptNodeIfPresent()
{
	...
	// This mechanism is how wbx::signal connections get cleaned up.
	if (node)
	{
		node->eraseAllRefs();
		node.reset();
