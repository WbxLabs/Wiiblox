...
bool LuaAllocator::hasSpace(const long diff)
{
	if (heapLimit > 0 && diff > 0 && diff + heapSize > heapLimit &&
		(wbx::Security::Context::current().identity == wbx::Security::GameScript_ || wbx::Security::Context::current().identity == wbx::Security::WiibloxGameScript_) )
		return false;
	return true;
}
...
void* LuaAllocator::alloc(void *ptr, size_t osize, size_t nsize)
{
	...
	if (!memPools.empty())
	{
		if (nsize == 0)
		{
			...
			if (index > -1)
			{
				WBXASSERT(osize == memPools[index]->get_requested_size());
		...
		else // allocate new memory
		{
			...
			if (index1 > -1)
			{
				WBXASSERT(nsize == memPools[index1]->get_requested_size());
				...
				if (index2 > -1)
				{
					WBXASSERT(osize == memPools[index2]->get_requested_size());
    ...
    WBXPROFILER_COUNTER_ADD("memory/lua/heap", diff);
