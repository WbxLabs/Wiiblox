...
{
	return WiibloxExtraSpace::get(thread)->getNode();
}
...
static void onAsyncResult(ThreadRef thread, weak_ptr<ScriptContext> context, IAsyncResult* result)
{
	...
	catch (wbx::base_exception& e)
	{
		...
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "Exception caught from async call. %s", e.what());
		return;
	}

	if (shared_ptr<wbx::ScriptContext> lockedContext = context.lock())
...
static int callGenericAsyncFunctionBridge (lua_State *L) 
{
	...
	(*f)(args, boost::bind(&onAsyncResult, ThreadRef(L), weak_from(WiibloxExtraSpace::get(L)->context()), _1));
...
wbx::Lua::detail::LiveThreadRef::LiveThreadRef (lua_State* thread)
:L(thread)
{
	WBXASSERT(L);
...
wbx::Lua::detail::LiveThreadRef::~LiveThreadRef()
{
...
namespace WBX
{
	...
	{
		template<>
		const Type& Type::getSingleton<wbx::Lua::WeakFunctionRef>()
		{
			static TType<wbx::Lua::WeakFunctionRef> type("Function");
			return type;
		}

		template<>
		wbx::Lua::WeakFunctionRef& Variant::convert<wbx::Lua::WeakFunctionRef>(void)
		{
			// Interpret "void" (Lua nil) as a null function
			// TODO: Is that correct behavior????
			if (isType<void>())
			{
				value = wbx::Lua::WeakFunctionRef();
				_type = &Type::singleton<wbx::Lua::WeakFunctionRef>();
			}

			wbx::Lua::WeakFunctionRef* val = tryCast<wbx::Lua::WeakFunctionRef>();
			if (val==NULL)
