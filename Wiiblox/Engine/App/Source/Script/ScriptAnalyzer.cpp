...
void* operator new(size_t size, wbx::ScriptParser::Allocator& alloc)
{
...
void operator delete(void*, wbx::ScriptParser::Allocator& alloc)
{
}
...
namespace WBX
{
    namespace ScriptParser
    {
        ...
        {
        public:
            Error(const Location& location, const char* format, ...) WBX_PRINTF_ATTR(3, 4)
                : location(location)
            {
                va_list args;
        ...
        struct Lexeme
        {
            ...
            Lexeme(const Location& location, Type type, const std::string* data)
                ...
                , data(data)
            {
                WBXASSERT(type == String || type == Number);
            }
		...
		WBX_PRINTF_ATTR(4, 5) void emitWarning(ScriptAnalyzer::Result& result, ScriptAnalyzer::WarningCode code, const Location& location, const char* format, ...)
		{
    ...
    {
        Security::Identities identity = wbx::Security::GameScript_;
    ...
    ScriptAnalyzer::Result ScriptAnalyzer::analyze(DataModel* dm, shared_ptr<Instance> script, const std::string& code)
    {
        ...
        try
        {
            ...
            WBXASSERT(dm->currentThreadHasWriteLock());
