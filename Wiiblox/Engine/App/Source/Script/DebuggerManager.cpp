...
void DebuggerManager::setDataModel(wbx::DataModel *pDataModel)
{	
...
wbx::DataModel* DebuggerManager::getDataModel()
{	return dataModel; }
...
{
	shared_ptr<Instances> result = wbx::make_shared<Instances>(debuggers.size());
	int i = 0;
...
ScriptDebugger* DebuggerManager::findDebugger( lua_State* L )
{
	...
	ScriptDebugger* scriptDebugger = findDebugger(WiibloxExtraSpace::get(L)->script.lock().get());
	if (scriptDebugger)
...
void DebuggerManager::addDebugger(shared_ptr<ScriptDebugger> debugger)
{
	...
	wbx::Instance* script = debugger->getScript();
...
void DebuggerManager::addUnaddedDebuggerForAddedDescendant(shared_ptr<wbx::Instance> instance)
{
	if (instance && (instance->isA<wbx::Script>() || instance->isA<wbx::ModuleScript>()))
	{
		...
		{
			WBXASSERT(debuggers.find(instance.get()) == debuggers.end());
...
void DebuggerManager::addScriptDebugger(Instance* instance)
{
	if (ScriptDebugger* debugger = Instance::fastDynamicCast<ScriptDebugger>(instance))
	{
		...
		{
			WBXASSERT(debuggers.find(debugger->getScript()) == debuggers.end());
...
void DebuggerManager::resume()
{
	...
	while (!errorThreads.empty())
	{
		...
		if (wbx::Lua::ThreadRef thread = weakref.lock())
		{
...
void DebuggerManager::onHook(lua_State* L, lua_Debug *ar)
{
	...
	if (paused)
	{
		...
		//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "paused debugger = %d", L);
	}
}
...
{
	setName(wbx::format("%sDebugger", script.getName().c_str()));
	setScript(&script);
}
...
{
	shared_ptr<Instances> result = wbx::make_shared<Instances>(breakpoints.size());
	int i = 0;
...
{
	shared_ptr<Instances> result = wbx::make_shared<Instances>(watches.size());
	int i = 0;
...
{
	wbx::StandardOut::singleton()->print(wbx::MESSAGE_INFO, "setIndexInfo");
	return 0;
}
...
Reflection::Variant DebuggerManager::readWatchValue(std::string expression, int stackDepth, lua_State* L)
{
	...
	WBXASSERT_BALLANCED_LUA_STACK2(evalThread);
	...
	{
		return std::string(wbx::format("*** %s ***", exp.what()));
	}
...
{
	WBXASSERT(ar->event == LUA_HOOKLINE);
	WBXASSERT(currentLine == ar->currentline);

	try
	{
		r = f(L, ar);
	}
	catch (wbx::base_exception& ex)
	{
		error = wbx::make_shared<std::string>(ex.what());
	}

	WBXASSERT(pausedThread.lock() == L);
	//pausedThread = L;
	WBXASSERT(!WiibloxExtraSpace::get(L)->yieldCaptured);
...
void ScriptDebugger::debuggerBreak( lua_State* L, lua_Debug *ar )
{
	...
	WBXASSERT(currentLine > 0);

	//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Encountered breakpoint: %s line %d", ar->short_src, currentLine);
	...
	if (pausedThread.empty())
	{
		...
		//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Resuming: %s line %d", ar->short_src, ar->currentline);
		resuming();
		currentLine = 0;
	}
	else
...
shared_ptr<Reflection::ValueMap> ScriptDebugger::readLocals(int stackIndex, lua_State* L)
{
	...
	shared_ptr<Reflection::ValueMap> locals = wbx::make_shared<Reflection::ValueMap>();
	WBXASSERT_BALLANCED_LUA_STACK(L);
	int n = 1;
...
shared_ptr<Reflection::ValueMap> ScriptDebugger::readGlobals(lua_State* L)
{
	...
	lua_getfenv(L, t);
#endif
	shared_ptr<Reflection::ValueMap> result = wbx::make_shared<Reflection::ValueMap>();
	...
	while (lua_next(L, t))
	{
		if (lua_isstring(L, -2))
		{
			...
			WBXASSERT(result->find(name) == result->end());
...
shared_ptr<Reflection::ValueMap> ScriptDebugger::readUpvalues(int stackIndex, lua_State* L)
{
	...
	shared_ptr<Reflection::ValueMap> arguments = wbx::make_shared<Reflection::ValueMap>();
	...
	while (const char* name = lua_getupvalue(L, -1, n++))
	{
		if (name[0] > 0)
		{
			Reflection::Variant value;
			Lua::LuaArguments::get(L, -1, value, false);
			WBXASSERT(arguments->find(name) == arguments->end());
...
bool ScriptDebugger::onLineHook( lua_State* L, lua_Debug *ar )
{
	...
	{
		lua_getinfo(L, "f", ar);
		wbx::Instance* scriptInstance = getScriptForLuaState(L);
		if (scriptInstance)
		{
...
bool ScriptDebugger::shouldBreak(DebuggerBreakpoint* bp, lua_State* L)
{
	...
	{
		std::string err = Lua::safe_lua_tostring(evalThread, -1);
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Breakpoint %d condition syntax error: %s", bp->getLine(), err.c_str());
	...
	{
		const char* message = lua_tostring(evalThread, -1);
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Breakpoint %d condition runtime error: %s", bp->getLine(), message);
...
void ScriptDebugger::onChildAdded( Instance* child )
{
	if (DebuggerBreakpoint* breakpoint = Instance::fastDynamicCast<DebuggerBreakpoint>(child))
	{
		WBXASSERT(breakpoint->getLine() > 0);
...
void ScriptDebugger::setScript( Script* value )
{
	...
	WBXASSERT(!script);	// Changing the script is not supported. DebuggerManager doesn't track changes like this
...
void ScriptDebugger::onScriptStarting( lua_State* L )
{
	...
	DebuggerManager& debuggerManager = wbx::Scripting::DebuggerManager::singleton();
	if (!debuggerManager.getEnabled())
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "Can't debug script %s because debugging is disabled", script->getName().c_str());
		return;
	}
...
void ScriptDebugger::onScriptParentChanged(shared_ptr<wbx::Instance> newParent)
{
	if (!newParent)
	{
...
void ScriptDebugger::onScriptCloned(shared_ptr<wbx::Instance> clonedScript)
{
	boost::shared_ptr<ScriptDebugger> clonedDebugger = createClone(clonedScript);
	if (clonedDebugger)
		wbx::Scripting::DebuggerManager::singleton().addDebugger(clonedDebugger);
}
...
void ScriptDebugger::setLocal( std::string name, Reflection::Variant value, int stackFrame)
{
	if (!withPausedThread<bool>(boost::bind(&doSetLocal, name, boost::cref(value), stackFrame, _1)))
		throw std::runtime_error(wbx::format("Bad local %s", name.c_str()));
}
...
void ScriptDebugger::setUpvalue( std::string name, Reflection::Variant value, int stackFrame)
{
	if (!withPausedThread<bool>(boost::bind(&doSetUpvalue, name, boost::cref(value), stackFrame, _1)))
		throw std::runtime_error(wbx::format("Bad upvalue %s", name.c_str()));
}
...
void ScriptDebugger::setGlobal( std::string name, Reflection::Variant value)
{
	if (!withPausedThread<bool>(boost::bind(&doSetGlobal, name, boost::cref(value), _1)))
		throw std::runtime_error(wbx::format("Bad global %s", name.c_str()));
}
...
shared_ptr<const Reflection::ValueArray> ScriptDebugger::getStack_Reflection()
{
	...
	{
		shared_ptr<Reflection::ValueMap> item = wbx::make_shared<Reflection::ValueMap>();
...
wbx::Instance* ScriptDebugger::getScriptForLuaState(lua_State* L)
{
	WBXASSERT(lua_isnil(L, -1) || lua_isfunction(L, -1));
	...
	if (varResult.isType<boost::shared_ptr<wbx::Instance> >())
		return varResult.get<boost::shared_ptr<wbx::Instance> >().get();
...
static std::string getHashString(wbx::Instance* pInstance)
{
...
{ 
	...
	const wbx::Instance* pParent = script->getParent();
	const wbx::Instance* pChild = script.get();
...
{
	wbx::Instance* pParent = wbx::DataModel::get(wbx::Scripting::DebuggerManager::singleton().getDataModel());
	wbx::Instance* pChild = NULL;
...
void ScriptDebugger::setLuaHook(ScriptDebugger* scriptDebugger, int hookMask, lua_State *L)
{
	...
	{
		const WiibloxExtraSpace* pExtraSpace = WiibloxExtraSpace::get(L);
		if (pExtraSpace)
		{
			...
			{
				shared_ptr<wbx::ModuleScript> pModuleScript = wbx::Instance::fastSharedDynamicCast<wbx::ModuleScript>(scriptDebugger->script);
				if (pModuleScript)
					lua_sethook(L, DebuggerManager::hook, hookMask, ScriptContext::hookCount);
			}
		}
...
void ScriptDebugger::updateHook()
{
	...
	WiibloxExtraSpace* pExtraSpace = WiibloxExtraSpace::get(rootThread.lock());
	if (pExtraSpace)
	{
		int hookMask = LUA_MASKLINE | LUA_MASKCALL | LUA_MASKRET | LUA_MASKCOUNT;
		if (wbx::Scripting::DebuggerManager::singleton().getBreakOnErrorMode() == BreakOnErrorMode_AllExceptions)
...
void ScriptDebugger::handleError(lua_State* thread)
{
	...
	std::string errorMessage = wbx::Lua::safe_lua_tostring(thread, -1);
...
boost::shared_ptr<ScriptDebugger> ScriptDebugger::createClone(boost::shared_ptr<Instance> clonedScript)
{
	...
	for (wbx::Scripting::ScriptDebugger::Breakpoints::const_iterator iter = breakpoints.begin(); iter != breakpoints.end(); ++iter)
	{
		if (iter->second)
		{
			shared_ptr<Instance> breakpoint = iter->second->clone(wbx::EngineCreator);
			if (breakpoint)
	...
	for (wbx::Scripting::ScriptDebugger::Watches::const_iterator iter = watches.begin(); iter != watches.end(); ++iter)
	{
		if (*iter)
		{
			shared_ptr<Instance> watch = (*iter)->clone(wbx::EngineCreator);
			if (watch)
...
{
	WBXASSERT(line > 0);
	setName(wbx::format("Breakpoint%d", line));
}
