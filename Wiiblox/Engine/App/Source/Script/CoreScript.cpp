...
namespace WBX
{
	...
	void CoreScript::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
	{
		...
		{
			wbx::ScriptContext* sc = ServiceProvider::find<ScriptContext>(oldProvider);
			WBXASSERT(sc);

			WBXASSERT(sc->hasScript(this));
	...
	BaseScript::Code CoreScript::requestCode(ScriptInformationProvider* scriptInfoProvider)
	{
        ...
        {
            throw wbx::runtime_error("Error loading core script %s", getName().c_str());
        }
    }
	...
	void CoreScript::extraErrorReporting(lua_State *thread)
	{
		...
		const char* error = wbx::Lua::safe_lua_tostring(thread, -1);
		...
		fileOut << wbx::FileSystem::getUserDirectory(true, wbx::DirAppData, "logs") << this->getName() << "_ln" << line << "_" << ".cse";
		...
		if(errorLog.is_open())
		{
			wbx::Log::timeStamp(errorLog, true);
