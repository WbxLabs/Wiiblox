...
static uint32_t WBXDaxEncodeOp(uint32_t x, uint32_t mulEven, uint32_t addEven, uint32_t mulOdd, uint32_t addOdd)
{
    uint32_t result      = 0;
    uint32_t mask        = 1;
...
namespace LuaVM
{
    ...
    std::string compile(const std::string& source)
    {
        ...
        std::string result = LuaSerializer::serialize(L, source, WBXDaxEncode, gLuaKeyPair.first);
    ...
    std::string compileLegacy(const std::string& source)
    {
        ...
        std::string result = LuaSerializer::serialize(L, source, WBXOldEncode, gLuaKeyPair.first);
    ...
    int load(lua_State* L, const wbx::ProtectedString& source, const char* chunkname, unsigned int modkey)
    {
    ...
    std::string compileCore(const std::string& source)
    {
        ...
        std::string result = LuaSerializer::serialize(L, source, WBXDaxEncode, LUAVM_INTERNAL_CORE_ENCODE_KEY);
    ...
    unsigned int WBXOldEncode(unsigned int i, int pc, unsigned int key)
    {
        (void)(pc);
        return LUAVM_ENCODEINSN(i, key);
    } 

    unsigned int WBXDaxEncode(unsigned int i, int pc, unsigned int key) 
    {
        Instruction enc = i;
        ...
        case OP_RETURN:
        case OP_CLOSURE:
            enc = WBXDaxEncodeOp(i, LUAVM_DAX_ME, pc, LUAVM_DAX_MO, LUAVM_DAX_AO);
            SET_OPCODE(enc, op);
            break;
        case OP_MOVE:
