...
{
	WBXASSERT(idx >= 0 && idx <= lua_gettop(L));
...
int Bridge<G3D::Color3>::on_index(const G3D::Color3& object, const char* name, lua_State *L)
{
	...
	// Failure
	throw wbx::runtime_error("%s is not a valid member", name);
}
...
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}

/// wbx::WBXRay has a default implementation for on_tostring() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
/// wbx::WBXRay has a default implementation for registerClass() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
template<>
const char* Bridge<wbx::WBXRay>::className("Ray");

const luaL_reg WBXRayBridge::classLibrary[] = {
	{"new", newWBXRay},
	{NULL, NULL}
};

void WBXRayBridge::registerClassLibrary (lua_State *L) {
	// Register the "new" function
	luaL_register(L, className, classLibrary); 
	lua_setreadonly(L, -1, true);
	lua_pop(L,1);		// Pop table from stack.   http://lua-users.org/lists/lua-l/2003-12/msg00139.html
...
int WBXRayBridge::newWBXRay(lua_State *L)
{
	Vector3 origin;
	Vector3 direction;
	...
	pushNewObject(L, wbx::WBXRay::fromOriginAndDirection(origin, direction));
...
{
	wbx::WBXRay& self	= Bridge<wbx::WBXRay>::getObject(L, 1);
...
template<>
int Bridge<wbx::WBXRay>::on_index(const wbx::WBXRay& object, const char* name, lua_State *L)
{
	...
	{
		Bridge<wbx::WBXRay>::pushNewObject(L, object.unit());
		return 1;
	}
...
template<>
void Bridge<wbx::WBXRay>::on_newindex(wbx::WBXRay& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
template<>
const char* Bridge<wbx::Region3>::className("Region3");
...
//Lower case are legacy
template<>
int Bridge<wbx::Region3>::on_index(const wbx::Region3& object, const char* name, lua_State *L)
{
...
template<>
void Bridge<wbx::Region3>::on_newindex(wbx::Region3& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
template<>
const char* Bridge<wbx::Region3int16>::className("Region3int16");
...
//Lower case are legacy
template<>
int Bridge<wbx::Region3int16>::on_index(const wbx::Region3int16& object, const char* name, lua_State *L)
{
...
template<>
void Bridge<wbx::Region3int16>::on_newindex(wbx::Region3int16& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
int PhysicalPropertiesBridge::newPhysicalProperties(lua_State *L)
{
	...
	if (FFlag::PhysPropConstructFromMaterial && count == 1)
	{
		...
		{
			if(!item->owner.isType<wbx::PartMaterial>())
				throw wbx::runtime_error("PhysicalProperties.new with 1 argument expects Enum.Material inputs");

			wbx::PartMaterial partMaterial = (wbx::PartMaterial)item->value;
	...
	else
	{
		if (FFlag::PhysPropConstructFromMaterial)
			throw wbx::runtime_error("Invalid number of arguments: %d, PhysicalProperties objects expect 1,3 or 5", count);
		else
			throw wbx::runtime_error("Invalid number of arguments: %d, PhysicalProperties objects expect 3 or 5", count);
	}
...
int Bridge<PhysicalProperties>::on_index(const PhysicalProperties& object, const char* name, lua_State *L)
{
	...
	// Failure
	throw wbx::runtime_error("%s is not a valid member of PhysicalProperties", name);
}
...
void Bridge<PhysicalProperties>::on_newindex(PhysicalProperties& object, const char* name, lua_State *L)
{
	...
	throw wbx::runtime_error("PhysicalProperties.%s cannot be assigned to", name);
}
...
int Rect2DBridge::newRect2D(lua_State *L)
{
    ...
    {
        throw wbx::runtime_error("Invalid number of arguments: %d", count);
	}
...
int Vector3Bridge::newVector3FromNormalId(lua_State *L)
{
	...
	if (EnumItem::getItem(L, 1, item))
	{
		if(!item->owner.isType<wbx::NormalId>()){
			throw wbx::runtime_error("Vector3.FromNormalId expects Enum.NormalId input");
		}
		Bridge<G3D::Vector3>::pushNewObject(L,wbx::normalIdToVector3((wbx::NormalId)item->value));
	}
	else {
		throw wbx::runtime_error("Vector3.FromNormalId expects Enum.NormalId input");
	}
...
int Vector3Bridge::newVector3FromAxis(lua_State *L)
{
	...
	if (EnumItem::getItem(L, 1, item))
	{
		if(!item->owner.isType<wbx::Vector3::Axis>()){
			throw wbx::runtime_error("Vector3.FromAxis expects Enum.Axis input");
		}
		Bridge<G3D::Vector3>::pushNewObject(L,
			wbx::normalIdToVector3(
				Axes::axisToNormalId((wbx::Vector3::Axis)item->value)
				)
			);
	}
	else {
...
template<>
const char* Bridge<wbx::Vector3int16>::className("Vector3int16");
...
{
	const wbx::Vector3int16& a = Vector3int16Bridge::getObject(L, 1);
	const wbx::Vector3int16& b = Vector3int16Bridge::getObject(L, 2);
...
int Vector3int16Bridge::on_div(lua_State *L)
{
	...
	if (Vector3int16Bridge::getValue(L, 1, a))
	{
		...
		if (Vector3int16Bridge::getValue(L, 2, b))
		{
			...
			{
				// Failure
				throw wbx::runtime_error("Divide by zero exception");
			}
...
template<>
int Bridge<wbx::Vector3int16>::on_index(const wbx::Vector3int16& object, const char* name, lua_State *L)
{
	if (strcmp(name,"x")==0 || strcmp(name,"X")==0)
	{
...
template<>
void Bridge<wbx::Vector3int16>::on_newindex(wbx::Vector3int16& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
template<>
const char* Bridge<wbx::Vector2int16>::className("Vector2int16");
...
{
	const wbx::Vector2int16& a = Vector2int16Bridge::getObject(L, 1);
	const wbx::Vector2int16& b = Vector2int16Bridge::getObject(L, 2);
...
int Vector2int16Bridge::on_div(lua_State *L)
{
	...
	else 
	{
		...
		pushVector2int16(L, wbx::Vector2int16(b,b) / a);
	}
	return 1;
};
...
template<>
int Bridge<wbx::Vector2int16>::on_index(const wbx::Vector2int16& object, const char* name, lua_State *L)
{
	if (strcmp(name,"x")==0 || strcmp(name,"X")==0)
	{
		lua_pushinteger(L, object.x);
	...
	//{
	//	Bridge<wbx::Vector2int16>::pushNewObject(L, object.direction());
	//	return 1;
	//}
...
template<>
void Bridge<wbx::Vector2int16>::on_newindex(wbx::Vector2int16& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}

/// wbx::Vector2 has a default implementation for on_tostring() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
template<>
const char* Bridge<wbx::Vector2>::className("Vector2");
...
{
	const wbx::Vector2& a = Vector2Bridge::getObject(L, 1);
	const wbx::Vector2& b = Vector2Bridge::getObject(L, 2);
	pushVector2(L, a + b);
	return 1;
};
...
int Vector2Bridge::on_div(lua_State *L)
{
	...
	else 
	{
		a = Vector2Bridge::getObject(L, 2);
		float b = lua_tofloat(L, 1);
		pushVector2(L, wbx::Vector2(b,b) / a);
	}
	return 1;
};
...
{
	wbx::Vector2& self = Bridge<wbx::Vector2>::getObject(L, 1);
	wbx::Vector2& v = Bridge<wbx::Vector2>::getObject(L, 2);
	float alpha = lua_tofloat(L, 3);
	Bridge<wbx::Vector2>::pushNewObject(L, self.lerp(v, alpha));
	return 1;
}

template<>
int Bridge<wbx::Vector2>::on_index(const wbx::Vector2& object, const char* name, lua_State *L)
{
	if (strcmp(name,"x")==0 || strcmp(name,"X")==0)
	{
		lua_pushnumber(L, object.x);
	...
	if (strcmp(name,"unit")==0)
	{
		Bridge<wbx::Vector2>::pushNewObject(L, object.direction());
		return 1;
	}
	if (strcmp(name,"magnitude")==0)
	{
...
template<>
void Bridge<wbx::Vector2>::on_newindex(wbx::Vector2& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}

/// wbx::BrickColor has a default implementation for on_tostring() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
/// wbx::BrickColor has a default implementation for registerClass() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
template<>
const char* Bridge<wbx::BrickColor>::className("BrickColor");
...
int BrickColorBridge::paletteBrickColor(lua_State *L)
{
	...
	if(index < 0 || index >= int(BrickColor::colorPalette().size())){
		throw wbx::runtime_error("palette index out of bounds (%d)", index);
	}
...
template<>
int Bridge<wbx::BrickColor>::on_index(const wbx::BrickColor& object, const char* name, lua_State *L)
{
...
template<>
void Bridge<wbx::BrickColor>::on_newindex(wbx::BrickColor& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
template<>
const char* Bridge<wbx::UDim>::className("UDim");
...
template<>
int Bridge<wbx::UDim>::on_index(const wbx::UDim& object, const char* name, lua_State *L)
{
	...
	if(name && !isupper(name[0]))
		throw wbx::runtime_error("%s is not a valid member, did you forget to capitalize the first letter?", name);
...
template<>
void Bridge<wbx::UDim>::on_newindex(wbx::UDim& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
template<>
const char* Bridge<wbx::UDim2>::className("UDim2");
...
template<>
int Bridge<wbx::UDim2>::on_index(const wbx::UDim2& object, const char* name, lua_State *L)
{
...
template<>
void Bridge<wbx::UDim2>::on_newindex(wbx::UDim2& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}

/// wbx::Faces has a default implementation for on_tostring() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
/// wbx::Faces has a default implementation for registerClass() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
template<>
const char* Bridge<wbx::Faces>::className("Faces");
...
int FacesBridge::newFaces(lua_State *L)
{
	...
	int normalIdMask = wbx::NORM_NONE_MASK;
	for(int index = 1; index<= count; index++){
		...
		{
			if(!item->owner.isType<wbx::NormalId>())
				throw wbx::runtime_error("Faces.new expects Enum.NormalId inputs");

			normalIdMask |= normalIdToMask((wbx::NormalId)item->value);
		}
	}
...
template<>
int Bridge<wbx::Faces>::on_index(const wbx::Faces& object, const char* name, lua_State *L)
{
	if (strcmp(name,"Top")==0)
	{
		lua_pushboolean(L, object.getNormalId(wbx::NORM_Y));
		return 1;
	}
	if (strcmp(name,"Bottom")==0)
	{
		lua_pushboolean(L, object.getNormalId(wbx::NORM_Y_NEG));
		return 1;
	}
	if (strcmp(name,"Back")==0)
	{
		lua_pushboolean(L, object.getNormalId(wbx::NORM_Z));
		return 1;
	}
	if (strcmp(name,"Front")==0)
	{
		lua_pushboolean(L, object.getNormalId(wbx::NORM_Z_NEG));
		return 1;
	}
	if (strcmp(name,"Right")==0)
	{
		lua_pushboolean(L, object.getNormalId(wbx::NORM_X));
		return 1;
	}
	if (strcmp(name,"Left")==0)
	{
		lua_pushboolean(L, object.getNormalId(wbx::NORM_X_NEG));
		return 1;
	}
	...
	if(name && !isupper(name[0]))
		throw wbx::runtime_error("%s is not a valid member, did you forget to capitalize the first letter?", name);
	// Failure
	throw wbx::runtime_error("%s is not a valid member. Valid members are Top,Bottom,Left,Right,Back,Front", name);
}

template<>
void Bridge<wbx::Faces>::on_newindex(wbx::Faces& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}

/// wbx::Axes has a default implementation for on_tostring() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
/// wbx::Axes has a default implementation for registerClass() invoked from LuaBridge.cpp. It is important you read LuaBridge.cpp if you are adding or removing any specialization
template<>
const char* Bridge<wbx::Axes>::className("Axes");
...
int AxesBridge::newAxes(lua_State *L)
{
	...
	for(int index = 1; index<= count; index++){
		...
		if (EnumItem::getItem(L, index, item))
		{
			if(!item->owner.isType<wbx::Vector3::Axis>() && !item->owner.isType<wbx::NormalId>() )
				throw wbx::runtime_error("Axes.new expects Enum.Axis or Enum.NormalId inputs");
			if(item->owner.isType<wbx::Vector3::Axis>()){
				axisMask |= wbx::Axes::axisToMask((wbx::Vector3::Axis)item->value);
			}
			else{
				axisMask |= wbx::Axes::axisToMask(wbx::Axes::normalIdToAxis((wbx::NormalId)item->value));
	...
	pushNewObject(L, wbx::Axes(axisMask));

	return 1;
}

template<>
int Bridge<wbx::Axes>::on_index(const wbx::Axes& object, const char* name, lua_State *L)
{
	if (strcmp(name,"X")==0)
	{
		lua_pushboolean(L, object.getAxis(wbx::Vector3::X_AXIS));
		return 1;
	}
	if (strcmp(name,"Y")==0)
	{
		lua_pushboolean(L, object.getAxis(wbx::Vector3::Y_AXIS));
		return 1;
	}
	if (strcmp(name,"Z")==0)
	{
		lua_pushboolean(L, object.getAxis(wbx::Vector3::Z_AXIS));
		return 1;
	}
	...
	if (strcmp(name,"Top")==0)
	{
		lua_pushboolean(L, object.getAxisByNormalId(wbx::NORM_Y));
		return 1;
	}
	if (strcmp(name,"Bottom")==0)
	{
		lua_pushboolean(L, object.getAxisByNormalId(wbx::NORM_Y_NEG));
		return 1;
	}
	if (strcmp(name,"Back")==0)
	{
		lua_pushboolean(L, object.getAxisByNormalId(wbx::NORM_Z));
		return 1;
	}
	if (strcmp(name,"Front")==0)
	{
		lua_pushboolean(L, object.getAxisByNormalId(wbx::NORM_Z_NEG));
		return 1;
	}
	if (strcmp(name,"Right")==0)
	{
		lua_pushboolean(L, object.getAxisByNormalId(wbx::NORM_X));
		return 1;
	}
	if (strcmp(name,"Left")==0)
	{
		lua_pushboolean(L, object.getAxisByNormalId(wbx::NORM_X_NEG));
		return 1;
	}
	...
	if(name && !isupper(name[0]))
		throw wbx::runtime_error("%s is not a valid member, did you forget to capitalize the first letter?", name);
	// Failure
	throw wbx::runtime_error("%s is not a valid member, valid members are X,Y,Z,Top,Bottom,Left,Right,Front,Back", name);
}

template<>
void Bridge<wbx::Axes>::on_newindex(wbx::Axes& object, const char* name, lua_State *L)
{
	// Failure
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
...
int Bridge<CellID>::on_index(const CellID& object, const char* name, lua_State *L)
{
	...
	// Failure
	throw wbx::runtime_error("%s is not a valid member, valid members are IsNil,Location,TerrainPart", name);
}
...
template<> const char* Bridge<wbx::NumberSequence>::className("NumberSequence");
...
int NumberSequenceBridge::newNumberSequence(lua_State* L)
{
    if (lua_isnumber(L,-1))
    {
        pushNumberSequence(L, wbx::NumberSequence(lua_tonumber(L,-1)));
        return 1;
    }
    ...
    for( int j=1; ;++j)
    {
        ...
        {
            throw wbx::runtime_error("NumberSequence ctor: expected 'NumberSequenceKeypoint' at index %d", j);
        }
...
int Bridge<NumberSequence>::on_index(const NumberSequence& object, const char* name, lua_State *L)
{
    ...
    throw wbx::runtime_error("'%s' is not a member of NumberSequence", name);
}
...
template<> const char* Bridge<wbx::ColorSequence>::className("ColorSequence");
...
int ColorSequenceBridge::newColorSequence(lua_State* L)
{
    ...
    for( int j=1; ;++j)
    {
        ...
        {
            throw wbx::runtime_error("ColorSequence ctor: expected 'ColorSequenceKeypoint' at index %d", j);
        }
...
int Bridge<ColorSequence>::on_index(const ColorSequence& object, const char* name, lua_State *L)
{
    ...
    throw wbx::runtime_error("'%s' is not a member of ColorSequence", name);
}
...
template<> const char* Bridge<wbx::NumberSequenceKeypoint>::className("NumberSequenceKeypoint");
...
int Bridge<NumberSequenceKeypoint>::on_index(const NumberSequenceKeypoint& object, const char* name, lua_State *L)
{
    ...
    throw wbx::runtime_error("'%s' is not a valid member of NumberSequenceKeypoint", name);
}
...
template<> const char* Bridge<wbx::ColorSequenceKeypoint>::className("ColorSequenceKeypoint");
...
int ColorSequenceKeypointBridge::newColorSequenceKeypoint(lua_State* L)
{
    ...
    if (!b)
        throw wbx::runtime_error("could not parse arg #2 to ColorSequenceKeypoint.new(), Color3 expected.");
...
int Bridge<ColorSequenceKeypoint>::on_index(const ColorSequenceKeypoint& object, const char* name, lua_State *L)
{
    ...
    */
    throw wbx::runtime_error("'%s' is not a valid member of ColorSequenceKeypoint", name);
}
...
template<> const char* Bridge<wbx::NumberRange>::className("NumberRange");
...
int NumberRangeBridge::newNumberRange(lua_State* L)
{
    ...
    if (b < a)
        throw wbx::runtime_error("NumberRange: invalid range");
...
int Bridge<NumberRange>::on_index(const NumberRange& object, const char* name, lua_State *L)
{
    ...
    throw wbx::runtime_error("'%s' is not a valid member of NumberRange", name);
}
...
// The default implementation for registerClass() is available in LuaBridge.cpp. This is a specialization.		
template<>
void Bridge<wbx::Vector2>::registerClass (lua_State *L)
{
	// Register the object events
	luaL_newmetatable(L, className);
	Lua::protect_metatable(L, -1);
...
// The default implementation for registerClass() is available in LuaBridge.cpp. This is a specialization.
template<>
void Bridge<wbx::Rect2D>::registerClass(lua_State *L)
{   
    // Register the object events
    luaL_newmetatable(L, className);
    Lua::protect_metatable(L, -1);
...
// The default implementation for registerClass() is available in LuaBridge.cpp. This is a specialization.		
template<>
void Bridge<wbx::UDim>::registerClass (lua_State *L)
{
	// Register the object events
	luaL_newmetatable(L, className);
	Lua::protect_metatable(L, -1);
...
// The default implementation for registerClass() is available in LuaBridge.cpp. This is a specialization.		
template<>
void Bridge<wbx::UDim2>::registerClass (lua_State *L)
{
	// Register the object events
	luaL_newmetatable(L, className);
	Lua::protect_metatable(L, -1);
