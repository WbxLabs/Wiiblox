...
template<>
const char* Bridge<Library>::className("WBXLibrary");
...
static int getApi(lua_State *L)
{
	std::list<std::string> api;
	{
		...
		WBXASSERT(!lua_isnil( L, -1 ));							// did you forget to call registerClassLibrary
		...
		WBXASSERT(!lua_isnil( L, -1 ));							// A library was pushed without being created
		WBXASSERT(lua_istable( L, -1 ));						// A library was pushed without being created
