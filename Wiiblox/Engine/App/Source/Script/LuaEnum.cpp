...
{
	const Reflection::EnumDescriptor* desc = Reflection::EnumDescriptor::lookupDescriptor(wbx::Name::lookup(name));
	if (desc)
	{
		Enum::push(L, desc);
		return 1;
...
{
	throw wbx::runtime_error("Enum cannot be modified");
}
...
{
	throw wbx::runtime_error("EnumItem cannot be modified");
}
...
bool Enums::getValue(lua_State *L, unsigned int index, wbx::Reflection::Variant& value)
{
...
namespace WBX
{
	namespace Lua
	{
		...
		{
			std::string s = wbx::format("Enum.%s.%s", object->owner.name.c_str(), object->name.c_str());
