...
template<>
const char* Bridge<EventInstance>::className("WBXScriptSignal"); 		
...
int Bridge<EventInstance>::on_index(const EventInstance& object, const char* name, lua_State *L)
{
	...
	// TODO: Remove this when we remove it from any Wiiblox scripts.
	if (strcmp(name, "connectFirst")==0)
	{
		wbx::Security::Context::current().requirePermission(wbx::Security::LocalUser, "connectFirst");
	...
	{
		wbx::Security::Context::current().requirePermission(wbx::Security::LocalUser, "connectLast");
	...
	if (strcmp(name, "disconnect")==0)
		throw wbx::runtime_error("Event:disconnect() has been deprecated. Use connection object returned by connect()");
...
public:
	...
	void doEventFire(const Reflection::EventArguments& arguments)
	{
		...
		wbx::ScopedAssign<int> assign(executionDepth, executionDepth+1);
		...
		if (ThreadRef functionThread = function.lock())
		{
			...
			{
				WBXASSERT_BALLANCED_LUA_STACK(functionThread);				//oldTop
				...
				else
				{
					...
					WBXASSERT(lua_isthread(functionThread, -1));					// slotThread
				...
				if (function.empty())
				{
					...
					WBXASSERT(cachedSlotThread.empty());
					disconnect();					
					return;
				}
...
{
	WiibloxExtraSpace* space = WiibloxExtraSpace::get(L);
	EventInstance& ei(getObject(L, 1));
	
	wbx::signals::connection connection;
	...
	{
		if (ei.descriptor->security!=wbx::Security::None)
			wbx::Security::Context::current().requirePermission(ei.descriptor->security, ei.descriptor->name.c_str());

		Reflection::Variant varResult;
		Lua::LuaArguments::get(L, 2, varResult, false);
		if (!varResult.isType<Lua::WeakFunctionRef>())
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Attempt to connect failed: Passed value is not a function");
		...
		shared_ptr< wbx::Reflection::TGenericSlotWrapper<FunctionScriptSlot> > wrapper(
			wbx::make_shared< wbx::Reflection::TGenericSlotWrapper<FunctionScriptSlot> >(FunctionScriptSlot(ei.descriptor, L, 2, useSubmitTask))
		);
		if (useSubmitTask)
		{
...
{
	{
		...
		if (source && ei.descriptor->isScriptable())
		{
			...
			shared_ptr<wbx::Reflection::TGenericSlotWrapper<WaitScriptSlot> > wrapper(
				wbx::make_shared< wbx::Reflection::TGenericSlotWrapper<WaitScriptSlot> >(WaitScriptSlot(L, useSubmitTask))
			);
			if (useSubmitTask)
			{
				wrapper->slot.setWeakSelf(wrapper);
...
template<>
const char* Bridge< wbx::signals::connection >::className("WBXScriptConnection"); 		

template<>
int Bridge< wbx::signals::connection >::on_index(const wbx::signals::connection& object, const char* name, lua_State *L)
{
...
template<>
void Bridge< wbx::signals::connection >::on_newindex(wbx::signals::connection& object, const char* name, lua_State *L)
{
	throw wbx::runtime_error("%s cannot be assigned to", name);
}
