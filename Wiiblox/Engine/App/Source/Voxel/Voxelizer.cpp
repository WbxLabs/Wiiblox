...
#if (defined(WBX_PLATFORM_IOS) && !TARGET_IPHONE_SIMULATOR) || defined(__ANDROID__)
#include <arm_neon.h>
#endif
...
#if ((defined(_WIN32) && !defined(_M_ARM) && !defined(_M_ARM64)) || (defined(__APPLE__) && !defined(WBX_PLATFORM_IOS)))
#define SIMD_SSE2
#endif

#if (defined(_WIN32) && defined(_M_ARM)) || (defined(_WIN32) && defined(_M_ARM64)) || (defined(WBX_PLATFORM_IOS) && !TARGET_IPHONE_SIMULATOR) || defined(__ANDROID__)
#define SIMD_NEON
#endif
...
namespace WBX { namespace Voxel {
    ...
    void Voxelizer::addMeshToPartCache(std::vector<DataModelPartCache>& partCache, PartInstance* part, OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency)
    {
        ...
        TriangleMesh* triangleMesh = WBX_static_cast<TriangleMesh*>(geometry);
...
//		TIMERFUNC();
        ...
        {
            WBXPROFILER_SCOPE("Voxel", "getPrimitivesOverlapping");
		...
		if (terrain)
		{
            WBXPROFILER_SCOPE("Voxel", "occupancyFillTerrain");
		...
		for (DenseHashSet<Primitive*>::const_iterator it = result.begin(); it != result.end(); ++it)
		{
			...
			if (part->getCookie() & wbx::PartCookie::IS_HUMANOID_PART)
				continue;
	...
	void Voxelizer::occupancyFillTerrainMega(OccupancyChunk& chunk, Voxel::Grid& terrain, const Vector3int32& chunkOffset, const Extents& chunkExtents)
	{
		...
		for (Grid::Region::xline_iterator it = region.xLineBegin(); it != region.xLineEnd(); ++it)
		{
			...
			WBXASSERT(it.getLineSize() == kVoxelChunkSizeXZ);
			WBXASSERT(isInsideChunkLocal(Vector3int32(0, y, z)));
	...
	void Voxelizer::occupancyFillTerrainSmooth(OccupancyChunk& chunk, Voxel2::Grid& terrain, const Extents& chunkExtents)
	{
		...
		for (int y = 0; y < kVoxelChunkSizeY; ++y)
            ...
            {
				WBXASSERT(box.getSizeX() == kVoxelChunkSizeXZ);
                WBXASSERT(isInsideChunkLocal(Vector3int32(0, y, z)));
