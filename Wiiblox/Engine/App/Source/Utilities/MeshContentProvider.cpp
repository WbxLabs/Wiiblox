...
namespace WBX {
	...
	MeshContentProvider::MeshContentProvider()
		:DescribedNonCreatable<MeshContentProvider, CacheableContentProvider, sMeshContentProvider, wbx::Reflection::ClassDescriptor::RUNTIME_LOCAL>(CACHE_ENFORCE_MEMORY_SIZE, 1024 * 1024 * 32)
	{
	...
	TaskScheduler::StepResult MeshContentProvider::ProcessTask(const std::string& id, shared_ptr<const std::string> data)
	{
		if(data)
        {
			...
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "MeshContentProvider failed to process %s because 'could not fetch'", id.c_str());
		}
