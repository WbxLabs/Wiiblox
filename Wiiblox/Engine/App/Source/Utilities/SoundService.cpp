...
using namespace WBX;
using namespace wbx::Soundscape;
...
namespace
{
void writeFMODLog(const char *error, const std::string &fmodOperation, const void *WBXFmodParent, const void *fmodObject)
{
    std::stringstream ss;
    ss << "FMOD object: " << fmodObject << ", parent: " << WBXFmodParent << ", fmodOperation: " << fmodOperation << ", error: " << error;
...
template<>
wbx::Soundscape::ListenerType& Variant::convert<wbx::Soundscape::ListenerType>(void)
{
	return genericConvert<wbx::Soundscape::ListenerType>();
}
} //namespace Reflection
template<>
bool StringConverter<wbx::Soundscape::ListenerType>::convertToValue(const std::string& text, wbx::Soundscape::ListenerType& value)
{
	return Reflection::EnumDesc<wbx::Soundscape::ListenerType>::singleton().convertToValue(text.c_str(),value);
}
} //namespace WBX
...
Reflection::BoundFuncDesc<SoundService, void(SoundType)> func_playSound(&SoundService::playSound, "PlayStockSound", "sound", Security::WiibloxScript);
Reflection::BoundFuncDesc<SoundService, void(ListenerType, shared_ptr<const wbx::Reflection::Tuple>)> func_setListener(&SoundService::setListener, "SetListener", "listenerType", "listener", Security::None);
Reflection::BoundFuncDesc<SoundService, shared_ptr<const wbx::Reflection::Tuple>()> func_getListener(&SoundService::getListener, "GetListener", Security::None);
...
void SoundService::openFmod()
{
	...
	if (!soundDisabled && wbx::GameSettings::singleton().soundEnabled)
	{
		try
        {
            ...
            if (num_drivers)
            {
                ...
                if (driver>=0)
                {
                    checkResultNoThrow(system->getDriverInfo(driver, wbx::WbxDbgInfo::s_instance.AudioDeviceName, DBG_STRING_MAX, NULL, NULL, NULL, NULL), "getDriverInfo", this, system.get());					
                }
            }
            else
            {
			...
			channelMaster->setVolume(wbx::GameBasicSettings::singleton().getMasterVolume());

			// listen for master volume changes
			gameSettingsChangedConnection = wbx::GameBasicSettings::singleton().propertyChangedSignal.connect(boost::bind(&SoundService::gameSettingsChanged,this,_1));
...
FMOD::DSP* SoundService::createDSP(FMOD_DSP_DESCRIPTION &dspdesc)
{
	...
	catch (const wbx::base_exception& ex)
	{
...
SoundService::~SoundService()
{
	WBXASSERT(stockSounds.size()==0);
	WBXASSERT(loadedSounds.size()==0);
	WBXASSERT(loaded3DSounds.size()==0);
	WBXASSERT(!system);
	WBXASSERT(!soundJob);
}
...
void SoundService::setListener(ListenerType listenerType, shared_ptr<const wbx::Reflection::Tuple> value)
{
	if (!((value && value->values.size() > 0) || listenerType == CameraListener))
	{
		throw wbx::runtime_error("SoundService:SetListener called with an incorrect argument.");
		return;
	}
	...
	if (listenerType == CFrame)
	{
		...
		{
			throw wbx::runtime_error("SoundService:SetListener value given is not a valid CFrame when given Enum.ListenerType.CFrame");
			return;
		}
	}
	else if (listenerType == ObjectPosition || listenerType == ObjectCFrame)
	{
		...
		if (tupleIter->isType<shared_ptr<Instance> >())
		{
			...
			{
				throw wbx::runtime_error("SoundService:SetListener value given does not have a location");
				return;
			}
		}
		else
		{
			throw wbx::runtime_error("SoundService:SetListener value given is not a valid instance");
			return;
		}
	}
...
shared_ptr<const wbx::Reflection::Tuple> SoundService::getListener()
{
	shared_ptr<wbx::Reflection::Tuple> result(new wbx::Reflection::Tuple(2));
				...
				if (convert(cf.translation, listener_pos))
				{
					...
					if (convert(-cf.lookVector(), listener_forward))
					{
						...
						if (convert(cf.upVector(), listener_up))
						{
							...
							wbx::PVInstance* pv = dynamic_cast<PVInstance*>(camera->getCameraSubject());
							wbx::PartInstance* part = pv ? pv->getPrimaryPart() : NULL;
							if (part)
...
void SoundService::updateMasterChannelGroup(const Time::Interval& timeSinceLastStep)
{
	...
	const float storedMasterVolume = muting ? getMasterVolume() : wbx::GameBasicSettings::singleton().getMasterVolume();
...
{
	if (propertyDescriptor == &wbx::GameBasicSettings::singleton().prop_masterVolume)
	{
		setMasterVolume(wbx::GameBasicSettings::singleton().getMasterVolume());
	}
}
...
void SoundService::checkResultNoThrow(FMOD_RESULT result, const char* fmodOperation, const void *WBXFmodParent, const void *fmodObject)
{
    if (FMOD_OK != result)
    {
        if (DebugSettings::singleton().soundWarnings || FLog::FMOD)
        {
            ...
            writeFMODLog(err, fmodOperation, WBXFmodParent, fmodObject);
            WBXASSERT(!FFlag::DebugBreakOnFMODErrors || FMOD_OK == result);
        }
    }
}

void SoundService::checkResult(FMOD_RESULT result, const char* fmodOperation, const void *WBXFmodParent, const void *fmodObject)
{
    if (FMOD_OK != result)
    {
        std::string message = wbx::format("FMOD %d: %s during %s", result, FMOD_ErrorString(result), fmodOperation);
        StandardOut::singleton()->print(MESSAGE_WARNING, message.c_str());
        writeFMODLog(FMOD_ErrorString(result), fmodOperation, WBXFmodParent, fmodObject);
        if (FMOD_OK != result)
        {
