...
namespace JNI
{
std::string fileSystemCacheDir; // Set in JNIWiibloxSettings.cpp
} // namespace JNI
} // namespace WBX
...
void logFileError(const void* caller, const char* operation, bool isError, bool doThrow = true)
{
    if (isError)
    {
        ...
        if (doThrow)
        {
            throw wbx::runtime_error("%s(%p): %s", operation, caller, ec.message().c_str());
        }
        else
        {
...
{
#ifdef __ANDROID__
    WBXASSERT(!JNI::fileSystemCacheDir.empty());
...
boost::filesystem::path FileSystem::getCacheDirectory(bool create, const char* subDirectory)
{
        ...
        if (!boost::filesystem::exists(path))
        {
            if (create)
            {
                ...
                if (!boost::filesystem::create_directories(path, ec) && ec.value())
                {
                    ...
                    throw wbx::runtime_error("FileSystem unable to create %s, %s", path.c_str(), ec.message().c_str());
                }
            }
            else
            {
    ...
    else
    {
        ...
        if (!boost::filesystem::exists(path))
        {
            throw wbx::runtime_error("Path does not exist: %s", path.c_str());
        }

        if (!boost::filesystem::is_directory(path))
        {
            throw wbx::runtime_error("Path is not a directory: %s", path.c_str());
        }
#ifndef __ANDROID__
    } // if (!DFFlag::FileSystemGetCacheDirectoryLikeAndroid)
#endif // #ifndef __ANDROID__
