...
{
	WBXASSERT(!insertEdge->inSpanningTree());
...
{
	WBXASSERT(removeEdge->inSpanningTree());
...
{
	if (!activate) {
		WBXASSERT(deactivate);
	}

	if (deactivate) {
		WBXASSERT_IF_VALIDATING(validateTree(deactivate->getChildSpanningNode()->getRoot<SpanningNode>()));
	}

	swap(deactivate, activate, newParent);

	if (activate) {
		WBXASSERT_IF_VALIDATING(validateTree(activate->getChildSpanningNode()->getRoot<SpanningNode>()));
	}
}
