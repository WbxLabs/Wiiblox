...
namespace WBX {
	...
	WBX_REGISTER_CLASS(Pages);
	WBX_REGISTER_CLASS(StandardPages);
	WBX_REGISTER_CLASS(FriendPages);
    ...
    void StandardPages::fetchNextChunk(boost::function<void ()> resumeFunction, boost::function<void (std::string)> errorFunction)
    {
		...
		if (HttpWbxApiService::isAPIHttpRequest(url))
		{
			if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(dm.get()))
			{
				apiService->getAsync(HttpWbxApiService::getApiUrlPath(url), true, wbx::PRIORITY_DEFAULT,
		...
		{
			wbx::Http(url).get(boost::bind(&StandardPages::processFetch, shared_from(this), _1, _2, resumeFunction, errorFunction));
		}
    }
	...
	{
		errorFunction( wbx::format("StandardPages: Request Failed because %s", error.c_str()) );
	}
    ...
    void FriendPages::fetchNextChunk(boost::function<void ()> resumeFunction, boost::function<void (std::string)> errorFunction)
    {
		...
		if (!firstTime) {
			...
			{
				wbx::Http(url).get(boost::bind(&FriendPages::processFetch, shared_from(this), _1, _2, resumeFunction, errorFunction));
			}
		} 
		else
		{
	...
	{
		errorFunction( wbx::format("FriendPages: Request Failed because %s", error.c_str()) );
	}
...
bool LuaWebService::isApiAccessEnabled()
{
	if (checkApiAccess)
	{
			...
			(Time::now<Time::Fast>() > timeToRecheckApiAccess))
		{
			...
			try
			{
				if (wbx::HttpWbxApiService* apiService = wbx::ServiceProvider::find<wbx::HttpWbxApiService>(dm))
				{
					apiService->get( format("universes/get-info?placeId=%d",dm->getPlaceID()), false, wbx::PRIORITY_EXTREME, response );
				}
			} 
			...
			if (!response.empty())
			{
				...
				{
					wbx::Reflection::ValueTable::const_iterator itr = v->find("StudioAccessToApisAllowed");
