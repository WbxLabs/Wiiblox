...
static wbx::mutex& moo2()
{
	static wbx::mutex mutex2;
	return mutex2;
}
static void initMoo()
{
...
void Name::setOrderIndex()
{
	...
	for (; i > 0; --i)
	{
		WBXASSERT(ordered[i-1]->str != str);
		if (str > ordered[i-1]->str)
			break;

		ordered[i-1]->sortIndex++;
		WBXASSERT(ordered[i-1]->sortIndex == i);
	}
	this->sortIndex = i;
	...
	for (size_t j = 0; j < ordered.size(); ++j)
		WBXASSERT(ordered[j]->sortIndex == j);
#endif
}
...
const Name& Name::declare(const char* const& sName) {
	...
	Name* result;
	{
		...
		wbx::mutex::scoped_lock lock(mutex());
        const Name& (*thisFunction)(const char* const &) = &wbx::Name::declare; // required due to overload.
        checkWBXCaller<kCallCheckCallersCode, callCheckSetApiFlag<kNameApiOffset> >(reinterpret_cast<void*>(thisFunction));
	...
	WBXASSERT(sName[0]=='\0' || (!result->str.empty() && lookup(sName) != Name::getNullName()) );
	return *result;
}
...
const Name& Name::lookup(const char* const& sName) {
	...
	{
		wbx::mutex::scoped_lock lock(mutex());
        const Name& (*thisFunction)(const char* const &) = &wbx::Name::lookup; // required due to overload.
        checkWBXCaller<kCallCheckCallersCode, callCheckSetApiFlag<kNameApiOffset> >(
...
std::ostream& wbx::operator<<( std::ostream& os, const Name& name )
{
