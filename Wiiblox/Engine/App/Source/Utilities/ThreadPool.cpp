...
namespace WBX
{
		...
		,kMaxScheduleSize(maxScheduleSize)
	{
		...
		for (int i=0; i<count; ++i){
			poolLocks[i].reset(new wbx::spin_mutex());
	...
	BaseThreadPool::~BaseThreadPool()
	{
		...
		case LockAndKill:
		{
			WBXASSERT(pool.size() == poolLocks.size());
			boost::posix_time::milliseconds timeout(50);
			for(unsigned i = 0; i<pool.size(); ++i){
				wbx::spin_mutex::scoped_lock lock(*poolLocks[i]);
	...
	void BaseThreadPool::loop(boost::shared_ptr<PoolData> poolData, boost::shared_ptr<wbx::spin_mutex> lock, ShutdownPolicy shutdownPolicy)
	{
		wbx::set_thread_name("WBX_BaseThreadPool");
		while (true)
		{
			{
			...
			{
				boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> task;
	...
	bool ThreadPool::schedule(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> task)
	{
	...
	bool PriorityThreadPool::schedule(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> task, float priority)
	{
		if (!shouldSchedule(poolData))
		{
			return false;
	...
	bool PriorityThreadPool::PriorityThreadPoolData::getNextTask(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)>& task)
	{
