...
namespace WBX {
	...
	TextureContentProvider::TextureContentProvider()
		:DescribedNonCreatable<TextureContentProvider, CacheableContentProvider, sTextureContentProvider, wbx::Reflection::ClassDescriptor::RUNTIME_LOCAL>(CACHE_ENFORCE_MEMORY_SIZE, 1024 * 1024 * 32)
	{
		setName(sTextureContentProvider);
	}

	void TextureContentProvider::setTextureAllocator(boost::function<wbx::Image*(std::istream&, const std::string&)> textureAllocator)
	{
	...
	TaskScheduler::StepResult TextureContentProvider::ProcessTask(const std::string& id, shared_ptr<const std::string> data)
	{
		...
		if(data){
			if(mTextureAllocator){
				...
				FASTLOG2(FLog::TextureContentProvider, "Allocated, CachedImage: %p, WBXImage: %p", img.get(), img->data.get());
			...
			if(ContentId(id).isHttp())
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "TextureContentProvider failed to process %s because 'could not fetch'", id.c_str());
		}
