...
DYNAMIC_FASTFLAGVARIABLE(WiibloxAnalyticsTrackingEnabled, false)
DYNAMIC_FASTSTRINGVARIABLE(WiibloxAnalyticsURL, "http://test.public.ecs.freblx.xyz/")
...
namespace
{
	...
	static std::string WiibloxSessionKey;
	static bool canUseGA = false, canUserWiibloxEvents = false;
	static bool initialized = false;
	static bool atteptedToUseBeforeInit = false;
  	static wbx::signal<void()> analyticTrackedSignalVar;

	static int analyticsUserID = 0, analyticsPlaceID = 0;

	static const char* WiibloxProductName = NULL;
	...
	static inline bool checkUsage()
	{
		...
		WBXASSERT(threadPool);
		WBXASSERT(googleAccountPropertyID.length() > 0);
		WBXASSERT(googleClientID.length() > 0);
		return true;
	}
...
namespace WBX
{
namespace WiibloxGoogleAnalytics
{
	bool isInitialized()
	{
		return initialized;
    ...
    void lotteryInit(const std::string &accountPropertyID, size_t maxThreadScheduleSize, int lotteryThreshold,
		...
		int WiibloxAnalyticsLottery /* = -1 */,
		const std::string &sessionKey /* = "sessionID=" */)
    {
		if (DFFlag::UseNewAnalyticsApi)
		{
			wbx::Analytics::GoogleAnalytics::lotteryInit(accountPropertyID, lotteryThreshold, productName ? productName : "", WiibloxAnalyticsLottery, sessionKey);
			return;
		}

		WiibloxSessionKey = sessionKey;

		WiibloxGoogleAnalytics::init(accountPropertyID, maxThreadScheduleSize, productName);

		if(WiibloxAnalyticsLottery == -1)
		{
			WiibloxAnalyticsLottery = lotteryThreshold;
		}
		...
		if (lottery < (std::size_t)WiibloxAnalyticsLottery || FFlag::DebugAnalyticsForceLotteryWin)
		{
			canUserWiibloxEvents = true;
		}
    }
	...
	void init(const std::string &accountPropertyID, size_t maxThreadScheduleSize, const char* productName /* = NULL */)
	{
		if (DFFlag::UseNewAnalyticsApi)
		{
			wbx::Analytics::GoogleAnalytics::init(accountPropertyID, productName ? productName : "");
			return;
		}

		if(atteptedToUseBeforeInit)
		{
			WBXASSERT(false); // Did you try to send events before analytics is initialized? Set breakpoints to trackEvent* to find out.
		}
		...
		WiibloxProductName = productName;
	...
	{
		if (DFFlag::UseNewAnalyticsApi)
			return wbx::Analytics::GoogleAnalytics::getSessionId();
	...
	{
		if (DFFlag::UseNewAnalyticsApi)
			return wbx::Analytics::GoogleAnalytics::getCanUse();
		else
			return canUseGA || canUserWiibloxEvents;
	}

	void setCanUseAnalytics()
	{
		if (DFFlag::UseNewAnalyticsApi)
			wbx::Analytics::GoogleAnalytics::setCanUse();
		else
		{
			canUseGA = true;
			canUserWiibloxEvents = true;
	...
	void sendEventWiiblox(const char* category,
		const char* action,
		const char* label,
		int value,
		bool sync)
    {
		if (DFFlag::UseNewAnalyticsApi)
		{
			wbx::Analytics::GoogleAnalytics::sendEventWiiblox(category, action, label, value, sync);
			return;
		}

		if(DFFlag::WiibloxAnalyticsTrackingEnabled && WiibloxProductName)
		{
			std::stringstream params;
			params
				<< (FFlag::SendStudioEventsWithStudioSID ? WiibloxSessionKey : "sessionID=") << googleClientID
			...
			postOrGet(wbx::format("%s%s/e.png",DFString::WiibloxAnalyticsURL.c_str(), WiibloxProductName), params, sync, true);
		}
	}
		...
		bool sync)
	{
		if (DFFlag::UseNewAnalyticsApi)
		{
			wbx::Analytics::GoogleAnalytics::trackEvent(category, action, label, value, sync);
			return;
		}
		...
		if(canUserWiibloxEvents)
		{
			sendEventWiiblox(category, action, label, value, sync);
		}
									 ...
									 bool sync)
	{
		if (DFFlag::UseNewAnalyticsApi)
		{
			wbx::Analytics::GoogleAnalytics::trackEventWithoutThrottling(category, action, label, value, sync);
			return;
		}
                         ...
                         bool sync)
	{
		if (DFFlag::UseNewAnalyticsApi)
		{
			wbx::Analytics::GoogleAnalytics::trackUserTiming(category, variable, milliseconds, label, sync);
			return;
		}
    ...
    wbx::signal<void()>& analyticTrackedSignal()
    {
