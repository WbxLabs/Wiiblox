...
static void callback(AsyncHttpQueue::CallbackWrapper f, Instance* owner, AsyncHttpQueue::RequestResult result, shared_ptr<std::string> response, shared_ptr<std::exception> exception)
{
	...
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "ContentProvider callback exception: %s", e.what());
	}
}
...
void AsyncHttpQueue::processRequests(boost::weak_ptr<AsyncHttpQueue> weakHttpQueue, RequestHandle request, boost::shared_ptr<wbx::spin_mutex> criticalSection)
{
	...
	try
	{
		try
		{
			try
			{
				...
				{
					wbx::spin_mutex::scoped_lock lock(*criticalSection);
				...
				{	
					...
					boost::shared_ptr<Http> httpRequest;
						httpRequest.reset(new wbx::Http(url));
	...
	catch (std::logic_error& e)
	{
		if(url.find("http") != std::string::npos){
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_SENSITIVE, "Content failed for %s because %s", url.c_str(), e.what());
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Content failed because %s", e.what());
		}
	...
	AsyncHttpQueue::FailedUrl::FailedUrl(const char* url)
		:url(url)
#ifdef _DEBUG
		,expiration(wbx::Time::now<Time::Fast>() + Time::Interval(10 * 60))
#else
		,expiration(wbx::Time::now<Time::Fast>() + Time::Interval(5 * 60))
#endif
	{
	}
