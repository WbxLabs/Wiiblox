...
DYNAMIC_FASTSTRING(WiibloxAnalyticsURL)
DYNAMIC_FASTFLAG(WiibloxAnalyticsTrackingEnabled)
...
namespace EphemeralCounter
{
	...
	void reportStats(const std::string& category, float value, bool blocking)
	{
		...
		std::string valueStr = wbx::format("%f", value);
        std::string url;
		if (FFlag::UseBuildGenericGameUrl)
		{
			url = BuildGenericGameUrl(baseUrl, wbx::format("game/report-stats?name=%s&value=%s", Http::urlEncode(category).c_str(), Http::urlEncode(valueStr).c_str()));
		}
		else
		{
			url = wbx::format("%sgame/report-stats?name=%s&value=%s", baseUrl.c_str(), Http::urlEncode(category).c_str(), Http::urlEncode(valueStr).c_str());
		}
	...
	void reportCounter(const std::string& counterName, int amount, bool blocking)
	{
		...
		std::string url = wbx::format("%s&counterName=%s&amount=%d", counterUrl.c_str(), counterName.c_str(), amount);
...
namespace GoogleAnalytics {
	...
	bool canUseGA = false;
	bool canUserWiibloxEvents = false;
	std::string googleClientID;
	std::string googleAccountPropertyID;
	std::string WiibloxProductName;
	std::string experimentString;
	std::string WiibloxSessionKey;
	...
	void lotteryInit(const std::string &accountPropertyID, int lotteryThreshold, const std::string& productName /*= ""*/, int WiibloxAnalyticsLottery /* = -1 */, const std::string &sessionKey /* = "sessionID=" */)
	{
		...
		bool sync)
	{
		...
		postOrGet(defaultThreadPool, WiibloxGoogleAnalytics::kGoogleAnalyticsBaseURL, params.str(), Http::kContentTypeDefaultUnspecified, sync, false, true);
	}
		...
		bool sync)
	{
		...
		if(DFFlag::WiibloxAnalyticsTrackingEnabled && !WiibloxProductName.empty())
		{
			std::stringstream params;
			params
			...
			postOrGet(defaultThreadPool, wbx::format("%s%s/e.png",DFString::WiibloxAnalyticsURL.c_str(), WiibloxProductName.c_str()), params.str(), Http::kContentTypeDefaultUnspecified, sync, true, true);
		}
	}
...
namespace InfluxDb {
	...
	using namespace wbx::Analytics::InfluxDb;
	...
	void reportPointsV2(const std::string& resource, const boost::unordered_set<Point>& points, int throttleHundredthsPercentage, bool blocking, const std::string& userIdOverride)
	{
        ...
        // tags
        std::stringstream ss;
		ss << wbx::format("%s,placeid=%s,userid=%s,reporter=%s,platform=%s,device=%s,appversion=%s,location=%s,osversion=%s ",
			resource.c_str(), 
