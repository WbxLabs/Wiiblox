...
DYNAMIC_FASTINTVARIABLE(NodeIntervalCapMS, 100)
    ...
    {
        WBXASSERT(startIndex < MH_NUM_MAX_NODES);
        WBXASSERT(size <= MH_NUM_MAX_NODES);
        WBXASSERT(result.size() == 0);
        int numNodeToSkip = 0;
        ...
        if (size > 0)
        {
            ...
            if (estimatedNodeInterval > 0.f)
            {
                ...
                WBXASSERT(numNodeToSkip * estimatedNodeInterval <= nodeIntervalCap);
            }
        }
        ...
        if (crossPacketCompression)
        {
            ...
            WBXASSERT(baselineCFrameDeltaNode.translation.precisionLevel == 255 || (outCalculatedBaselineCFrame.translation-baselineCFrame.translation).magnitude() < (baselineCFrameDeltaNode.translation.precisionLevel+1)*MH_MIN_PRECISION*2);
        }
        ...
        if (size > 0)
        {
            ...
            WBXASSERT(!Math::isNanInfVector3(outCalculatedLinearVelocity));
    ...
    void MovementHistory::pushBack(MovementNode node)
    {
        ...
        if (size == MH_NUM_MAX_NODES)
        {
            ...
            popFront();
            WBXASSERT(size == MH_NUM_MAX_NODES-1);
        }
    ...
    {
        WBXASSERT(numNodesToConcat <= size);
        WBXASSERT(lastIndex < MH_NUM_MAX_NODES);
