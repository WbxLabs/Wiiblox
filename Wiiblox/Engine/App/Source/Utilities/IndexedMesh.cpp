...
IndexedMesh::~IndexedMesh() 
{
	WBXASSERT(!lower);
	WBXASSERT(!upper);
	WBXASSERT(!computedUpper);
	WBXASSERT(!getTypedParent<IndexedMesh>());
	...
	if (newLower != lower)
	{
		if (lower)			// pull out of the chain - I have no lower, so am out of commission!
		{
			...
			while (numChildren() > 0) {
				...
				WBXASSERT(backChild->getTypedParent<IndexedMesh>() == this);
...
{
	if (oldParent) {
		IndexedMesh* oldMeshParent = WBX_static_cast<IndexedMesh*>(oldParent);
...
const IndexedMesh* IndexedMesh::getConstIndexedMeshParent() const
{
	...
	WBXASSERT(answer == computeParentFromLower());
	return answer;
}
...
{
	WBXASSERT_SLOW(computedUpper == IndexedMesh::computeConstUpper(this));
...
bool IndexedMesh::isUpperRoot(const IndexedMesh* lower) 
{
	...
	WBXASSERT(answer == (lower && (computeConstUpper(lower)->lower == lower)));
