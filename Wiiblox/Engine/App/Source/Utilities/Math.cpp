...
{
	WBXASSERT((angle >= -Math::pi()) && (angle <= Math::pi()));
...
{
	WBXASSERT(G3D::fuzzyGe(limitLo, -Math::pif()) && G3D::fuzzyLe(limitLo, Math::pif()));
	WBXASSERT(G3D::fuzzyGe(limitHi, -Math::pif()) && G3D::fuzzyLe(limitHi, Math::pif()));
	rot = radWrap(rot);
	if ( limitLo <= limitHi )
	{
		if ( rot < limitLo )
...
{
	WBXASSERT((newRotationNotWound >= -pi()) && (newRotationNotWound <= pi()));
	...
	WBXASSERT(fabs(answer - currentRotationWound) < 0.5);
...
void Math::lerpArray(const G3D::Array<float>& before,
					...
					float alpha)
{
	WBXASSERT(before.size() == after.size());
	WBXASSERT(before.size() == answer.size());
...
{
	WBXASSERT(angle <= (Math::pif()+0.0001f));
	WBXASSERT(angle >= -(Math::pif()+0.0001f));
...
unsigned char Math::rotationToByte(float angle)
{
	...
	WBXASSERT(delta <= segSizeRadians());
#endif
...
#ifdef _DEBUG
	WBXASSERT(isOrthonormal(answer));
...
Vector3 Math::safeDirection(const Vector3& v)
{
    ...
    if (fMagnitude > 1e-12f) {
		...
		WBXASSERT(answer.magnitude() < 1.01f);
		return answer;
    } else {
		WBXASSERT(0);
...
{
	WBXASSERT_FISHING(v0.isUnit());
	WBXASSERT_FISHING(v1.isUnit());
...
{
	WBXASSERT_VERY_FAST(look.y >= -1.0f);
	WBXASSERT_VERY_FAST(look.y <= 1.0f);
...
{
	WBXASSERT_VERY_FAST(angle(v0, v1) < (Math::pif() * 0.10));
...
{
	WBXASSERT_VERY_FAST(Math::fuzzyEq(v0.magnitude(), 1.0f, 1e-4f));		// lower tolerance
	WBXASSERT_VERY_FAST(Math::fuzzyEq(v1.magnitude(), 1.0f, 1e-4f));		// lower tolerance
...
bool Math::orthonormalizeIfNecessary(Matrix3& m)
{
	if (!isOrthonormal(m)) {
		m.orthonormalize();
		WBXASSERT_IF_VALIDATING(isOrthonormal(m));
		return true;
	}
	else {
		return false;
...
Matrix3 Math::alignAxesClosest(const Matrix3& align, const Matrix3& target)
{
	...
	WBXASSERT_VERY_FAST(isOrthonormal(answer));
...
Vector3 Math::sortVector3(const Vector3& v)
{
	...
	WBXASSERT_VERY_FAST(ans.z >= ans.y);
	WBXASSERT_VERY_FAST(ans.y >= ans.x);
	return ans;
}
...
Vector3 Math::closestPointOnRay(const wbx::WBXRay& pointOnRay, 
								const wbx::WBXRay& otherRay)
{
...
bool Math::intersectRayConvexPolygon(const wbx::WBXRay& ray,  const std::vector<Vector3>& poly, Vector3& hit, bool oneSided)
{
    if( poly.size() < 3 )
    {
        WBXASSERT(0);
...
bool Math::intersectRayPlane(const wbx::WBXRay& ray, const G3D::Plane& plane, G3D::Vector3& hit)
{
...
bool intersectLinePlaneOld(const wbx::WBXRay& ray, const G3D::Plane& plane, G3D::Vector3& hit)
{
	if (Math::intersectRayPlane(ray, plane, hit)) {
		return true;
	}

	wbx::WBXRay oppositeRay = wbx::WBXRay::fromOriginAndDirection(ray.origin(), -ray.direction());
...
#ifdef _DEBUG
	WBXRay tempRay = wbx::WBXRay::fromOriginAndDirection(line.point(), line.direction());
	Vector3 tempHit;
	intersectLinePlaneOld(tempRay, plane, tempHit);
	WBXASSERT(tempHit.fuzzyEq(hit));
#endif
...
const Matrix3& Math::getAxisRotationMatrix(int face)
{
	...
	switch (face) {
		...
		case (4):	return y_neg;
		case (5):	return z_neg;
		default:	WBXASSERT(0); return Matrix3::identity();
	}
}
