...
namespace
{			
	const char* kNamedUniverseAssetBase = "WBXgameasset://";
		...
		else if (isNamedAsset())
		{
			...
			if (!baseUrl.empty())
			{
				...
				ss << "asset/"
					...
					<< "&" << kNamedUniverseAssetAssetNameParam << wbx::Http::urlEncode(id.substr(strlen(kNamedUniverseAssetBase)))
	...
	bool ContentId::reconstructUrl(const std::string& baseUrl, const char* const paths[], const int pathCount)
	{
		...
		*urlIter = '\0';
        ...
        if (DFFlag::UseNewUrlClass)
        {
            ...
            static const std::string testsite_domain = "Wiibloxlabs.com";

            const wbx::Url baseUrlParsed = wbx::Url::fromString(baseUrl);
            ...
            for (int i = 0; i < pathCount; i++)
            {
                if (parsed.pathEqualsCaseInsensitive(paths[i]))
                {
                    ...
                    id = wbx::Url::fromComponents(scheme, host, path, parsed.query(), parsed.fragment()).asString();
        ...
        for (int i = 0; i < pathCount; i++)
        {
            ...
            {
                static const char* domain = ".Wiibloxlabs.com";
                ...
                {
                    size_t WiibloxLabsPos = host.find(domain);
                    if (WiibloxLabsPos != std::string::npos)
                        id = baseUrl + "/" + path;
                    else
                        id = "http://www.freblx.xyz/" + path;
                }
