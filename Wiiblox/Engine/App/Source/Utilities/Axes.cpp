...
std::string StringConverter<Axes>::convertToString(const Axes& value)
{
	...
	for(int axis = 0; axis < 3; axis++){
		if(value.getAxis((wbx::Vector3::Axis)axis)){
			result += seperator;
			switch((wbx::Vector3::Axis)axis){
				case wbx::Vector3::X_AXIS:		result += "X";	break;
				case wbx::Vector3::Y_AXIS:		result += "Y";	break;
				case wbx::Vector3::Z_AXIS:		result += "Z";	break;
                default: break;
			}
			seperator = ", ";
		}
...
{
	template<>
	EnumDesc<wbx::Vector3::Axis>::EnumDesc()
	:EnumDescriptor("Axis")
	{
		addPair(wbx::Vector3::X_AXIS, "X");
		addPair(wbx::Vector3::Y_AXIS, "Y");
		addPair(wbx::Vector3::Z_AXIS, "Z");
	}

	template<>
	wbx::Vector3::Axis& Variant::convert<wbx::Vector3::Axis>(void)
	{
		return genericConvert<wbx::Vector3::Axis>();
	}
}

template<>
bool wbx::StringConverter<G3D::Vector3::Axis>::convertToValue(const std::string& text, wbx::Vector3::Axis& value)
{
	if(text.find("Y") != std::string::npos || text.find("Top") != std::string::npos || text.find("Bottom") != std::string::npos){
		value = wbx::Vector3::Y_AXIS;
		return true;
	}
	if(text.find("Z") != std::string::npos || text.find("Back") != std::string::npos || text.find("Front") != std::string::npos ){
		value = wbx::Vector3::Z_AXIS;
		return true;
	}
	if(text.find("X") != std::string::npos || text.find("Right") != std::string::npos || text.find("Left") != std::string::npos ){
		value = wbx::Vector3::X_AXIS;
		return true;
	}
	return false;
}
template<>
bool StringConverter<wbx::Axes>::convertToValue(const std::string& text, Axes& value)
{
	value.clear();
