...
namespace Reflection {
    ...
    EnumDesc<KeyCode>::EnumDesc() : EnumDescriptor("KeyCode")
    {
		...
		for (int i = SDLK_WORLD_0; i <= SDLK_WORLD_95; ++i)
		{
			...
			addPair((wbx::KeyCode)i, ss.str().c_str());
		}
    ...
    template<>
    KeyCode& Variant::convert<wbx::KeyCode>(void)
    {
        return genericConvert<wbx::KeyCode>();
    }
    
} // namespace Reflection

template<>
bool StringConverter<wbx::KeyCode>::convertToValue(const std::string& text, wbx::KeyCode& value)
{
    return Reflection::EnumDesc<wbx::KeyCode>::singleton().convertToValue(text.c_str(),value);
}
