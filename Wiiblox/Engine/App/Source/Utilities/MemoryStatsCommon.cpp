...
#ifdef WBX_POOL_ALLOCATION_STATS
...
{
    WBXASSERT(MemoryStats::totalMemoryBytes() > 0);
...
size_t slowGetMemoryPoolAllocation()
{
    ...
    for (iter = wbx::poolAllocationList.begin(); iter != wbx::poolAllocationList.end(); iter++)
    {
...
size_t slowGetMemoryPoolAvailability()
{
    ...
    std::vector<size_t*>::iterator iter;
    for (iter = wbx::poolAvailabilityList.begin(); iter != wbx::poolAvailabilityList.end(); iter++)
    {
...
void releaseAllPoolMemory()
{
    ...
    for (iter = wbx::poolReleaseMemoryFuncList.begin(); iter != wbx::poolReleaseMemoryFuncList.end(); iter++)
    {
        (*iter)();
    }
}
