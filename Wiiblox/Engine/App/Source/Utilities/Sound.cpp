...
namespace WBX 
{
	...
	namespace Reflection {
		template<>
		const Type& Type::getSingleton<wbx::Soundscape::SoundId>()
		{
			return Type::singleton<wbx::ContentId>();
		}

		template<>
		wbx::Soundscape::SoundId& Variant::convert<wbx::Soundscape::SoundId>(void)
		{
			if (_type->isType<std::string>())
			{
				value = wbx::Soundscape::SoundId(cast<std::string>());
				_type = &Type::singleton<wbx::Soundscape::SoundId>();
			}
			return genericConvert<wbx::Soundscape::SoundId>();
		}


		template<>
		void TypedPropertyDescriptor<wbx::Soundscape::SoundId>::readValue(DescribedBase* instance, const XmlElement* element, IReferenceBinder& binder) const
		{
			if (!element->isXsiNil())
			{
				ContentId value;
		...
		template<>
		void TypedPropertyDescriptor<wbx::Soundscape::SoundId>::writeValue(const DescribedBase* instance, XmlElement* element) const
		{
		...
		template<>
		int TypedPropertyDescriptor<wbx::Soundscape::SoundId>::getDataSize(const DescribedBase* instance) const 
		{
			return sizeof(wbx::Soundscape::SoundId) + getValue(instance).toString().size();
		}

		template<>
		bool TypedPropertyDescriptor<wbx::Soundscape::SoundId>::hasStringValue() const {
			return true;
		}

		template<>
		std::string TypedPropertyDescriptor<wbx::Soundscape::SoundId>::getStringValue(const DescribedBase* instance) const{
			return StringConverter<wbx::Soundscape::SoundId>::convertToString(getValue(instance));
		}

		template<>
		bool TypedPropertyDescriptor<wbx::Soundscape::SoundId>::setStringValue(DescribedBase* instance, const std::string& text) const {
			wbx::Soundscape::SoundId value;
			if (StringConverter<wbx::Soundscape::SoundId>::convertToValue(text, value))
			{
				setValue(instance, value);
				return true;
			}
	...
	namespace Soundscape
    {
		...
		// People were uploading audio as a model/plugin.  This requires "<Wiiblox" 
		static bool isFileModel(const char* filename)
		{
			std::ifstream is(utf8_decode(filename).c_str(), std::ios_base::in | std::ios_base::binary);
			char buf[7];
			is.read(buf,7);
			return (0 == strncmp(buf, "<Wiiblox", 7));
		}
		...
		static bool isMemModel(const std::string* data)
		{
			if (data->size() > 7)
			{
				return (0 == strncmp(data->c_str(), "<Wiiblox", 7));
			}
			return false;
		}

		FMOD::Sound* Sound::tryLoad(const wbx::Instance* context) // throw on failure, returns null if not available yet
		{
            ...
            try
			{
                WBXPROFILER_SCOPE("Sound", "Sound::get");
                ...
                {
                    WBXPROFILER_SCOPE("Sound", "getFile");
                ...
                WBXPROFILER_LABELF("Sound", "%s (%d bytes)", id.c_str(), fileSize);
				...
				if ( !fileName.empty() && (fileSize >= DFInt::MinSoundStreamSizeBytes) )
				{
                    ...
                    WBXPROFILER_SCOPE("Sound", "createStream");
				...
				else
				{
                    ...
                    WBXPROFILER_SCOPE("Sound", "createSound");
