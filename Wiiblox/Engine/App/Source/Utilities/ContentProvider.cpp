...
#ifdef _WIN32
#define APPLOG(message) do { if (appLog != NULL) {wbx::mutex::scoped_lock lock(*appLogLock); appLog->writeEntry(wbx::Log::Information, message); } } while(0)
#else
...
namespace {
    ...
    {
#ifdef _WIN32
		return wbx::utf8_encode(path.wstring());
#else
        return path.string();
#endif
    }
	...
	{
#ifdef _WIN32
		return wbx::utf8_decode(str.c_str());
#else
        return fs::path(str);
#endif
	}
...
namespace WBX {
	...
	wbx::mutex *ContentProvider::appLogLock = NULL;
	...
	ContentProvider::~ContentProvider()
	{
			wbx::FileSystem::clearCacheDirectory("ContentProvider");
		}
	...
	void ContentProvider::verifyRequestedScriptSignature(const ProtectedString& source, const std::string& assetId, bool required)
	{
		...
		try
		{
            ...
            // looks like "--WBXassetid%1818%"
            const char* assetHeader = "--WBXassetid%";
	...
	void ContentProvider::verifyScriptSignature(const ProtectedString& source, bool required)
	{
		...
		try
		{
            ...
            // looks like "--WBXsig%MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCtfLLFT36v5r9bNP7STBteDU5a%"
            const char* sigHeader = "--WBXsig%";
	...
	bool ContentProvider::hasContent(const wbx::ContentId& id) {
		return isContentLoaded(id);
	}

	bool ContentProvider::isUrlBad(wbx::ContentId id) {
		...
		shared_ptr<std::exception> requestError)
	{
		...
		if (result==AsyncHttpQueue::Succeeded)
		{
			...
			{
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Content failed to load for %s because %s", id.c_str(), e.what());
		...
		shared_ptr<std::exception> requestError)
	{
		...
		if (result==AsyncHttpQueue::Succeeded)
		{
			...
			{
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Content failed to parse for %s because %s", id.c_str(), e.what());
				result = AsyncHttpQueue::Failed;
			}
		}
	...
	void ContentProvider::preloadContentWithCallback(wbx::ContentId id, float priority, boost::function<void (AsyncHttpQueue::RequestResult)> callback, AsyncHttpQueue::ResultJob jobType, const std::string& expectedType)
	{
	...
	void ContentProvider::preloadContentResultCallback(AsyncHttpQueue::RequestResult results, ContentId id)
	{
		...
		{
			wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "ContentProvider:Preload() failed for %s", id.c_str());
		}
	}

	void ContentProvider::preloadContent(wbx::ContentId id)
	{
	...
	void ContentProvider::preloadContentBlockingListHelper(AsyncHttpQueue::RequestResult results, boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction, PreloadAsyncRequest *pRequest, ContentId id)
	{
		...
		if (results != AsyncHttpQueue::Succeeded)
		{
			if (DFFlag::ImageFailedToLoadContext) 
			{
				wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "ContentProvider:PreloadAsync() failed for %s", id.c_str());
			}
			pRequest->failed++;
		}
	...
	void ContentProvider::preloadContentBlockingList(shared_ptr<const Reflection::ValueArray> idList, boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
	{
		...
		if (pList && pList->size() > 0) 
		{
			for (size_t i = 0; i < pList->size(); i++)
			{
				...
				ContentId id = v.get<wbx::ContentId>();
	...
	shared_ptr<const std::string> ContentProvider::getContentString(wbx::ContentId id)
	{
		...
		if (!result)
		{
			...
			if (!blockingLoadContent(id, &item))
				throw wbx::runtime_error("Unable to load %s", id.c_str());

			result = requestContentString(id, ContentProvider::PRIORITY_SCRIPT);
			if (!result)
				throw wbx::runtime_error("Unable to retrieve cache of %s", id.c_str());
		}
		WBXASSERT(result);
		return result;
	}

	shared_ptr<const std::string> ContentProvider::requestContentString(const wbx::ContentId& id, float priority)
	{
		WBXASSERT(this!=NULL);
		APPLOG("ContentProvider::requestContentString - enter");
		wbx::ContentId activeId = id;
	...
	{
		APPLOG(wbx::format("ContentProvider::requestContentString %s", id.c_str()).c_str());
	...
	AsyncHttpQueue::RequestResult ContentProvider::privateLoadContent(wbx::ContentId& id, RequestType httpRequestType, float priority, CachedContent* result, AsyncHttpQueue::RequestCallback* callback, AsyncHttpQueue::ResultJob jobType, const std::string& expectedType)
	{
		...
		if( httpRequestType != FullAsyncRequest )
		{
			...
			if (id.isFile())
			{
				...
				else
				{
					...
					// parent directory.  The logic for this is that WiibloxStudio
					// will look for scripts relative to the WBXL, but WiibloxTest
					// will look for scripts relative to the ProjectDir for WiibloxTest,
					// which is one higher than the WBXL.  So, in the case of
					// WiibloxStudio, we forcefully push one directory higher if the file
					// wasn't found next to the WBXL.
	...
	bool ContentProvider::isUrl(const std::string& s)
	{
		if (isHttpUrl(s))
			return true;
		if (s.find("WBXasset://")==0 && s.length() > std::string("WBXasset://").length())
			return true;
        if (s.find("wbxassetid://")==0 && s.length() > std::string("wbxassetid://").length())
            return true;
	...
	std::string ContentProvider::getFile(wbx::ContentId ticket)
	{
		CachedContent item;
		if (!blockingLoadContent(ticket, &item) || !registerFile(ticket, &item))
			throw wbx::runtime_error("Unable to load %s", ticket.c_str());
...
namespace WBX
{
	...
	{
		// Returns something like "C:\Documents and Settings\All Users\Application Data\Wiiblox\Cache"
        if (createPath)
        {
            static const fs::path path = wbx::FileSystem::getCacheDirectory(true, subFolder);
            return path;
        }
        else
        {
            static const fs::path path = wbx::FileSystem::getCacheDirectory(false, subFolder);
            return path;
        }	
	}
	...
	wbx::ContentId ContentProvider::registerContent(std::istream& content)
	{
		wbx::ContentId contentId;
		{
			// Get the hash
			boost::scoped_ptr<MD5Hasher> hasher(MD5Hasher::create());
			hasher->addData(content);
			contentId = wbx::ContentId(hasher->toString());
		}
	...
	std::string ContentProvider::findAsset(wbx::ContentId contentId)
	{
		WBXASSERT(contentId.isAsset() || contentId.isAppContent());
		...
		else if (contentId.isAppContent())
		{
			...
			{
				filePath = wbx::FileSystem::getUserDirectory(true, DirAppData, "");
	...
	std::string ContentProvider::findHashFile(wbx::ContentId contentId)
	{
	...
	void ContentProvider::setAssetFolder(const char* sPath)
	{
		...
		{
            if (wbx::Log::current())
                wbx::Log::current()->writeEntry(Log::Information, wbx::format("setAssetFolder %s", sPath).c_str());
            ...
            if (!fs::exists(path))
                throw wbx::runtime_error("The path '%s' does not exist", path.string().c_str());
            if (!is_directory(path))
                throw wbx::runtime_error("'%s' is not a directory", path.string().c_str());
...
#error Unsupported Platform
	...
	std::auto_ptr<std::istream> ContentProvider::getContent(const wbx::ContentId& ticket, const std::string& expectedType)
	{
