...
{
template<>
EnumDesc<wbx::Soundscape::RollOffMode>::EnumDesc()
	:EnumDescriptor("RollOffMode")
{
	addPair(wbx::Soundscape::Inverse, "Inverse");
	addPair(wbx::Soundscape::Linear, "Linear");
}
template<>
wbx::Soundscape::RollOffMode& Variant::convert<wbx::Soundscape::RollOffMode>(void)
{
	return genericConvert<wbx::Soundscape::RollOffMode>();
}
} //namespace Reflection
template<>
bool StringConverter<wbx::Soundscape::RollOffMode>::convertToValue(const std::string& text, wbx::Soundscape::RollOffMode& value)
{
	return Reflection::EnumDesc<wbx::Soundscape::RollOffMode>::singleton().convertToValue(text.c_str(),value);
}
}
...
static Reflection::RemoteEventDesc<SoundChannel, void(int)> event_timePositionUpdatedFromServer(&SoundChannel::timePositionUpdatedFromServerSignal, "TimePositionUpdated", "newPositionSeconds", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);
static Reflection::RemoteEventDesc<SoundChannel, void(int)> event_timePositionUpdatedFromServerScript(&SoundChannel::timePositionUpdatedFromServerScriptSignal, "TimePositionUpdatedFromScript", "newPositionSeconds", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);
static Reflection::RemoteEventDesc<SoundChannel, void(int)> event_soundResumedFromServer(&SoundChannel::soundResumedFromServerSignal, "SoundResumedFromServer", "currentTimePosition", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);
...
void SoundChannel::releaseChannel()
{
	...
	if (fmod_channel)
	{
		...
		WBXASSERT(oldSound==0 || oldSound==this);
#endif
...
void SoundChannel::updateListenState(const Time::Interval& timeSinceLastStep)
{
	...
	if (controlledByAndIsServer() && (timeSinceLastStep.msec() > 0))
	{
		...
		if (isPlaying() && !isPaused())
		{
			...
			{
				position = 0;
				DataModel::scoped_write_request request(wbx::DataModel::get(this));
				stop();
			}
			else
			{
...
void SoundChannel::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
	...
	if (newProvider)
	{
		...
		if (wbx::Network::Players::clientIsPresent(newProvider))
		{
...
namespace {
	void onSoundLoadedForSoundChannel(shared_ptr<SoundChannel> soundChannel, const SoundId &soundId, const Instance *context, bool shouldPlayOnLoad, AsyncHttpQueue::RequestResult result, shared_ptr<const std::string> item)
	{
		...
		case AsyncHttpQueue::Failed:
			...
			{
				wbx::StandardOut::singleton()->printf(MESSAGE_ERROR, "Sound failed to load %s : %s", context->getFullName().c_str(), soundId.c_str());
			}
			break;
...
bool SoundChannel::isPlaying() const
{
	...
	if (!fmod_channel)
	{
		if (wbx::Network::Players::clientIsPresent(this, false))
		{
			return false;
		}
...
void SoundChannel::loadSound(const Instance *context, bool shouldPlayOnLoad)
{
	...
	if (!SoundService::soundDisabled && !soundDisabled && wbx::GameSettings::singleton().soundEnabled)
	{
...
{
	if (isHeardGlobally() && !wbx::Network::Players::serverIsPresent(this))
	{
...
void SoundChannel::setPlayCount(int value)
{
	...
	{
	case -1:
		if (isHeardGlobally() && wbx::Network::Players::clientIsPresent(this))
		{
			soundPositionSeconds = 0;
		}
		stop();
...
void SoundChannel::onChannelEnd(const FMOD_CHANNEL *channel) 
{
	...
	if (fmod_channel && reinterpret_cast<const void*>(channel) == reinterpret_cast<const void*>(fmod_channel))
	{
		...
		{
			sound->unacquire();
				if (DataModel* dm = wbx::DataModel::get(this)) 
				{
					dm->submitTask(boost::bind(&SoundChannel::soundEnded, shared_from(this), getSoundId().toString()), wbx::DataModelJob::Write);
				}
			}
		}
	}
...
FMOD_RESULT F_CALLBACK callbackChannelEnd(FMOD_CHANNELCONTROL *channelControl, FMOD_CHANNELCONTROL_TYPE type, FMOD_CHANNELCONTROL_CALLBACK_TYPE callbacktype, void *commanddata1, void *commanddata2)
{
		...
		if (callbacktype==FMOD_CHANNELCONTROL_CALLBACK_END)
	{
		...
		wbx::Soundscape::SoundChannel* sound;
...
{
	return SoundService::soundDisabled && wbx::Network::Players::serverIsPresent(this) && isHeardGlobally();
}
