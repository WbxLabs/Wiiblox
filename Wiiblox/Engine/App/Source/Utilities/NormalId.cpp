...
{
	WBXASSERT(validNormalId(normalId));
	Vector3 answer;
	int xyz = normalId % 3;
...
Matrix3 normalIdToMatrix3Internal(NormalId normalId)
{
	...
	WBXASSERT(uInObject == uvwToObject(objectToUvw(uInObject, normalId), normalId));
	WBXASSERT(vInObject == uvwToObject(objectToUvw(vInObject, normalId), normalId));
	WBXASSERT(wInObject == uvwToObject(objectToUvw(wInObject, normalId), normalId));
...
NormalId Vector3ToNormalId(const Vector3& v)
{
	...
	if (v.x == 1.0f) {
		...
		WBXASSERT(0);		// This function assumes a unit, orthogally aligned Vector3
