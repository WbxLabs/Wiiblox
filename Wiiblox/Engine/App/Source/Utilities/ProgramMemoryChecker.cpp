...
__forceinline bool getSectionInfo(const HMODULE& module, const char* name, uintptr_t& baseAddr, size_t& size)
{
    ...
    unsigned int dest = ((unsigned int)(name) + WBX_BUILDSEED);
    volatile unsigned int tmp = dest;
    const char* const sectionName = (const char* const)(tmp - WBX_BUILDSEED);
    baseAddr = 0;
    size = 0;
...
namespace WBX
{
    namespace Security{
        ...
        volatile const size_t WBXGoldHash = 0;

        // The lower part of .text
        volatile const uintptr_t WBXLowerBase = 0x00400000;
        volatile const size_t WBXLowerSize = 1024;

        // The upper part of .text
        volatile const uintptr_t WBXUpperBase = 0x00400000;
        volatile const size_t WBXUpperSize = 1024;

        // the .rdata section
        volatile const uintptr_t WBXRdataBase = 0x00400000;
        volatile const size_t WBXRdataSize = 1024;

        // the vmp sections
        volatile const uintptr_t WBXVmpBase = 0x00400000;
        volatile const size_t WBXVmpSize = 1024;

        // the Import Address (thunk) Table
        volatile const uintptr_t WBXIatBase = 0x00400000;
        volatile const size_t WBXIatSize = 1024;

        // the vmp sections (plain .text section)
        volatile const uintptr_t WBXVmpPlainBase = 0x00400000;;
        volatile const size_t WBXVmpPlainSize = 1024;;

        // the vmp sections (mutation .text section)
        volatile const uintptr_t WBXVmpMutantBase = 0x00400000;;
        volatile const size_t WBXVmpMutantSize = 1024;;

        // the vmp sections (don't know)
        volatile const uintptr_t WBXVmp0MiscBase = 0x00400000;;
        volatile const size_t WBXVmp0MiscSize = 1024;;

        // the vmp sections (don't know)
        volatile const uintptr_t WBXVmp1MiscBase = 0x00400000;;
        volatile const size_t WBXVmp1MiscSize = 1024;;

        // the .rdata section without IAT
        volatile const uintptr_t WBXRdataNoIatBase = 0x00400000;
        volatile const size_t WBXRdataNoIatSize = 1024;
    }
        ...
        , ntdllTextBase(0)
    {
        ...
        uintptr_t pfn = reinterpret_cast<uintptr_t>(WBXNtdllProcAddress(ntdll, CryptStrings::cmpNtQueryVirtualMemory));
        uintptr_t callTemplate = reinterpret_cast<uintptr_t>(WBXNtdllProcAddress(ntdll, CryptStrings::cmpZwFilterToken));
        initApiFunction(pfn, ntQvmAsUint, callTemplate, ntQvmCallHash, hashEndSize, kNtQvmEndToken);

        pfn = reinterpret_cast<uintptr_t>(WBXNtdllProcAddress(ntdll, CryptStrings::cmpNtGetContextThread));
        callTemplate = reinterpret_cast<uintptr_t>(WBXNtdllProcAddress(ntdll, CryptStrings::cmpZwLoadKey));
    ...
    , lastCompletedTime(Time::nowFast())
{
    ...
    scanningRegions[kGoldHashStart] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXLowerBase), Security::WBXLowerSize));
    scanningRegions[kGoldHashEnd] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXUpperBase), Security::WBXUpperSize));
    scanningRegions[kGoldHashRot] = nonceLocation;

    // Set up the diff-only hashes
    scanningRegions[kRdataHash] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXRdataNoIatBase), Security::WBXRdataNoIatSize));
    scanningRegions[kVmpPlainHash] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXVmpPlainBase), Security::WBXVmpPlainSize));
    scanningRegions[kVmpMutantHash] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXVmpMutantBase), Security::WBXVmpMutantSize));
    scanningRegions[kVmp0MiscHash] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXVmp0MiscBase), Security::WBXVmp0MiscSize));
    scanningRegions[kVmp1MiscHash] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXVmp1MiscBase), Security::WBXVmp1MiscSize));
    scanningRegions[kIatHash] = ScanRegionTest(ScanRegion(reinterpret_cast<char*>(Security::WBXIatBase), Security::WBXIatSize));
...
bool ProgramMemoryChecker::areMemoryPagePermissionsSetupForHacking() 
{
    ...
    if (::checkG3dConsts())
    {
        wbx::Security::setHackFlagVmp<LINE_RAND4>(wbx::Security::hackFlag8, HATE_CONST_CHANGED);
    }
    #if defined(I_AM_GOY_THAT_LOVES_VMPROTECT)
    VMProtectEnd();
    #endif
...
#define PMC_ROT 3
        ...
        while (p <= limit)
        {
            v1 += modifyArg<(WBX_BUILDSEED>>0)%4>(p) * kPrime32_2; v1 = _rotl(v1, 13); v1 *= kPrime32_1; p+=4;
            v2 += modifyArg<(WBX_BUILDSEED>>2)%4>(p) * kPrime32_2; v2 = _rotl(v2, 13); v2 *= kPrime32_1; p+=4;
            v3 += modifyArg<(WBX_BUILDSEED>>4)%4>(p) * kPrime32_2; v3 = _rotl(v3, 13); v3 *= kPrime32_1; p+=4;
            v4 += modifyArg<(WBX_BUILDSEED>>6)%4>(p) * kPrime32_2; v4 = _rotl(v4, 13); v4 *= kPrime32_1; p+=4;
        }  
...
{
#if !defined(WBX_STUDIO_BUILD) && defined(_WIN32)
#if defined(I_AM_GOY_THAT_LOVES_VMPROTECT)
    VMProtectBeginMutation(NULL);
#endif
    // This is an added check
    size_t checkStart = wbx::Security::WBXTextBase;
    size_t checkSize = wbx::Security::WBXTextSize;
...
{
#if defined(_WIN32) && !defined(WBX_STUDIO_BUILD)
#if defined(I_AM_GOY_THAT_LOVES_VMPROTECT)
    VMProtectBeginMutation(NULL);
#endif
    ...
    const void* hsceBaseAddress = wbx::HUMAN::HumanoidState::getComputeEventBaseAddress();
    if (hsceBaseAddress)
    {
...
unsigned int protectVmpSections()
{
    ...
    VirtualQuery(reinterpret_cast<void*>(wbx::Security::WBXVmpBase), &thisMemInfo, sizeof(thisMemInfo));
    if (thisMemInfo.State != MEM_COMMIT || !(thisMemInfo.Protect & PAGE_EXECUTE_READ))
    {
        return wbx::Security::WBXVmpSize;
    }
    return thisMemInfo.RegionSize - wbx::Security::WBXVmpSize;
}
