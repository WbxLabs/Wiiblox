...
template<>
bool XmlNameValuePair::getValue(wbx::ProtectedString& value) const {
	std::string tmp;
	if (getValue(tmp)) {
		value = wbx::ProtectedString::fromTrustedSource(tmp);
		return true;
	}
	return false;
}
...
namespace WBX
{
	...
	{		
		...
		{
			static TType<wbx::ProtectedString> type("ProtectedString");
			return type;
		}
