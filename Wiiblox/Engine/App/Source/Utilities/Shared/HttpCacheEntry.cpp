...
using namespace WBX;
using namespace wbx::HttpPlatformImpl::Cache;
using namespace wbx::HttpCache;
...
{
    static const uint32_t m = htonl(WBX_CACHE_FILE_MAGIC);
    return m;
}
...
static void ceDeleter( wbx::HttpPlatformImpl::Cache::CacheEntry * e ) { delete[] ( (char*)e ); }
...
CacheResult CacheResult::open(const char* assetUrl, const char* cdnUrl)
{
    try
    {
        ...
        if (WBX_CACHE_FILE_VERSION != hdr.version)
        {
...
CacheResult CacheResult::update(const char* assetUrl, const char* cdnUrl, const uint32_t responseCode, const Data& headers, const Data& body)
{
	...
	Header cacheHeader = {
		gMagic(),                                                               // magic
		WBX_CACHE_FILE_VERSION,                                                 // version
		std::min(strlen(cdnUrl), static_cast<size_t>(WBX_CACHE_URL_MAX_LENGTH)),// urlBytes
		{'\0'},                                                                 // url
