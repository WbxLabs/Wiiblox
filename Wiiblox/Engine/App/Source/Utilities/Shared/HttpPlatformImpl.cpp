...
using namespace WBX;
using namespace wbx::HttpPlatformImpl;

namespace
{
static std::string WiibloxCookieOverrideDomain;
static std::string WiibloxCookieOverride;
...
print_cookies(const char* tag, CURL *curl)
{
  ...
  StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "print_cookies() from:%s Curl cookies:\n", tag);
  res = curl_easy_getinfo(curl, CURLINFO_COOKIELIST, &cookies);
  if (res != CURLE_OK) {
    StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "print_cookies() Curl curl_easy_getinfo failed\n");
    return;
  }
  nc = cookies, i = 1;
  while (nc) {
    StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "print_cookies() [%d]: %s\n", i, nc->data);
    nc = nc->next;
    i++;
  }
  if (i == 1) {
    StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "print_cookies() (none)\n");
  }
...
public:
	static void initializeCachingThreadHandler()
	{
		...
		boost::thread(thread_wrapper(g, "WBX_http_cache_clean"));
...
void addHeader(curl_slist*& headers, const char* header)
{
    ...
    if (NULL == headers)
    {
        throw wbx::runtime_error("Error adding header %s", header);
    }
}
...
public:
        ...
        ,curlsh(gCurlsh)
    {
        ...
        // Setup User-Agent.
        logCurlError("CURLOPT_USERAGENT", curl_easy_setopt(curl, CURLOPT_USERAGENT, Http::WBXUserAgent.c_str()));
        ...
        WBXASSERT(initialized ? Http::CookieSharingUndefined != cookieSharingPolicy : Http::CookieSharingUndefined == cookieSharingPolicy);
        ...
        )
    {
        ...
        if (compressData)
        {
            ...
            catch (io::gzip_error& e)
            {
                ...
                throw wbx::runtime_error("Upload GZip error %d, ZLib error %d, \"%s\"", e1, e2, url.c_str());
            }
        ...
        if (isPost && !externalRequest)
        {
            ...
            if (foundCsrfHeader)
            {
                ...
                if (!dupdata)
                {
                    ...
                    throw wbx::runtime_error("CurlHandle(%p), strdup failure: %s", this, errmsg);
                }
            ...
            case CURLE_SSL_CONNECT_ERROR:
                {
                    ...
                    if (count < DFInt::HttpCurlDeepErrorReportingCount)
                    {
                        ...
                        {
                            WiibloxGoogleAnalytics::sendEventWiiblox("CURLE_SSL_CONNECT_ERROR", "custom", Http::urlEncode(line).c_str());
                        }
                    }
                    }
                }
...
void init(Http::CookieSharingPolicy cookieSharingPolicy)
{
    ...
    if (!initialized)
    {
        ...
        setCookiesForDomain(WiibloxCookieOverrideDomain, WiibloxCookieOverride);

        {
            boost::function0<void> f = boost::bind(&CacheStatistics::initializeCachingThreadHandler);
            boost::function0<void> g = boost::bind(&StandardOut::print_exception, f, MESSAGE_ERROR, false);
            boost::thread(thread_wrapper(g, "WBX_http_cache_stats_report"));
        }
...
void setCookiesForDomain(const std::string& domain, const std::string& cookies)
{
    if (gCurlsh)
    {
        ...
        // This allows to propogate cookies accross www, m, web, api for all Wiiblox baseURL sub domains
    ...
    {
        WiibloxCookieOverrideDomain = domain;
        WiibloxCookieOverride = cookies;
    }
}
...
void perform(HttpOptions& options, std::string& response)
{
	...
	else
	{
		if (DFFlag::HttpZeroLatencyCaching && HttpCache::PolicyFinalRedirect == options.cachePolicy && curlHandle->getOldCachedData(response))
		{
			...
			threadPool->schedule(boost::bind(&wbx::HttpPlatformImpl::updateCachedData, curlHandle));
			return;
		}
	}
#endif
	...
	if ( statusCode < 200 || statusCode > 299 || statusCode == 202 )
    {
        ...
        throw wbx::http_status_error(statusCode, curlHandle->getResponseCodeReason());
    }
}
} // namespace HttpPlatformImpl
} // namespace WBX
