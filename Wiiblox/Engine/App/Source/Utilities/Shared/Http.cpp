...
DYNAMIC_FASTINTVARIABLE(HttpWBXEventFailureReportHundredthsPercent, 0)
...
#ifdef __APPLE__
extern "C" {
    int WBX_trustCheckBrowser(const char* url)
    {
        return wbx::Http::trustCheckBrowser(url) ? 1 : 0;
    }
}

// NOTICE: Remove these calls after transition for Apple is complete.
int WBX_isWiibloxSite(const char* url);
int WBX_isMoneySite(const char* url);
...
static bool inline sendHttpFailureToEvents()
{
    ...
    return r < DFInt::HttpWBXEventFailureReportHundredthsPercent;
}
...
class HTTPStatistics
{
    ...
    void addFailure(double delay, const char* url, const char* reason)
    {
        ...
        {
            WiibloxGoogleAnalytics::trackEvent(GA_CATEGORY_GAME, "HTTPFailure", reason, delay);
        }

        if (sendHttpFailureToEvents())
        {
            int placeID = atoi(Http::placeID.c_str());
            WiibloxGoogleAnalytics::sendEventWiiblox(GA_CATEGORY_GAME, "HTTPFailure", reason, placeID);
        }
    }
    ...
    static ServiceCategory getServiceCategory(const char* url)
    {
        ...
        {
            wbx::Url parsed = wbx::Url::fromString(url);
    ...
    static void sendInfluxEvent(double delay, const char* url, size_t bytes, const char* reason, int httpStatusCode = 0, bool blocking = false)
    {
        ...
        wbx::Analytics::InfluxDb::Points points;
...
#if defined (__APPLE__) && !defined(WBX_PLATFORM_IOS)
std::string Http::WBXUserAgent = "Wiiblox/Darwin";
#elif defined(WBX_PLATFORM_DURANGO)
std::string Http::WBXUserAgent = "Wiiblox/XboxOne";
#elif defined (_WIN32)
std::string Http::WBXUserAgent = "Wiiblox/WinInet";
#else
std::string Http::WBXUserAgent;
#endif
...
wbx::atomic<int> Http::cdnSuccessCount = 0;
wbx::atomic<int> Http::cdnFailureCount = 0;
wbx::atomic<int> Http::alternateCdnSuccessCount = 0;
wbx::atomic<int> Http::alternateCdnFailureCount = 0;
double Http::lastCdnFailureTimeSpan = 0;
wbx::atomic<int> Http::WiibloxSuccessCount = 0;
wbx::atomic<int> Http::WiibloxFailureCount = 0;
WindowAverage<double, double> Http::WiibloxResponce(kWindowSize);
WindowAverage<double, double> Http::cdnResponce(kWindowSize);

wbx::mutex *Http::WiibloxResponceLock = NULL;
wbx::mutex *Http::cdnResponceLock = NULL;
...
void Http::httpGetPostImpl(bool isPost, std::istream& data, const std::string& contentType, bool compressData, const HttpAux::AdditionalHeaders& additionalHeaders, bool externalRequest, std::string& response)
{
    ...
    if (!externalRequest)
    {
    	...
    	{
            headers[kPlayerCountHeaderKey] = wbx::format("%d", Http::playerCount);
    	}
    }

    // call Mac specific http implementation
    wbx::Cocoa::httpGetPostCocoa(url, authDomainUrl, isPost, data, contentType, compressData, headers, externalRequest, response, useDefaultTimeouts, responseTimeoutMillis);
}
#endif // ifdef __APPLE__
} // WBX
...
{
    WiibloxResponceLock = new wbx::mutex();
    cdnResponceLock = new wbx::mutex();
}

Http::MutexGuard::~MutexGuard()
{
    delete WiibloxResponceLock;
    WiibloxResponceLock = NULL;

    delete cdnResponceLock;
    cdnResponceLock = NULL;
}

wbx::mutex *Http::getWiibloxResponceLock()
{
    return WiibloxResponceLock;
}

wbx::mutex *Http::getCdnResponceLock()
{
    return cdnResponceLock;
}

http_status_error::http_status_error(int statusCode)
    : std::runtime_error(wbx::format("HTTP %d", statusCode))
    , statusCode(statusCode)
{
}

http_status_error::http_status_error(int statusCode, const std::string& message)
    : std::runtime_error(wbx::format("HTTP %d (%s)", statusCode, message.c_str()))
    , statusCode(statusCode)
{
}
...
void Http::init(API api, CookieSharingPolicy sharingPolicy)
{
    ...
    threadPool = tp.get();
    Http::defaultApi = api;
    FASTLOG2(FLog::Http, "Http initialization M1 = 0x%x M2=0x%x", &Http::WiibloxResponceLock, &Http::cdnResponceLock);
...
void Http::SetUseStatistics(bool value)
{
    if (value)
    {
        ...
        boost::thread(thread_wrapper(g, "WBX_http_stats_report"));
    }
}
...
#if defined(_WIN32) && !defined(WBX_PLATFORM_DURANGO)
...
#else
        throw wbx::runtime_error("%s: %s", url, message);
#endif
    }
}
...
void Http::ThrowLastError(int err, const char* url, const char* message)
{
    ...
    if (::FormatMessage(
        ...
        throw wbx::runtime_error("%s: %s, err=0x%X", url, message, err);
    else
        throw wbx::runtime_error("%s: %s, %s", url, message, buffer);
}
#endif // ifdef _WIN32
...
{
    wbx::Timer<wbx::Time::Fast> httpTimer;
#ifdef __APPLE__
	if (!useCurlHttpImpl || forceNativeHttp)
    {
        WBXASSERT(isPost == !contentType.empty());
        try
        {
        ...
        catch (const wbx::http_status_error& e)
        {
            if (recordStatistics)
            {
    ...
    if (externalRequest)
    {
        if (isPost)
        {
            ...
            {
                throw wbx::runtime_error("Post data too large. Limit: %d KB. Post size: %d KB",
...
void Http::onWinHttpRedirect(unsigned long dwInternetStatus, std::string redirectUrl)
{
    ...
    // bitgravity:
    int pos = redirectUrl.find("bg.Wiiblox");
    ...
    // cloudfront:
    pos = redirectUrl.find("-cf.Wiiblox");
    if (pos != std::string::npos)
    {
        alternateUrl = redirectUrl.substr(0, pos);
	...
	catch (wbx::base_exception& e)
    {
        if (shouldRetry)
        {
            ...
            if (!alternateUrl.empty())
            {
                ...
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING,
...
// This allows just Wiiblox domains, not Wiiblox trusted domains like facebook.
bool Http::isStrictlyWiibloxSite(const char* url)
{
    if (DFFlag::UseNewUrlClass)
    {
        wbx::Url parsed = wbx::Url::fromString(url);
    
        return parsed.isSubdomainOf("freblx.xyz") || parsed.isSubdomainOf("Wiibloxlabs.com");
    }

    std::string host(HTParse(url, NULL, PARSE_HOST));
    if ("freblx.xyz" != host && !hasEnding(host, ".freblx.xyz") 
        && "Wiibloxlabs.com" != host && !hasEnding(host, ".Wiibloxlabs.com"))
    {
        return false;
    }
    return true;
...
bool Http::isWiibloxSite(const char* url)
{
#ifdef __APPLE__
    if (!useCurlHttpImpl)
    {
        return WBX_isWiibloxSite(url) != 0;
    }
#endif // ifdef __APPLE__
    ...
    if (!useCurlHttpImpl)
    {
        ...
        case ATL_URL_SCHEME_FTP:
        {
			...
			if (hostName.Right(10)=="freblx.xyz" || hostName.Right(14)=="Wiibloxlabs.com")
				return true;
    ...
    if (DFFlag::UseNewUrlClass)
    {
        ...
        const bool isWiiblox =
            parsed.isSubdomainOf("freblx.xyz") ||
            parsed.isSubdomainOf("Wiibloxlabs.com");
        ...
        return isWiiblox || isFacebook || isYoutube || isGoogle;
    } // if (DFFlag::UseNewUrlClass)
...
bool Http::isExternalRequest(const char* url)
{
    ...
    if (!useCurlHttpImpl)
    {
        ...
        if(hostname.find("freblx.xyz") != std::string::npos || hostname.find("Wiibloxlabs.com") != std::string::npos)
            return false;
...
bool Http::doHttpGetPostWithNativeFallbackForReporting(bool isPost, std::istream& dataStream, const std::string& contentType, bool compressData, const HttpAux::AdditionalHeaders& additionalHeaders, bool allowExternal, std::string& response)
{
	...
	std::string googleAnalyticsBaseURL = WiibloxGoogleAnalytics::kGoogleAnalyticsBaseURL;
	std::string counterMultiUrl = GetCountersMultiIncrementUrl(::GetBaseURL(), wbx::Stats::countersApiKey);
	std::string counterSingleUrl = GetCountersUrl(::GetBaseURL(), wbx::Stats::countersApiKey);
	std::string statsURL = wbx::format("%sgame/report-stats", ::GetBaseURL().c_str());
...
void Http::applyAdditionalHeaders(wbx::HttpAux::AdditionalHeaders& outHeaders)
{
	for (wbx::HttpAux::AdditionalHeaders::const_iterator itr = outHeaders.begin(); itr != outHeaders.end(); ++itr)
	{
