...
static Reflection::EventDesc<RunService, 
	void(double), 
	wbx::signal<void(double)>,
	wbx::signal<void(double)>* (RunService::*)(bool)> event_RenderStepped(&RunService::getOrCreateScriptRenderSteppedSignal, "RenderStepped", "step", Security::None);
...
static Reflection::BoundFuncDesc<RunService, bool()> func_isRunning(&RunService::isRunning, "IsRunning",  Security::WiibloxScript);
...
public:
	...
	{ 
		...
		else
		{
			...
			wbx::Time lastCountedTimestamp = stepRecords.begin() != stepRecords.end() ? (stepRecords.end() - 1)->endTime : wbx::Time();
...
public:
	...
	HeartbeatTask(shared_ptr<RunService> runService)
		...
		,lastHeartbeatStamp(wbx::Time::now<wbx::Time::Fast>())
	{
		cyclicExecutive = true;
	...
	TaskScheduler::StepResult stepDataModelJob(const Stats& stats)
	{
		...
		if (!DFFlag::VariableHeartbeat && (wbx::TaskScheduler::singleton().isCyclicExecutive() && cyclicExecutive))
		{
			...
			{
				wbx::Time currentTime = wbx::Time::now<wbx::Time::Fast>();
...
{
	WBXPROFILER_SCOPE("Physics", "gameStepped");
...
void RunService::unbindFunctionFromRenderStepEarly(std::string name)
{
	...
	{
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING,"RunService:UnbindFromRenderStep removed different functions with same reference name %s %i times.", name.c_str(), erasedFuncCount);
	}
}
	...
	for (EventCallbackMap::iterator iter = renderSteppedEarlyCallbackMap.begin(); iter != renderSteppedEarlyCallbackMap.end(); ++iter)
	{
		...
		for (std::vector<FunctionNameRefPair>::iterator funcIter = functionVector.begin(); funcIter != functionVector.end(); ++funcIter)
		{
			try
			{
				...
				{
					InvokeCallback(weak_from(this), funcIter->second, wbx::make_shared<Reflection::Tuple>());
				}
			}
			catch(const std::runtime_error& e)
			{
				throw wbx::runtime_error("RunService::InvokeCallback failed because %s", e.what());
			}
		}
	}
}
...
wbx::signal<void(double)>* RunService::getOrCreateScriptRenderSteppedSignal(bool create)
{
