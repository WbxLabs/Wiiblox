	...
	std::streamsize read(char* s, std::streamsize n) 
	{
		...
		if(numBytesAvailable == 0)
		{
			...
			while (!::InternetQueryDataAvailable(request, &numBytesAvailable, 0, 0))
			{
				...
				{
					wbx::Http::ThrowLastError(err, "(unknown)", "InternetQueryDataAvailable");
				}
			}
		}
		...
		wbx::Http::ThrowIfFailure(TRUE == ::InternetReadFile(request, (LPVOID) s, std::min(numBytesAvailable, (DWORD)n), &bytesRead), "(unknown)", "InternetReadFile");
	...
	void Http::httpGetPostWinInet(bool isPost, std::istream& data, const std::string& contentType, bool compressData, const HttpAux::AdditionalHeaders& additionalHeaders, bool externalRequest, std::string& response)
	{
		...
		if (theUrl.size()==0)
 		...
 		if (!Http::trustCheck(theUrl.c_str(), externalRequest))
 			throw wbx::runtime_error("trust check failed for %s", theUrl.c_str());
		...
		if (u.GetHostNameLength()==0)
			throw wbx::runtime_error("'%s' is missing a hostName", theUrl.c_str());

		// Initialize the User Agent
		WININETHINTERNET session = InternetOpen("Wiiblox/WinInet", PRE_CONFIG_INTERNET_ACCESS, NULL, NULL, 0);
		if (!session)
			throw wbx::runtime_error("InternetOpen failed for %s", theUrl.c_str());
		...
		if (!connection)
			throw wbx::runtime_error("InternetConnect failed for %s", theUrl.c_str());
		...
		WININETHINTERNET request = ::HttpOpenRequest(
			...
			0); 
		if (!request)
			throw wbx::runtime_error("HttpOpenRequest failed for %s", theUrl.c_str());
		...
		std::string csrfTokenForRequest;
		{
			...
			{
				WBXASSERT(!isPost || !boost::iequals("Content-Type", iter->first));
		...
		else
		{
			...
			catch (io::gzip_error& e)
			{
				...
				throw wbx::runtime_error("Upload GZip error %d, ZLib error %d, \"%s\"", e1, e2, theUrl.c_str());
			}
			...
			if (uploadSize>10000)
			{
				...
				std::string contentSizeHeader = wbx::format("Wiiblox-Content-Size: %d\r\n", uploadSize);
				...
				std::string contentFirstHeader = wbx::format("Wiiblox-Content-First: %d\r\n", uploadData.c_str()[0]);
				...
				std::string contentLastHeader = wbx::format("Wiiblox-Content-Last: %d\r\n", uploadData.c_str()[uploadSize-1]);
				...
				std::string contentHashHeader = wbx::format("Wiiblox-Content-Hash: %s\r\n", hasher->c_str());
			...
			// Send the request
			{
                ...
                {
                    throw wbx::runtime_error("HttpSendRequestEx failed: %d", GetLastError());
                }
		...
		if (externalRequest)
		{
				...
				(LPVOID)transferCoding, &transferCodingLength, 0))
			{
				...
				{
					throw wbx::runtime_error("External http request uses unsupported transfer encoding.");
				}
			}
...

		{
			...
			catch (io::gzip_error& e)
			{
				int e1 = e.error();
				int e2 = e.zlib_error_code();
				throw wbx::runtime_error("GZip error %d, ZLib error %d, \"%s\"", e1, e2, theUrl.c_str());
			}
			catch (io::zlib_error& e)
			{
				int e1 = e.error();
				throw wbx::runtime_error("ZLib error %d, \"%s\"", e1, theUrl.c_str());
			}
		}
		...
		if (statusCode < 200 || statusCode >= 300)
		{
			...
			if(response != "" && externalRequest)
				wbx::StandardOut::singleton()->printf(MESSAGE_WARNING, "Error response: %s", response.c_str());

			if (::HttpQueryInfo(request, HTTP_QUERY_STATUS_TEXT, (LPVOID) buffer, &length, 0))
				throw wbx::http_status_error(statusCode,buffer);
			else
				throw wbx::http_status_error(statusCode);
		}
			...
			{ 
				...
				if ( certificateInfo.lpszSubjectInfo )
				{
					...
					if (currString.size() >= 11 &&
                        (currString.find(".freblx.xyz") != std::string::npos || currString.find(".Wiibloxlabs.com") != std::string::npos))
						foundIt = true;
