...
void StandardOut::printf(MessageType type, const char* format, ...)
{
	...
	std::string message = wbx::vformat(format, arg_list);
	va_end(arg_list);
	print(type, message);
}
...
void StandardOut::print(MessageType type, const std::string& message)
{
	if (wbx::Log::current())
	{
		...
		{
		case MESSAGE_ERROR:
			wbx::Log::current()->writeEntry(Log::Error, message.c_str());
			break;
		case MESSAGE_WARNING:
			wbx::Log::current()->writeEntry(Log::Warning, message.c_str());
			break;
		case MESSAGE_INFO:
			wbx::Log::current()->writeEntry(Log::Information, message.c_str());
			break;
		case MESSAGE_OUTPUT:
			...
			if (message.size() < 256)
				...
				wbx::Log::current()->writeEntry(Log::Information,message.substr(0, 256).c_str());
			break;
		case MESSAGE_SENSITIVE:
			wbx::Log::current()->writeEntry(Log::Warning, message.c_str());
			break;
        default:
            wbx::Log::current()->writeEntry(Log::Error,"Standard Message Out set with incorrect Message Type");
            break;
