...
public:
    ...
    virtual HRESULT __stdcall OnRedirect( IXMLHTTPRequest2 *pXHR, const WCHAR *pwszRedirectUrl) override
    {
        ...
        timeRedirect = wbx::Time::nowFastSec() - timeStart;
        ...
        wbx::HttpPlatformImpl::Cache::CacheResult cacheResult = wbx::HttpPlatformImpl::Cache::CacheResult::open(url.c_str(), redirect.c_str());

        if (cacheResult.isValid())
        {
            timeData = wbx::Time::nowFastSec() - timeStart;
            response.clear();
            response = cacheResult.getResponseBody().toString();
            timeDone = timeData = wbx::Time::nowFastSec() - timeStart;
            cacheValid = true;
    ...
    {
        status = dwStatus;
        timeHeaders = wbx::Time::nowFastSec() - timeStart;
        return S_OK;
    }
...
static void addHeader( IXMLHTTPRequest2* request, const std::wstring& header, const std::wstring value, const std::string& theUrl )
{
    ...
    if( FAILED(hr) )
        throw wbx::runtime_error( "http failed to add header '%S' : '%S' (0x%x) [%s]", header.c_str(), value.c_str(), hr, theUrl.c_str());
}
...
namespace WBX
{
    void Http::httpGetPostXbox(bool isPost, std::istream& data, const std::string& contentType, bool compressData, const HttpAux::AdditionalHeaders& additionalHeaders, bool externalRequest, HttpCache::Policy cachePolicy, std::string& response)
    {
        ...
        WBXPROFILER_SCOPE("Http", __FUNCTION__);
        WBXPROFILER_LABEL("Http", theUrl.c_str());

        wbx::Timer<wbx::Time::Precise> timer;
        
        static const std::wstring userAgentHeader   = L"User-Agent";
        static const std::wstring userAgentValue    = s2ws(Http::WBXUserAgent) + L" Wiiblox Xbox App 1.0.0";
        ...
        if (theUrl.size()==0)
            throw wbx::runtime_error("http empty url");

        if (!Http::trustCheck(theUrl.c_str(), externalRequest))
            throw wbx::runtime_error("http trust check failed for %s", theUrl.c_str());
        ...
        if(FAILED(hr) || !request)
            throw wbx::runtime_error("http is not available at the moment (0x%x) [%s]", hr, theUrl.c_str());
        ...
        if (FAILED(hr))
            throw wbx::runtime_error("http could not open connection (0x%x) [%s]", hr, theUrl.c_str());
        ...
        site1Cookie.pwszUrl = L".sitetest1.Wiibloxlabs.com/";
        ...
        if( !isPost )
        {
            ...
            hr = request->Send( 0, 0 );
            if( FAILED(hr) )
                throw wbx::runtime_error("http failed to send (0x%x) [%s]", hr, theUrl.c_str() );
        }
        else
        {
            // POST
            ...
            if( FAILED(hr) )
                wbx::runtime_error("http send failed (0x%x) [%s]", hr, theUrl.c_str() );
        }
        ...
        if( !callback->wait() )
        {
            // timeout
            request->Abort();
            throw wbx::runtime_error( "http TIMEOUT [%s]", theUrl.c_str() );
        }
        
        hr = callback->getHresult();
        if (FAILED(hr))
            throw wbx::runtime_error( "http error hr=0x%x [%s]", hr, theUrl.c_str() );
...
namespace WBX{ namespace HttpPlatformImpl {

void init(Http::CookieSharingPolicy cookieSharingPolicy) { WBXASSERT(0); }
void setCookiesForDomain(const std::string& domain, const std::string& cookies) { WBXASSERT(0); }
void getCookiesForDomain(const std::string& domain, std::string& cookies) { WBXASSERT(0); }
boost::filesystem::path getWiibloxCookieJarPath() { WBXASSERT(0); return ""; }
void setProxy(const std::string& host, long port) { WBXASSERT(0); }
void perform(HttpOptions& options, std::string& response) { WBXASSERT(0); }
