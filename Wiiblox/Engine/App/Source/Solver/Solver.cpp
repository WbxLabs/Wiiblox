...
static WBX_SIMD_INLINE int getBodyIndex( 
...
{
    WBXPROFILER_SCOPE("Physics", "gatherConstraintPairData");
...
static WBX_SIMD_INLINE void integrateVelocities( SolverBodyDynamicProperties& _variableData, const SolverBodyMassAndInertia& _massAndInertia, const Vector3& _externalImpulse, const Vector3& _externalAngularImpulse, float _dt, const SolverConfig& _config )
{
    ...
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInfVector3( _variableData.integratedLinearVelocity ) );
    ...
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInfVector3( _variableData.integratedAngularVelocity ) );
}
...
{
    WBXPROFILER_SCOPE("Physics", "integrateVelocitiesAndInitSimulatedObjects");
...
{
    WBXPROFILER_SCOPE("Physics", "initAnchoredObjects");
...
static WBX_SIMD_INLINE void integratePositions( SolverBodyDynamicProperties& _dynamicProps, const SolverBodyStaticProperties& _staticProps, 
                                                   const VirtualDisplacementPOD& _velStageVirD, const VirtualDisplacementPOD& _posStageVirD, float _dt )
{
    WBXASSERT_VERY_FAST(!wbx::Math::isNanInfVector3(_velStageVirD.lin));
    WBXASSERT_VERY_FAST(!wbx::Math::isNanInfVector3(_velStageVirD.ang));
    WBXASSERT_VERY_FAST(!wbx::Math::isNanInfVector3(_posStageVirD.lin));
    WBXASSERT_VERY_FAST(!wbx::Math::isNanInfVector3(_posStageVirD.ang));
    ...
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInfVector3( _dynamicProps.position ) );
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInfVector3( _dynamicProps.orientation.row(0) ) );
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInfVector3( _dynamicProps.orientation.row(1) ) );
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInfVector3( _dynamicProps.orientation.row(2) ) );
}
...
void PGSSolver::integratePositionsAndUpdateSimBodies( 
    ...
    float _dt )
{
    WBXPROFILER_SCOPE("Physics", "integratePositionsAndUpdateSimBodies");
...
static WBX_SIMD_INLINE void integratePositionsIgnoreVelocities( SolverBodyDynamicProperties& _dynamicProps, const SolverBodyStaticProperties& _staticProps, 
...
void PGSSolver::integratePositionsIgnoreVelocitiesAndUpdateSimBodies( 
    ...
    float _dt )
{
    WBXPROFILER_SCOPE("Physics", "integratePositionsIgnoreVelocitiesAndUpdateSimBodies");
...
//#define ENABLE_INCONSISTENT_CONSTRAINT_DETECTOR_DEBUG
    ...
    wbx::StandardOut::singleton()->printf(wbx::MESSAGE_OUTPUT, out.str().c_str() );
#endif
...
static void breakIntoIslands( ArrayDynamic< SimBody* >& _islandSimBodies, 
                            ...
                            const ArrayBase< Constraint* >& _constraints )
{
    WBXPROFILER_SCOPE("Physics", "breakIntoIslands");
    // Index the bodies
...
{
    WBXPROFILER_SCOPE("Physics", "PGSSolver::solve");
    ...
    {
        WBXPROFILER_SCOPE("Physics", "fillConstraintSet");
                            ...
                            const SolverConfig& solverConfig )
{
    ...
    {
        WBXPROFILER_SCOPE("Physics", "generateBodyIndices");
    ...
    {
        WBXPROFILER_SCOPE("Physics", "buildConstraintEquations");
    ...
    {
        WBXPROFILER_SCOPE("Physics", "updateConstraintCache");
...
void PGSSolver::addConstraint( Constraint* _constraint )
{
    ...
    WBXASSERT( _constraint->getType() != Constraint::Types_Collision );
...
void PGSSolver::removeContactManifold( boost::uint64_t _uidA, boost::uint64_t _uidB )
{
    ...
    auto it = contactManifolds.find( pair );
    WBXASSERT( it != contactManifolds.end() );
    if( it != contactManifolds.end() )
    {
        it->second->referenceCount--;
...
static WBX_SIMD_INLINE void updateCollision( ConstraintCollision& _collision, const OrderedConnector& _inputConnector )
{
    ...
    WBXASSERT_VERY_FAST( !wbx::Math::isNanInf( params.normal.length() ) && std::abs( 1.0f - params.normal.length() ) < 0.1f );
...
ContactManifold* PGSSolver::updateContactManifold( const BodyUIDPair& _pairId, const ArrayBase< OrderedConnector >& _inputManifold )
{
    ...
    WBXASSERT( cachedManifoldIt != contactManifolds.end() );
...
size_t PGSSolver::addContactConnectors( ArrayDynamic< ContactManifold* >& _activeManifolds, 
                                       ...
                                       const boost::unordered_set< SimBody* >& _simBodies )
{
    WBXPROFILER_SCOPE("Physics", "addContactConnectors");
