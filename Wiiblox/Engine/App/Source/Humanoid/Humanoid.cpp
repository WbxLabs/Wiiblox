...
static Reflection::EnumPropDescriptor<Humanoid, wbx::Camera::CameraMode> prop_CameraMode("CameraMode", category_Data, &Humanoid::getCameraMode, &Humanoid::setCameraMode, Reflection::PropertyDescriptor::REPLICATE_ONLY);
...
static Reflection::BoundFuncDesc<Humanoid, void(bool)> func_SetClickToWalkEnabled(&Humanoid::setClickToWalkEnabled, "SetClickToWalkEnabled","enabled", Security::WiibloxScript);
...
static Reflection::BoundFuncDesc<Humanoid, void(HUMAN::StateType)> func_ChangeState(&Humanoid::changeState, "ChangeState", "state", wbx::HUMAN::xx, Security::None);
...
static Reflection::RemoteEventDesc<Humanoid, void(shared_ptr<Instance>)> desc_serverEquipTool(&Humanoid::serverEquipToolSignal, "ServerEquipTool", "tool", Security::Wiiblox, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
...
{
	addPair(wbx::HUMAN::FALLING_DWN, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::FALLING_DWN));
	addPair(wbx::HUMAN::RUNNING, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::RUNNING));
	addPair(wbx::HUMAN::RUNNING_NO_PHYS, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::RUNNING_NO_PHYS));
	addPair(wbx::HUMAN::CLIMBING, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::CLIMBING));
	addPair(wbx::HUMAN::STRAFING_NO_PHYS, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::STRAFING_NO_PHYS));
	addPair(wbx::HUMAN::RAGDOLL, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::RAGDOLL));
	addPair(wbx::HUMAN::GETTING_UP, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::GETTING_UP));
	addPair(wbx::HUMAN::JUMPING, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::JUMPING));
	addPair(wbx::HUMAN::LANDED, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::LANDED));
	addPair(wbx::HUMAN::FLYING, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::FLYING));
	addPair(wbx::HUMAN::FREE_FALL, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::FREE_FALL));
	addPair(wbx::HUMAN::SEATED, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::SEATED));
	addPair(wbx::HUMAN::PLATFORM_STANDING, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::PLATFORM_STANDING));
	addPair(wbx::HUMAN::DEAD, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::DEAD));
	addPair(wbx::HUMAN::SWIMMING, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::SWIMMING));
	addPair(wbx::HUMAN::PHYSICS, wbx::HUMAN::HumanoidState::getStateNameByType(wbx::HUMAN::PHYSICS));
	addPair(wbx::HUMAN::xx, "None");
}
template<>
...
{
	if(text == wbx::HUMAN::HumanoidState::getStateNameByType(compare)) {
		value = compare;
		return true;
	}
	return false;
...
template<>
bool wbx::StringConverter<HUMAN::StateType>::convertToValue(const std::string& text, HUMAN::StateType& value)
{
	if (Humanoid::isStateInString(text, wbx::HUMAN::FALLING_DWN, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::RUNNING, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::RUNNING_NO_PHYS, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::CLIMBING, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::STRAFING_NO_PHYS, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::RAGDOLL, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::GETTING_UP, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::JUMPING, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::LANDED, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::FLYING, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::FREE_FALL, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::SEATED, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::PLATFORM_STANDING, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::DEAD, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::SWIMMING, value)) return true;
	if (Humanoid::isStateInString(text, wbx::HUMAN::PHYSICS, value)) return true;
	if(text.find("None")){
		value = wbx::HUMAN::xx;
		return true;
	}
...
static const std::string& getAppendageString(size_t appendage, bool R15)
{
    ...
    {
        WBXASSERT(!"Unknown appendage type");
...
Humanoid::~Humanoid()
{
	WBXASSERT(!currentState.get());
	WBXASSERT(world==NULL);
	FASTLOG1(FLog::ISteppedLifetime, "Humanoid destroyed - %p", this);
}

void collectStatus(shared_ptr<wbx::Instance> instance, Reflection::ValueArray* result)
{
...
void Humanoid::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
	if(newProvider)
	{
		...
		if (DFFlag::UseR15Character && wbx::Network::Players::backendProcessing(this))
		{
			DataModel* dataModel = Instance::fastDynamicCast<DataModel>(newProvider);
			if (dataModel)
			{
				wbx::Security::Impersonator impersonate(wbx::Security::Replicator_);
...
bool Humanoid::getDead() const
{
	if (this->world != NULL) {
		WBXASSERT(currentState.get());
		return (currentState->getStateType() == wbx::HUMAN::DEAD);
	}
	else {
		return true;
	}
...
{
	wbx::PartInstance* tempTorso = getTorsoSlow();
	if (tempTorso) {
...
{ 
	try {
		wbx::Security::Context::current().requirePermission(wbx::Security::WiibloxScript, "setRigType");
	} 
	catch (wbx::base_exception& e) 
	{
		raisePropertyChanged(propRigType);
		wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Insufficient permissions to setRigType");
		throw e;
	}
...
void Humanoid::buildJointsFromAttachments(PartInstance* part, std::vector<PartInstance*>& characterParts)
{
	...
	if (shared_ptr<const Instances> children = part->getChildren2())
	{
		...
		{
			if(wbx::Attachment* attachment = Instance::fastDynamicCast<wbx::Attachment>((*iter).get()))
			{
...
// Server side
void Humanoid::buildJoints(wbx::DataModel* dm)
{
	if(!status){
		status = Creatable<Instance>::create<wbx::StatusInstance>();
	...
	{	
		...
		else
		{
			...
			WBXASSERT(head && torso && rightArm && leftArm && rightLeg && leftLeg);
...
bool Humanoid::hasWalkToPoint(Vector3& worldPosition) const
{
	...
	if(player && (wbx::GameBasicSettings::singleton().inHybridMode() || wbx::GameBasicSettings::singleton().inMousepanMode()) )
		return false;
...
void Humanoid::setPreviousStateType(wbx::HUMAN::StateType newState)
{
...
void Humanoid::changeState(HUMAN::StateType state)
{
    ...
    checkWBXCaller<kCallCheckCodeOnly, callCheckSetApiFlag<kChangeStateApiOffset> >(
...
void Humanoid::equipTool(wbx::Tool* tool)
{
	if(tool)
	{
		if(ModelInstance* character = getCharacterFromHumanoid(this))
		{
			...
			if (wbx::Network::Players::backendProcessing(this))
			{
				...
				if(ModelInstance* character = getCharacterFromHumanoid(this))
				{
					...
					if(player && !fromWorkspace) 
					{
						...
						const wbx::SystemAddress addr = player->getRemoteAddressAsWbxAddress();
...
void Humanoid::unequipTools()
{
	if(ModelInstance* character = getCharacterFromHumanoid(this))
    {
		if(character->getChildren())
        {
			...
			{
				if(wbx::Backpack* backpack = player->findFirstChildOfType<Backpack>())
				{
					...
					{
						if(wbx::Tool* tool = Instance::fastDynamicCast<wbx::Tool>((*iter).get()))
...
Velocity Humanoid::calcDesiredWalkVelocity() const
{
	...
	if (autorotate)
	{
		if (player && wbx::GameBasicSettings::singleton().getRotationType() == wbx::GameBasicSettings::ROTATION_TYPE_CAMERA_RELATIVE)
		{
...
{
	{
		...
		WBXASSERT(value.x <= 1.0f && value.z <= 1.0f);
	...
	if (walkDirection != wbx::Vector3::zero())
	{
...
{
	return ( (currentState.get()) ? currentState->getStateType() == wbx::HUMAN::SWIMMING : false);
}
...
{
	if(DataModel* dataModel = wbx::DataModel::get(this))
	{
		if(walkVector == Vector3::zero())
		{
			setLuaMoveDirection(wbx::Vector3(0,0,0));
			rawMovementVector = wbx::Vector3(0,0,0);
			return;
		}
		...
		if (relativeToCamera)
		{
			...
			const wbx::Vector3 northDirection(0,0,-1);
...
JointInstance* Humanoid::getRightShoulder()
{
	...
	return temp ? WBX_static_cast<JointInstance*>(temp->findFirstChildByName("Right Shoulder"))
				: NULL;
}
...
void Humanoid::computeForce(bool throttling)
{
	...
	{
		WBXASSERT(getEngineBody() && getEngineBody()->getRootSimBody());
	...
	if (world && currentState.get())			// put this here for safety
	{
		...
		G3D::Vector3 temp;
		// The following WBXASSERT is just a way to get around the system to build this line on DEBUG build
...
void Humanoid::onEvent_SiblingPropertyChanged(const wbx::Reflection::PropertyDescriptor* desc)
{
...
void Humanoid::onStepped(const Stepped& event)
{
    ...
    WBXPROFILER_SCOPE("Physics", "Humanoid::onStepped");
	...
	if (localSimulating || ownedByLocalPlayer) {
		...
		if (Instance::fastDynamicCast<ModelInstance>(getParent()))
		{
			...
			// The return value of getTorsoFast()->getCoordinateFrame().translation is not bool & is Vector3.
			WBXASSERT(getTorsoFast() ? ((temp = getTorsoFast()->getCoordinateFrame().translation) == temp) : true);

			if (!ownedByLocalPlayer && !wbx::TaskScheduler::singleton().isCyclicExecutive())
			{
				if (event.longStep)
				{
			...
			WBXASSERT(getTorsoFast() ? ((temp - getTorsoFast()->getCoordinateFrame().translation).length() < 10.0f) : true);
		}
...
{
    if(wbx::GameBasicSettings::singleton().inClassicMode())
...
void Humanoid::renderMultiplayer(Adorn* adorn, const wbx::Camera& camera)
{
	...
	{	
		...
		if(DFFlag::EnableMotionAnalytics)
		{
			...
			Primitive * HeadPrimitive = wbx::PartInstance::getPrimitive(foundHead);
			if(HeadPrimitive)
			{
...
void Humanoid::renderBillboard(Adorn* adorn, const wbx::Camera& camera)
{
	if (displayDistanceType == Humanoid::HUMANOID_DISPLAY_DISTANCE_TYPE_NONE)
		return;
...
static void collectAllDescendantCharacterPrims(shared_ptr<wbx::Instance> descendant, std::vector<Primitive*>& primitives)
{
...
static void collectAllDescendantCharacterParts(shared_ptr<wbx::Instance> descendant, std::vector<PartInstance*>& parts)
{
...
static void setLocalTransparencyModifierDecendants(shared_ptr<wbx::Instance> descendant, float transparencyModifier)
{
	PartInstance *part = Instance::fastDynamicCast<PartInstance>(descendant.get());
	if (part) 
	{
...
namespace Reflection {
template<>
EnumDesc<wbx::Humanoid::NameOcclusion>::EnumDesc()
:EnumDescriptor("NameOcclusion")
{
	addPair(wbx::Humanoid::NAME_OCCLUSION_ALL, "OccludeAll");
	addPair(wbx::Humanoid::NAME_OCCLUSION_ENEMY, "EnemyOcclusion");
	addPair(wbx::Humanoid::NAME_OCCLUSION_NONE, "NoOcclusion");
}

template<>
EnumDesc<wbx::Humanoid::HumanoidDisplayDistanceType>::EnumDesc()
:EnumDescriptor("HumanoidDisplayDistanceType")
{
	addPair(wbx::Humanoid::HUMANOID_DISPLAY_DISTANCE_TYPE_VIEWER, "Viewer");
	addPair(wbx::Humanoid::HUMANOID_DISPLAY_DISTANCE_TYPE_SUBJECT, "Subject");
	addPair(wbx::Humanoid::HUMANOID_DISPLAY_DISTANCE_TYPE_NONE, "None");
}

template<>
EnumDesc<wbx::Humanoid::HumanoidRigType>::EnumDesc()
	:EnumDescriptor("HumanoidRigType")
{
	addPair(wbx::Humanoid::HUMANOID_RIG_TYPE_R6, "R6");
	addPair(wbx::Humanoid::HUMANOID_RIG_TYPE_R15, "R15");
}


template<>
wbx::Humanoid::NameOcclusion& Variant::convert<wbx::Humanoid::NameOcclusion>(void)
{
	return genericConvert<wbx::Humanoid::NameOcclusion>();
}

template<>
EnumDesc<wbx::Humanoid::Status>::EnumDesc()
:EnumDescriptor("Status")
{
	addPair(wbx::Humanoid::POISON_STATUS, "Poison");
	addPair(wbx::Humanoid::CONFUSION_STATUS, "Confusion");
}

template<>
wbx::Humanoid::Status& Variant::convert<wbx::Humanoid::Status>(void)
{
	return genericConvert<wbx::Humanoid::Status>();
}
}  // namespace Reflection

template<>
bool wbx::StringConverter<wbx::Humanoid::NameOcclusion>::convertToValue(const std::string& text, wbx::Humanoid::NameOcclusion& value)
{
	if(text.find("OccludeAll")){
		value = wbx::Humanoid::NAME_OCCLUSION_ALL;
		return true;
	}
	if(text.find("EnemyOcclusion")){
		value = wbx::Humanoid::NAME_OCCLUSION_ENEMY;
		return true;
	}
	if(text.find("NoOcclusion")){
		value = wbx::Humanoid::NAME_OCCLUSION_NONE;
		return true;
	}
	return false;
}
...
template<>
bool wbx::StringConverter<wbx::Humanoid::Status>::convertToValue(const std::string& text, wbx::Humanoid::Status& value)
{
	if(text.find("Poison")){
		value = wbx::Humanoid::POISON_STATUS;
		return true;
	}
	if(text.find("Confusion")){
		value = wbx::Humanoid::CONFUSION_STATUS;
		return true;
	}
	return false;
}
...
void Humanoid::setupAnimator()
{
	if(!animator)
	{
		...
		if (!animator)
		{
			animator = shared_ptr<Animator>(Creatable<Instance>::create<wbx::Animator>(this));
...
bool Humanoid::getStateTransitionEnabled(HUMAN::StateType state)
{
    ...
    {
        wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Invalid state passed to GetStateEnabled.");
    } 
    else 
    {        
...
void Humanoid::setStateTransitionEnabled(HUMAN::StateType state, bool enabled)
{
    if (state < HUMAN::FALLING_DWN || state >= HUMAN::NUM_STATE_TYPES)
    {
        wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Invalid state passed to SetStateEnabled.");
    } 
    else 
    {        
...
bool foundShortestDistanceFromCornerToObject(PartInstance* floorPart, Humanoid* charHumanoid, const Vector3& deltaDisplacementGoal, const Vector3& torsoSpaceClosestCorner, Vector3& allowedDisplacementOut)
{
	...
	wbx::ContactManager* contactManager = characterWorld->getContactManager();
	...
	for (size_t i = 0; i < 3; i++)
	{
		...
		// Raycast Setup
		WBXRay newRay(worldSpaceCurrentCorner, deltaDisplacementGoal);
...
Vector3 Humanoid::getSimulatedFrictionVelocityOffset(PartInstance* floorPart, Vector3& newCharPosition, const Vector3& previousCharPosition, const Vector3& charPosInFloorSpace, const wbx::Velocity& previousFloorVelocity, float& netDt)
{
	...
	if (floorPrim && (getCurrentFloorFilterPhase() >= Assembly::NoSim_SendIfSim) && floorPrim->getWorld())
	{
		...
		if (getWorld()->getUsingPGSSolver())
			multiplyer = wbx::HUMAN::HumanoidState::runningKMovePForPGS() * netDt * (1000.0f/(DFInt::HumanoidFloorManualFrictionVelocityMultValue));
		else
			multiplyer = wbx::HUMAN::HumanoidState::runningKMoveP() * netDt * (1000.0f/(DFInt::HumanoidFloorManualFrictionVelocityMultValue));

		Vector3 approxXZForce(deltaVelocity.x * multiplyer , 0 , deltaVelocity.z * multiplyer);
		float forceMagnitudeOverMax = (approxXZForce.magnitude() / (wbx::HUMAN::HumanoidState::maxLinearGroundMoveForce()));  
...
void Humanoid::updateNetworkFloorPosition(PartInstance* floorPart, CoordinateFrame& previousFloorPosition, wbx::Velocity& lastFloorVelocity, float& netDt)
{
