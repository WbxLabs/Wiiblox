	...
	:Named<HumanoidState, sMovingNoPhysicsBase>(humanoid, priorState)
{
	...
	WBXASSERT(!torsoPart.get() || torsoPart->getPartPrimitive()->getEngineType() == Primitive::DYNAMICS_ENGINE);
...
void MovingNoPhysicsBase::onComputeForceImpl()
{
	...
	if (torsoPart.get()) {
		WBXASSERT(!torsoPart->getPartPrimitive()->inKernel());
	}
#endif
}
...
void MovingNoPhysicsBase::disconnectTorso()
{
	...
	if (torsoPart.get())
	{
		WBXASSERT(torsoPart->getPartPrimitive()->getEngineType() == Primitive::HUMANOID_ENGINE);
...
void MovingNoPhysicsBase::onSimulatorStepImpl(float stepDt)
{
	...
	if (Primitive* torso = getHumanoid()->getTorsoSlow()->getPartPrimitive()) 
	{
		...
		{
			WBXASSERT(!(torso->getBody()->getRootSimBody()->isInKernel()));		// make sure engine not simulating with physics
