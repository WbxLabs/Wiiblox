...
{
	WBXASSERT(STATE_TABLE[oldState+1][0] == oldState);
	WBXASSERT(STATE_TABLE[0][eventType+1] == eventType);
...
HumanoidState::~HumanoidState()
{
	...
	std::vector<wbx::signals::connection>::iterator buoyancyConnectionsIterator;
...
void HumanoidState::preStepSimulatorSide(float dt)
{
	...
	if (DFFlag::CheckForHeadHit)
	{
		...
		if (foundBody && foundHead) 
		{
			...
			WBXRay caster(foundHead->getCoordinateFrame().translation, headUp);
			Vector3 hitLocation;
...
void HumanoidState::simulate(shared_ptr<HumanoidState>& state, float dt)
{
	...
	state->onStepImpl();			
	WBXASSERT(!state->getAssembly() || state->getStateType() == state->stateFromAssembly());
...
void HumanoidState::updateHumanoidFloorStatus(shared_ptr<HumanoidState>& state)
{
	...
	if (isValidSubscriptionHumanoidState(state) && lastFloorPrim)
	{
		...
		if (floorAssembly)
		{
			...
			PartInstance* floorRootPart = PartInstance::fromPrimitive(wbx::Mechanism::getRootMovingPrimitive(lastFloorPrim));
...
void HumanoidState::doSimulatorStateTable(shared_ptr<HumanoidState>& state, float dt)
{
	...
	for (int i = 0; i < NUM_EVENT_TYPES; ++i)
	{
		...
		WBXASSERT(newType != RUNNING_SLAVE);							// only should come to slave side, never out of state table
    ...
    if (isBadType)
    {
        wbx::Security::setHackFlagVmp<LINE_RAND1>(wbx::Security::hackFlag5, HATE_HSCE_EBX);
    }
    if (isBadNvReg)
    {
        Tokens::sendStatsToken.addFlagFast(HATE_HSCE_EBX);
        Tokens::simpleToken |= HATE_HSCE_EBX;
        wbx::Security::setHackFlagVmp<LINE_RAND1>(wbx::Security::hackFlag7, HATE_HSCE_EBX);
    }
	#if defined(WIN32) && defined(I_AM_GOY_THAT_LOVES_VMPROTECT) 
    VMProtectEnd();
	#endif
...
bool HumanoidState::computeEvent(EventType eventType) 
{
	...
	switch (eventType) 
	{
	case HAS_BUOYANCY:			WBX_JUNK; returnValue = computeHasBuoyancy(); break;
	case NO_BUOYANCY:			WBX_JUNK; returnValue = !computeHasBuoyancy(); break;
	case NO_HEALTH:				WBX_JUNK; returnValue = (humanoid->getHealth() <= 0.0f); break;
	case NO_NECK:				WBX_JUNK; returnValue = (!humanoid->getNeck()); break;
	case JUMP_CMD:				WBX_JUNK; returnValue = computeJumped(); break;
	case SIT_CMD:				WBX_JUNK; returnValue = humanoid->getSit(); break;
	case NO_SIT_CMD:			WBX_JUNK; returnValue = !humanoid->getSit(); break;
	case PLATFORM_STAND_CMD:	WBX_JUNK; returnValue = humanoid->getPlatformStanding(); break;
	case NO_PLATFORM_STAND_CMD:	WBX_JUNK; returnValue = !humanoid->getPlatformStanding(); break;
	case TIPPED:				WBX_JUNK; returnValue = computeTipped(); break;
	case UPRIGHT:				WBX_JUNK; returnValue = computeUpright(); break;
	case FACE_LDR:				WBX_JUNK; returnValue = facingLadder; break;
	case AWAY_LDR:				WBX_JUNK; returnValue = !facingLadder; break;
	case STRAFE_CMD:			WBX_JUNK; returnValue = false; break;
	case NO_STRAFE_CMD:			WBX_JUNK; returnValue = !humanoid->getStrafe(); break;
	case ON_FLOOR:				WBX_JUNK; returnValue = (floorPart != NULL) && (!DFFlag::FixJumpGracePeriod || getRelativeMovementVelocity().y <= 0.0f); break;
	case OFF_FLOOR:				WBX_JUNK; returnValue = (floorPart == NULL); break;
	case OFF_FLOOR_GRACE:		WBX_JUNK; returnValue = (noFloorTimer > fallDelay()); break;
	case TOUCHED:				WBX_JUNK; returnValue = computeTouched(); break;
	case NEARLY_TOUCHED:		WBX_JUNK; returnValue = computeNearlyTouched(); break;
	case TOUCHED_HARD:			WBX_JUNK; returnValue = computeTouchedHard(); break;
	case ACTIVATE_PHYSICS:		WBX_JUNK; returnValue = computeActivatePhysics(); break;
	case TIMER_UP:				WBX_JUNK; returnValue = (timer <= 0.0f); break;
	case NO_TOUCH_ONE_SECOND:
        ...
        break;
	case FINISHED:		WBX_JUNK; returnValue = finished; break;
    case HAS_GYRO:		WBX_JUNK; returnValue = computeHasGyro(); break;
	default:
		{
			returnValue = false;
		}
...
{
	WBXASSERT(humanoid->getWorld());
	if (World* world = humanoid->getWorld()) 
	{
		if (PartInstance* foundTorso = humanoid->getTorsoSlow()) {
			wbx::SystemAddress myLocalAddress = wbx::Network::Players::findLocalSimulatorAddress(humanoid);
			...
			WBXASSERT(0);	// no torso - why am I here?
		}
	}
	return false;
}
										...
										const Vector3& offset, const float maxDistance,	Assembly* humanoidAssembly, const CoordinateFrame& torsoC)
{
	...
	WBXRay tryRay = WBXRay::fromOriginAndDirection(pointOnTorso, -Vector3::unitY());
...
shared_ptr<PartInstance> HumanoidState::tryFloor(const WBXRay& ray, Vector3& hitLocation, Vector3& hitNormal, float maxDistance, Assembly* humanoidAssembly, PartMaterial& recentFloorMaterial)
{
	WBXASSERT(humanoid->getWorld());

	WBXASSERT(humanoidAssembly);
	filteringAssembly = humanoidAssembly;
	Primitive* floorPrim = NULL;

	WBXASSERT(ray.direction().isUnit());
	WBXRay worldRay = WBXRay::fromOriginAndDirection(ray.origin(), ray.direction());
...
const Velocity HumanoidState::getFloorPointVelocity() 
{
	...
	{
		WBXASSERT(p->getWorld());
		if (p->getWorld())
		{
			WBXASSERT(floorTouchInWorld != unitializedFloorTouch());
...
{
	wbx::Primitive *torso = humanoid->getTorsoPrimitiveSlow();
	if (torso)
	{
...
void HumanoidState::fireMovementSignal(wbx::signal<void(float)>& movementSignal, float movementVelocity)
{
...
void HumanoidState::doLadderRaycast(GeometryService *geom, const WBXRay& caster,
...
void HumanoidState::doAutoJump() 
{
	...
	float rayLength = 1.5; 
	// This is how high a Wiibloxian jumps from the mid point of his torso 
	...
	if (geom) 
	{
		...
		WBXRay torsoRay(torsoPos + Vector3(0, -torso->getConstPartPrimitive()->getSize().y/2, 0), torsoLookVector * rayLength); 
		WBXRay jumpRay(torsoPos + Vector3(0, jumpHeight - torso->getConstPartPrimitive()->getSize().y, 0), torsoLookVector * rayLength); 
...
bool HumanoidState::findLadder(Adorn* adorn)
{
	...
	for (int i = 1; i < topRay; i++) // search for 3 studs, starting at toes		NOTE - removed bottom search
	{
		...
		WBXRay caster(torsoCoord.translation + originOnTorso, torsoLook * ladderSearchDistance(humanoid) );
		...
		if (hitPrim) 
		{
			...
			if (p)
			{
				if (p->getPartType() == wbx::TRUSS_PART) return true;
			}
		}
		...
		if (adorn) { 
			...
			WBXRay rayInPart = WBXRay::fromOriginAndDirection(	caster.origin() - torsoCoord.translation, 
