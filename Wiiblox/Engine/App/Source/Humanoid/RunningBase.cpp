	...
	,desiredAltitude(0.0f)
{
	...
	WBXASSERT(!humanoid->getTorsoSlow() || humanoid->getTorsoSlow()->getPartPrimitive()->getEngineType() == Primitive::DYNAMICS_ENGINE);
}
...
void RunningBase::onComputeForceImpl()
{
	...
	if (!root)
		...
		if (getFloorPrimitive())
		{
			...
			debugRayList.push_back(DebugRay(WBXRay(getFloorTouchInWorld(), (force) / 500.0f ), Color3::green()));
		}
#endif
	...
	{
		WBXASSERT(floor->getAssembly()->getAssemblyPrimitive()->getBody() != root);
...
#if CHARACTER_FORCE_DEBUG
				debugRayList.push_back(DebugRay(WBXRay(getFloorTouchInWorld(), Vector3(0.0, deltaForce/100.0f, 0.0)), Color3::red()));
				Vector3 force = root->getBranchForce();
				debugRayList.push_back(DebugRay(WBXRay(getFloorTouchInWorld() + Vector3(3.0f, 0.0f, 0.0f) , (force) / 500.0f ), Color3::yellow()));
#endif
			}
		}
	}
