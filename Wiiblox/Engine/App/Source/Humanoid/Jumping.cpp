...
void Jumping::onComputeForceImpl()
{
 	...
 	if (!getFinished())
	{
		...
		//StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "%f", root->getVelocity().linear.y);
		...
		else
		{
			for (int x = -1; x <= 1; x+=2) {
				for (int z = -1; z <= 1; z+=2) 
				{
					...
					tryRay = WBXRay::fromOriginAndDirection(pointOnTorso, Vector3::unitY());
...
shared_ptr<PartInstance> Jumping::tryCeiling(const WBXRay& ray, float maxDistance, Assembly* humanoidAssembly)
{
	Humanoid* humanoid = getHumanoid();
	float distance;
