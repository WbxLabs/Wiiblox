...
Assembly::~Assembly()
{
	WBXASSERT(history == NULL);
	WBXASSERT(state == Sim::ANCHORED);
	WBXASSERT(simJob == NULL);
	WBXASSERT(recursiveDepth == -1);
}
/*
...
{
	WBXASSERT((state == Sim::ANCHORED) || (getCurrentStage()->getStageType() >= IStage::HUMANOID_STAGE));
...
Assembly* Assembly::getPrimitiveAssemblyFast(Primitive* p)
{
	...
	WBXASSERT_IF_VALIDATING(!answer || dynamic_cast<Assembly*>(answer));
	return answer;
}
...
const Assembly* Assembly::getConstPrimitiveAssembly(const Primitive* p)
{
	if (const IndexedMesh* clump = p->getConstComputedUpper()) {
		...
		WBXASSERT_IF_VALIDATING(!answer || dynamic_cast<const Assembly*>(answer));
		return answer;
	}
	return NULL;
}
...
void Assembly::getPhysics(G3D::Array<CompactCFrame>& motorAngles) const
{
	...
	for (int i = 0; i < motorAngles.size(); ++i) 
	{
		...
		{
			const MotorJoint* m1d = WBX_static_cast<const MotorJoint*>(motors[i]);
			motorAngles[i] = CompactCFrame(Vector3::zero(), Vector3::unitZ(), m1d->getCurrentAngle());
		}
		else
		{
            const Motor6DJoint* m6d = WBX_static_cast<const Motor6DJoint*>(motors[i]);
...
void Assembly::setPhysics(const G3D::Array<CompactCFrame>& motorAngles, const PV& pv)
{
	...
	if (assemblyMotors.size() == motorAngles.size()) 
	{
		for (int i = 0; i < assemblyMotors.size(); ++i) 
		{
			...
			{
				MotorJoint* m1d = WBX_static_cast<MotorJoint*>(assemblyMotors[i]);
				// TODO: Put this assertion back in and debug http://Wiiblox.onjira.com/browse/CLIENT-250
				//WBXASSERT(G3D::fuzzyEq(fabs(motorAngles[i].getAxis().z), 1.0f));

				WBXASSERT(motorAngles[i].translation.isZero());
			...
			{
				Motor6DJoint* m6d = WBX_static_cast<Motor6DJoint*>(assemblyMotors[i]);
		...
		//{
		//	wbx::StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "motors: %f,%f,%f,%f", motorAngles[0].rotationangle,  motorAngles[1].rotationangle,  motorAngles[2].rotationangle,  motorAngles[3].rotationangle);
		//}
...
bool Assembly::isAssemblyRootPrimitive(const Primitive* p)
{
	...
	WBXASSERT(!answer || (getConstPrimitiveAssembly(p)->getConstTypedLower<Clump>()->getConstTypedLower<Primitive>() == p));
	return answer;
}
...
{
	WBXASSERT(this->getCurrentStage()->getStageType() >= IStage::TREE_STAGE);
...
void notifyAssemblyPrimitiveMoved(Primitive* p, bool resetContacts)
{
	...
	if (resetContacts)
	{
		...
		{
			WBXASSERT(p->getContact(i));
			p->getContact(i)->primitiveMovedExternally();
		}
		WBXASSERT(temp == p->getNumContacts());
	}
}
