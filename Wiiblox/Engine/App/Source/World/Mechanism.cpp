...
{
	WBXASSERT(isMovingAssemblyRoot(a));		// shouldn't be calling this otherwise
	...
	for (int i = 0; i < a->numChildren(); ++i)
	{
		...
		const Joint* j = WBX_static_cast<const Joint*>(spanningEdge);
		WBXASSERT(Joint::isSpringJoint(j));
	}
#endif
...
Mechanism* Mechanism::getPrimitiveMechanism(Primitive* p)
{
	if (Assembly* assembly = Assembly::getPrimitiveAssembly(p)) {
		return WBX_static_cast<Mechanism*>(assembly->getComputedUpper());
	}
	return NULL;
}
...
const Mechanism* Mechanism::getConstPrimitiveMechanism(const Primitive* p)
{
	if (const Assembly* assembly = Assembly::getConstPrimitiveAssembly(p)) {
		return WBX_static_cast<const Mechanism*>(assembly->getConstComputedUpper());
	}
	return NULL;
}
...
bool Mechanism::isMechanismRootPrimitive(const Primitive* p)
{
	...
	WBXASSERT(!answer || (getConstPrimitiveMechanism(p)->getConstTypedLower<Assembly>()->getConstTypedLower<Clump>()->getConstTypedLower<Primitive>() == p));
	return answer;
}
