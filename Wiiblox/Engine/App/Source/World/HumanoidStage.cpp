...
{
	WBXASSERT(movingHumanoidAssemblies.size() == 0);
}


void HumanoidStage::toDynamics(Assembly* a)
{
	WBX_static_cast<SleepStage*>(getDownstreamWS())->onAssemblyAdded(a);
}

void HumanoidStage::fromDynamics(Assembly* a)
{
	WBX_static_cast<SleepStage*>(getDownstreamWS())->onAssemblyRemoving(a);
}

void HumanoidStage::toHumanoid(Assembly* a)
{
	WBXASSERT(a->getAssemblyState() == Sim::ANCHORED);
...
void HumanoidStage::fromHumanoid(Assembly* a)
{
	...
	WBXASSERT(a->getAssemblyState() == Sim::AWAKE);
