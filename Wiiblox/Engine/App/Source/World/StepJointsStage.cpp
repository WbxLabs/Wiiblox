...
{
	WBXASSERT(worldStepJoints.empty());
}
...
void StepJointsStage::removeJoint(Joint* j)
{
	if (j->canStepWorld()) {
		...
		WBXASSERT(!j->StepJointsStageHook::is_linked());
	}
}
...
void StepJointsStage::onSimulateAssemblyAdded(Assembly* a)
{
	...
	WBX_static_cast<HumanoidStage*>(getDownstreamWS())->onAssemblyAdded(a);
}


void StepJointsStage::onSimulateAssemblyRemoving(Assembly* a) 
{
	WBX_static_cast<HumanoidStage*>(getDownstreamWS())->onAssemblyRemoving(a);
...
{
	wbx::Profiling::Mark mark(*profilingJointUpdate, false);

	for (Joints::iterator it = worldStepJoints.begin(); it != worldStepJoints.end(); ++it) 
	{
		WBXASSERT(it->canStepWorld());
		it->stepWorld();
	}
}
