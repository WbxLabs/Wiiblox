...
{
	WBXASSERT(WBX_static_cast<Ball*>(p1->getGeometry()));
	WBXASSERT(WBX_static_cast<MegaClusterPoly*>(p0->getGeometry()));
...
{
	return WBX_static_cast<const Ball*>(getConstPrimitive(1)->getConstGeometry());
}

const Poly* BallCellContact::poly() const
{
	return WBX_static_cast<const MegaClusterPoly*>(getConstPrimitive(0)->getConstGeometry());
}
