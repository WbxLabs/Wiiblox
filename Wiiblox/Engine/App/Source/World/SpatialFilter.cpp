...
SpatialFilter::~SpatialFilter()
{
	for (int i = 0; i < Assembly::NUM_PHASES; ++i) {
		WBXASSERT(assemblies[i].empty());
	}
}

MechToAssemblyStage* SpatialFilter::getMechToAssemblyStage()
{
	return WBX_static_cast<MechToAssemblyStage*>(this->getDownstreamWS());
}
...
{
	WBXASSERT(mi.to != mi.from);
	WBXASSERT(mi.a->getFilterPhase() == mi.from);
...
void SpatialFilter::moveInto(MoveInstructions& mi)
{
...

	{
		...
		if (wasSimulating) {
			WBXASSERT(mi.a->downstreamOfStage(this));
...
{
	WBXASSERT(filter.networkAddress == Network::NetworkOwner::Server());
	const wbx::SystemAddress owner = a->getAssemblyPrimitive()->getNetworkOwner();
...
bool SpatialFilter::inClientSimRegion(Assembly* a)
{
	...
	return filter.region.contains(pos2d, wbx::Network::DistributedPhysics::CLIENT_SLOP());
}
	...
	case SimSendFilter::dPhysClient:
		{
			...
			else
            {
				if (inClientSimRegion(a) 
				    ...
				    && (a->getAssemblyPrimitive()->getNetworkOwner() != wbx::Network::NetworkOwner::ServerUnassigned())
...
{
	WBXASSERT((filter.mode == SimSendFilter::dPhysClient) || (filter.mode == SimSendFilter::dPhysServer));
...
{
	WBXASSERT(Mechanism::isMovingAssemblyRoot(a));
	WBXASSERT(!a->computeIsGrounded());
...
void SpatialFilter::onAssemblyRootRemoving(Assembly* a) 
{
	if (a->inPipeline()) {
		...
		WBXASSERT(a->getFilterPhase() == Assembly::NOT_ASSIGNED);
	}
}
