...
public:
    ...
    : data(geometry)
    {
        ...
        WBXPROFILER_COUNTER_ADD("memory/terrain/physics", getMemorySize());
    }
    
    ~ChunkMeshShape()
    {
        WBXPROFILER_COUNTER_SUB("memory/terrain/physics", getMemorySize());
    }
...
struct SmoothClusterGeometry::ChunkMesh
{
    ...
    {
		WBXPROFILER_SCOPE("Physics", "generateShape");

        WBXASSERT(state == State_Dummy);
		state = State_Ready;
...
{
	grid = WBX_static_cast<MegaClusterInstance*>(p->getOwner())->getSmoothGrid();
...
bool SmoothClusterGeometry::hitTest(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal)
{
...
bool SmoothClusterGeometry::hitTestTerrain(const WBXRay& rayInMe, Vector3& localHitPoint, int& surfId, CoordinateFrame& surfCf)
{
	Vector3 normal;
    unsigned char material;
	...
	if (castRay(rayInMe, localHitPoint, normal, material,  MC_SEARCH_RAY_MAX, /* ignoreWater= */ true))
	{
		...
		WBXASSERT(surfId != NORM_UNDEFINED);
...
bool SmoothClusterGeometry::castRay(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, unsigned char& surfaceMaterial, float maxDistance, bool ignoreWater)
{
	Vector3 from = rayInMe.origin();
	Vector3 to = rayInMe.origin() + maxDistance * rayInMe.direction();

    MegaClusterInstance* mci = WBX_static_cast<MegaClusterInstance*>(myPrim->getOwner());
...
{
	Voxel2::Grid* grid = WBX_static_cast<MegaClusterInstance*>(myPrim->getOwner())->getSmoothGrid();
	...
	for (size_t j = 0; j < visitCount; ++j)
	{
		...
		if (mesh->shape && mesh->shape.unique())
		{
			...
			if (gcUnusedMemory > size_t(FInt::SmoothTerrainPhysicsCacheSize))
			{
				...
				WBXASSERT(mesh->state == ChunkMesh::State_Ready);
