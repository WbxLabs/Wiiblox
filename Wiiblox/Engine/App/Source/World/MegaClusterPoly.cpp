...
MegaClusterPoly::MegaClusterPoly(Primitive* p)
    : myPrim(p)
{
	Grid* grid = WBX_static_cast<MegaClusterInstance*>(myPrim->getOwner())->getVoxelGrid();
...
bool MegaClusterPoly::hitTest(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, float searchRayMax, bool treatCellsAsBlocks, bool ignoreWater)
{
    int dummySurf = -1;
...
bool MegaClusterPoly::hitTestTerrain(const WBXRay& rayInMe, Vector3& localHitPoint, int& surfId, CoordinateFrame& surfCf)
{
...
bool MegaClusterPoly::hitTestMC(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, int& surfId, CoordinateFrame& surfaceCf, float searchRayMax, bool treatCellsAsBlocks, bool ignoreWater)
{
	...
	for (int index = 0; index < 3; ++index)
    {
		...
		WBXASSERT(boost::math::isfinite(rayInMe.origin()[index]));
		WBXASSERT(boost::math::isfinite(rayInMe.direction()[index]));
...
{
	WBXASSERT(false); // not implemented
    return false;
}
...
std::vector<Vector3> MegaClusterPoly::findCellIntersectionWithGeom( const Vector3int16& cell, const CoordinateFrame& myCf, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId ) const
{
    ...
    const Poly* otherPoly = WBX_static_cast<const Poly*>(&otherGeom);
...
bool MegaClusterPoly::hitLocationOnCornerWedgeCell(const WBXRay& rayInMe, const Vector3int16& testCell, const int& orientation, Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
...
bool MegaClusterPoly::hitLocationOnHorizontalWedgeCell(const WBXRay& rayInMe, const Vector3int16& testCell, const int& orientation, Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
    // Depending on orientation, specify faces and check for intersection
...
bool MegaClusterPoly::hitLocationOnVerticalWedgeCell(const WBXRay& rayInMe, const Vector3int16& testCell, const int& orientation, Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
    // Depending on orientation, specify faces and check for intersection
...
bool MegaClusterPoly::hitLocationOnInverseCornerWedgeCell(const WBXRay& rayInMe, const Vector3int16& testCell, const int& orientation, Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
    // Depending on orientation, specify faces and check for intersection
...
bool MegaClusterPoly::hitLocationOnBlockCell(const WBXRay& rayInMe, const Vector3int16& testCell, Vector3& localHitPoint, Vector3& surfaceNormal, int& surfId, CoordinateFrame& hitSurfaceCoord) const
{
    Vector3 t0(1, 1, 1);
    Vector3 t1(1, 1, -1);
    Vector3 t2(-1, 1, -1);
...
{
	WBXASSERT(shape >= 0 && shape < CELL_BLOCK_Empty);
