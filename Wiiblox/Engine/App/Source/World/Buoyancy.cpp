...
namespace WBX
{
	...
	void BuoyancyContact::deleteConnectors()
	{
		...
		for (size_t i = 0; i < connectors.size(); ++i) {
			WBXASSERT(!connectors[i]->isInKernel());
			delete connectors[i];
		}
	...
	{
		if (wbx::PartInstance::fromPrimitive(primitive)->getPartType() == CYLINDER_PART)
	...
	void BuoyancyContact::updateBuoyancyFloatingForce()
	{
		...
		if (getPrimitive(0)->getWorld()->getUsingNewPhysicalProperties())
		{
			...
			fullBuoyancy.y = - floaterPrim->getGeometry()->getVolume() * Units::kmsAccelerationToRbx( wbx::Constants::getKmsGravity() ) * waterDensity;
		}
		else
		{
			// Buoyant force:			Fb = p_water * grav_constant * Volume_displaced
			// Can be expanded into:	Fb = p_o * g * V / (p_w/p_o)
			// Unfortunately legacy Wiiblox never knew the correct mass of an object, so what the hell?
			fullBuoyancy.y = - floaterPrim->getConstBody()->getMass() * (Units::kmsAccelerationToRbx( wbx::Constants::getKmsGravity() ) / floaterPrim->getSpecificGravity());
		}
	}
	...
	void BuoyancyContact::updateConnectors()
	{
		...
		float viscosity_K = -waterViscosity / wbx::Constants::kernelDt() / connectors.size();
	...
	{
		WBXASSERT(connectors.size() == 1);  // Ball has only one connector	
	...
	wbx::Vector3 BuoyancyBallContact::getWaterVelocity( int )
	{
		Vector3 center = floaterPrim->getPV().position.translation;
	...
	void BuoyancyBoxContact::getSurfaceAreaInDirection(const Vector3& relativeVelocityDir, float& crossArea, float& tangentArea)
	{
		...
		WBXASSERT(Math::fuzzyEq(componentWeights.x + componentWeights.y + componentWeights.z, 1.0f, 1.0e-4f));
	...
	void wbx::BuoyancyCylinderContact::updateSubmergeRatio()
	{
