...
void MovingAssemblyStage::addAnimatedJoint(Joint* j)	
{
	if (j->canStepUi() && Joint::isMotorJoint(j))
	{
		...
		{
			WBXASSERT(!j->MovingAssemblyStageHook::is_linked());
...
void MovingAssemblyStage::addJoint(Joint* j)
{
	if (j->canStepUi()) {
		...
		WBXASSERT(animatedJoints.find(j) == animatedJoints.end());
	}
}
...
{
	a->putInStage(this);							
	WBX_static_cast<StepJointsStage*>(getDownstreamWS())->onSimulateAssemblyAdded(a);
}


void MovingAssemblyStage::onSimulateAssemblyRemoving(Assembly* a) 
{
	WBX_static_cast<StepJointsStage*>(getDownstreamWS())->onSimulateAssemblyRemoving(a);
