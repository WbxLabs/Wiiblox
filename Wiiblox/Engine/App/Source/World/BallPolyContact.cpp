...
BallPolyContact::BallPolyContact(Primitive* p0, Primitive* p1)
	: PolyContact(p0, p1)
{
	WBXASSERT(WBX_static_cast<Ball*>(p0->getGeometry()));
	WBXASSERT(WBX_static_cast<Poly*>(p1->getGeometry()));
}
...
{
	return WBX_static_cast<const Ball*>(getConstPrimitive(0)->getConstGeometry());
}

const Poly* BallPolyContact::poly() const
{
	return WBX_static_cast<const Poly*>(getConstPrimitive(1)->getConstGeometry());
}
