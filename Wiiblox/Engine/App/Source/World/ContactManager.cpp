...
bool ContactManager::intersectingMySimulation(Primitive* check, wbx::SystemAddress myLocalAddress, float overlapIgnored)
{
	WBXASSERT_FISHING(check->getNetworkOwner() != myLocalAddress);
...
shared_ptr<const Instances> ContactManager::getPartCollisions(Primitive* check)
{
	...
	while (c) {
		...
		if (check != other)
		{
			...
			if (wbx::Instance::fastDynamicCast<MegaClusterInstance>(partInstance))
			{
										...
										bool& stopped) const
{
	...
	for (int i = 0; i < primitives.size(); ++i) 
	{
		...
		if (!p->getBody()) {
			WBXCRASH("getSlowHit with deleted primitive");
		}
...
Primitive* ContactManager::getHit(	const WBXRay& worldRay, 
...
Primitive* ContactManager::getFastHit(	const WBXRay& worldRay, 
										const G3D::Array<const Primitive*>& ignorePrim,
										const HitTestFilter* filter,
                                        ...
                                        PartMaterial& surfaceMaterial) const
{
	WBXPROFILER_SCOPE("Physics", "getFastHit");
	...
	WBXASSERT(worldRay.direction().magnitude() < 5000.0f);
	...
	WBXASSERT(maxDistance < 5000.0f);
	maxDistance = std::min(5000.0f, maxDistance);
	const WBXRay unitRay = worldRay.unit();
			...
			for (DenseHashSet<Primitive*>::const_iterator it = primsFound.begin(); primsFound.end() != it; ++it) {
				...
				if (primExtents.overlapsOrTouches(searchExtents) && primExtents.max().y <= searchExtents.center().y) {
					...
					if (intersectingOthersChecks < FInt::IntersectingOthersCallsAllowedOnSpawn) {
						...
						boost::shared_ptr<wbx::PartInstance> tempPart;
...
{
	WBXASSERT(!p0->getGeometry()->isTerrain() && !p1->getGeometry()->isTerrain());
	...
	WBXCRASH("ContactManager::createContact"); 
	return NULL;
}

void ContactManager::onNewPair(Primitive* p0, Primitive* p1)
{
	WBXASSERT_VERY_FAST(Primitive::getContact(p0, p1) == NULL);
...
void ContactManager::releasePair(Primitive* p0, Primitive* p1)
{
    if( p0 && p1 )
    {
        ...
        else
        {
            ...
            {
		        WBXASSERT(0);			// this should never happen - something amiss in adding/removing contacts from Spatial Hash stage
	        }
        }
        WBXASSERT_VERY_FAST(Primitive::getContact(p0, p1) == NULL);		// should be clear now
    }
}
...
Primitive* ContactManager::getHitLegacy(const WBXRay& originDirection, 
...
{
	WBXASSERT(originDirection.direction().isUnit());
	WBXRay worldRay = WBXRay::fromOriginAndDirection(originDirection.origin(), originDirection.direction());
														 ...
														 const Vector3int16& extentSize, bool cellChanged)
{
    ...
    WBXASSERT(extentSize.x * extentSize.y * extentSize.z <= LARGE_TERRAIN_CONTACT_VOLUME);
...
{
	WBXASSERT(myMegaClusterPrim && getSmoothGrid());
    ...
    for (int i = 0; i < otherPrim->getNumContacts(); )
    {
        if (otherPrim->getContactOther(i) == myMegaClusterPrim)
		{
			...
			WBXASSERT(contactType == Contact::Contact_Cell || contactType == Contact::Contact_Buoyancy);
...
{
	WBXPROFILER_SCOPE("Physics", "applyDeferredTerrainChanges");
