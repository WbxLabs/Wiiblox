...
SendPhysics::~SendPhysics()
{
	WBXASSERT(simJobs.empty());
}

void SendPhysics::buildSimJob(SimJob* job)
{
	if (job->getAssembly()) {
		WBXASSERT(!job->getConstAssembly()->getConstSimJob());
...
void SendPhysics::destroySimJob(SimJob* job)
{
	...
	if (job->getAssembly()) {
		WBXASSERT(job->getAssembly()->getSimJob() == job);
...
//	WBXASSERT_VERY_FAST(Mechanism::isMovingAssemblyRoot(a));					// todo - get rid of this assert by refactoring primitive anchoring
