...
World::~World() 
{
	...
	WBXASSERT(groundPrimitive->getNumEdges() == 0);
	groundPrimitive->setWorld(NULL);

	WBXASSERT(numJoints == 0);
	WBXASSERT(numContacts == 0);
	WBXASSERT(primitives.size() == 0);
	WBXASSERT(breakableJoints.empty());
...
void World::loadProfilers(std::vector<wbx::Profiling::CodeProfiler*>& worldProfilers) const
{
...
{
	return WBX_static_cast<TreeStage*>(cleanStage->findStage(IStage::TREE_STAGE));
}
...
{
	return WBX_static_cast<SpatialFilter*>(cleanStage->findStage(IStage::SPATIAL_FILTER));
}
...
{
	return WBX_static_cast<AssemblyStage*>(cleanStage->findStage(IStage::ASSEMBLY_STAGE));
}
...
{
	return WBX_static_cast<MovingAssemblyStage*>(cleanStage->findStage(IStage::MOVING_ASSEMBLY_STAGE));
}
...
{
	return WBX_static_cast<StepJointsStage*>(cleanStage->findStage(IStage::STEP_JOINTS_STAGE));
}
...
{
	return WBX_static_cast<GroundStage*>(cleanStage->findStage(IStage::GROUND_STAGE));
}

HumanoidStage* World::getHumanoidStage() 
{
	return WBX_static_cast<HumanoidStage*>(cleanStage->findStage(IStage::HUMANOID_STAGE));
}

SleepStage* World::getSleepStage()
{
	return WBX_static_cast<SleepStage*>(cleanStage->findStage(IStage::SLEEP_STAGE));
}
...
{
	return WBX_static_cast<SimulateStage*>(cleanStage->findStage(IStage::SIMULATE_STAGE));
}
...
{
	WBXASSERT(p->getWorld());
	WBXASSERT(p->getAssembly());
	WBXASSERT(p->inPipeline());
	WBXASSERT(!inStepCode);
...
void World::assemble()
{
    WBXPROFILER_SCOPE("Physics", "assemble");
...
{
    WBXPROFILER_SCOPE("Physics", "notifyMovingAssemblies");

	wbx::Profiling::Mark mark(*profilingAssembly, false);
...
{
	WBXPROFILER_SCOPE("Physics", "uiStep");

	WBXASSERT(isAssembled());				// testing - assemble in 

	WBXASSERT(!inStepCode);

	if (longStep)
	{
		wbx::Profiling::Mark mark(*profilingBreak, false);
	...
	{
		wbx::Profiling::Mark mark(*profilingFilter, false);
		getSpatialFilter()->filterStep();
	}

	{
		wbx::Profiling::Mark mark(*profilingUiStep, false);
...
{
	WBXPROFILER_SCOPE("Physics", "doWorldStep");

	inStepCode = true;

	WBXASSERT(isAssembled());																// Assembly - Tree Stage
	...
	{
        WBXPROFILER_SCOPE("Physics", "updateBroadphase");

		wbx::Profiling::Mark mark(*contactManager->profilingBroadphase, false);
		std::for_each(
...
int World::updateStepsRequiredForCyclicExecutive(float desiredInterval)
{
	...
	if (DFFlag::CyclicExecutiveThrottlingCancelWorldStepAccum)
	{
		...
		// we can get into a state where Wiiblox wants to run 15 physics WorldSteps in 1 frame,
...
#ifdef _WIN32
	WBXASSERT_IF_VALIDATING(_CrtCheckMemory() != 0);
#endif

//	WBXASSERT(desiredInterval > 0.01);	// less than 100 FPS
//	WBXASSERT(desiredInterval < 0.10); // more than 10 FPS
//	WBXASSERT((worldStepId % Constants::worldStepsPerUiStep()) == 0);

	wbx::Profiling::Mark mark(*profilingWorldStep, true, true);

	if( !wbx::TaskScheduler::singleton().isCyclicExecutive() )
	{
	...
	if (longStep)
	{
		...
		{
			wbx::Profiling::Mark mark(*contactManager->profilingBroadphase, false);
			WBXASSERT(!((*i)->SimulateStageHook::is_linked()));
			WBXASSERT((*i)->getCurrentStage()->getStageType() != IStage::HUMANOID_STAGE);
...
{
	WBXASSERT(fallen.size() == 0);

	const SleepStage* sleepStage = getSleepStage();
	WBXASSERT(sleepStage);
...
// Re-entrancy problem - please capture and show complete stack to David B.
	WBXASSERT(inJointNotification == NULL);		
...
void World::removeFromBreakable(Joint* j)
{
	if (j->isBreakable()) {
		...
		WBXASSERT_SLOW(breakableJoints.find(j) == breakableJoints.end());
	}
}
...
{
	Primitive* parent = WBX_static_cast<Primitive*>(j->getParentSpanningNode());
	Primitive* child = WBX_static_cast<Primitive*>(j->getChildSpanningNode());

	if (parent && child && (parent != child))
	{
		if (Joint::isRigidJoint(j))
		{						
			RigidJoint* r = WBX_static_cast<RigidJoint*>(j);
...
//	WBXASSERT(numJoints == 0);
	...
	for (int i = 0; i < primitives.size(); ++i) 
	{
		...
		if(PartOperation* partOp = wbx::Instance::fastDynamicCast<PartOperation>(part))
		{
...
void World::insertPrimitive(Primitive* p) 
{
	...
	WBXASSERT(!p->getAssembly());
	WBXASSERT(!p->inPipeline());
	WBXASSERT(!p->getFirstEdge());
	WBXASSERT(!p->getNumEdges());
	WBXASSERT(!p->getWorld());
...
void World::destroyAutoJoints(Primitive* p, std::set<Primitive*>* ignoreGroup, bool includeExplicit, bool includeAuto)
{
	...
	WBXASSERT(startIndex == tempJoints.size());
}
...
{
	WBXASSERT(usingPGSSolver == getKernel()->getUsingPGSSolver());
...
void World::sendAnalytics(void)
{
	if(passCount > 0.0)
	{
		...
		wbx::Analytics::EphemeralCounter::reportStats("SmoothnessHumanoidStopsperK", (int)stopsperk, false);
		passCount = 0.0;
		errorCount = 0.0;
	}
	...
	{
		wbx::Analytics::EphemeralCounter::reportStats("SmoothnessHumanoidFrameinfosSize", (int)( .5 + frameinfosSize/frameinfosCount), false);
		wbx::Analytics::EphemeralCounter::reportStats("SmoothnessHumanoidFrameinfosTarget", (int)( .5 + frameinfosTarget/frameinfosCount), false);
		wbx::Analytics::EphemeralCounter::reportStats("SmoothnessHumanoidTargetDelayTenths", (int)( .5 + targetDelayTenths/frameinfosCount), false);
		wbx::Analytics::EphemeralCounter::reportStats("SmoothnessHumanoidFrameinfosSizeTenths", (int)( .5 + infosSizeTenths/frameinfosCount), false);
		wbx::Analytics::EphemeralCounter::reportStats("SmoothnessHumanoidMaxDelta", (int)( .5 + maxDelta/frameinfosCount), false);
