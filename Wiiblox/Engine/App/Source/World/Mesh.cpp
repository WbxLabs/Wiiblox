...
void Mesh::makeParallelRamp( const Vector3& size, Vector3& cofm )
{
	...
	//float connectionThickness = wbx::PartInstance::brickHeight();
...
Edge* Mesh::findOrMakeEdge(size_t v0, size_t v1)
{
	...
	if (Edge* found = vert0->findEdge(vert1)) {
		WBXASSERT(found->getVertex(NULL, 0) == vert1);		// should be backwards - this is second face
		WBXASSERT(found->getVertex(NULL, 1) == vert0);
		WBXASSERT(found->getForward());
		WBXASSERT(!found->getBackward());
		return found;
	}
	else {
...
{
	WBXASSERT(pointInMesh(inside));
	WBXASSERT(!pointInMesh(outside));
	
	WBXRay ray = WBXRay::fromOriginAndDirection(inside, (outside-inside).direction());
...
{
	WBXASSERT(!pointInMesh(p0));
	WBXASSERT(!pointInMesh(p1));
	f0 = NULL;
	f1 = NULL;
...
bool Mesh::rayIntersectsFace(const WBXRay& ray, const Face* face, Vector3& intersection) const
{
...
bool Mesh::hitTest(const WBXRay& ray, Vector3& hitPoint, Vector3& surfaceNormal) const
{
...
{
	WBXASSERT(pointInInternalExtrusion(pBelowInside));
	WBXASSERT(!pointInInternalExtrusion(pBelowOutside));
...
{
	WBXASSERT(!pointInInternalExtrusion(p0));
	WBXASSERT(!pointInInternalExtrusion(p1));
