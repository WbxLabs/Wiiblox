...
{
	return WBX_static_cast<const Poly*>(primitive[0]->getConstGeometry());
}

const Poly* PolyCellPair::poly1() const
{
	return WBX_static_cast<const Poly*>(primitive[1]->getConstGeometry());
}
...
{
	WBXASSERT(verticesInObject.size() == 0);
...
bool CellFaceFacePair::pairIsValid()
{
	...
	primitive[0]->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER ? 
		terrain0 = WBX_static_cast<MegaClusterInstance*>(primitive[0]->getOwner()) :
		terrain1 = WBX_static_cast<MegaClusterInstance*>(primitive[1]->getOwner());

	WBXASSERT(primitive[0]->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER || primitive[1]->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER);
								...
								ConnectorArray& newConnectors)
{
	...
	WBXASSERT(!Math::isNanInf(answer->computeOverlap()));
...
EdgeEdgeConnector* CellEdgeEdgePair::newEdgeEdgeConnector()
{
	EdgeEdgeConnector* answer = new EdgeEdgeConnector(	primitive[0]->getBody(),
														...
														bestEdge0->getId(),
														bestEdge1->getId()			);		
	WBXASSERT(answer->computeOverlap());
	return answer;
}	
