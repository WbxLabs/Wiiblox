...
{
	return WBX_static_cast<EdgeStage*>(getDownstreamWS());
}
...
{
	WBXASSERT(!Joint::findConstJoint(p, Joint::ANCHOR_JOINT));
	WBXASSERT(!Joint::findConstJoint(p, Joint::FREE_JOINT));
...
void GroundStage::onEdgeAdded(Edge* e)
{
	if (Joint::isKernelJoint(e)) {
		onKernelJointAdded(WBX_static_cast<KernelJoint*>(e));
	}
	...
	if (Joint::isRigidJoint(e)) {	// do this after - uses the primitive list
		checkForFreeGroundJoint(WBX_static_cast<RigidJoint*>(e));
	}
}
...
void GroundStage::onEdgeRemoving(Edge* e)
{
	...
	if (Joint::isKernelJoint(e)) {
		onKernelJointRemoving(WBX_static_cast<KernelJoint*>(e));
	}
}
...
{
	WBXASSERT(k->getPrimitive(0));
	WBXASSERT(k->getPrimitive(1));
	WBXASSERT(k->getPrimitive(0)->downstreamOfStage(this));
}
...
void GroundStage::checkForFreeGroundJoint(RigidJoint* r)
{
	...
	WBXASSERT(p0->downstreamOfStage(this));
	WBXASSERT(p1->downstreamOfStage(this));
