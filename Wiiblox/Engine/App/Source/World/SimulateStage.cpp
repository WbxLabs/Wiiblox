...
{
	WBXASSERT(movingDynamicAssemblies.size() == 0);
	WBXASSERT(realTimeAssemblies.size() == 0);
	WBXASSERT(movingAssemblyRoots.size() == 0);
}
...
{
	WBXASSERT(!a->getAssemblyPrimitive()->requestFixed());
...
void SimulateStage::putFirstMovingRootInSendPhysics(Assembly* a)
{
	...
	WBXASSERT(Mechanism::isMovingAssemblyRoot(movingRoot));
