...
{
	return WBX_static_cast<Ball*>(getPrimitive(i)->getGeometry());
}
...
Ball* BallBlockContact::ball() {
	return WBX_static_cast<Ball*>(ballPrim()->getGeometry());
}
Block* BallBlockContact::block() {
	return WBX_static_cast<Block*>(blockPrim()->getGeometry());
}
...
Block* BlockBlockContact::block(int i) {
	return WBX_static_cast<Block*>(getPrimitive(i)->getGeometry());
}
...
GeoPairConnector* BlockBlockContact::findGeoPairConnector(	Body* b0, 
															...
															int param1) 
{
    WBXASSERT( myData && contactParams );
											...
											int edge1) 
{
	...
	WBXASSERT(geoPair->getContactPoint().length < 0);
	WBXASSERT(geoPair->getContactPoint().normal.magnitude() > 0.99f);
												...
												NormalId planeFaceID) 
{
	...
	else
	{
		if(geoFeaturesOverlap(pointBody, planeBody, pointID, pointFaceID, planeFaceID))
		{
			...
			geoPair->updateContactPoint();
			getKernel()->insertConnector(geoPair);
			WBXASSERT(geoPair->getContactPoint().length < -ContactConnector::overlapGoal() + 0.001);
			WBXASSERT(geoPair->getContactPoint().normal.magnitude() > 0.99f);
