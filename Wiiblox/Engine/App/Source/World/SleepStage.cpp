...
{
	WBXASSERT(numContactsInStage == 0);
	WBXASSERT(numContactsInKernel == 0);
	WBXASSERT(debugReentrant == 0);

	WBXASSERT(recursiveWakePending.empty());				// only on impact...
	WBXASSERT(wakePending.empty());
	WBXASSERT(awake.empty());
	WBXASSERT(sleepingChecking.empty());					// Edges that are awake
	WBXASSERT(sleepingDeeply.empty());						// no Edges that are awake
	WBXASSERT(removing.empty());
	WBXASSERT(steppingContacts[Sim::CAN_NOT_THROTTLE].size() == 0);
	WBXASSERT(touchingContacts[Sim::CAN_NOT_THROTTLE].size() == 0);
	WBXASSERT(steppingContacts[Sim::CAN_THROTTLE].size() == 0);
	WBXASSERT(touchingContacts[Sim::CAN_THROTTLE].size() == 0);
}
...
bool SleepStage::validate()
{
	for (AssemblySetIt aIt = awake.begin(); aIt != awake.end(); ++aIt) 
	{
		...
		WBXASSERT(assembly->getAssemblyState() == Sim::AWAKE);
...
{
	WBXPROFILER_SCOPE("Physics", "stepSleepStage");
	...
	WBXASSERT_IF_VALIDATING(validate());

	stepAssembliesRecursiveWakePending();
	stepAssembliesWakePending();

	WBXASSERT(wakePending.empty());
	WBXASSERT(recursiveWakePending.empty());
	...
	WBXASSERT(recursiveWakePending.empty());
	WBXASSERT_IF_VALIDATING(validate());
	WBXASSERT_IF_VALIDATING(validateJoints());
}

void SleepStage::doContacts(ContactLists& contactLists)
{
	wbx::Profiling::Mark mark(*profilingCollision, false);
...
{
    WBXPROFILER_SCOPE("Physics", "stepAssembliesRecursiveWakePending");
...
{
    WBXPROFILER_SCOPE("Physics", "stepAssembliesWakePending");
...
{
	wbx::Profiling::Mark mark(*profilingWake, false);
	WBXASSERT_IF_VALIDATING(validate());
	...
	{
		Assembly* a = *it;
		WBXASSERT(a->getAssemblyState() == checkState);
		WBXASSERT(a->getRecursiveDepth() == -1);
	...
	while (!aDeque.empty())
	{
		...
		aDeque.pop_front();
		WBXASSERT(a->getRecursivePassId() == recursivePassId);
		WBXASSERT(a->getRecursiveDepth() >= 0);
		WBXASSERT(a->getRecursiveDepth() <= maxDepth);
...
{
	WBXASSERT(e->getThrottleType() != Sim::UNDEFINED_THROTTLE);
...
{
	WBXPROFILER_SCOPE("Physics", "stepContacts");

	WBXASSERT(toSleeping.empty());
	WBXASSERT(toStepping.empty());
	WBXASSERT(toContacting.empty());
	WBXASSERT(toContactingSleeping.empty());
	...
	for (int i = 0; i < contacts.size(); ++i) 
	{
		...
		WBXASSERT(initialSize == contacts.size());
		WBXASSERT(!throttling || !canThrottle(c));	// if throttling, the contact should be one that "can't"
		WBXASSERT(c->inOrDownstreamOfStage(this));
		WBXASSERT(p0);
		WBXASSERT(p1);
		WBXASSERT(p0->getAssembly());
		WBXASSERT(p1->getAssembly());

#ifdef __WBX_CRASH_ASSERT_HP
		Joint* test = Primitive::getJoint(p0, p1);
		WBXASSERT_VERY_FAST(!test || !test->inOrDownstreamOfStage(this));
#endif
		...
		WBXASSERT(c->getEdgeState() != Sim::UNDEFINED);
	...
	WBXPROFILER_LABELF("Physics", "%d contacts, %d connectors", initialSize, numConnectors);
}

void SleepStage::stepJoints()
{
	wbx::Profiling::Mark mark(*profilingJointSleep, false);
	WBXASSERT(toSleepingJoint.empty());
	...
	{
		Joint* j = *it;
		WBXASSERT(j->inOrDownstreamOfStage(this));
...
{
	wbx::Profiling::Mark mark(*profilingWake, false);
	WBXASSERT(toDeep.empty());
	WBXASSERT(toSleepingChecking.empty());
...
{
	wbx::Profiling::Mark mark(*profilingSleep, false);
	WBXASSERT(toWake.empty());
	WBXASSERT(toDeep.empty());

	for (AssemblySetIt it = sleepingChecking.begin(); it != sleepingChecking.end(); ++it) 
	{
		Assembly* assembly = *it;
		WBXASSERT(assembly->getAssemblyState() == Sim::SLEEPING_CHECKING);
			...
			if (e->inOrDownstreamOfStage(this)) 
			{
				...
				if (isAffecting(e))	
				{
					...
					if (currentDepth <  maxDepth) 
					{
						...
						{
							WBXASSERT(other->getRecursiveDepth() == -1);
...
{
	WBXASSERT(e->inOrDownstreamOfStage(this));
	WBXASSERT(e->getEdgeState() != Sim::UNDEFINED);
...
void SleepStage::wakeEdge(Edge* e)
{
	...
	if (Contact::isContact(e)) {
		if (state == Sim::CONTACTING_SLEEPING) {
			changeContactState(WBX_static_cast<Contact*>(e), Sim::CONTACTING);
		}
		else if (state == Sim::SLEEPING) {
			changeContactState(WBX_static_cast<Contact*>(e), Sim::STEPPING);
		}
	}
	else {
		WBXASSERT(Joint::isJoint(e));
		if (state == Sim::SLEEPING) {
			changeJointState(WBX_static_cast<Joint*>(e), Sim::STEPPING);
		}
	}
}
...
void SleepStage::changeContactState(Contact* c, Sim::EdgeState newState)
{
	...
	WBXASSERT(currentState != newState);
...
{
	WBXASSERT(!debugReentrant);
	debugReentrant = true;

	if (!a->inOrDownstreamOfStage(this)) {
		WBXASSERT(a->inStage(IStage::HUMANOID_STAGE));
	}
	else {
		Sim::AssemblyState currentState = a->getAssemblyState();
		WBXASSERT(currentState != newState);
		WBXASSERT(currentState != Sim::ANCHORED);
		...
		if (newState == Sim::AWAKE) {		// INTO KERNEL
			if (!a->downstreamOfStage(this)) {
				...
				WBX_static_cast<SimulateStage*>(getDownstreamWS())->onAssemblyAdded(a);
			}
		}
		if (Sim::outOfKernelAssemblyState(newState)) {		// OUT OF KERNEL - note "wake pending" do not force into or out of kernel"
			if (a->downstreamOfStage(this)) {
				...
				WBX_static_cast<SimulateStage*>(getDownstreamWS())->onAssemblyRemoving(a);
			}
		}
...
SleepStage::AssemblySet& SleepStage::stateToSet(Sim::AssemblyState state)
{
	...
	default:							WBXASSERT(0);	return awake;
	}
}
...
{
	WBXASSERT(!assembly->computeIsGrounded());
	WBXASSERT(assembly->getAssemblyState() == Sim::ANCHORED);
	...
	else 
	{
		...
		WBXASSERT(ok);
		WBXASSERT(assembly->getAssemblyState() == Sim::WAKE_PENDING);
	}
}

void SleepStage::onAssemblyRemoving(Assembly* assembly)
{
	WBXASSERT(assembly->inOrDownstreamOfStage(this));
	WBXASSERT(assembly->getAssemblyState() != Sim::ANCHORED);
	WBXASSERT(assembly->getAssemblyState() != Sim::REMOVING);
...
void SleepStage::onEdgeAdded(Edge* e)
{
	...
	if (Contact::isContact(e)) {
		Contact* c = WBX_static_cast<Contact*>(e);
...
void SleepStage::onEdgeRemoving(Edge* e)
{
	...
	if (Contact::isContact(e)) {
		...
		if (e->getEdgeState() != Sim::SLEEPING) {
			changeContactState(WBX_static_cast<Contact*>(e), Sim::SLEEPING);
		}
		numContactsInStage--;
	}
	else {
		if (e->getEdgeState() == Sim::SLEEPING) {
			...
			changeJointState(WBX_static_cast<Joint*>(e), Sim::SLEEPING);
		}
	}

	WBXASSERT(e->inStage(this));
	WBXASSERT(e->getEdgeState() == Sim::SLEEPING);
	WBXASSERT(e->getThrottleType() != Sim::UNDEFINED_THROTTLE);
