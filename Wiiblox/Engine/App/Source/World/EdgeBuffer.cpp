...
{
	WBXASSERT_SLOW(debugAddAssembly(a));
...
void EdgeBuffer::beforeAssemblyRemoving(Assembly* a)
{
	...
	WBXASSERT_SLOW(debugRemoveAssembly(a));
}
...
{
	if (e->inStage(this)) {
		WBXASSERT(Joint::isKinematicJoint(e) == Joint::isMotorJoint(e));
		...
		if (ok) {
			...
			WBXASSERT_SLOW(debugPushEdgeToDownstream(e));
		}
	}
}
...
{
	WBXASSERT(e->inStage(this));
	WBXASSERT(Joint::isMotorJoint(e));

	Assembly* a0 = e->getPrimitive(0)->getAssembly();

	WBX_ENGINE_ASSERT(a0 == e->getPrimitive(1)->getAssembly());
...
void EdgeBuffer::removeEdgeIfDownstream(Edge* e)
{
	if (e->downstreamOfStage(this)) {
		...
		WBXASSERT_SLOW(debugRemoveEdgeFromDownstream(e));
	}
}
...
{
	WBXASSERT(!Joint::isGroundJoint(e));
	WBXASSERT(!Joint::isRigidJoint(e));
	e->putInStage(this);
	pushEdgeIfOk(e);
}
...
bool EdgeBuffer::debugPushEdgeToDownstream(Edge* e)
{
	...
	WBXASSERT(e->downstreamOfStage(this));
...
bool EdgeBuffer::debugAddAssembly(Assembly* a)
{
	...
	//WBXASSERT(assemblyEdges.emptyLeft(a));
	return true;
}
