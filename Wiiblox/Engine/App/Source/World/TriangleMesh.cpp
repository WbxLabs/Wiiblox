...
bool TriangleMesh::validateIsBlockData(const std::string& data)
{
	...
	boost::call_once(flag, boost::bind(&WiibloxGoogleAnalytics::trackEvent,
...
bool TriangleMesh::hitTest(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal)
{
	if (!kdTreeMesh)
		return false;
