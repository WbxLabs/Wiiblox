...
, Edge(NULL, NULL)
{
	...
	WBXASSERT(jointCoord0 == CoordinateFrame());
	WBXASSERT(jointCoord1 == CoordinateFrame());
}
...
{
	WBXASSERT((value == NULL) != (jointOwner == NULL));
	jointOwner = value;
}
...
static int biggerJointGuid2(const Joint* j0, const Joint* j1)
{
	...
	WBXASSERT(answer == Guid::compare(j01, j00, j10, j11));
	WBXASSERT(answer == Guid::compare(j00, j01, j11, j10));
	WBXASSERT(answer == -Guid::compare(j11, j10, j00, j01));
	WBXASSERT(answer == -Guid::compare(j10, j11, j01, j00));
	return answer;
}
...
bool Joint::isHeavierThan(const SpanningEdge* other) const
{
	...
	const Joint* j1 = WBX_static_cast<const Joint*>(other);
...
{
	Primitive* p = WBX_static_cast<Primitive*>(n);
...
{
	const Primitive* p = WBX_static_cast<const Primitive*>(n);
...
wbx::SurfaceType Joint::getSurfaceTypeFromNormal( const Primitive& primitive, const NormalId& normalId )
{
