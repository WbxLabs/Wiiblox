...
bool Block::hitTest(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal)
{
...
const Vector3* Block::getEdgePoint(const Vector3int16& clip, wbx::NormalId& normalID) const {
...
const Vector3* Block::getPlanePoint(const Vector3int16& clip, wbx::NormalId& normalID) const {

	// normal is from the negative "zero" point positive
	if (clip.x != 0) {
		normalID = clip.x > 0 ? wbx::NORM_X : wbx::NORM_X_NEG;
		int x = clip.x > 0 ? 0 : 1;
		return &vertices[x*4];		// either +x,+y,+z or -x,+y,+z
	}
	if (clip.y != 0) {
		normalID = clip.y > 0 ? wbx::NORM_Y : wbx::NORM_Y_NEG;
		int y = clip.y > 0 ? 0 : 1;
		return &vertices[y*2];		
	}
	WBXASSERT(clip.z);
	{
		normalID = clip.z > 0 ? wbx::NORM_Z : wbx::NORM_Z_NEG;
		int z = clip.z > 0 ? 0 : 1;
		return &vertices[z];
	}
}
...
GeoPairType Block::getBallInsideInfo(const Vector3& ray, const Vector3* &offset, wbx::NormalId& normalID) 
{
	...
	for (int i = 0; i < 3; i++) {
		...
		if (temp < min) {
			min = temp;
			normalID = static_cast<wbx::NormalId>(i);
		}
		temp = ray[i] + l[i];
		if (temp < min) {
			min = temp;
			normalID = static_cast<wbx::NormalId>(i+3);
		}
	}
	WBXASSERT(min != FLT_MAX);

	offset = (normalID > wbx::NORM_Z) ? &vertices[7] : &vertices[0];
	return BALL_PLANE_PAIR;
}
...
Vector2 Block::getProjectedVertex(const Vector3& vertex, wbx::NormalId normalID) {

	Vector2 ans;
	switch (normalID) {	
				case (wbx::NORM_X):		ans.x = vertex.y;	ans.y = vertex.z;	return ans;
				case (wbx::NORM_Y):		ans.x = vertex.z;	ans.y = vertex.x;	return ans;
				case (wbx::NORM_Z):		ans.x = vertex.x;	ans.y = vertex.y;	return ans;
				case (wbx::NORM_X_NEG):	ans.x = vertex.z;	ans.y = vertex.y;	return ans;
				case (wbx::NORM_Y_NEG):	ans.x = vertex.x;	ans.y = vertex.z;	return ans;
				case (wbx::NORM_Z_NEG):	ans.x = vertex.y;	ans.y = vertex.x;	return ans;
