...
MultiJoint::~MultiJoint() 
{
	WBXASSERT(connector[0] == NULL);
	WBXASSERT(numConnector == 0);
}
...
void MultiJoint::addToMultiJoint(Point* point0, Point* point1, Connector* _connector)
{
	...
	WBXASSERT_IF_VALIDATING(validateMultiJoint());
}

Point* MultiJoint::getPoint(int id)
{
	WBXASSERT(id < 8);
	WBXASSERT(point[id] != NULL);
	WBXASSERT(point[id]->getBody() == getPrimitive(id % 2)->getBody());
	return point[id];
}
...
{
	WBXASSERT(id < 4);
	WBXASSERT(connector[id] != NULL);
	return connector[id];
}

#ifdef __WBX_VALIDATE_ASSERT
bool MultiJoint::pointsAligned() const
{
	WBXASSERT(numBreakingConnectors <= numConnector);
...
bool MultiJoint::validateMultiJoint()
{
	...
	{{
		Point *p = point[i*2 + j];
		WBXASSERT(p->getBody() == this->getPrimitive(j)->getBody());
		
		Connector* c = connector[i];
		WBXASSERT(c->getBody((Connector::BodyIndex)j) == this->getPrimitive(j)->getBody());
	}}
#endif
	return true;
}
...
//	WBXASSERT_IF_VALIDATING(pointsAligned());

	WBXASSERT_IF_VALIDATING(validateMultiJoint());

	for (int i = 0; i < numConnector; i++) {
		WBXASSERT(connector[i]);
