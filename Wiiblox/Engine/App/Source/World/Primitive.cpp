...
void Primitive::setSizeMultiplier(SizeMultiplier value)
{
	if (value != sizeMultiplier) 
	{
		if (!world) {
			...
			WBXASSERT(0);		// show to dave - changing primitive name to/from "Torso" while in world
		}
	}
}
...
{
	WBXASSERT(!Math::isNanInfVector3(getBody()->getPos()));
...
const Extents& Primitive::getFastFuzzyExtents()  
{
	...
	WBXASSERT_VERY_FAST(computeFuzzyExtents() == fuzzyExtents);	
	WBXASSERT_VERY_FAST(fuzzyExtentsStateId == getBody()->getStateIndex());
...
Primitive* Primitive::downstreamPrimitive(Joint* j)
{
	...
	if (!p1) {
		...
		Body* b0 = p0->getBody();
		Body* b1 = p1->getBody();
		WBXASSERT((b0->getParent() == b1) || (b1->getParent() == b0));
...
void EdgeList::removeEdge(Edge* e)
{
	...
	WBXASSERT(removeIndex >= 0);
	WBXASSERT(list[removeIndex].edge == e);
...
{
	return WBX_static_cast<Joint*>(joints.getEdge(id));
}
...
{
	return WBX_static_cast<Contact*>(contacts.getEdge(id));
}
...
void Primitive::insertEdge(Edge* e)
{
	...
	if (Joint::isJoint(e)) {
		...
		if (p1) {
			...
			WBXASSERT(AnchorJoint::isAnchorJoint(WBX_static_cast<Joint*>(e)) || FreeJoint::isFreeJoint(WBX_static_cast<Joint*>(e)));
		}
	}
	else {
		WBXASSERT_VERY_FAST(Contact::isContact(e));
...
{
	return WBX_static_cast<Joint*>(joints.getFirst());
}
...
{
	WBXASSERT_VERY_FAST(prev);

	Edge* e = joints.getNext(this, const_cast<Joint*>(prev));

	return WBX_static_cast<const Joint*>(e);
}
...
{
	return WBX_static_cast<Contact*>(contacts.getFirst());
}
...
Contact* Primitive::getNextContact(Contact* prev) 
{
	...
	return WBX_static_cast<Contact*>(contacts.getNext(this, prev));
}
...
RigidJoint* Primitive::getFirstRigidAt(Joint* start)
{
	while (start) {
		if (RigidJoint::isRigidJoint(start)) {
			return WBX_static_cast<RigidJoint*>(start);
		}
...
Joint* Primitive::getJoint(Primitive* p0, Primitive* p1, int index)
{
	...
	for (int i = 0; i < leastJoints->getNumJoints(); ++i)
	{
		...
		if (other == mostJoints)
		{
			WBXASSERT(leastJoints->getJoint(i)->links(p0, p1));
			if (foundId == index)
			{
...
Contact* Primitive::getContact(Primitive* p0, Primitive* p1)
{
	...
	for (int i = 0; i < leastContacts->getNumContacts(); ++i)
	{
		...
		{
			WBXASSERT(leastContacts->getContact(i)->links(p0, p1));
...
static void reportOverlap(Primitive* touchReporting, Primitive* touchOther)
{
	...
	WBXASSERT(touchReportingAssembly);
	WBXASSERT(touchOtherAssembly);
...
{
	// This if statement is because we have WBX Assert that we don't want to trigger
...
{
	WBXASSERT_VERY_FAST(!hasAutoJoints());
...
bool Primitive::hitTest(const WBXRay& worldRay, Vector3& worldHitPoint, Vector3& surfaceNormal)
{
    const CoordinateFrame& cframe = getBody()->getCoordinateFrame();

	WBXRay localRay = cframe.toObjectSpace(worldRay);
...
Vector3 Primitive::clipToSafeSize(const Vector3& newSize)
{
	...
	if ((safeSize.x * safeSize.y * safeSize.z) > maxVolume)			// up from 1 million
	{
		...
		WBXASSERT_VERY_FAST(safeSize.x * safeSize.y * safeSize.z <= (maxVolume * 1.01f));
	}
	return safeSize;
}
...
void Primitive::setPV(const PV& newPv)
{
	...
	if (newPv != bodyPv) 
	{
		...
		WBXASSERT((world == NULL) == (assembly == NULL));
								...
								SurfaceType			newSurfaceType)
{
	...
	WBXASSERT_IF_VALIDATING(!hasAutoJoints());
...
void Primitive::calculateSortSize()
{
	...
	WBXASSERT(planar * 1000.0f + 1 < (float)std::numeric_limits<unsigned int>::max());
	WBXASSERT(planar > 0.0f);
