...
namespace WBX
{
	bool WebParser::parseWebListResponse(std::istream& stream, wbx::Reflection::ValueArray& result)
	{
	...
	bool WebParser::parseWebGenericResponse(std::istream& stream, wbx::Reflection::Variant& result)
	{
		TextXmlParser machine(stream.rdbuf());
		std::auto_ptr<XmlElement> root(machine.parse());
		return parseWebGenericResponse(root.get(), result);
	}
	bool WebParser::parseWebGenericResponse(const XmlElement* root, wbx::Reflection::Variant& result)
	{
		if (root->getTag() == tag_WebTable) 
		{
			shared_ptr<Reflection::ValueMap> table(new Reflection::ValueMap());
			if(loadTable(root, *table)){
				result = shared_ptr<const wbx::Reflection::ValueMap>(table);
				return true;
			}
		}
		else if(root->getTag() == tag_WebList)
		{
			shared_ptr<Reflection::ValueArray> list(wbx::make_shared<Reflection::ValueArray>());
			if(loadList(root, *list)){
				result = shared_ptr<const wbx::Reflection::ValueArray>(list);
				return true;
			}
		}
	...
	bool WebParser::loadList(const XmlElement* listElement, wbx::Reflection::ValueArray& result)
	{
	...
	bool WebParser::loadTable(const XmlElement* tableElement, wbx::Reflection::ValueMap& result)
	{
	...
	bool WebParser::loadValue(const XmlElement* valueElement, wbx::Reflection::Variant& value)
	{
		if(const XmlElement* tableElement = valueElement->findFirstChildByTag(tag_WebTable)){
			shared_ptr<Reflection::ValueMap> table(new Reflection::ValueMap());
			if(loadTable(tableElement, *table)){
				value = shared_ptr<const wbx::Reflection::ValueMap>(table);
				return true;
			}
		}
			...
			if(loadList(tableElement, *list)){
				value = shared_ptr<const wbx::Reflection::ValueArray>(list);
				return true;
			}
		}
		else{
			if(const XmlAttribute* typeAttribute = valueElement->findAttribute(tag_WebType))
			{
				...
				if(typeAttribute->getValue(type))
				{
					...
					{
						if(const XmlElement* WiibloxRoot = valueElement->findFirstChildByTag(tag_Wiiblox)){
							Serializer serializer;
							Instances instances;
							serializer.loadInstancesFromText(WiibloxRoot, instances);
							if(instances.size() == 1)
							{
								value = instances[0];
								return true;
	...
	bool WebParser::loadEntry(const XmlElement* entryElement, std::string& key, wbx::Reflection::Variant& value)
	{
		bool gotKey = false;
		bool gotValue = false;
		...
		try
		{
			if(propTree.size() > 0)
			{
				...
				for (boost::property_tree::ptree::const_iterator it = propTree.begin(); it != end; ++it)
				{
					...
					{
						shared_ptr<Reflection::ValueTable> subMap(wbx::make_shared<Reflection::ValueTable>());
						if(!legacyPopulateValueTableFromPtree(it->second,subMap))
							return false;

						Reflection::Variant variantValue = shared_ptr<const wbx::Reflection::ValueTable>(subMap);
		...
		if(propTree.size() > 0)
		{
			...
			for (boost::property_tree::ptree::const_iterator it = propTree.begin(); it != end; ++it)
			{
				...
				if(key.empty())
				{
					WBXASSERT(!subMap);
					if(!subArray)
						subArray = wbx::make_shared<Reflection::ValueArray>();
				...
				else
				{
					WBXASSERT(!subArray);
					if(!subMap)
						subMap = wbx::make_shared<Reflection::ValueTable>();
					(*subMap)[key] = value;
				}
	...
	Reflection::Variant populateValueTableFromRapidJson(rapidjson::Value& node)
	{
		...
		{
			shared_ptr<Reflection::ValueArray> subArray = wbx::make_shared<Reflection::ValueArray>();
	...
	bool WebParser::parseJSONObject(const std::string& rawWebResponse, Reflection::Variant& result)
	{
		...
		WBXASSERT(root.IsObject() || root.IsArray());
	...
	bool WebParser::parseJSONTable(const std::string& rawWebResponse, shared_ptr<const Reflection::ValueTable>& valueTable)
	{
		...
		valueTable = result.cast<shared_ptr<const wbx::Reflection::ValueTable> >();
		return true;
	...
	bool WebParser::parseJSONArray(const std::string& rawWebResponse, shared_ptr<const Reflection::ValueArray>& valueArray)
	{
		...
		valueArray = result.cast<shared_ptr<const wbx::Reflection::ValueArray> >();
		return true;
	...
	bool WebParser::legacyParseWebJSONResponse(std::stringstream& rawWebResponse, shared_ptr<const Reflection::ValueTable>& valueTable)
	{
		...
		shared_ptr<Reflection::ValueTable> table(wbx::make_shared<Reflection::ValueTable>());
