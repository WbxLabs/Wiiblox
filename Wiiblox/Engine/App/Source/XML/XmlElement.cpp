...
const wbx::Name& value_IDREF_null = Name::declare("null");
const wbx::Name& value_IDREF_nil = Name::declare("nil");
...
const XmlTag& tag_Wiiblox = Name::declare("Wiiblox");
...
bool XmlNameValuePair::isValueEqual(const wbx::Name* value) const {
	switch (valueType) {
	case STRING:
...
bool XmlNameValuePair::getValue(const wbx::Name*& value) const
{
	if (valueType==NAME) {
		value = nameValue;
		return true;
	...
	if (valueType==STRING) {
		value = &wbx::Name::declare(stringValue->c_str());
		clearValue();
		nameValue = value;
		valueType = NAME;
		return true;
...
template<>
bool XmlNameValuePair::isValueType<const wbx::Name*>() const
{
	return valueType==NAME;
}

template<>
bool XmlNameValuePair::isValueType<wbx::InstanceHandle>() const
{
...
bool XmlNameValuePair::getValue(wbx::ContentId& value) const
{
...
bool XmlNameValuePair::getValue(float& value) const
{
	...
	if (valueType==STRING) {
		if (wbx::StringConverter<float>::convertToValue(*stringValue, value)) {
			clearValue();
			floatValue = value;
			valueType = FLOAT;
			return true;
	...
	WBXASSERT(valueType!=DOUBLE);	// No provision (yet) for converting from double back to float
...
bool XmlNameValuePair::getValue(double& value) const
{
	...
	if (valueType==STRING) {
		if (wbx::StringConverter<double>::convertToValue(*stringValue, value)) {
			clearValue();
			doubleValue = value;
			valueType = DOUBLE;
			return true;
...
bool XmlNameValuePair::getValue(wbx::InstanceHandle &value) const {
	
	if (valueType==NAME && *nameValue==value_IDREF_null) {
		clearValue();
		handleValue = new wbx::InstanceHandle(NULL);
		valueType = HANDLE;
...
std::string XmlNameValuePair::toString(XmlWriter* writer) const
{
	...
	case BOOL:
		{
			 return wbx::StringConverter<bool>::convertToString(boolValue);
		};

	case INT:
		{
			 return wbx::StringConverter<int>::convertToString(intValue);
		};

	case UINT:
		{
			 return wbx::StringConverter<unsigned int>::convertToString(uintValue);
		};

	case FLOAT:
		{
			 return wbx::StringConverter<float>::convertToString(floatValue);
		};

	case DOUBLE:
		{
			 return wbx::StringConverter<double>::convertToString(doubleValue);
		};
