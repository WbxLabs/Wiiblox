	...
	while (true) {
		...
		if (isCloseTag(currentTag.c_str())) {
			...
			if (currentElement == NULL)
				throw wbx::runtime_error("TextXmlParser::parse - Got close tag %s without open tag.", currentTag.c_str());
			...
			if (newElement->getTag()==wbx::Reflection::Type::singleton<wbx::ContentId>().tag)
			{
				...
				else
				{
					...
					else
					{
						...
						if (tagName.compare(0, 6, "binary") == 0)	// The binary tag may have attributes, so we have to compare a substring
						{
							...
							wbx::StandardOut::singleton()->printf(wbx::MESSAGE_WARNING, "Not reading binary data");
							readText(false);
							newElement->setValue(wbx::ContentId());
						}
						...
						{
							newElement->setValue(wbx::ContentId(this->readText(true).c_str()));
						}
						else if (tag_null==tagName)
						{
							...
							throw wbx::runtime_error("TextXmlParser::parse - Unknown tag '%s'.", tagName.substr(0, 32).c_str());

						std::string closingTag = readTag();	// closing tag
						if (!isCloseTag(closingTag.c_str()))
							throw wbx::runtime_error("TextXmlParser::parse - '%s' should be a closing tag", closingTag.substr(0, 32).c_str());
					}
				}
			}
...
{
	// Special handling for wbx::ContentId
	// TODO: move to Reflection::Property?
	if (xmlNode->isValueType<wbx::ContentId>())
	{
