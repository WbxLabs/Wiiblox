...
namespace WBX { namespace Voxel2 {
    ...
    void MaterialTable::load(const std::string& file)
    {
       ...
       std::ifstream in(utf8_decode(file).c_str(), std::ios::in | std::ios::binary);
        ...
        if (!in)
            throw wbx::runtime_error("Error opening file %s", file.c_str());
        ...
        if (root.HasParseError())
			throw wbx::runtime_error("Failed to parse JSON: %s at %d", root.GetParseError(), int(root.GetErrorOffset()));
		...
		{
			throw wbx::runtime_error("Failed to find atlas definition for platform %s", platform.c_str());
		}
