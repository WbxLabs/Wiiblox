...
namespace WBX { namespace Voxel2 {
    ...
    static unsigned char readUInt8(const std::string& data, unsigned int& readOffset)
    {
        if (readOffset >= data.size())
            throw wbx::runtime_error("Error while decoding data: unexpected end at offset %u", readOffset);
    ...
    static void decodeChunk(const std::string& data, unsigned int& readOffset, Box& result, std::vector<Cell>& cells)
	{
        ...
        while (offset < cells.size())
		{
			...
			if (offset + run.second > cells.size())
				throw wbx::runtime_error("Error while decoding data: chunk overflow at %u cells", offset + run.second);
    ...
    struct DeallocateCells
    {
        ...
        {
            WBXPROFILER_COUNTER_SUB("memory/terrain/voxel", size);
	...
	{
        WBXASSERT(size != 0 && (size & (size - 1)) == 0);
    ...
    void Box::allocate()
    {
        ...
        WBXPROFILER_COUNTER_ADD("memory/terrain/voxel", size);
	...
	{
        WBXASSERT(std::find(listeners.begin(), listeners.end(), listener) == listeners.end());
	...
	void Grid::disconnectListener(GridListener* listener)
	{
		...
		WBXASSERT(it != listeners.end());
		listeners.erase(it);
	}
    
    Box Grid::read(const Region& region, int lod) const
    {
        WBXPROFILER_SCOPE("Voxel", "read");

		WBXASSERT(region.aligned(1 << lod));
    ...
    {
        WBXPROFILER_SCOPE("Voxel", "write");

		WBXASSERT(region.size() == box.getSize());
    ...
    void Grid::deserialize(const std::string& data)
	{
        ...
        if (version != 1)
			throw wbx::runtime_error("Error while decoding data: unsupported version");

		int chunkSizeLog2 = readUInt8(data, readOffset);
		int chunkSize = 1 << chunkSizeLog2;

		if (chunkSizeLog2 > 8)
			throw wbx::runtime_error("Error while decoding data: malformed chunk size");
