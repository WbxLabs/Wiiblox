...
namespace WBX
{
	...
	{
        static wbx::atomic<int> index(-1);
		return ++index;
	}

	shared_ptr<Instance> ServiceProvider::getPublicServiceByClassNameString(std::string sName)
	{
		const Name& serviceName = wbx::Name::lookup(sName);
		if (serviceName.empty())
			throw wbx::runtime_error("'%s' is not a valid Service name", sName.c_str());
	...
	shared_ptr<Instance> ServiceProvider::findServiceByClassName(const wbx::Name& className) const
	{
		std::map<const wbx::Name*, shared_ptr<Instance> >::iterator iter = serviceMap.find(&className);
	...
	{
		std::map<const wbx::Name*, shared_ptr<Instance> >::iterator iter = serviceMap.find(&instance->getClassName());
		if (iter!=serviceMap.end())
	...
	void ServiceProvider::onChildAdded(Instance* instance)
	{
		...
		WBXASSERT(Instance::fastDynamicCast<ServiceProvider>(instance)==NULL);
		...
		if (service!=NULL)
		{
			...
			if (instance->getClassName()!=wbx::Name::getNullName())
			{
				WBXASSERT_SLOW(findServiceByClassName(instance->getClassName())==NULL);
				instance->lockParent();
	...
	shared_ptr<Instance> ServiceProvider::createChild(const wbx::Name& className, wbx::CreatorRole creatorRole)
	{
	...
	shared_ptr<Instance> ServiceProvider::create(Instance* context, const wbx::Name& name)
	{
		while (context!=NULL)
		{
