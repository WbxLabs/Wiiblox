...
static Reflection::PropDescriptor<Instance, int> prop_dataCost("DataCost", category_Data, &Instance::getPersistentDataCost, NULL, Reflection::PropertyDescriptor::UI, Security::WiibloxPlace);
...
const Reflection::PropDescriptor<Instance, bool> Instance::propWiibloxLocked("WiibloxLocked", category_Data, &Instance::getWiibloxLocked, &Instance::setWiibloxLocked, Reflection::PropertyDescriptor::SCRIPTING, Security::Plugin);

Reflection::EventDesc<
	Instance, 
	void(shared_ptr<Instance>), 
	wbx::signal<void(shared_ptr<Instance>)>, 
	wbx::signal<void(shared_ptr<Instance>)>* (Instance::*)(bool)>
...
shared_ptr<Instance> Instance::createChild(const wbx::Name& className, wbx::CreatorRole creatorRole)
{
...
void Instance::readChild(const XmlElement* childElement, IReferenceBinder& binder, CreatorRole creatorRole)
{
	...
	if (childInstance!=NULL) {
		...
		message += "\"";
		wbx::StandardOut::singleton()->print(wbx::MESSAGE_WARNING, message.c_str());
#endif
...
void Instance::readChildren(const XmlElement* element, IReferenceBinder& binder, wbx::CreatorRole creatorRole)
{
	if (!element)
		return;
...
void Instance::read(const XmlElement* element, IReferenceBinder& binder, wbx::CreatorRole creatorRole)
{
	// Read the referent attribute, if it exists
...
void Instance::writeChildren(XmlElement* container, wbx::CreatorRole creatorRole, const SaveFilter saveFilter)
{
	writeChildren(container, itIsInScope, creatorRole, saveFilter);
}

void Instance::writeChildren(XmlElement* container, const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole, const SaveFilter saveFilter)
{
	if (children)
	{
...
void Instance::writeProperties(XmlElement* container) const
{
	...
	wbx::Reflection::ConstPropertyIterator iter = properties_begin();
...
XmlElement* Instance::writeXml(const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole)
{
	// TODO: archivable==false messes up functions like clone()
	//       find a better way to do this
	if (!getIsArchivable() || (getClassName()==wbx::Name::getNullName()))
		return NULL;
...
{
	securityCheck(wbx::Security::Context::current());
}

void Instance::securityCheck(wbx::Security::Context& context) const
{
...
{	
	...
	if (!ignoreLock && getIsParentLocked()) {
		std::string message = wbx::format("The Parent property of %s is locked, current parent: %s, new parent %s", getFullName().c_str(), 
	...
	if (this==newParent) {
		std::string message = wbx::format("Attempt to set %s as its own parent", getFullName().c_str());
		throw std::runtime_error(message);
	}

	if (this->isAncestorOf(newParent)) {
		WBXASSERT(newParent);
		std::string message = wbx::format("Attempt to set parent of %s to %s would result in circular reference", getFullName().c_str(), newParent->getFullName().c_str());
		throw std::runtime_error(message);
	}
	...
	if (isSettingParent)
	{
		...
		std::string message = wbx::format("Something unexpectedly tried to set the parent of %s to %s while trying to set the parent of %s. Current parent is %s.", 
    ...
    checkWBXCaller<kCallCheckCallArg, callCheckSetBasicFlag<HATE_RETURN_CHECK> >(thisFunction);    

	// signals for the child being added
#if !defined(WBX_RCC_SECURITY) && !defined(WBX_STUDIO_BUILD) && !defined(_NOOPT) && !defined(_DEBUG) && defined(_WIN32) && !defined(WBX_PLATFORM_DURANGO) && !defined(WBX_PLATFORM_UWP)
    ...
    {
        detectedExploit = (detectDllByExceptionChainStack<4>(&newParent, wbx::Security::kCheckDefault) != 0);
    }
#endif
    ...
    if (detectedExploit)
    {
		...
		#endif
        wbx::Security::setHackFlagVs<0>(wbx::Security::hackFlag3, HATE_SEH_CHECK);
        #if defined(I_AM_GOY_THAT_LOVES_VMPROTECT) 
		VMProtectEnd();
		#endif
    }
...
XmlElement* Instance::toNewXmlRoot(Instance* instance, wbx::CreatorRole creatorRole)
{
...
private:
	...
	shared_ptr<Instance> createClones(shared_ptr<const Instance> instance)
	{
		...
		if (!copy)
			throw std::runtime_error(wbx::format("%s cannot be cloned", instance->getFullName().c_str()));

		WBXASSERT(map.find(instance.get()) == map.end());
	...
	void copyProperty(const wbx::Reflection::ConstProperty& prop, const Instance* source, Instance* destination)
	{
		...
		WBXASSERT (prop.getDescriptor() != Instance::propParent);

		WBXASSERT(source == prop.getInstance());
	...
	void setParent(shared_ptr<const Instance> source)
	{
		...
		WBXASSERT(source->getParent());
		WBXASSERT(map.find(source->getParent()) != map.end());
	...
	void notifyCloning(const Map::value_type& value)
	{
		...
		if(source->onDemandRead())
			const_cast<wbx::Instance*>(source)->onDemandWrite()->instanceClonedSignal(value.second);
	}
};
...
{
	return clone(wbx::ScriptingCreator);
}


// Declare some dictionary entries
static const wbx::Name& nameName = wbx::Name::declare("Name");
static const wbx::Name& nameParent = wbx::Name::declare("Parent");
...
void Instance::destroy()
{
	if (getIsParentLocked() && getParent())
		throw wbx::runtime_error("The Parent property of %s is locked", getFullName().c_str());
...
void Instance::setWiibloxLocked(bool value)
{
	if(WiibloxLocked != value){
		WiibloxLocked = value;
		raisePropertyChanged(propWiibloxLocked);
	}
    void (Instance::* thisFunction)(bool) = &Instance::setWiibloxLocked;
    checkWBXCaller<kCallCheckCodeOnly, callCheckSetApiFlag<kWBXLockedApiOffset> >(
...
void Instance::destroyAllChildrenLua()
{
    ...
    checkWBXCaller<kCallCheckRegCall, callCheckSetBasicFlag<HATE_DESTROY_ALL> >(
        reinterpret_cast<void*>((void*&)(thisFunction)));
	...
	while (children) 
	{
        ...
        if ( !checkWBXCaller<kCallCheckCodeOnly, callCheckNop >(reinterpret_cast<void*>((void*&)(thisFunction))) )
        {
		    child->destroy();
        }
	}
...
void Instance::raisePropertyChanged(const wbx::Reflection::PropertyDescriptor& descriptor)
{
    PropertyChanged event(wbx::Reflection::Property(descriptor, this));
...
void Instance::raiseEventInvocation(const wbx::Reflection::EventDescriptor& descriptor, const wbx::Reflection::EventArguments& args, const SystemAddress* target)
{
    if (this->parent && DFFlag::LockViolationInstanceCrash) // fast-path for replication and serialization
    {
        validateThreadAccess(this);
...
const void* Instance::getSetParentAddr()
{
    ...
    bool (wbx::Instance::*thisPmfn)(wbx::Instance*, bool) = &setParentInternal;
...
namespace WBX{ namespace Security {
    // the .rdata section
    volatile const uintptr_t WBXTextEndNeg = 0x00400000;
    volatile const size_t WBXTextSizeNeg = 1024;
}
}
