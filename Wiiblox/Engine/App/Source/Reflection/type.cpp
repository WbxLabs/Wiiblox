...
namespace WBX { namespace Reflection {
...
SignatureDescriptor::Item::Item(const wbx::Name* name, const Type* type, const Variant& defaultValue)
	:name(name)
	,type(type)
	,defaultValue(defaultValue)
{
	// Arguments must be camelCase
	WBXASSERT(name->c_str()[0] == tolower(name->c_str()[0]));
}

SignatureDescriptor::Item::Item(const wbx::Name* name, const Type* type)
	:name(name)
	,type(type)
	,defaultValue()
{
	// Arguments must be camelCase
	WBXASSERT(name->c_str()[0] == tolower(name->c_str()[0]));
	WBXASSERT(defaultValue.isVoid());
}

void SignatureDescriptor::addArgument(const wbx::Name& name, const Type& type)
{
	Item i(&name, &type);
	arguments.push_back(i);
}

void SignatureDescriptor::addArgument(const wbx::Name& name, const Type& type, const Variant& defaultValue)
{
	WBXASSERT(defaultValue.isVoid() || defaultValue.type()==type);
