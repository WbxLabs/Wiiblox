...
unsigned int ClassDescriptor::checksum()
{
    ...
    if (checksumValue == 0)
    {
        StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "Calculating checksum...");
    ...
    StandardOut::singleton()->printf(wbx::MESSAGE_INFO, "API checksum %d", checksumValue);
	...
	,security(security)
{
...

	{
		...
		WBXASSERT(iter == base.derivedClasses.end() || *iter != this);
