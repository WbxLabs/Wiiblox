...
static Reflection::BoundFuncDesc<LuaDragger, void(wbx::WBXRay)> func_MouseMove(&LuaDragger::mouseMove, "MouseMove", "mouseRay", Security::None);
static Reflection::BoundFuncDesc<LuaDragger, void()> func_MouseUp(&LuaDragger::mouseUp, "MouseUp", Security::None);
static Reflection::BoundFuncDesc<LuaDragger, void(wbx::Vector3::Axis)> func_AxisRotate(&LuaDragger::axisRotate, "AxisRotate", "axis", wbx::Vector3::X_AXIS, Security::None);
REFLECTION_END();
...
LuaDragger::~LuaDragger()
{
	wbx::GameBasicSettings::singleton().setCanMousePan(true);
}
...
{
	WBXASSERT(Workspace::getWorkspaceIfInWorkspace(_mousePart.get()));

	wbx::GameBasicSettings::singleton().setCanMousePan(false);

	if ((dragPhase == MOUSE_DOWN) || (dragPhase == DRAGGING)) {
		throw std::runtime_error("Call to LuaDragger::mouseDown when already dragging");
	}

	WBXASSERT(jointsIMade.size() == 0);
	WBXASSERT(runDragger.get() == NULL);
...
void LuaDragger::mouseMove(WBXRay mouseRay)
{
	WBXRay unitRay = mouseRay.unit();
	...
	{
		if(wbx::GameBasicSettings::singleton().inMousepanMode())
			wbx::GameBasicSettings::singleton().setCanMousePan(false);
		doDrag(unitRay);
	}
	else if(!wbx::GameBasicSettings::singleton().getCanMousePan())
		wbx::GameBasicSettings::singleton().setCanMousePan(true);
}
...
void LuaDragger::tryStartDragging(const WBXRay& unitMouseRay)
{
...
{
	if(shared_ptr<PartInstance> safeMousePart = mousePart.lock()){
		WBXASSERT(Workspace::getWorkspaceIfInWorkspace(safeMousePart.get()));
...
void LuaDragger::doDrag(const WBXRay& unitMouseRay)
{
	WBXASSERT(dragPhase == DRAGGING);
	if (shared_ptr<PartInstance> safeMousePart = mousePart.lock()){
		if (Workspace::getWorkspaceIfInWorkspace(safeMousePart.get())) 
		{
			if( dragParts.size() == 1 )
...
bool LuaDragger::getSnapHitPoint(PartInstance* part, const WBXRay& unitMouseRay, Vector3& hitPoint)
{
