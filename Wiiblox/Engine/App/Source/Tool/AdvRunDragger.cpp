...
void AdvRunDragger::SnapInfo::updateHitFromSurface(const WBXRay& mouseRay)
{
	//Extents snapExtents(snap->getExtentsLocal());
	const CoordinateFrame& snapCoord = snap->getCoordinateFrame();
	WBXRay rayInSnap = snapCoord.toObjectSpace(mouseRay);
    ...
    else
    {
	    ...
	    static WBXRay lastRay = rayInSnap;
...
#endif
	WBXASSERT(!dragPart.expired());
#ifdef _DEBUG
	WBXASSERT(Workspace::getWorldIfInWorkspace(dragPart.lock().get()) == world);
#endif
	WBXASSERT(dragPart.lock()->getPartPrimitive() == drag);
	WBXASSERT(handleMultipleParts || PartInstance::nonNullInWorkspace(dragPart.lock()));

	shared_ptr<PartInstance> p(snapPart.lock());
	if (PartInstance::nonNullInWorkspace(p)) {
#ifdef _DEBUG
		WBXASSERT(Workspace::getWorldIfInWorkspace(p.get()) == world);
#endif
...
void AdvRunDragger::snapPartFromSnapInfo()
{
	if (snapInfo.snap) {
		...
		WBXASSERT(Workspace::getWorldIfInWorkspace(snapPart.lock().get()) == workspace->getWorld());
	}
	else {
		snapPart.reset();
	}
...
{
	WBXASSERT(!_dragPart.expired());
	WBXASSERT(PartInstance::nonNullInWorkspace(_dragPart.lock()));
	...
	for( unsigned int i = 0; i < _dragParts.size(); i++ )
	{
		...
		WBXASSERT(!partFromGroup.expired());
#ifdef _DEBUG
		WBXASSERT(Workspace::getWorldIfInWorkspace(partFromGroup.lock().get()) == world);
#endif
		WBXASSERT(PartInstance::nonNullInWorkspace(partFromGroup.lock()));
	...
	if (handleMultipleParts)
	{
		...
		if (selection.size() == 1)
		{
			wbx::ModelInstance* mInstance = wbx::Instance::fastDynamicCast<wbx::ModelInstance>(selection[0]);
			if (mInstance)
	...
	WBXASSERT(handleMultipleParts || (Workspace::getWorldIfInWorkspace(dragPart.lock().get()) == workspace->getWorld()));
						...
						const Vector3&			_dragPointWorld)
{
	...
	WBXASSERT(Workspace::getWorldIfInWorkspace(dragPart.lock().get()) == workspace->getWorld());
}
...
bool AdvRunDragger::moveDragPart()
{
	...
	WBXASSERT(Math::isOrthonormal(dragInWorld));
	WBXASSERT(Math::isOrthonormal(snapRotation));
	...
	WBXRay throughDragHit = WBXRay::fromOriginAndDirection(dragHitWorld, mouseRay.direction());
	WBXRay rayInDrag = dragCoord.toObjectSpace(throughDragHit);
	...
	WBXASSERT_IF_VALIDATING(hitPlane);
...
{
    WBXASSERT_VERY_FAST(!drag->hasAutoJoints());
	if (handleMultipleParts)
	{
...
void AdvRunDragger::findNoSnapPosition(const CoordinateFrame& original)
{
    ...
    if (!foundPart)
    {
        ...
        if (Math::intersectRayPlane(MouseCommand::getSearchRay(mouseRay),Plane(Vector3::unitY(), Vector3::zero()), hitPoint))
        {
            ...
            if (handleMultipleParts)
			{
				WBXASSERT(primaryPart);
				wbx::CoordinateFrame cFrameBefore = primaryPart->getCoordinateFrame();
...
void AdvRunDragger::tiltPart(PartInstance* part, const CoordinateFrame& camera)
{
	...
	WBXASSERT(0); // test: are we even hitting this?
...
void AdvRunDragger::findSafeY()
{
	if (colliding()) 
    {
		...
		WBXASSERT(moved.x == 0.0);
		WBXASSERT(moved.y != 0.0);
...
bool AdvRunDragger::snap(const WBXRay& _mouseRay)
{
...
bool AdvRunDragger::snapGroup(const WBXRay& _mouseRay)
{
	mouseRay = _mouseRay;
...
bool AdvRunDragger::getSnapHitPoint(PartInstance* part, const WBXRay& unitMouseRay, Vector3& hitPoint)
{
...
Contact* AdvRunDragger::getNextContact(Primitive*& prim, Contact* c)
{
	...
	{
		if (dragPart.lock())
			wbx::Draw::selectionBox(dragPart.lock()->getPart(), adorn, Color4(Color3::orange()));
	}
#endif
