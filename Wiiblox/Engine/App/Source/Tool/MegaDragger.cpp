...
void MegaDragger::setToSelection(const Workspace* workspace)
{
	...
	std::vector<wbx::PVInstance*> pvInstances;

	for (wbx::Instances::const_iterator iter = selection->begin(); iter != selection->end(); ++iter)
	{
		if ((*iter)->isDescendantOf(workspace))
			if (shared_ptr<wbx::PVInstance> pvInstance = wbx::Instance::fastSharedDynamicCast<wbx::PVInstance>(*iter))
