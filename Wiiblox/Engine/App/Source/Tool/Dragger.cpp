...
void Dragger::movePrimitivesDelta(	const G3D::Array<Primitive*>& primitives, 
									...
									Vector3& movedSoFar)
{
	//WBXASSERT(delta == DragUtilities::toGrid(delta));
	WBXASSERT(delta != Vector3::zero());
...
{
	WBXASSERT(primitives.size() > 0);
	WBXASSERT(!intersectingWorldOrOthers(primitives, contactManager, Tolerance::maxOverlapOrGap(), bottomPlaneHeight));
                                  ...
                                  bool snapToWorld)
{
	...
	if (!intersectingWorldOrOthers(primitives, contactManager, overlapTolerance, maxDragDepth())) {		
		...
		WBXASSERT(middleMove != startMove);
                                    ...
                                    bool snapToWorld )
{
	if (primitives.size() == 0) {
		WBXASSERT(0);			// just wanted to see where this happens
		return Vector3::zero();
	}
	...
	if (tryMove != Vector3::zero()) {
		...
		WBXASSERT(		(movedSoFar == Vector3::zero())
								...
								const float customPlaneHeight )
{
	...
	if (primitives.size() > wbx::ClientAppSettings::singleton().GetValueMinPartsForOptDragging())
											...
											const float customPlaneHeight)
{
	...
	// populate data
	wbx::Extents tmpExtent;
	std::vector<wbx::Extents> primExtents;
	...
	for (G3D::Array<Primitive*>::const_iterator iter = primitives.begin(); iter != primitives.end(); ++iter) {
		// extents data
		tmpExtent = wbx::Extents(Math::toGrid((*iter)->getFastFuzzyExtents().min(), Tolerance::mainGrid()), 
	...
	//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "TryMove: %f - Moved: %f - Total Time: %lf", tryMove.y, moved.y, (std::clock() - start ) / (double) CLOCKS_PER_SEC);
...
void Dragger::searchUpGross_EXT(	std::vector<wbx::Extents> &primExtents,
...
void Dragger::searchDownGross_EXT(	std::vector<wbx::Extents> &primExtents,
									const G3D::Array<Primitive*>& primitives, 
									const boost::unordered_set<const Primitive*> &ignorePrimitives,
									ContactManager& contactManager,
									const float bottomPlaneHeight,
...
void Dragger::searchUpFine_EXT(	std::vector<wbx::Extents> &primExtents,
								const G3D::Array<Primitive*>& primitives, 
								const boost::unordered_set<const Primitive*> &ignorePrimitives,
								ContactManager& contactManager,
								const float bottomPlaneHeight,
...
void Dragger::searchDownFine_EXT(	std::vector<wbx::Extents> &primExtents,
									const G3D::Array<Primitive*>& primitives, 
									const boost::unordered_set<const Primitive*> &ignorePrimitives,
									ContactManager& contactManager,
									const float bottomPlaneHeight,
...
bool Dragger::intersectingWorldOrOthers_EXT(std::vector<wbx::Extents> &primExtents,
											const G3D::Array<Primitive*>& primitives, 
											const boost::unordered_set<const Primitive*> &ignorePrimitives,
											ContactManager& contactManager,
											const float bottomPlaneHeight,  
	...
	for (int a = 0; a < (int)primExtents.size(); ++a) {
		...
		if (foundPrims.size()) {
			...
			for (int zz = 0; zz < foundPrims.size(); ++zz) {
				wbx::Extents tmpExtent(Math::toGrid(foundPrims[zz]->getFastFuzzyExtents().min(), Tolerance::mainGrid()), 
...
bool Dragger::intersectingGroundPlane_EXT(	const std::vector<wbx::Extents>& primExtents, 
											const G3D::Array<Primitive*>& primitives, 
											const float yHeight, 
											const Vector3& movedSoFar)
{
...
bool Dragger::checkBallPolyIntersection(const Primitive* ballPrim, const CoordinateFrame &ballCFrame, const Primitive* polyPrim, const CoordinateFrame &polyCFrame)
{
	...
	for (unsigned int j = 0; j < polyMesh->numEdges(); j++)
	{
		...
		edgeRay = WBXRay(p1, (p2 - p1).direction());
...
void Dragger::moveExtents(std::vector<wbx::Extents> &primExtents, const Vector3& delta)
{
	std::vector<wbx::Extents>::iterator end_iter = primExtents.end();
	for (std::vector<wbx::Extents>::iterator iter = primExtents.begin(); iter != end_iter; ++iter) 
		(*iter).shift(delta);
}

void Dragger::moveExtentsDelta(std::vector<wbx::Extents> &primExtents, const Vector3& delta, Vector3& movedSoFar)
{
