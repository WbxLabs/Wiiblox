...
shared_ptr<MouseCommand> AdvMoveToolBase::onMouseDown(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (getOverHandle(inputObject, tempPt, normalId))
    {
		...
		{
			wbx::Selection* selection = ServiceProvider::find<Selection>(workspace);
			if (selection)
				selectionChangedConnection = selection->selectionChanged.connect(boost::bind(&wbx::AdvMoveToolBase::setToSelection, this));
		}
...
void AdvMoveToolBase::onMouseMove(const shared_ptr<InputObject>& inputObject)
{
		...
		&&	megaDragger->anyDragPartAlive()) 
	{
        ...
        else if ( getDragType() == HANDLE_ROTATE )
        {
            ...
            const WBXRay    mouse_ray = getUnitMouseRay(inputObject);
            ...
            const WBXRay    projection_ray  = camera.worldRay(mid_screen.x,mid_screen.y);
            ...
            if ( snaped_angle > 0.001f )
            {
				...
				snapped_rotate = megaDragger->rotateDragParts(snapped_rotate, wbx::AdvArrowTool::advCollisionCheckMode);
...
shared_ptr<MouseCommand> AdvMoveToolBase::onMouseUp(const shared_ptr<InputObject>& inputObject)
{
	...
	if (DFFlag::RestoreTransparencyOnToolChange)
		AdvArrowToolBase::restoreSavedPartsTransparency();
	else if (!wbx::AdvArrowTool::advCollisionCheckMode)
...
void AdvMoveTool::getGridXYUsingCamera(wbx::PartInstance *part, G3D::Vector3 &gridXDir, G3D::Vector3 &gridYDir)
{
