...
void RunDragger::SnapInfo::updateHitFromSurface(const WBXRay& mouseRay)
{
	//Extents snapExtents(snap->getExtentsLocal());
	const CoordinateFrame& snapCoord = snap->getCoordinateFrame();
...
void RunDragger::snapInfoFromSnapPart()
{
	...
	WBXASSERT(PartInstance::nonNullInWorkspace(dragPart.lock()));

	shared_ptr<PartInstance> p(snapPart.lock());
	if (PartInstance::nonNullInWorkspace(p)) {
		WBXASSERT(Workspace::getWorldIfInWorkspace(p.get()) == workspace->getWorld());
...
bool RunDragger::snap(const WBXRay& _mouseRay)
{
