...
void AdvLuaDragger::mouseMove(WBXRay mouseRay)
{
...
void AdvLuaDragger::tryStartDragging(const WBXRay& unitMouseRay)
{
	if (shared_ptr<PartInstance> safeMousePart = mousePart.lock()){
		if (Workspace::getWorkspaceIfInWorkspace(safeMousePart.get())) {
...
void AdvLuaDragger::doDrag(const WBXRay& unitMouseRay)
{
			...
			else 
			{
				if (AdvRunDragger::dragMultiPartsAsSinglePart)
				{
					if (advRunDragger.get()) {	
						...
						advRunDragger->snap(unitMouseRay);
						//wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Time to snap (multiple part new): %lf", ( std::clock() - start ) / (double) CLOCKS_PER_SEC);
					}	
				}
				else
				{
...
bool AdvLuaDragger::getSnapHitPoint(PartInstance* part, const WBXRay& unitMouseRay, Vector3& hitPoint)
{
