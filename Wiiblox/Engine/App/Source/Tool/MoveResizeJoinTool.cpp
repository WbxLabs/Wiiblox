...
void MoveResizeJoinTool::findTargetPV(const shared_ptr<InputObject>& inputObject)
{
	...
	for (it = selection->begin(); it != selection->end(); ++it) {
		if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(it->get())) {
			...
			WBXRay gridRay(getUnitMouseRay(inputObject));
...
shared_ptr<MouseCommand> MoveResizeJoinTool::onMouseDown(const shared_ptr<InputObject>& inputObject)
{
	...
	else
	{
		...
		if (!wbx::AdvArrowTool::advCollisionCheckMode)
		{
...
void MoveResizeJoinTool::onMouseMove(const shared_ptr<InputObject>& inputObject)
{
	...
	if (!targetPV.expired()) {
		...
		WBXRay axisRay = WBXRay::fromOriginAndDirection(hitPointGrid, worldNormal);
		WBXRay gridRay = getUnitMouseRay(inputObject);
		Vector3 closePoint = wbx::Math::closestPointOnRay(axisRay, gridRay);
...
shared_ptr<MouseCommand> MoveResizeJoinTool::onMouseUp(const shared_ptr<InputObject>& inputObject)
{
	...
	if( part )
	{
		if (!wbx::AdvArrowTool::advCollisionCheckMode && !DFFlag::RestoreTransparencyOnToolChange)
...
{
	if (targetPV.expired()) {
		WBXASSERT(0);	// should be caught higher up
	}
	else {
		// Debug version confirms the dynamic_cast of targetInstance
		shared_ptr<PartInstance> part = shared_polymorphic_downcast<PartInstance>(targetPV.lock());

		resizeFloat(part, localNormalId, axisDelta, wbx::AdvArrowTool::advCollisionCheckMode);
	}
}
...
bool MoveResizeJoinTool::advResizeImpl(shared_ptr<PartInstance> part, NormalId localNormalId, float amount, bool checkIntersection)
{
    ...
    {
		WBXASSERT(part->getFormFactor() == PartInstance::SYMETRIC);
...
void MoveResizeJoinTool::setSelection(shared_ptr<wbx::Instance> oldSelection, shared_ptr<wbx::Instance> newSelection)
{
	if (shared_ptr<wbx::PVInstance> partInstance = wbx::Instance::fastSharedDynamicCast<wbx::PVInstance>(oldSelection))
	{
		if (partInstance == scalingPart.lock())
		{
			if (shared_ptr<wbx::PVInstance> pvInstance = wbx::Instance::fastSharedDynamicCast<wbx::PVInstance>((newSelection)))
			{
				scalingPart = pvInstance;
				if (DFFlag::RestoreTransparencyOnToolChange && !wbx::AdvArrowTool::advCollisionCheckMode)
				{
					shared_ptr<PartInstance> selectedPart = wbx::Instance::fastSharedDynamicCast<wbx::PartInstance>(newSelection);
