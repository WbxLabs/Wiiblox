...
Vector3 DragUtilities::safeMoveYDrop(const PartArray& parts, const Vector3& tryDrag, ContactManager& contactManager, const float customPlaneHeight)
{
	...
	WBXASSERT(!contactManager.intersectingOthers(primitives, Tolerance::maxOverlapOrGap()));
...
Vector3 DragUtilities::hitObjectOrPlane(const PartArray& parts, const WBXRay& unitMouseRay, const ContactManager& contactManager, bool snapToGrid)
{
	Vector3 hit;
...
bool DragUtilities::hitObject(const PartArray& parts, const WBXRay& unitMouseRay, const ContactManager& contactManager, Vector3& hit, bool snapToGrid)
{
	std::vector<const Primitive*> primitives;
	partsToPrimitives(parts, primitives);
...
void DragUtilities::unJoinFromOutsiders(const PartArray& parts)
{
	...
	WBXASSERT(notJoinedToOutsiders(parts));
}
...
bool DragUtilities::hitObjectOrPlane(const ContactManager& contactManager,
									const WBXRay& unitSearchRay,
									const std::vector<const Primitive*>& ignorePrims,
									Vector3& hit,
                                    bool snapToGrid)
{
	WBXASSERT(unitSearchRay.direction().isUnit());
	WBXRay searchRay = MouseCommand::getSearchRay(unitSearchRay);	// length determines how far to search;
...
Vector3 DragUtilities::toLocalGrid(const Vector3& deltaIn)
{
    ...
    WBXASSERT(DragUtilities::getGrid().x == DragUtilities::getGrid().y &&
