...
NewNullTool::NewNullTool(Workspace* workspace) 
	...
	, hasWaypoint(false)
{
	if (wbx::ServiceProvider::find<UserInputService>(workspace))
	{
...
bool NewNullTool::isInFirstPerson()
{
	...
	if (ModelInstance* character = wbx::Network::Players::findLocalCharacter(workspace))
	{
		...
		bool* clickable, Vector3* waypoint)
{
    ...
    wbx::Network::Player* localPlayer = wbx::Network::Players::findLocalPlayer(DataModel::get(workspace));
...
{
	WBXASSERT(inputObject->getUserInputType() == InputObject::TYPE_MOUSEIDLE);
	...
	UserInputService* userInputService = wbx::ServiceProvider::find<UserInputService>(workspace);
	...
	else 
	{
		...
		if (localPlayer || player || wbx::Network::Players::serverIsPresent(DataModel::get(workspace)))
		{
			if (userInputService)
			{
				cursor = userInputService->getDefaultMouseCursor(false, true).toString();
...
{
	if( wbx::Network::Player* player = wbx::Network::Players::findLocalPlayer(DataModel::get(workspace)) )
	{
...
{
	WBXASSERT(inputObject->getUserInputType() == InputObject::TYPE_MOUSEMOVEMENT);
...
shared_ptr<MouseCommand> NewNullTool::onRightMouseUp(const shared_ptr<InputObject>& inputObject)
{
	...
	if(rightMouseClickPart)
	{
		wbx::Network::Player* localPlayer = wbx::Network::Players::findLocalPlayer(workspace);
