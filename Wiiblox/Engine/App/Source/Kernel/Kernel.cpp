...
{
	WBXASSERT_VERY_FAST(!inStepCode); 
	WBXASSERT_FISHING(Math::longestVector3Component(b->getBranchForce()) < 1e9f);
	return true;
}
...
{
	WBXASSERT(b->getRootSimBody()->getDt() == 0.0f);
	
	kernelData->insertBody(b);				//bodies.fastAppend(b);
	WBXASSERT(validateBody(b));
...
void Kernel::removeBody(Body* b)
{
	...
	WBXASSERT(!inStepCode); 
	WBXASSERT((!b->getRootSimBody()->isContactBody() && !b->getRootSimBody()->isFreeFallBody()) ||
...
void Kernel::preStep() 
{
	...
	{
		wbx::Profiling::Mark mark(*profilingKernelBodies, false);
	...
	{
		wbx::Profiling::Mark mark(*profilingKernelConnectors, false);
		...
		if (!usingPGSSolver)
		{
			...
			{
				ContactConnector* conn = WBX_static_cast<ContactConnector*>(contactConnectors[i]);		
...
{
    WBXPROFILER_SCOPE("Physics", "Kernel::stepWorld");
    ...
    if( usingPGSSolver )
    {
        ...
        for (int j = 0; j < secondPassConnectors.size(); ++j) {
            WBXASSERT(validateConnector(secondPassConnectors[j]));
	...
	else // legacy solver
	{
		...
		if (contactConnectors.size() > 0)
		{
			...
			for (i = 0; i < Constants::impulseSolverMaxIterations(); ++i)
			{
				...
				for (int j = 0; j < contactConnectors.size(); ++j)
				{
					WBXASSERT(validateConnector(contactConnectors[j]));
		...
		{
			{
				...
				for (int j = 0; j < realtimeConnectors.size(); ++j) {	
					WBXASSERT(validateConnector(realtimeConnectors[j]));
					realtimeConnectors[j]->computeForce(false);
				}

				for (int j = 0; j < jointConnectors.size(); ++j) {	
					WBXASSERT(validateConnector(jointConnectors[j]));
...
{
    WBXPROFILER_SCOPE("Physics", "Kernel::stepWorldThrottled");
