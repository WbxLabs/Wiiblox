...
{
	WBXASSERT(!throttling || !canThrottle());
	...
	if (params.length < 0.0) 
	{
		...
		WBXASSERT(fabs(force.x) < Math::inf());
	}
	else {
		outOfContactHit++;
		reset();
...
{
	WBXASSERT(geoPair.body0->getRootSimBody()->isInKernel() || geoPair.body1->getRootSimBody()->isInKernel());
