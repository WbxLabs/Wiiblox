...
{
	WBXASSERT(std::abs(grid1*10.0f - Math::iRound(grid1*10.0f)) < 0.01f);		// Can't enter with values too far off grid
	WBXASSERT(std::abs(grid2*10.0f - Math::iRound(grid2*10.0f)) < 0.01f);		// indicates a bad snap
...
{
	WBXASSERT(clippedSortedSize.y >= clippedSortedSize.x);
	WBXASSERT(clippedSortedSize.z >= clippedSortedSize.y);
	WBXASSERT(clippedSortedSize.max(Vector3(1,1,1)) == clippedSortedSize);
