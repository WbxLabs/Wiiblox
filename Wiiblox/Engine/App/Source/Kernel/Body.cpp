...
static wbx::atomic<int> gBodyStateIndex;
...
Body::~Body() 
{
	WBXASSERT(connectorUseCount == 0);
	WBXASSERT(!link);
	WBXASSERT(simBody);
	WBXASSERT(leafBodyIndex == -1);
...
void Body::makeCofmDirty()
{
	...
	{
		WBXASSERT(this->validateParentCofmDirty());
		WBXASSERT(this->getRootSimBody()->getDirty());
	}
	else 
	{
		if (getParent()) {
		...
		if (cofm) {
			cofm->makeDirty();
			WBXASSERT( (numChildren() > 0) || (cofmOffset != Vector3::zero()) );
		}
		else {
			WBXASSERT(numChildren() == 0);
		}
	}
}
...
{
	WBXASSERT_VERY_FAST(!getParent() || (!(getParent()->getConstRootSimBody()->isInKernel())));	// confirm not happening to bodies in kernel
	...
	if (getParent()) {
		WBXASSERT(root == getParent()->getRoot());		// I keep my pvIndex, as do my children
		WBXASSERT(!simBody);		
	}
	else {
		WBXASSERT(root = this);
...
void Body::onParentChanged(IndexedTree* oldParent)
{
	...
	WBXASSERT_VERY_FAST(!getParent() || (!(getParent()->getConstRootSimBody()->isInKernel())));
...
void Body::updatePV()
{
	WBXASSERT((getParent() != NULL) != (getRoot() == this));
	...
	if (!getParent() || (stateIndex == getRoot()->getStateIndex())) {
		...
		WBXASSERT(stateIndex != getRoot()->getStateIndex());		// concurrency check
		stateIndex = getRoot()->getStateIndex();
	
		WBXASSERT_SLOW(Math::isOrthonormal(pv.position.rotation));
		WBXASSERT_SLOW(!Math::isNanInfDenormVector3(pv.position.translation)); // - asserting all the time - John
	}
}
...
{
	WBXASSERT_FISHING(Math::longestVector3Component(_meInParent.translation) < 1e6);

	WBXASSERT_FISHING(Math::isOrthonormal(_meInParent.rotation));
...
void Body::setPv(const PV& _pv, const BodyPvSetter& bpv)
{
	if (getParent()) 
	{
		WBXASSERT(0);	// bad news here - this object has a parent, so setting it's position should only be done
...
//WBXASSERT(Math::longestVector3Component(pv.position.translation) < 1e6);
	WBXASSERT_FISHING(Math::isOrthonormal(pv.position.rotation));
	WBXASSERT_SLOW(!Math::isNanInfDenormVector3(pv.position.translation));
//	WBXASSERT(!Math::isNanInfDenormMatrix3(pv.position.rotation)); -- This was asserting all the time. I had to comment out - John
	WBXASSERT_SLOW(!Math::isNanInfDenormVector3(pv.velocity.linear));
	WBXASSERT_SLOW(!Math::isNanInfDenormVector3(pv.velocity.rotational));
}
...
{
	WBXASSERT(simBody);
	WBXASSERT((numChildren() > 0) || !getCofm() || getCofm()->getCofmInBody() == cofmOffset);
...
float Body::potentialEnergy() 
{
	...
	float WBXGravity = Units::kmsAccelerationToRbx(Constants::getKmsGravity());

	return -WBXGravity * tempPv.position.translation.y * mass;
}
