...
void BulletShapeConnector::updateConnectorPointFromManifold(bool refreshContacts)
{
	...
	WBXASSERT(manifoldArray.size() > bulletManifoldIndex);
...
void BulletShapeConnector::realignConnectorsToBulletContacts()
{
	...
	{
		BulletShapeConnector* conn = WBX_static_cast<BulletShapeConnector*>(manifoldArray[bulletManifoldIndex]->getContactPoint(i).m_userPersistentData);
		if (conn)
