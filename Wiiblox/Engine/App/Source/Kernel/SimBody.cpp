...
{
	WBXASSERT(dt == 0.0f);
	WBXASSERT(freeFallBodyIndex == -1);
	WBXASSERT(realTimeBodyIndex == -1);
	WBXASSERT(jointBodyIndex == -1);
	WBXASSERT(contactBodyIndex == -1);
}
...
void SimBody::update()
{
	...
	WBXASSERT_VERY_FAST(Math::longestVector3Component(force) < 1e20f);
	WBXASSERT_VERY_FAST(constantForceY < 1e20f);
	WBXASSERT_VERY_FAST(body->cofmIsClean());
	WBXASSERT_FISHING(!Math::isNanInfDenormVector3(qOrientation.imag()));

	WBXASSERT(dirty);		// concurrency check
...
{
	WBXASSERT(!dirty);
	WBXASSERT_SLOW(Math::longestVector3Component(force) < 1e19);
	WBXASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
	WBXASSERT_SLOW(Math::longestVector3Component(pv.velocity.linear) < 1e6);
	WBXASSERT_SLOW(Math::longestVector3Component(force) < 1e19);
	WBXASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
	WBXASSERT_SLOW(Math::longestVector3Component(pv.velocity.linear) < 1e6);
	...
	WBXASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.position.translation));
	WBXASSERT_FISHING(!Math::isNanInfDenormVector3(qOrientation.imag()));
	WBXASSERT_VERY_FAST(!Math::isNanInfDenormMatrix3(pv.position.rotation));
	WBXASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.velocity.linear));
	WBXASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.velocity.rotational));
...
void SimBody::stepPosition()
{
	...
	WBXASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
	WBXASSERT(!hasExternalForceOrImpulse());
