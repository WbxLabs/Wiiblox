...
namespace WBX {
	...
	private:
		...
		mutable wbx::spin_mutex changeTrackerMutex;

		void setTrackerSimJob(SimJobTracker& tracker, SimJob* simJob) const
		{
			wbx::spin_mutex::scoped_lock lock(changeTrackerMutex);
			tracker.setSimJob(simJob);
		}

	public:
		wbx::signal<void(Primitive*)> assemblyPhysicsOnSignal;
		wbx::signal<void(Primitive*)> assemblyPhysicsOffSignal;
