...
namespace WBX {
    ...
    public:
		...
		bool hitTest(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal) override;
		bool collidesWithGroundPlane(const CoordinateFrame& c, float yHeight) const override;
		bool setUpBulletCollisionData() override;

		bool hitTestTerrain(const WBXRay& rayInMe, Vector3& localHitPoint, int& surfId, CoordinateFrame& surfCf) override;

        // Terrain specific API
		bool castRay(const WBXRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, unsigned char& surfaceMaterial, float maxDistance, bool ignoreWater);
