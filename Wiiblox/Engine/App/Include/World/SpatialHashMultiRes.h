...
namespace WBX {
	...
	public:
		...
		bool getNextGrid(Vector3int32& grid, const WBXRay& unitRay, float maxDistance);
	...
	#ifdef _WBX_DEBUGGING_SPATIAL_HASH
	...
	private:
		...
		object_pool<TreeNode, Wiiblox_allocator> treeNodeAllocator;
		object_pool<SpatialNode, Wiiblox_allocator> spatialNodeAllocator;
		//
		...
		void visitPrimitivesInSpaceWorker(TreeNode* tn, int level, int hashId, const wbx::Vector3int32& gridId, int* visitOrder, IntersectResult intersectResult, SpaceFilter* filter, const Vector3& visitDir);
