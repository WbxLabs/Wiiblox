...
namespace WBX { 
	...
	// Used internally
		namespace detail {
			class LiveThreadRef 
				: public wbx::quick_intrusive_ptr_target<LiveThreadRef>
	...
	class WeakThreadRef
		: public wbx::quick_intrusive_ptr_target<WeakThreadRef>
		, boost::noncopyable
		, public Diagnostics::Countable<WeakThreadRef>
	{
		// TODO: boost::mutex would be safer
		typedef wbx::spin_mutex Mutex;
		static Mutex sync;
	public:
		class Node
			: public wbx::quick_intrusive_ptr_target<Node>
			, boost::noncopyable
		{
			friend class WeakThreadRef;
			WeakThreadRef* first;
