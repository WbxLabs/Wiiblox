...
namespace WBX
{
	...
	public:
		...
		struct ScriptStartOptions
		{
			...
			ScriptStartOptions():identity(wbx::Security::GameScript_)
			{
			}
		};
	...
	private:
		...
		class ScriptImpersonator : public wbx::Security::Impersonator
		{
		public:
		...
		typedef boost::array<GlobalState, wbx::Security::COUNT_VM_Classes> GlobalStates; // separate Lua top-level states
		GlobalStates globalStates;
		Lua::WeakThreadRef commandLineSandbox;
		std::set<BaseScript*> scripts;
		wbx::Time nextPendingScripts;
		struct ScriptStart
		{
        ...
        struct SecurityAnchor
        {
            ...
            FORCEINLINE void update(const void* ptr)
            {
                ...
                localValue[0] ^= localValue[1]*WBX_BUILDSEED | 20151112;
                localValue[1] ^= localValue[0]*20151112 | WBX_BUILDSEED;
		...
		wbx::safe_queue<WaitingThread> waitingThreads;
        ...
        wbx::atomic<int> timedout;     // == scripts should stop running
	...
	public:
		...
		wbx::signal<void(std::string, std::string, shared_ptr<Instance>)> errorSignal;
		// A temporary signals used for diagnostic purposes
		wbx::signal<void(shared_ptr<Instance>, std::string, shared_ptr<Instance>)> camelCaseViolation;
		wbx::signal<void(lua_State*)> scriptErrorDetected;

		////////////////////////////////////////////////
		// Configuration
		void setWiibloxPlace(bool WiibloxPlace);
		...
		Reflection::Variant evaluateStudioCommandItem(const char* itemToEvaluate, shared_ptr<wbx::LuaSourceContainer> script);
		...
		void executeInNewThread(wbx::Security::Identities identity, const ProtectedString& script, const char* name);
		std::auto_ptr<Reflection::Tuple> executeInNewThread(wbx::Security::Identities identity, const ProtectedString& script, const char* name, const Reflection::Tuple& arguments);
		void executeInNewThreadWithExtraGlobals(wbx::Security::Identities identity,
		...
		Result resume(wbx::Lua::ThreadRef thread, int narg);
	...
	private:
		...
		static void setThreadIdentityAndSandbox(lua_State* thread, wbx::Security::Identities identity, shared_ptr<BaseScript> script);
		static wbx::Security::Identities getThreadIdentity(lua_State* thread);
	...
	public:
		...
		lua_State* getGlobalState(wbx::Security::Identities identity);
	...
	private:
		...
		static int WbxTime(lua_State* thread);
		...
		static int loadWiibloxLibrary(lua_State* L);
		...
		wbx::signals::connection camelCaseViolationConnection;
