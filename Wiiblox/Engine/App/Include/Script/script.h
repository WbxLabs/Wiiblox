...
namespace WBX
{
	...
	private:
		...
		weak_ptr<wbx::Network::Player> localPlayer;
	...
	public:
		...
		weak_ptr<wbx::Network::Player> getLocalPlayer() { return localPlayer; }
		void setLocalPlayer(const shared_ptr<wbx::Network::Player>& localPlayer) { this->localPlayer = localPlayer; }

		// Thread management
		boost::intrusive_ptr<Lua::WeakThreadRef::Node> threadNode;
		wbx::signal<void(lua_State*)> starting;
		wbx::signal<void()> stopped;
	...
	public:
		...
		/*override*/ XmlElement* writeXml(const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole)
		{
	...
	public:
		...
		void assignConnection(const wbx::signals::connection& connection)
		{
