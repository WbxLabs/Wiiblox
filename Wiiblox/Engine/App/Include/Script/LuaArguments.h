...
namespace WBX {
	...
	R withVariantValue(const Reflection::Variant& value, F f)
	{
		...
		if (value.isType<wbx::ProtectedString>())
			return f(value.cast<wbx::ProtectedString>());
		...
		if (const Reflection::EnumDescriptor* desc = Reflection::EnumDescriptor::lookupDescriptor(value.type()))
		{
			...
			if (item == NULL)
				throw wbx::runtime_error("Invalid value for enum %s", desc->name.c_str());
			return f(*item);
		}
		...
		if (value.isType<wbx::WBXRay>())
			return f(value.cast<wbx::WBXRay>());
		...
		if (value.isType<wbx::Region3>())
			return f(value.cast<wbx::Region3>());
		if( value.isType<wbx::Region3int16>())
			return f(value.cast<wbx::Region3int16>());
		...
		if (value.isType<wbx::signals::connection>())
			return f(value.cast<wbx::signals::connection>());
	...
	{
		typedef RBX::DenseHashMap<const void*, bool> TablesCollection;
	...
	public:
		...
		static shared_ptr<Reflection::Tuple> getValues(lua_State* L)
		{
			...
			shared_ptr<Reflection::Tuple> args(wbx::make_shared<Reflection::Tuple>(argCount));

			for (int i = 0; i<argCount; ++i)
			{
				Reflection::Variant& v = args->values.at(i);
				bool success = wbx::Lua::LuaArguments::get(L, i+1, v, false);
				RBXASSERT(success);
			}
