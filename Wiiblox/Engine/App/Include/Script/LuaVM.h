#pragma once
     ...
     (defined(__APPLE__) && !defined(WBX_PLATFORM_IOS))) && \
     !defined(WBX_STUDIO_BUILD)
#define WBX_SECURE_DOUBLE
#endif

#ifdef _WIN32
#define WBX_ALIGN(s) _declspec(align(s)) 
#else
#define WBX_ALIGN(s) __attribute__((__aligned__(s))) 
#endif

#include <boost/unordered_map.hpp>
#include <string>
#if defined(WBX_SECURE_DOUBLE) && !defined(_M_ARM) && !defined(_M_ARM64)
#include <emmintrin.h>
#endif
...
typedef unsigned int (*WBXOpEncoder)(unsigned int i, int pc, unsigned key);
...
namespace LuaVM
{
    ...
    int load(lua_State* L, const wbx::ProtectedString& source, const char* chunkname, unsigned int modkey = 1);
...
private:
    double storage;
public:
    static WBX_ALIGN(16) int luaXorMask[4];
