...
namespace Lua
{
	...
	public:
		...
		wbx::signal<void(const Stepped&)> highPrioritySteppedSignal;
		wbx::signal<void(const Stepped&)> steppedSignal;
		wbx::signal<void(const Stepped&)> renderSteppedSignal;
		wbx::signal<void()>				  earlyRenderSignal;
		wbx::signal<void(const Heartbeat&)> heartbeatSignal;
		wbx::signal<void(RunTransition)> runTransitionSignal;

		TaskScheduler::Job* getPhysicsJob();
		TaskScheduler::Job* getHeartbeat();

		wbx::signal<void(double, double)> scriptSteppedSignal;
		wbx::signal<void(double)> scriptHeartbeatSignal;
		wbx::signal<void(double)> scriptRenderSteppedSignal;
		wbx::signal<void()> scriptRenderSteppedEarlySignal;

		wbx::signal<void(double)>* getOrCreateScriptRenderSteppedSignal(bool create = true);
