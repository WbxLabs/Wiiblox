...
namespace WBX
{
	...
	public:
		...
		static shared_ptr<T> create()
		{
            ...
            checkWBXCaller<kCallCheckCallersCode, callCheckSetBasicFlag<HATE_RETURN_CHECK> >(reinterpret_cast<void*>(thisFunction));
			return obj;
		}
	...
	class FactoryProduct : public BaseClass {
		...
		private:
			...
			const Name& getClassNameUnconstructed() const {
				return Name::declare<sClassName>();
			}
	...
	public:
		...
		static const Name& className() { return static_getCreator().getClassName(); };
		static bool isNullClassName() { 
			RBXASSERT(!className().empty());
			return false; 
		};
		const Name& getClassName() const { return static_getCreator().getClassName(); };
