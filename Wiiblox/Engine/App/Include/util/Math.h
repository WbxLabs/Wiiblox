...
namespace WBX {
	...
	namespace Math 
	{
		...
		inline Vector2 expandVector2(const Vector2& v, int expand) {
			Vector2 answer(v);
			for (int i = 0; i < 2; ++i) {
				answer[i] += expand * wbx::Math::sign(v[i]);
			}
			return answer;
		}
		...
		Vector3 closestPointOnRay(const wbx::WBXRay& pointOnRay, const wbx::WBXRay& otherRay);
...
//			return (objId < 3) ? getColumn(objectR, objId) : -getColumn(objectR, objId - 3);
		...
		bool intersectRayPlane(const WBXRay& ray, const Plane& plane, Vector3& hit);
        bool intersectRayConvexPolygon(const wbx::WBXRay& ray,  const std::vector<Vector3>& poly, Vector3& hit, bool oneSided);
