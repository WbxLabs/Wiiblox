...
#include "wbx/atomic.h"
	...
	#define WBX_USE_CONCURRENCY_VALIDATOR(expr) (expr)
#else
	#define WBX_USE_CONCURRENCY_VALIDATOR(expr) ((void)0)
#endif
...
{
private:
    wbx::atomic<int> writing;
	std::string writeLocation;
    mutable wbx::atomic<int> reading;
...
public:
	ReadOnlyValidator(const ConcurrencyValidator& c) : concurrencyValidator(c) {
		WBX_USE_CONCURRENCY_VALIDATOR(concurrencyValidator.preRead());
	}

	~ReadOnlyValidator() {
		WBX_USE_CONCURRENCY_VALIDATOR(concurrencyValidator.postRead());
	}
};
...
public:
	WriteValidator(ConcurrencyValidator& c) : concurrencyValidator(c) {
		WBX_USE_CONCURRENCY_VALIDATOR(concurrencyValidator.preWrite());
	}

	WriteValidator(ConcurrencyValidator& c, const std::string& writeWhere) : concurrencyValidator(c) {
		WBX_USE_CONCURRENCY_VALIDATOR(concurrencyValidator.preWrite(writeWhere));
	}
	...
	~WriteValidator() {
		WBX_USE_CONCURRENCY_VALIDATOR(concurrencyValidator.postWrite());
	}
};
