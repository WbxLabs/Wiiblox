...
namespace WBX
{
	...
	public:
		...
		{			
			...
			virtual bool getNextTask(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)>& task) = 0;
		};
	private:
		int count;
		const size_t kMaxScheduleSize;
		std::vector< boost::shared_ptr<wbx::spin_mutex> > poolLocks;
		std::vector< boost::shared_ptr<boost::thread> > pool;
		ShutdownPolicy shutdownPolicy;

		static void loop(boost::shared_ptr<PoolData> poolData, boost::shared_ptr<wbx::spin_mutex> lock, ShutdownPolicy shutdownPolicy);
	...
	private:
		struct ThreadPoolData
			: public PoolData
		{
			typedef wbx::safe_queue<boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> > Queue;
			Queue queue;

			/*override*/ bool getNextTask(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)>& task)
			{
	...
	public:
		...
		bool schedule(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> task);
	};
	...
	private:
		struct PriorityTask
		{
			boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> func;
			float priority;
			PriorityTask(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> func, float priority)
				: func(func)
				, priority(priority)
			{}
			PriorityTask()
		...
		struct PriorityThreadPoolData
			: public PoolData
		{
			typedef wbx::safe_heap<PriorityTask> Heap;
			Heap heap;
	...
	public:
		...
		bool schedule(boost::function<void(boost::shared_ptr<wbx::spin_mutex>)> task, float priority);
	};
}
