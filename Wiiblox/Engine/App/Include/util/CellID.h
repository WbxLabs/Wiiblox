...
namespace WBX 
{
	...
	private:
		bool isNil;
		wbx::Vector3 location;
		shared_ptr<Instance> terrainPart;
	public:
		CellID();
		CellID( bool isNil, const wbx::Vector3& location, shared_ptr<Instance> terrainPart )
			:isNil(isNil)
			,location(location)
		...
		void setLocation( wbx::Vector3 newLocation ) { location = newLocation; }
