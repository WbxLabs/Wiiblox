...
namespace WBX {
	...
	public:
		...
		Type* getTypedChild(int i) {
			return WBX_static_cast<Type*>(children[i]);
		}
		...
		const Type* getRoot() const {
			return (parent) 
				? parent->getRoot<Type>()
				: WBX_static_cast<const Type*>(this);
		}
		...
		Type* getOneBelowRoot() {
			...
			return WBX_static_cast<Type*>(answer);
		}
		...
		{
			Type* t = WBX_static_cast<Type*>(this);
			func(t);
		...
		{
			const Type* t = WBX_static_cast<const Type*>(this);
			func(t);
			for (int i = 0; i < children.size(); ++i) {
