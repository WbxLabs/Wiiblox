...
{
public:
	wbx::signal<void(shared_ptr<Instance>)> childAddedSignal;
	wbx::signal<void(shared_ptr<Instance>)> childRemovedSignal;
	wbx::signal<void(shared_ptr<Instance>)> descendantAddedSignal;
	wbx::signal<void(shared_ptr<Instance>)> descendantRemovingSignal;
	wbx::signal<void(shared_ptr<Instance>)> instanceClonedSignal;
...
public:
	...
	void securityCheck(wbx::Security::Context& context) const;
	...
	static const Reflection::PropDescriptor<Instance, bool> propWiibloxLocked;
	...
	wbx::signal<void(shared_ptr<Instance>)>* getOrCreateChildAddedSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->childAddedSignal : NULL; };

	void childRemovedSignal(shared_ptr<Instance>& inst) { if(onDemandRead()) onDemandWrite()->childRemovedSignal(inst); }
	wbx::signal<void(shared_ptr<Instance>)>* getOrCreateChildRemovedSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->childRemovedSignal : NULL; };

	void descendantAddedSignal(shared_ptr<Instance>& inst) { if(onDemandRead()) onDemandWrite()->descendantAddedSignal(inst); }
	wbx::signal<void(shared_ptr<Instance>)>* getOrCreateDescendantAddedSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->descendantAddedSignal : NULL; };

	void descendantRemovingSignal(const shared_ptr<Instance>& inst) { if(onDemandRead()) onDemandWrite()->descendantRemovingSignal(inst); }
	wbx::signal<void(shared_ptr<Instance>)>* getOrCreateDescendantRemovingSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->descendantRemovingSignal : NULL; };

	wbx::signal<void(shared_ptr<Instance>, shared_ptr<Instance>)> ancestryChangedSignal;
	wbx::signal<void(const Reflection::PropertyDescriptor*)> propertyChangedSignal;
	...
	wbx::signal<void(CombinedSignalType, const ICombinedSignalData*)> combinedSignal;
	...
	static XmlElement* toNewXmlRoot(Instance* instance, wbx::CreatorRole creatorRole);		// DB 12/5/05 - added here to centralize the spawner behavior - to, from XML
...
public:
	...
	bool getWiibloxLocked() const { return WiibloxLocked; }
	void setWiibloxLocked(bool value);
	...
	Type* getTypedChild(int index)	{
		return WBX_static_cast<Type*>((*children)[index].get());
	}

	template<class Type>
	const Type* getTypedChild(int index) const	{
		return WBX_static_cast<const Type*>((*children)[index].get());
	}
	...
	template<class Type>
	Type* getTypedParent() {
		return WBX_static_cast<Type*>(parent);
	}

	template<class Type>
	const Type* getTypedParent() const {
		return WBX_static_cast<const Type*>(parent);
	}
	...
	virtual shared_ptr<Instance> createChild(const Name& className, wbx::CreatorRole creatorRole);
	void read(const XmlElement* element, IReferenceBinder& binder, wbx::CreatorRole creatorRole);
	void readChildren(const XmlElement* element, IReferenceBinder& binder, wbx::CreatorRole creatorRole);
	void readChild(const XmlElement* childElement, IReferenceBinder& binder, wbx::CreatorRole creatorRole);

	virtual XmlElement* writeXml(const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole);

	void writeChildren(XmlElement* container, const boost::function<bool(Instance*)>& isInScope, wbx::CreatorRole creatorRole, const SaveFilter saveFilter = SAVE_ALL);
	void writeChildren(XmlElement* container, wbx::CreatorRole creatorRole, const SaveFilter saveFilter = SAVE_ALL);

	void raisePropertyChanged(const wbx::Reflection::PropertyDescriptor& descriptor);
	void raiseEventInvocation(const wbx::Reflection::EventDescriptor& descriptor, const wbx::Reflection::EventArguments& args, const SystemAddress* target);
...
protected:
	...
	void raiseChanged(const wbx::Reflection::PropertyDescriptor& descriptor)
	{
