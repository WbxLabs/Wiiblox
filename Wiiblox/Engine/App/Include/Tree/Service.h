...
namespace WBX
{
	...
	private:
		...
		mutable std::map<const Name*, shared_ptr<Instance> > serviceMap;
	public:
		wbx::signal<void()> closingSignal;
		wbx::signal<void()> closingLateSignal;
		wbx::signal<void(shared_ptr<Instance> service)> serviceAddedSignal;
		wbx::signal<void(shared_ptr<Instance> service)> serviceRemovingSignal;
		...
		//mutable wbx::reentrant_concurrency_catcher threadGuard;
		template<class ServiceClass>
		ServiceClass* create() const
		{
			//wbx::reentrant_concurrency_catcher::scoped_lock lock(threadGuard);
			...
			if (service==NULL)
			{
				...
				RBXASSERT((ServiceClass::className()==Name::getNullName()) || serviceMap.find(&ServiceClass::className())!=serviceMap.end());
		...
		static shared_ptr<Instance> create(Instance* context, const Name& name);
		shared_ptr<Instance> getPublicServiceByClassNameString(std::string name); 
		
	protected:
		/*override*/ shared_ptr<Instance> createChild(const Name& className, wbx::CreatorRole creatorRole);
	...
	private:
		...
		shared_ptr<Instance> findServiceByClassName(const Name& className) const;
