	...
	namespace Reflection
	{
		...
		protected:
			MemberDescriptor(const ClassDescriptor& owner, const char* name, const char* category, Attributes attributes, Security::Permissions security)
				...
				,owner(owner)
				,category(Name::declare(category))
				,security(security)
			{
			}
		...
		public:
			...
			typedef RBX::DenseHashMap<const char*, MemberDescriptorType*, StringHashPredicate, StringEqualPredicate> DescriptorLookup;
			...
			void declare(MemberDescriptorType* descriptor)
			{
				...
				// Add this to allDescriptors (in a determanistic order) 
				{
					...
					while (iter!=allDescriptors().end())
					{
						...
						if (desc==descriptor)
							goto SKIP;
						int compare = Name::compare(descriptor->name, desc->name);
						if (compare<0)
							break;
						if (compare==0)
						{
							// This descriptor name already exists in a different class
							compare = Name::compare(descriptor->owner.name, desc->owner.name);
