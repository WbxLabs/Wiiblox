...
namespace WBX {
	...
	namespace Reflection {
...
#if 0
			typedef boost::unordered_map<const Name*, const EnumDescriptor*> EnumNameTable;
#else
		...
		public:
			...
			static const EnumDescriptor* lookupDescriptor(const Name& name) {
...
template<typename Enum> class EnumRegistrar;
		...
		private:
			...
			std::vector< const Name* > enumToName;		// maps enum to Name (there may be gaps)
			...
			void addLegacy(int oldValue, const char* name, Enum value)
			{
				...
				nameToEnumLegacy[&Name::declare(name)] = value;
			}
		...
		public:
			const Name& convertToName(const Enum& value) const
			{
				RBXASSERT(value>=0);
			...
			{
				typename std::map<const Name*, Enum>::const_iterator iter = nameToEnum.find(&name);
			...
			{
				Enum e;
				if (convertToValue(Name::lookup(text), e))
			...
			{
				return convertToValue(Name::lookup(text), value);
			}
...
#define WBX_REGISTER_ENUM(Enum)		namespace WBX { namespace Reflection {								\
									template<>															\
