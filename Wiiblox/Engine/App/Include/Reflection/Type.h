...
namespace WBX
{
	namespace Reflection
	{
		...
		std::ostream& operator<<(std::ostream& os, const wbx::Reflection::Type& type);

		// Handy macro for registering a type
#define WBX_REGISTER_TYPE(mType)		template<> wbx::Reflection::TypeRegistrar<mType> wbx::Reflection::TypeRegistrar<mType>::registrar(0)
		...
		class Variant
		{
			...
			const Type* _type;
			wbx::placement_any<Storage> value;
		...
		public:
			...
			public:
				Item(const Name* name, const Type* type, const Variant& defaultValue);
			...
			void addArgument(const Name& name, const Type& type);
			void addArgument(const Name& name, const Type& type, const Variant& defaultValue);

			SignatureDescriptor();
		};

		template<class ValueType>
		ValueType& wbx::Reflection::Variant::genericConvert()
		{
			...
			throw wbx::runtime_error("Unable to cast %s to %s", _type->tag.c_str(), Type::singleton<ValueType>().tag.c_str() );
		}	
	}
}
