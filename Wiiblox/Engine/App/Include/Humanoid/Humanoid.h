...
namespace WBX {
	...
	private:
		...
		wbx::signals::scoped_connection characterChildAdded;		
		wbx::signals::scoped_connection characterChildRemoved;				
		void onEvent_ChildModified(shared_ptr<Instance> child);

		boost::unordered_map<shared_ptr<PartInstance>, wbx::signals::scoped_connection> siblingMap;
		void updateSiblingPropertyListener(shared_ptr<PartInstance> sibling);
		void onEvent_SiblingPropertyChanged(const wbx::Reflection::PropertyDescriptor* desc);

		shared_ptr<StatusInstance> status;

		wbx::signals::scoped_connection onCFrameChangedConnection;
		wbx::signals::scoped_connection humanoidEquipConnection;
		...
		void renderMultiplayer(Adorn* adorn, const wbx::Camera& camera);

		void renderBillboard(Adorn* adorn, const wbx::Camera& camera);
		...
		Vector3 getSimulatedFrictionVelocityOffset(PartInstance* floorPart, Vector3& newCharPosition, const Vector3& previousCharPosition, const Vector3& charPosInFloorSpace, const wbx::Velocity& previousFloorVelocity, float& netDt);
		const Velocity getHumanoidRelativeVelocityToPart(Primitive* floorPrim);
	
	public:
		void updateNetworkFloorPosition(PartInstance* floorPart, CoordinateFrame& previousFloorPosition, wbx::Velocity& lastFloorVelocity, float& netDt);
		...
		wbx::signal<void(Status)> statusAddedSignal;
		wbx::signal<void(Status)> statusRemovedSignal;
		wbx::signal<void(std::string)> customStatusAddedSignal;
		wbx::signal<void(std::string)> customStatusRemovedSignal;
		
		wbx::remote_signal<void(shared_ptr<Instance>)> serverEquipToolSignal;

		// Humanoid Network Update Connection
		wbx::signals::scoped_connection         onPositionUpdatedByNetworkConnection;
		...
		wbx::signal<void()> diedSignal;
		wbx::signal<void(float)> swimmingSignal;
		wbx::signal<void(float)> runningSignal;				// state change scripts			
		wbx::signal<void(float)> climbingSignal;			// state change scripts
		wbx::signal<void(bool)> jumpingSignal;				// state change scripts
		wbx::signal<void(bool)> freeFallingSignal;
		wbx::signal<void(bool)> strafingSignal;
		wbx::signal<void(bool)> gettingUpSignal;
		wbx::signal<void(bool)> fallingDownSignal;
		wbx::signal<void(bool)> ragdollSignal;
		wbx::signal<void(bool, shared_ptr<Instance>)> seatedSignal;
		wbx::signal<void(bool)> platformStandingSignal;
        wbx::signal<void(wbx::HUMAN::StateType, wbx::HUMAN::StateType)> stateChangedSignal;
        wbx::signal<void(wbx::HUMAN::StateType, bool)> stateEnabledChangedSignal;
		wbx::HUMAN::StateType getCurrentStateType();
		wbx::HUMAN::StateType getPreviousStateType() { return previousState; };
		void setPreviousStateType(wbx::HUMAN::StateType newState);
		void changeState(wbx::HUMAN::StateType state);
		static bool isStateInString(const std::string& text, const wbx::HUMAN::StateType &compare, wbx::HUMAN::StateType& value);

		wbx::signal<void(float)> healthChangedSignal;

		// Internal use only - no reflection - happens both client, server
		wbx::signal<void()> doneSittingSignal;
		wbx::signal<void()> donePlatformStandingSignal;

		void equipToolInstance(shared_ptr<Instance> instance);
		void equipTool(wbx::Tool* tool);
		void unequipTools();
        ...
        void testWalkSpeed(float walkSpeed, float percentWalkSpeed) const
        {
            ...
            {
                wbx::Security::setHackFlagVs<LINE_RAND4>(wbx::Security::hackFlag11, HATE_SPEEDHACK);
            }
		...
		wbx::signal<void(bool)> moveToFinishedSignal;
		...
		// Build Joints
		void buildJoints(wbx::DataModel* dm = NULL);
		...
		wbx::signal<void(shared_ptr<Instance>)> animationPlayedSignal;
