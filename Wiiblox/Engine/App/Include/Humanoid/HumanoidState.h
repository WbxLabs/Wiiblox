...
namespace WBX
{
	...
	namespace HUMAN
	{
		...
		public:
			...
			DebugRay(const WBXRay& _ray, const Color3& _color)
			{
				ray = _ray;
				color = _color;
			}
		...
		private:           
			...
			void doLadderRaycast(GeometryService *geom, const WBXRay& caster,Humanoid* humanoid, Primitive** hitPrimOut, 
									Vector3* hitLocationOut);
			void doAutoJump(); 
			void findFloor(shared_ptr<PartInstance>& oldFloor);
			shared_ptr<PartInstance> tryFloor(const WBXRay& ray, Vector3& hitLocation, Vector3& hitNormal, float maxDistance, Assembly* humanoidAssembly, PartMaterial& recentFloorMaterial);
		...
		protected:
			...
			void fireMovementSignal(wbx::signal<void(float)>& movementSignal, float movementVelocity);
		...
		public:
			...
			std::vector<wbx::signals::connection> buoyancyConnections;
            ...
            {
#ifdef _WIN32
                bool (wbx::HUMAN::HumanoidState::* hsce)(EventType) = &computeEvent;
