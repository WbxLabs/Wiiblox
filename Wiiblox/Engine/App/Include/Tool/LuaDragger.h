...
namespace WBX {
	...
	private:
		...
		void tryStartDragging(const WBXRay& unitMouseRay);
		void startDragging();
		void doDrag(const WBXRay& unitMouseRay);
		bool getSnapHitPoint(PartInstance* part, const WBXRay& unitMouseRay, Vector3& hitPoint);
	...
	public:
		...
		void mouseMove(WBXRay mouseRay);		// inefficient, but easier to have just one version
