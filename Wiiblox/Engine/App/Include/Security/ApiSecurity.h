...
namespace WBX{ namespace Security {
    extern volatile const uintptr_t WBXTextBase;
    extern volatile const size_t WBXTextSize;
    extern volatile const uintptr_t WBXTextEndNeg;
    extern volatile const size_t WBXTextSizeNeg;
    extern volatile const uintptr_t WBXVmpBase;
    extern volatile const size_t WBXVmpSize;
}
}
...
namespace WBX 
{
FORCEINLINE static bool isWBXTextAddr(const void* const ptr)
{
#if defined(_WIN32) && !defined(WBX_PLATFORM_DURANGO) && !defined(WBX_STUDIO_BUILD) && !defined(WBX_PLATFORM_UWP)
    return (reinterpret_cast<uintptr_t>(ptr) - Security::WBXTextBase < Security::WBXTextSize);
#else
    return true;
#endif
}
...
static const unsigned int kWBXLockedApiOffset = 3;
...
FORCEINLINE static unsigned int checkWBXCaller(const void* const funcAddress)
{
#if defined(_WIN32) && !defined(_NOOPT) && !defined(LOVE_ALL_ACCESS) && !defined(WBX_STUDIO_BUILD) && !defined(WBX_PLATFORM_DURANGO) && !defined(WBX_PLATFORM_UWP)
    unsigned int flags = 0;

    void* returnAddress = _ReturnAddress();
    flags |= isWBXTextAddr(returnAddress) ? 0 : (1<<0);
    if (!flags)
    {
        switch(level)
        {
        case kCallCheckCallersCode:
            {
                ...
                flags |= isWBXTextAddr(secondReturnAddress) ? 0 : (1<<2);
            }
            // continue
...
inline HMODULE WBXGetNtdll()
{
    ...
    while ((imoListWalk->Flink != imoListBase.Blink) && --moduleLimit)
    {
        ...
        //filter here
        unsigned int dest = ((unsigned int)(kNtDll) + WBX_BUILDSEED);
        volatile unsigned int tmp = dest;
        const WCHAR* const ntdllName = (const WCHAR* const)(tmp - WBX_BUILDSEED);
...
inline void* WBXNtdllProcAddress(HMODULE module, bool filter(const char*) )
{
    DWORD* loc = 0;
...
#if defined(_WIN32) && !defined(WBX_STUDIO_BUILD) && !defined(WBX_RCC_SECURITY) && !defined(WBX_PLATFORM_DURANGO) && !defined(WBX_PLATFORM_UWP)
...
template<size_t kMaxDepth> FORCEINLINE uint32_t detectDllByExceptionChain(void* addrOfChain, unsigned int kFlags)
{
    ...
    uintptr_t textEndNeg = Security::WBXTextEndNeg;
    size_t textSizeNeg = Security::WBXTextSizeNeg;
    uintptr_t vmpBase = Security::WBXVmpBase;
    size_t vmpSize = Security::WBXVmpSize;
    uint32_t result = 0;
    for (size_t i = 0; i < kMaxDepth; ++i)
    {
        ...
        {
            // check if handler chain init in Wiiblox. (equivalent to "addr - base >= size")
        ...
        // check if handler in Wiiblox code. (equivalent to "addr - base >= size")
