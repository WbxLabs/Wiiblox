...
namespace WBX {
    ...
    enum GameJoinType
    {
        ...
        GameJoin_Follow,   // id = Wiiblox player ID -or- playerName = name of the player to follow. non-empty playerName takes precedence
    ...
    {
        Award_OK        = 0, // ok
        Award_Fail      = 1, // tried sending but failed, (net/xblive/Wiiblox error)
	...
	enum AccountAuthResult
	{
		...
		AccountAuth_InvalidWiibloxUser		= 10,
		AccountAuth_WiibloxUserAlreadyLinked	= 11,
	...
	enum PlatformPurchaseResult
	{
		...
		PurchaseResult_ConsumeRequestFail = 2,	//issues with Wiiblox resolving purchase
    ...
    struct IPlatformAPI
    {
		...
		virtual int		performSetWiibloxCredentials(const std::string& accountName, const std::string& password, std::string* response) = 0;
		virtual AccountAuthResult	performHasWiibloxCredentials() = 0;
		...
		//			}
		virtual int		fetchInventoryInfo(shared_ptr<wbx::Reflection::ValueArray> values) = 0;
		...
		// result = list of MPSD members by {XUID: WiibloxId} pairs
    ...
    public:
        ...
        wbx::signal<void(int)> viewChanged;
		wbx::signal<void(int)> gameJoinedSignal;
		wbx::signal<void()> userAuthCompleteSignal;	
		wbx::signal<void(int)> userAccountChangeSignal;
		wbx::signal<void(int)>	robuxAmountChangedSignal;
		wbx::signal<void(std::string)> networkStatusChangedSignal;
        wbx::signal<void(std::string)> keyboardClosedSignal;

		wbx::signal<void(std::string)> lostActiveUser;
		wbx::signal<void(std::string)> gainedActiveUser;

		wbx::signal<void(std::string)> lostUserGamepad;
		wbx::signal<void(std::string)> gainedUserGamepad;

        wbx::signal<void(int)> voiceChatUserTalkingStartSignal;
        wbx::signal<void(int)> voiceChatUserTalkingEndSignal;

		wbx::signal<void()> suspendSignal;
		...
		void beginSetWiibloxCredentials(std::string accountName, std::string password, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction);
		void beginHasLinkedAccount(boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction);
		void beginHasWiibloxCredentials(boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction);
