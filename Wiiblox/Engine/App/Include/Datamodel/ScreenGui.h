...
namespace WBX {
	...
	public:
        ...
        /*override*/ GuiResponse processGesture(const UserInputService::Gesture gesture, shared_ptr<const wbx::Reflection::ValueArray> touchPositions, shared_ptr<const Reflection::Tuple> args);
		...
		bool hasModalDialog();
		void onModalButtonChanged(const wbx::Reflection::PropertyDescriptor* desc, wbx::GuiButton* guiButton);
	...
	private:
		...
		bool removeModalButton(wbx::GuiButton* guiButton);
		bool insertModalButton(wbx::GuiButton* guiButton);

		Rect2D bufferedViewport;		// grab on render (const - don't take action), resize on process;
		bool renderable;

		std::vector<GuiButton*> modalGuiObjects;
		boost::unordered_map<Instance*, wbx::signals::scoped_connection> connections;
