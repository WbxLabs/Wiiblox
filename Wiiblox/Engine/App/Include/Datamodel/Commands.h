...
namespace WBX {
	...
	void AddSelectionToRoot(XmlElement* root, Selection* sel, wbx::CreatorRole creatorRole);
...
private:
	DataModel* dataModel;
public:
	ToggleViewMode(wbx::DataModel* dm);

	/*override*/ bool isChecked() const;
	/*override*/ bool isEnabled() const;
	/*override*/ bool isSelected() const;
	/*override*/ void doIt(wbx::IDataState* dataState);
};
...
public:
	TToolVerb(DataModel* dataModel, bool toggle = true, bool blacklisted = true) :
	  ParentClass(
		  MouseCommandClass::name().toString() + "Tool",	// MouseCommand should be a Named<> descendant, or must implement a static name() function
		  dataModel,
          blacklisted
		 ),
	 toggle(toggle)
...
public:
	...
	static wbx::BrickColor getCurrentColor() { return m_currentColor; }
	static void setCurrentColor(wbx::BrickColor val) { m_currentColor = val; }

private:
	static wbx::BrickColor m_currentColor;
};
