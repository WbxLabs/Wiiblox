...
namespace WBX {
	...
	public:
		...
		wbx::signal<void()> deactivationSignal;

		void setAssetId(int assetId);

		void openScriptDoc(shared_ptr<wbx::Instance> script, int lineNumber);
	...
	public:
		...
		boost::mutex mutex;
		wbx::signal<void()> allPluginsDeactivatedSignal;
		...
		virtual void fireDragEnterEvent(DataModel* dataModel, shared_ptr<const wbx::Instances> instances, Vector2 location);
	};
};
