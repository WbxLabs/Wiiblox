...
private:
	...
	wbx::signals::scoped_connection sourceValueChangedConnection;
	float lastReceivedValue;

public:
	// public for interoperation with CustomEvent
	wbx::signal<void(float)> sourceValueChanged;
	// connection signals public for testing
	wbx::signal<void(shared_ptr<Instance>)> eventConnected;
	wbx::signal<void(shared_ptr<Instance>)> eventDisconnected;
