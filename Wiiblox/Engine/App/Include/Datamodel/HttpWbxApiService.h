...
namespace WBX {
	...
	extern const char* const sHttpWbxApiService;
	class HttpWbxApiService
		:public DescribedCreatable<HttpWbxApiService, Instance, sHttpWbxApiService, Reflection::ClassDescriptor::INTERNAL>
		,public Service
	{
	public:
		...
		public:
			...
			void execute(HttpWbxApiService* apiService);
	...
	private:
		typedef DescribedCreatable<HttpWbxApiService, Instance, sHttpWbxApiService, Reflection::ClassDescriptor::INTERNAL> Super;
		...
		// each queue can only store HttpWbxApiMaxThrottledQueueSize items
		// if HttpWbxApiMaxThrottledQueueSize is reached then the request returns a throttle error
		...
		shared_ptr<HttpWbxApiJob> HttpWbxApiJob;
		...
		bool clientPresent;
		bool isPlaySolo;
		wbx::signals::scoped_connection serviceAddedConnection;
		wbx::signals::scoped_connection playersChangedConnection;
		wbx::signals::scoped_connection contentProviderPropertyChangedConnection;

		unsigned int totalNumOfApiCalls;
		wbx::Timer<wbx::Time::Fast> instanceAliveTimer;
		...
		void playersPropertyChanged(const wbx::Reflection::PropertyDescriptor* desc);
		...
		static void httpHelper(weak_ptr<HttpWbxApiService> weakApiService, std::string* response, std::exception* exception, HttpApiRequest request, ThrottlingPriority throttlePriority,
			boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction);

		static bool retrySyncRequest(Http& http, std::string& syncResponse);

		void setStaticApiBaseUrl(const wbx::Reflection::PropertyDescriptor* pPropertyDescriptor);
	...
	public:
		...
		void postAsync(std::string urlPath, std::string data,bool useHttps, ThrottlingPriority throttlePriority, HttpService::HttpContentType content, 
			boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction);
		void postAsyncWithAdditionalHeaders(std::string urlPath, std::string data,bool useHttps, ThrottlingPriority throttlePriority, HttpService::HttpContentType content, wbx::HttpAux::AdditionalHeaders additionalHeaders,
			boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction);
