...
public:
	...
	wbx::signal<void()> interpolationFinishedSignal;
	wbx::signal<void(bool)> firstPersonTransitionSignal;
	wbx::signal<void(CoordinateFrame)> cframeChangedSignal;
	...
	// Taken from WBXCamera, a derivative of the G3D Camera.
	...
	WBXRay worldRay(float x, float y, float depth = 0.0f) const;
	WBXRay worldRayLua(float x, float y, float depth);
	WBXRay worldRayViewportLua(float x, float y, float depth);

	// Taken from WBXCamera, a derivative of the G3D Camera.
	// Returns the world space view frustum, which is a truncated pyramid describing
	// the volume of space seen by this camera.
	void frustum(const float farPlaneZ, wbx::Frustum& fr) const;

	wbx::Frustum frustum() const;
