...
public:
		...
		wbx::signal<void(MoveState,MoveState)> moveStateChangedSignal; // void(newState, oldState)

		// this is better:
		wbx::signal<void(shared_ptr<Instance>, shared_ptr<Instance>)> equippedSignal; // void(humanoid, controller)
		wbx::signal<void(shared_ptr<Instance>)> unequippedSignal; //void(humanoid)

		wbx::remote_signal<void(shared_ptr<Instance>)> createPlatformMotor6DSignal;
		wbx::remote_signal<void()> destroyPlatformMotor6DSignal;
