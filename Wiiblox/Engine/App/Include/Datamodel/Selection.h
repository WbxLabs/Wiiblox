...
namespace WBX {
	...
	class WBXInterface ISelectionBase
	{
		friend class Selection;
	private:
	...
	private:
		...
		typedef std::map<wbx::Instance*, wbx::signals::connection> Connections;
		Connections connections;

	public:
		wbx::signal<void(const SelectionChanged&)> selectionChanged;
		wbx::signal<void()> luaSelectionChanged;
	...
	private:
		...
		void propagateChangeSignalToLua(const wbx::SelectionChanged& event);
		bool instanceCanLiveInSelection(Instance* instance);

		wbx::signals::connection selectionChangedConnection;
	};
