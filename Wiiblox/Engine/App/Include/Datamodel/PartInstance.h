...
typedef wbx::Intrusive::Set< class PartInstance, class PhysicsService >::Hook PhysicsServiceHook;
...
public:
	...
	static void convertToNewPhysicalPropRecursive(wbx::Instance* instance);
	static bool instanceOrChildrenContainsNewCustomPhysics(wbx::Instance* instance);
...
public:
	wbx::remote_signal<void(wbx::SystemAddress)>* getOrCreateNetworkOwnerChangedSignal(bool create);

	//helper function
	static void printNetAPIDisabledMessage();

	// Networking - streaming, no UI
	static const Reflection::PropDescriptor<PartInstance, wbx::SystemAddress> prop_NetworkOwner;
	static const Reflection::PropDescriptor<PartInstance, bool> prop_NetworkIsSleeping;
	
    static const Reflection::PropDescriptor<PartInstance, wbx::SystemAddress> prop_ConfirmedNetworkOwner;
    static const Reflection::PropDescriptor<PartInstance, wbx::SystemAddress> prop_TransitionalNetworkOwner;
	...
	class TouchedSignal : public wbx::signal<void(shared_ptr<Instance>)>
	{
		private:
			typedef wbx::signal<void(shared_ptr<Instance>)> Super;
	...
	public:
		PartInstance* part;
		template<typename F>
		wbx::signals::connection connect(F slot)
		{
			FASTLOG1(FLog::TouchedSignal, "Signal connected (no upper message = lua) - %p", this);
			wbx::signals::connection con = Super::connect(TouchedSlot(slot, part));
	...
	public:
		...
		wbx::signal<void(bool)> sleepingChangedSignal;
        wbx::signal<void(shared_ptr<Instance>)> clumpChangedSignal;

		// Fired for humanoid state when CFrame changed from reflection
		wbx::signal<void()> cframeChangedFromReflectionSignal;

		TouchTransmitter* touchTransmitter;
		wbx::atomic<int> touchedSlotCount;
		TouchedSignal touchedSignal;
		TouchedSignal touchEndedSignal;

		wbx::signal<void(shared_ptr<Instance>)> deprecatedStoppedTouchingSignal;
		wbx::signal<void()> outfitChangedSignal;

		wbx::signal<void(shared_ptr<Instance>)> localSimulationTouchedSignal;
        wbx::signal<void(bool)> buoyancyChangedSignal;

        wbx::signal<void(PartInstance* , CoordinateFrame&, wbx::Velocity&, float&)> onPositionUpdatedByNetworkSignal;
	...
	wbx::signal<void(shared_ptr<Instance>)>* getOrCreateDeprecatedStoppedTouchingSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->deprecatedStoppedTouchingSignal : NULL; }
	wbx::signal<void(shared_ptr<Instance>)>* getOrCreateLocalSimulationTouchedSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->localSimulationTouchedSignal : NULL; }
	wbx::signal<void()>* getOrCreateOutfitChangedSignal(bool create = true) { return (onDemandRead() || create) ? &onDemandWrite()->outfitChangedSignal : NULL; }
...
public:
	...
	virtual void setPartSizeXml(const Vector3& WBXSize);
	...
	virtual Surface getSurface(const WBXRay& gridRay, int& surfaceId);
	...
	//
	const wbx::SystemAddress getNetworkOwner() const;
	void setNetworkOwner(const wbx::SystemAddress value);
	void setNetworkOwnerNotifyIfServer(const wbx::SystemAddress value, bool ownershipBecomingManual);
	...
	void setNetworkOwnerAndNotify(const wbx::SystemAddress value);
	void notifyNetworkOwnerChanged(const wbx::SystemAddress oldOwner);
#ifdef WBX_TEST_BUILD
	wbx::signal<void(float, float)> ownershipChangeSignal;
#endif
	...
	static void checkConsistentOwnerAndRuleResetRoots(wbx::SystemAddress& consistentAddress, bool& hasConsistentOwner, bool& hasConsistentManualOwnershipRule, std::vector<Primitive*>& primRoots);
	...
	//
	/*override*/ bool hitTestImpl(const WBXRay& worldRay, Vector3& worldHitPoint);
	/*override*/ Extents computeExtentsWorld() const;

	bool containedByFrustum(const wbx::Frustum& frustum) const;
    bool intersectFrustum(const wbx::Frustum& frustum) const;
