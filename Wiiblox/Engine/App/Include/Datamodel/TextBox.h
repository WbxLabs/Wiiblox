...
namespace WBX
{
	...
	private:
		...
		void keyUp(RepeatKeyState::KeyType keyType, wbx::KeyCode keyCode, char key);
		void keyDown(RepeatKeyState::KeyType keyType, wbx::KeyCode keyCode, char key);
		void textInput(RepeatKeyState::KeyType keyType, std::string textInput);

		int getCursorPos(wbx::Vector2 mousePos);
        ...
        wbx::signals::scoped_connection textBoxFinishedEditingConnection;
	...
	public:
		...
		wbx::signal<void(bool, const shared_ptr<Instance>)> focusLostSignal;
		wbx::signal<void()> focusGainedSignal;
