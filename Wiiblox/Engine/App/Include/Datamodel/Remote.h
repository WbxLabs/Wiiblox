...
namespace WBX {
    ...
    public:
        ...
        template <typename F> wbx::signals::connection connect(const F& function)
        {
            wbx::signals::connection result = BaseSignal<Signature>::connect(function);
            queue.process();
            return result;
        }
    ...
    private:
        ...
        void push(const boost::function<void ()>& item)
		{
			if (!queue.push(item))
			{
                ...
                wbx::StandardOut::singleton()->printf(wbx::MESSAGE_ERROR, "Remote event invocation queue exhausted for %s; did you forget to implement %s?", name.c_str(), eventName);
			}
		}
	...
	public:
		...
		wbx::remote_signal<void(int, shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)> remoteOnInvokeServer;
		wbx::remote_signal<void(int, shared_ptr<const Reflection::Tuple>)> remoteOnInvokeClient;

		wbx::remote_signal<void(int, shared_ptr<const Reflection::Tuple>)> remoteOnInvokeSuccess;
		wbx::remote_signal<void(int, std::string)> remoteOnInvokeError;
	...
	public:
		...
		LatchedSignal<wbx::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)>* getOrCreateOnServerEvent(bool create = true);
		LatchedSignal<wbx::remote_signal, void(shared_ptr<const Reflection::Tuple>)>* getOrCreateOnClientEvent(bool create = true);
    ...
    private:
        ...
        LatchedSignal<wbx::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)> onServerEvent;
		LatchedSignal<wbx::remote_signal, void(shared_ptr<const Reflection::Tuple>)> onClientEvent;
	};
