...
namespace WBX {
	...
	public:
		typedef wbx::Intrusive::Set< PartInstance, PhysicsService > Parts;
	...
	private:
		...
		wbx::signals::scoped_connection touchesConnection;
		wbx::atomic<int> touchSentCounter;
		int touchResetCount;
		int touchSendListId;

		// used to determine number of physics senders
		wbx::signals::connection playersChangedConnection;

		ConcurrencyValidator concurrencyValidator;

		/*override*/ void onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider);

		wbx::signals::connection assemblyPhysicsOnConnection;
		wbx::signals::connection assemblyPhysicsOffConnection;
	...
	public:
		...
		// Interface
		wbx::signal<void(shared_ptr<Instance>)> assemblyAddingSignal;				// state change scripts			
		wbx::signal<void(shared_ptr<Instance>)> assemblyRemovedSignal;			// state change scripts
