...
static inline void WiibloxScriptModifiedCheck(wbx::Security::Permissions perm)
{
#ifndef WBX_STUDIO_BUILD
    wbx::Security::Context::current().requirePermission(wbx::Security::WiibloxScript);
    if (perm != wbx::Security::WiibloxScript)
    {
...
public:
	...
	static wbx::atomic<int> count;
	std::auto_ptr<wbx::Verb> lockVerb;

	wbx::signal<void()> screenshotSignal;
	wbx::signal<void(const std::string &)> screenshotReadySignal;
	wbx::signal<void(bool)> screenshotUploadSignal;

	wbx::signal<void(bool)> graphicsQualityShortcutSignal;

	wbx::signal<void()> allowedGearTypeChanged;
	wbx::signal<void(const shared_ptr<InputObject>& event)> InputObjectProcessed;

	wbx::signal<void()> workspaceLoadedSignal;
	wbx::signal<void()> gameLoadedSignal;
	...
	static void setLoaderFunction(boost::function<void(wbx::DataModel*)> loader) { loaderFunc = loader; }
	...
	void setGame(wbx::Game* newGame) { game = newGame; }
...
private:			
	...
	static boost::function<void(wbx::DataModel*)> loaderFunc;
...
public:
	...
	wbx::signal<void(shared_ptr<Instance>, const Reflection::PropertyDescriptor*)> itemChangedSignal;

	void clearContents(bool resettingSimulation);
    
    void setInitialScreenSize(wbx::Vector2 newScreenSize);
	...
	void setPlaceID(int placeID, bool WiibloxPlace);
	...
	void saveToWiiblox(boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction);
	...
	static shared_ptr<DataModel> createDataModel(bool startHeartbeat, wbx::Verb* lockVerb, bool shouldShowLoadingScreen);
    
	static void closeDataModel(shared_ptr<DataModel> dataModel);
    
	~DataModel();
	DataModel(wbx::Verb* lockVerb);
	...
	wbx::signal<void()> saveFinishedSignal;
	...
	static bool canSave(const wbx::Instance* instance);
	...
	static void TakeScreenshotTask(weak_ptr<wbx::DataModel> weakDataModel);
	static void ScreenshotReadyTask(weak_ptr<wbx::DataModel> weakDataModel, const std::string &filename);

	// true: finished, false: started
	static void ScreenshotUploadTask(weak_ptr<wbx::DataModel> weakDataModel, bool finished);

	static void ShowMessage(weak_ptr<wbx::DataModel> weakDataModel, int slot, const std::string &message, double duration);
...
private:
    wbx::signals::scoped_connection unbindResourceSignal;
