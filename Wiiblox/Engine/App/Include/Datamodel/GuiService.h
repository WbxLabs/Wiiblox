...
namespace WBX {
	...
	private:
        ...
        wbx::signals::scoped_connection screenGuiConnection;
	...
	public:
		...
		GuiService();
		wbx::signal<void(std::string)> openUrlWindow;
		wbx::signal<void()> urlWindowClosed;

		wbx::signal<void(std::string, std::string)> keyPressed;
		wbx::signal<void()> escapeKeyPressed;
		wbx::signal<void(GuiService::SpecialKey, std::string)> specialKeyPressed;

		wbx::signal<void(std::string)> newErrorSignal;
		wbx::signal<void(UiMessageType, std::string)> newUiMessageSignal;

		wbx::signal<void()> showLeaveConfirmationSignal;

		wbx::signal<void()> menuOpenedSignal;
		wbx::signal<void()> menuClosedSignal;
		...
		bool processKeyDown(const shared_ptr<wbx::InputObject>& event);
