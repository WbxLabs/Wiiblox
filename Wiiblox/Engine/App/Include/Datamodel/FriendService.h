...
namespace WBX {
	...
	public:
		...
		//Should be private
		wbx::remote_signal<void(int, int, FriendEventType)> friendEventReplicatingSignal;
		wbx::remote_signal<void(int, int, FriendStatus)> friendStatusReplicatingSignal;		
	...
	private:
		...
		wbx::signals::scoped_connection friendStatusConnection;
		
		void friendEventReplicationChanged(int playerId, int otherPlayerId, FriendEventType status);
		wbx::signals::scoped_connection friendEventConnection;
