...
public:
	...
	bool startDecalDrag(Decal *decal, wbx::InsertMode insertMode);
...
private:
	...
	shared_ptr<wbx::InputObject>			idleMouseEvent;		// copy this for the idle event
...
public:
	...
	wbx::signal<void(const TouchPair&)> stepTouch;	// called for each touch on a step
	wbx::signal<void(shared_ptr<Camera>)> currentCameraChangedSignal;
...
public:
	...
	shared_ptr<const Reflection::Tuple> getRayHit(WBXRay ray, shared_ptr<IgnoreType> ignore, bool terrainCellsAreCubes = false, bool ignoreWaterCells = false);
...
private:
	...
	wbx::signals::scoped_connection setDefaultMouseCommandConnection;
...
public:
	static bool serverIsPresent(const Instance* context);		// shortcut for wbx::Network::Players::serverIsPresent
	static bool clientIsPresent(const Instance* context);		// shortcut for wbx::Network::Players::clientIsPresent
...
public:
    ...
    wbx::signal<void(int)> luaPhysicsAnalyzerIssuesFound;
