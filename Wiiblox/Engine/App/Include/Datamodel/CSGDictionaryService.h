...
namespace WBX
{
	...
	class CSGDictionaryService
		: public DescribedCreatable<CSGDictionaryService, FlyweightService, sCSGDictionaryService, Reflection::ClassDescriptor::PERSISTENT, Security::Wiiblox>
	{
	protected:
		typedef DescribedCreatable<CSGDictionaryService, FlyweightService, sCSGDictionaryService, Reflection::ClassDescriptor::PERSISTENT, Security::Wiiblox> Super;
		...
		void reparentChildData(shared_ptr<wbx::Instance> sharedInstance);

		virtual void refreshRefCountUnderInstance(wbx::Instance* instance);

        boost::shared_ptr<CSGMesh> insertMesh(const std::string key, const wbx::BinaryString& meshData);

		boost::shared_ptr<CSGMesh> insertCachedMesh(const std::string key, const wbx::BinaryString& meshData);
