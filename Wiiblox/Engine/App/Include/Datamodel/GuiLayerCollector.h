...
namespace WBX {
	...
	private:
		...
		static void LoadZ(const shared_ptr<wbx::Instance>& instance, GuiLayers guiVectors[]);
		void loadZVectors();
        ...
        GuiResponse doProcessGesture(const boost::shared_ptr<GuiBase>& guiBase, const UserInputService::Gesture& gesture, const shared_ptr<const wbx::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args);
		...
		GuiLayers mGuiVectors[wbx::GUIQUEUE_COUNT];		// temp arrays for rendering - never realloc, always fast clear
        
        boost::unordered_map<Instance*,wbx::signals::scoped_connection> propertyConnections;
	};
