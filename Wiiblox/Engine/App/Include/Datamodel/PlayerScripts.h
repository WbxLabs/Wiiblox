...
namespace WBX {
	...
	private:
		...
		wbx::signals::scoped_connection initializeDefaultScriptsConnection;

		wbx::signal<void()> defaultScriptsLoadedSignal;	
	...
	public:
		...
		void defaultScriptsSend(weak_ptr<wbx::Network::Player> p);
		void defaultScriptsReceived(int confirm);

		wbx::remote_signal<void(shared_ptr<Instance>)> requestDefaultScriptsSignal;
		wbx::remote_signal<void(int)> confirmDefaultScriptsSignal;
