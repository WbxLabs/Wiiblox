...
// TODO: Refactor: Call this wbx::DOM or something
class SerializerV2 {
protected:
...
public:
	...
	// reading:
	void loadInstancesFromText(const XmlElement* root, wbx::Instances& result);
	
	// Until DataModel becomes an Instance and it can handle "globals" like Workspace, we need to treat
	// it specially during reads:
	void load(std::istream& stream, wbx::DataModel* dataModel);
	void loadInstances(std::istream& stream, wbx::Instances& result);

private:
	void loadXML(std::istream& stream, wbx::DataModel* dataModel);
	void loadInstancesXML(const XmlElement* root, wbx::Instances& result, wbx::IReferenceBinder& binder, wbx::CreatorRole creatorRole);
	shared_ptr<wbx::Instance> loadInstanceXML(const XmlElement* itemElement, wbx::IReferenceBinder& binder, wbx::CreatorRole creatorRole);
};
