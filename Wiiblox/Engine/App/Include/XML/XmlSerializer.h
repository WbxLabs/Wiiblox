...
protected:
	std::map<wbx::InstanceHandle, int> handles;
	typedef boost::unordered_map<std::string, wbx::InstanceHandle> IdValidationMap;
...
public:
	...
	int getHandleIndex(wbx::InstanceHandle h) {
		int i;
		std::map<wbx::InstanceHandle, int>::iterator iter = handles.find(h);
		if (iter==handles.end()) {
	...
	bool isValidId(const std::string& id, const wbx::InstanceHandle& h) const
	{
	...
	void recordId(const std::string& id, const wbx::InstanceHandle& h)
	{
