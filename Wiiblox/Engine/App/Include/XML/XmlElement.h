...
extern const Name& value_IDREF_null;
extern const Name& value_IDREF_nil;

// TODO: Put these in a file that knows about the Wiiblox schema
extern const XmlTag& name_root;
extern const XmlTag& tag_Wiiblox;
...
private:
	...
	union {
		...
		mutable wbx::ContentId* contentIdValue;
		...
		mutable const Name* nameValue;
		mutable wbx::InstanceHandle* handleValue;		// TODO: with in-place constructor/destructor, could we avoid a "new"
	};
...
public:
	...
	XmlNameValuePair(const XmlTag& tag, const char* text)
		:tag(tag),stringValue(new std::string(text)),valueType(STRING)     {}
	XmlNameValuePair(const XmlTag& tag, wbx::ContentId contentId)
		:tag(tag),contentIdValue(new wbx::ContentId(contentId)),valueType(CONTENTID)     {}
	...
	XmlNameValuePair(const XmlTag& tag, const Name* value)
	...
	XmlNameValuePair(const XmlTag& tag, wbx::InstanceHandle value)
		:tag(tag),valueType(HANDLE),handleValue(new wbx::InstanceHandle(value)) {}
	~XmlNameValuePair() {
		clearValue();
	}
	...
	bool isValueEqual(wbx::ContentId contentId) const;
	...
	bool isValueEqual(const Name* value) const;
	bool isValueEqual(wbx::InstanceHandle value) const;
	...
	bool getValue(wbx::ContentId& contentId) const;
	...
	bool getValue(const Name* &value) const;
	bool getValue(wbx::InstanceHandle &value) const;
	// Templated version that can be implemented by clients
	template<class T>
	bool getValue(T& value) const;

	void setValue(std::string value) {clearValue(); stringValue = new std::string(value); valueType=STRING; }
	void setValue(wbx::ContentId contentId) {clearValue(); contentIdValue = new wbx::ContentId(contentId); valueType=CONTENTID; }
	...
	void setValue(const Name* value) {clearValue(); nameValue = value; valueType=NAME; };
	void setValue(wbx::InstanceHandle handle) {clearValue(); handleValue = new wbx::InstanceHandle(handle); valueType=HANDLE; };
	// Templated version that can be implemented by clients
	template<class T>
...
class XmlAttribute 
	: public wbx::Sibling<XmlAttribute>
	, public XmlNameValuePair
	, public wbx::Allocator<XmlAttribute> 
{
public:
...
class XmlElement 
	: public wbx::Sibling<XmlElement>
	, public wbx::Parent<XmlElement>
	, public XmlNameValuePair
	, public wbx::Allocator<XmlElement> 
{
#ifdef _DEBUG
	char leak[15];
	void recordLeak()
...
#endif
private:
	wbx::Parent<XmlAttribute> attributes;
...
public:
	...
	inline bool findAttributeValue(const XmlTag& _tag, const Name*& value) const {
