...
class TestJSON : public wbx::FastLogJSON
{
};
...
BOOST_AUTO_TEST_CASE(Serializer)
{
	...
	boost::filesystem::path jsonFolder = wbx::FileSystem::getUserDirectory(true, wbx::DirExe, "ClientSettings") / "Test.json";
...
BOOST_FIXTURE_TEST_CASE(Lua, DataModelFixture)
{
	...
	std::auto_ptr<wbx::Reflection::Tuple> result = execute("return settings():GetFVariable('TestTrueValue'), settings():GetFVariable('TestFalseValue'), settings():GetFFlag('TestTrueValue'), settings():GetFFlag('TestFalseValue')");
