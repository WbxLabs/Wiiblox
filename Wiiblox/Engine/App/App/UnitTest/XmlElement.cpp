...
{
	std::ifstream stream("C:\\Users\\erik.cassel\\Documents\\512balls_128chars_8000fixed.WBXl", std::ios_base::in | std::ios_base::binary);
	TextXmlParser machine(stream.rdbuf());
...
{
	XmlNameValuePair element(wbx::Name::getNullName(), "-12");
	int i;
	BOOST_CHECK(element.getValue(i));
	BOOST_CHECK_EQUAL(i, -12);

	XmlNameValuePair element2(wbx::Name::getNullName(), "x12");
	BOOST_CHECK(!element2.getValue(i));

	XmlNameValuePair element3(wbx::Name::getNullName(), "12x");
