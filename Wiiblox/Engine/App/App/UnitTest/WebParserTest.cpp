...
BOOST_AUTO_TEST_SUITE(WebParser)
	...
	BOOST_AUTO_TEST_CASE(TestJSONEmpty)
	{
		...
		BOOST_CHECK(wbx::WebParser::parseJSONObject("{}", v));
	...
	{
		Reflection::Variant v;
		BOOST_CHECK(wbx::WebParser::parseJSONObject(
	...
	BOOST_AUTO_TEST_CASE(TestJSONWriteBasic)
	{
		...
		(*table)["b"] = testValue;
		(*table)["c"] = false;
		(*table)["d"] = shared_ptr<const Reflection::ValueTable>(wbx::make_shared<Reflection::ValueTable>());
		(*table)["e"] = shared_ptr<const Reflection::ValueArray>(wbx::make_shared<Reflection::ValueArray>());
		(*table)["f"] = wbx::Vector3();
		...
		std::string result;
		BOOST_CHECK_EQUAL(wbx::WebParser::writeJSON(v, result, wbx::WebParser::FailOnNonJSON), false);
		BOOST_CHECK(wbx::WebParser::writeJSON(v, result, wbx::WebParser::SkipNonJSON));
		...
		BOOST_CHECK(wbx::WebParser::parseJSONObject(result, parsedV));
	...
	BOOST_AUTO_TEST_CASE(TestJSONRountrip)
	{
		...
		Reflection::Variant v = shared_ptr<const Reflection::ValueTable>(table);
		std::string result;
		BOOST_CHECK(wbx::WebParser::writeJSON(v, result));
	...
	{	
		...
		wbx::Reflection::Variant value;

		std::auto_ptr<std::istream> stream(new std::istringstream(response));
		BOOST_REQUIRE(wbx::WebParser::parseWebGenericResponse(*stream, value));
	...
	{	
		...
		std::auto_ptr<std::istream> stream(new std::istringstream(response));
		if(!wbx::WebParser::parseWebGenericResponse(*stream, value)){
			BOOST_CHECK(false);
		}
	...
	{	
		...
		}
		{
			...
			std::auto_ptr<std::istream> stream(new std::istringstream(response));
			BOOST_CHECK(wbx::WebParser::parseWebGenericResponse(*stream, value));
			BOOST_CHECK(value.isType<bool>());
		}
	...
	"<Wiiblox xmlns:xmime=\"http://www.w3.org/2005/05/xmlmime\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:noNamespaceSchemaLocation=\"http://www.Wiiblox.com/Wiiblox.xsd\" version=\"4\">"
	"<External>null</External>								"
	"<External>nil</External>								"
	"<Item class=\"Part\" referent=\"WBX0\">				"
	"	<Properties>										 "
