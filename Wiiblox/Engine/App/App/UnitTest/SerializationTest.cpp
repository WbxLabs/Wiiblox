	...
	: public DescribedCreatable<SerializationTestInstance, Instance, sSerializationTestInstance>
{
		...
		: DescribedCreatable<SerializationTestInstance, Instance, sSerializationTestInstance>(generateRandomString(*rngp).c_str())
	{
		...
		prop8 = WBXRay(Vector3(rng.uniform(), rng.uniform(), rng.uniform()), Vector3(rng.uniform(), rng.uniform(), rng.uniform()));
...
static Reflection::BoundProp<WBXRay> desc_prop8("prop8", category_Data, &SerializationTestInstance::prop8);
...
WBX_REGISTER_CLASS(SerializationTestInstance);
