...
static void initFlatDataHierarchy(wbx::Instance* parent, int numScripts)
{
	for (int ii = 0; ii < numScripts; ++ii)
	{
		//create script
		shared_ptr<wbx::Script> scriptInstance = wbx::Creatable<wbx::Instance>::create<wbx::Script>();
		scriptInstance->setEmbeddedCode(wbx::ProtectedString::fromTestSource(DEFAULT_SCRIPT_CODE));
		...
		shared_ptr<wbx::IntValue> intvalue = wbx::Creatable<wbx::Instance>::create<wbx::IntValue>();
		intvalue->setParent(parent);
	}
}

static void initNestedDataHierarchy(wbx::Instance* parent, int numScripts)
{
	for (int ii = 0; ii < numScripts; ++ii)
	{
		//create script
...
static void addDebuggerData(shared_ptr<wbx::Instance> instance, shared_ptr<wbx::Scripting::DebuggerManager> debuggerManager, const DebuggerData &debuggerData)
{
	shared_ptr<wbx::Script> script = wbx::Instance::fastSharedDynamicCast<wbx::Script>(instance);
	if (script)
	{
		//there shouldn't be any debugger present for this script
		wbx::Scripting::ScriptDebugger* debugger = debuggerManager->findDebugger(script.get());
		...
		{
			shared_ptr<wbx::Scripting::DebuggerBreakpoint> bp = debugger->setBreakpoint(debuggerData.breakpoints[ii]);
			BOOST_CHECK(bp != NULL);

			if (!debuggerData.breakpointConditions[ii].empty())
				wbx::Scripting::DebuggerBreakpoint::prop_Condition.setValue(bp.get(), debuggerData.breakpointConditions[ii]);			
		}
...
static void verifyDebuggerData(shared_ptr<wbx::Instance> instance, shared_ptr<wbx::Scripting::DebuggerManager> debuggerManager, const DebuggerData &debuggerData)
{
	shared_ptr<wbx::Script> script = wbx::Instance::fastSharedDynamicCast<wbx::Script>(instance);
	if (script)
	{
		...
		wbx::Scripting::DebuggerBreakpoint* bp;
		for (int ii = 0; ii < 3; ++ii)
		{
		...
		if (!debuggerData.watch.empty())
		{
			const wbx::Scripting::ScriptDebugger::Watches& watches = debugger->getWatches();
...
BOOST_AUTO_TEST_CASE(FlatDataSerialization)
{
	...
	wbx::Scripting::DebuggerManager& debuggerManager = wbx::Scripting::DebuggerManager::singleton();
	
	std::stringstream binaryDataStreamOriginal(std::ios_base::out | std::ios_base::in | std::ios_base::binary);

	{		
		DataModelFixture dm;
		wbx::Workspace* workspace = dm.dataModel->getWorkspace();
		...
		wbx::SerializerBinary::serialize(binaryDataStreamOriginal, &debuggerManager);
	...
	workspace->replenishCamera();
	initFlatDataHierarchy(workspace, 5);
	wbx::SerializerBinary::deserialize(binaryDataStreamOriginal, &debuggerManager);
	...
	wbx::SerializerBinary::serialize(binaryDataStreamNew, &debuggerManager);
