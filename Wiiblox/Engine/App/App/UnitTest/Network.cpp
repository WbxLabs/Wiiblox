...
WBX_REGISTER_CLASS(TheNewClass);
...
{
    WBXASSERT(Reflection::Descriptor::lockedDown);
...
BOOST_AUTO_TEST_CASE(FilterEnabled)
{
	...
	{
		wbx::DataModel::LegacyLock lock(&server, wbx::DataModelJob::Write);
	...
	{
		wbx::DataModel::LegacyLock lock(&client1, wbx::DataModelJob::Write);
		clientPart1 = client1->getWorkspace()->findFirstChildByName(BRICK_STRING);
		BOOST_REQUIRE(clientPart1);
	}
	Instance* clientPart2;
	{
		wbx::DataModel::LegacyLock lock(&client2, wbx::DataModelJob::Write);
...
public:
	...
	boost::shared_ptr<wbx::Instance> instance;

	wbx::signals::scoped_connection propChangedConnection;
	wbx::signals::scoped_connection ancestryChangedConnection;

	ParentChangeRecorder(boost::shared_ptr<wbx::Instance>_instance) : instance(_instance)
	{
	...
	void onParentChanged(const wbx::Reflection::PropertyDescriptor* desc)
	{
		if (*desc == wbx::Instance::propParent)
		{
...
static void NewInstanceParentAlwaysValid_onChildAdded(shared_ptr<Instance> child, shared_ptr<wbx::CEvent> event)
{
...
BOOST_AUTO_TEST_CASE(NewInstanceParentAlwaysValid)
{
	...
	// Confirm the client got it
	Instance* clientPart;
	shared_ptr<wbx::CEvent> event(new wbx::CEvent(true));
	{
		wbx::DataModel::LegacyLock lock(&clientDm, wbx::DataModelJob::Write);
		clientPart = clientDm->getWorkspace()->findFirstChildByName("TestPart");
		BOOST_REQUIRE(clientPart);
...
static void onChildAdded(shared_ptr<Instance> child, shared_ptr<wbx::CEvent> event)
{
...
static void onScriptLockGrantedOrNot(bool lockGranted, bool* state, wbx::CEvent* event)
{
	*state = lockGranted;
	event->Set();
}
...
BOOST_AUTO_TEST_CASE(ReplicationExplicitlyAssignDefaultPropertyValues)
{
	...
	BOOST_CHECK(requestLockWaitEvent.Wait(wbx::Time::Interval::from_seconds(SLEEPTIME)));
...
{
    if (wbx::Test::BaseGlobalFixture::fflagsAllOn)
    {
        commandLine.append(" --fflags=true");
    }
    else if (wbx::Test::BaseGlobalFixture::fflagsAllOff)
    {
        ...
        {
            {
                DataModel::LegacyLock l(&clientDm, DataModelJob::Write);
                G3D::Random rng(1);
                wbx::Network::Players* players = wbx::ServiceProvider::find<wbx::Network::Players>(&clientDm);
                WBXASSERT(players);
...
#ifndef _DEBUG
BOOST_FIXTURE_TEST_CASE(Perf, wbx::Test::PerformanceTestFixture)
{
	DataModelFixture serverDm;
	NetworkFixture::startServer(serverDm);
	...
	// Create a part on the server
	{
		...
		part->setCoordinateFrame(wbx::CoordinateFrame(G3D::Vector3(0, 0, 10)));
		part->setParent(serverDm->getWorkspace());
	}
...
static void on_dp_ChildAdded(shared_ptr<Instance> child, shared_ptr<wbx::CEvent> event)
{
	if (child->getName() == "LastPartDP")
		event->Set();
};

static void onGameLoaded( shared_ptr<wbx::CEvent> event)
{
...
BOOST_FIXTURE_TEST_CASE(DeserializeProcessTest, wbx::Test::PerformanceTestFixture)
{		
	wbx::Time now;
	wbx::Time was;
	...
	shared_ptr<wbx::CEvent> event1(new wbx::CEvent(true));
	shared_ptr<wbx::CEvent> event2(new wbx::CEvent(true));
	...
	event2->Wait(wbx::Time::Interval::from_seconds(2));	// if it takes this long it must be ready.
	...
	// Create parts on the client
	{
		...
		was = wbx::Time::now(wbx::Time::Benchmark);
	...
	// Confirm the client got it
	BOOST_CHECK(event1->Wait(wbx::Time::Interval::from_seconds(60)));
	now = wbx::Time::now(wbx::Time::Benchmark);
	double ptime = (now - was).seconds();

	BOOST_MESSAGE(wbx::format("time for parts to propagate to server %f seconds", ptime));
...
std::auto_ptr<wbx::Reflection::Tuple> DataModelFixture::execute(const char* script, const wbx::Reflection::Tuple& args, wbx::Security::Identities identity)
{
...
BOOST_AUTO_TEST_CASE(NewClass)
{
...

    {
        // Create an instance from the new class on the server
        {
            ...
            shared_ptr<PartInstance> ins = Creatable<Instance>::create<wbx::TheNewClass>();
...
BOOST_AUTO_TEST_CASE(PropTestServer)
{
    ...
    shared_ptr<PartInstance> serverIns;
    {
        ...
        serverIns = Creatable<Instance>::create<wbx::TheNewClass>();
        serverIns->setAnchored(true);
        serverIns->setName("TheNewClass");
        serverIns->setParent(serverDm->getWorkspace());
        serverIns->fastDynamicCast<wbx::TheNewClass>()->setExistingProp(1);
        serverIns->fastDynamicCast<wbx::TheNewClass>()->setTheNewProp(1);
    }
...

    {
        ...
        serverIns->fastDynamicCast<wbx::TheNewClass>()->setExistingProp(2);
        serverIns->fastDynamicCast<wbx::TheNewClass>()->setTheNewProp(2);
    }
...
BOOST_AUTO_TEST_CASE(PropTestClient)
{
    ...
    Instance* clientIns;
    {
        ...
        wbx::TheNewClass* newIns = clientIns->fastDynamicCast<wbx::TheNewClass>();
...
BOOST_AUTO_TEST_CASE(EventTestServer)
{
...

    {
        ...
        wbx::TheNewClass* newIns = serverIns->fastDynamicCast<wbx::TheNewClass>();
        newIns->connectExistingSignal();
        wbx::TheNewClass::event_ExistingEvent->fireAndReplicateEvent(newIns, 1);
        BOOST_CHECK(newIns->existingEvt.Wait(wbx::Time::Interval::from_seconds(1)));
        BOOST_CHECK_EQUAL(newIns->existingEventValue, 1);
        newIns->connectTheNewSignal();
        wbx::TheNewClass::event_TheNewEvent->fireAndReplicateEvent(newIns, 1);
        BOOST_CHECK(newIns->theNewEvt.Wait(wbx::Time::Interval::from_seconds(1)));
...
BOOST_AUTO_TEST_CASE(EventTestClient)
{
    ...
    //G3D::System::sleep(1);
    {
        ...
        wbx::signals::scoped_connection connection(newIns->connectExistingSignal());
        BOOST_CHECK(newIns->existingEvt.Wait(wbx::Time::Interval::from_seconds(2)));
        BOOST_CHECK_EQUAL(newIns->existingEventValue, 1);
