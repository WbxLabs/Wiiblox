...
BOOST_AUTO_TEST_CASE( SettingsAreAvailableThroughLua )
{
	...
	std::auto_ptr<wbx::Reflection::Tuple> result = dm.execute(
		"return settings()");
...
BOOST_AUTO_TEST_CASE( SettingsAreSharedBetweenDataModels )
{
	...
	std::auto_ptr<wbx::Reflection::Tuple> result = dm1.execute(
...
{
	wbx::TaskSchedulerSettings::singleton();
	SettingsTestFixture dm;
	std::auto_ptr<wbx::Reflection::Tuple> result = dm.execute(
...
BOOST_AUTO_TEST_CASE( AddingPartUnderSelectionDoesNukeSettings )
{
	...
	BOOST_CHECK_EQUAL((wbx::Instances*)NULL, sel->getChildren().read().get());
}
