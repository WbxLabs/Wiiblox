...
static unsigned int countChunkPackets(shared_ptr<ReplicatorTestWrapper>& sender, shared_ptr<ReplicatorTestWrapper>& receiver) {
    sender->getReplicationData().initialSendIterator = wbx::ClusterChunksIterator(sender->cluster->getVoxelGrid()->getNonEmptyChunks());
...
// 		ReadWriteBuffer buffer;
// 		wbx::VarInt<>::encode(buffer.writeStream.get(), i << 3);
// 		buffer.updateRead();
// 		unsigned int result;
// 		wbx::VarInt<>::decode(*(buffer.readStream), &result);
...
static CellMaterial produceRandomMaterial(unsigned hash)
{
	...
	WBXASSERT(material >= CELL_MATERIAL_Grass && material <= CELL_MATERIAL_Water);
...
static Cell produceRandomCell(CellMaterial material)
{
	...
	if (material != CELL_MATERIAL_Water) {
		...
		WBXASSERT(cellBlock < MAX_CELL_BLOCKS);
...
BOOST_AUTO_TEST_CASE(SendRandomTerrain) {
	// Disabling WBXASSERT checks speeds up this test somewhat:
	// FLog::Asserts = 0;
