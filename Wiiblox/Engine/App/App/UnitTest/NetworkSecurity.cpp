...
BOOST_AUTO_TEST_CASE(SecurePlayerReplication)
{
	...
	{
		wbx::DataModel::LegacyLock lock(&clientDm, wbx::DataModelJob::Write);
		wbx::Network::Player* player = wbx::Network::Players::findLocalPlayer(&clientDm);
	...
	{
		wbx::Network::Players* players = wbx::ServiceProvider::find<wbx::Network::Players>(&serverDm);
		BOOST_REQUIRE(players);

		wbx::Network::Player* player = Instance::fastDynamicCast<wbx::Network::Player>(players->getChild(0));
