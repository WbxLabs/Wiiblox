...
BOOST_AUTO_TEST_CASE(TestWBXRay)
{
	wbx::WBXRay ray = wbx::WBXRay::fromOriginAndDirection(G3D::Vector3(0, 12, 0), G3D::Vector3(2, 0, 0));	
...
BOOST_AUTO_TEST_CASE(TestCamera)
{
    ...
    ///Make sure you get the origin & direction WBXRay correct when you pass in the mouse coord & viewport rect.
    wbx::WBXRay WBXRay = camera.worldRay(377.0f, 149.0f);
    Vector3 rayOrigin = WBXRay.origin();
	...
	Vector3 direction = WBXRay.direction();
...
{
	G3D::CoordinateFrame coord(normalIdToMatrix3(wbx::NORM_Y), Vector3(0, -0.5, 0));
	G3D::CoordinateFrame dummy(normalIdToMatrix3(wbx::NORM_X_NEG), Vector3(0.5, 0.5, 0));	
	...
	wbx::WBXRay WBXRay;
	WBXRay.origin() = G3D::Vector3(0, 0, 0);
	WBXRay.direction() = G3D::Vector3(0.5f, 0.5f, 0.5f);

	wbx::WBXRay WBXRayObjectSpace = coord.toObjectSpace(WBXRay);

	BOOST_CHECK_EQUAL(WBXRayObjectSpace.origin(), G3D::Vector3(0, 0, 0.5));
	BOOST_CHECK_EQUAL(WBXRayObjectSpace.direction(), G3D::Vector3(-0.5, 0.5, 0.5));
	...
	G3D::CoordinateFrame snappedCFrame = wbx::Math::snapToGrid(presnapCFrame, 0.100000001f);
...
bool FailsPlane(const G3D::Vector3& pt, const wbx::Frustum& fr, wbx::Frustum::FrustumPlane plane)
{
...
bool FailsPlane(const wbx::Extents& ex, const wbx::Frustum& fr, wbx::Frustum::FrustumPlane plane)
{
	bool retVal = false;
...
BOOST_AUTO_TEST_CASE(TestFrustum)
{
	...
	wbx::Extents ex(G3D::Vector3(-0.5f, -0.5f, 1.f), G3D::Vector3(0.5f, 0.5f, 2.f));
	wbx::Frustum fr1(G3D::Vector3::zero(), G3D::Vector3(0, 0, 1), G3D::Vector3(0, 1, 0), kNearPlaneDist, kFarPlaneDist, kNinetyDegInRad, kNinetyDegInRad);
	wbx::Frustum fr2(G3D::Vector3::zero(), G3D::Vector3(0, 0, 1), G3D::Vector3(0, 1, 0), kNearPlaneDist, wbx::Math::inf(), kNinetyDegInRad, kNinetyDegInRad);

	// extents completely in frustum
	BOOST_CHECK(fr1.containsAABB(ex));
	BOOST_CHECK(fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneInvalid));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneInvalid));

	// crosses far plane
	ex = wbx::Extents(G3D::Vector3(-0.5f, -0.5f, kFarPlaneDist - 1.f), G3D::Vector3(0.5f, 0.5f, kFarPlaneDist + 1.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(fr2.containsAABB(ex));	// no far plane
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneFar));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneInvalid));

	// beyond far plane
	ex = wbx::Extents(G3D::Vector3(-0.5f, -0.5f, kFarPlaneDist + 1.f), G3D::Vector3(0.5f, 0.5f, kFarPlaneDist + 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(fr2.containsAABB(ex));	// no far plane
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneFar));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneInvalid));
	...
	ex = wbx::Extents(G3D::Vector3(-0.1f, -0.1f, kNearPlaneDist - 0.2f), G3D::Vector3(0.1f, 0.1f, kNearPlaneDist - 0.1f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneNear));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneNear));

	// crosses near plane
	ex = wbx::Extents(G3D::Vector3(-0.1f, -0.1f, kNearPlaneDist - 0.1f), G3D::Vector3(0.1f, 0.1f, kNearPlaneDist + 0.1f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneNear));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneNear));
	...
	ex = wbx::Extents(G3D::Vector3(-0.5f, -0.5f, -2.f), G3D::Vector3(0.5f, 0.5f, -1.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));

	// above top plane
	ex = wbx::Extents(G3D::Vector3(-0.5f, 5.f, 1.f), G3D::Vector3(0.5f, 6.f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneTop));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneTop));

	// crosses top plane
	ex = wbx::Extents(G3D::Vector3(-0.5f, 0.f, 1.f), G3D::Vector3(0.5f, 6.f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneTop));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneTop));
	...
	// below bottom plane
	ex = wbx::Extents(G3D::Vector3(-0.5f, -6.f, 1.f), G3D::Vector3(0.5f, -5.f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneBottom));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneBottom));

	// crosses bottom plane
	ex = wbx::Extents(G3D::Vector3(-0.5f, -6.f, 1.f), G3D::Vector3(0.5f, 0.f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneBottom));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneBottom));
	...
	// outside left plane
	ex = wbx::Extents(G3D::Vector3(-6.f, -0.5f, 1.f), G3D::Vector3(-5.f, 0.5f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneLeft));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneLeft));

	// crosses left plane
	ex = wbx::Extents(G3D::Vector3(-6.f, -0.5f, 1.f), G3D::Vector3(0.f, 0.5f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneLeft));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneLeft));
	...
	// outside right plane
	ex = wbx::Extents(G3D::Vector3(5.f, -0.5f, 1.f), G3D::Vector3(6.f, 0.5f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneRight));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneRight));

	// crosses right plane
	ex = wbx::Extents(G3D::Vector3(0.f, -0.5f, 1.f), G3D::Vector3(6.f, 0.5f, 2.f));
	BOOST_CHECK(!fr1.containsAABB(ex));
	BOOST_CHECK(!fr2.containsAABB(ex));
	BOOST_CHECK(FailsPlane(ex, fr1, wbx::Frustum::kPlaneRight));
	BOOST_CHECK(FailsPlane(ex, fr2, wbx::Frustum::kPlaneRight));
	...
	wbx::Frustum fr3(G3D::Vector3(0,0,10000), G3D::Vector3(0, 0, 1), G3D::Vector3(0, 1, 0), kNearPlaneDist, kFarPlaneDist, kNinetyDegInRad, kNinetyDegInRad);
	...
	wbx::Frustum fr4(G3D::Vector3::zero(), G3D::Vector3(0, 0, 1), G3D::Vector3(0, 1, 0), kNearPlaneDist, wbx::Math::inf(), kNinetyDegInRad, kNinetyDegInRad);
...
BOOST_AUTO_TEST_CASE(TestFrustumTransformed)
{
	...
	wbx::Frustum fr1(G3D::Vector3(100, 200, 300), G3D::Vector3(+1, 0, 0), G3D::Vector3(0, -1, 0), 1.f, 1000.f, kNinetyDegInRad, kNinetyDegInRad);
	wbx::Frustum fr2(G3D::Vector3(100, 200, 300), G3D::Vector3(-1, 0, 0), G3D::Vector3(0, -1, 0), 1.f, 1000.f, kNinetyDegInRad, kNinetyDegInRad);
