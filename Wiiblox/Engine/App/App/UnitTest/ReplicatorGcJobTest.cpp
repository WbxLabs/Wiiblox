...
bool SetMemoryLevel(DataModelFixture& dm)
{
	...
	if (replicator->getMemoryLevel() != wbx::MemoryStats::MEMORYLEVEL_ALL_CRITICAL_LOW)
	{
	...
	BOOST_WARN(replicator->getMemoryLevel() <= wbx::MemoryStats::MEMORYLEVEL_ONLY_PHYSICAL_CRITICAL_LOW);
...
{
    ScopedAssign<bool> disableSound(wbx::Soundscape::SoundService::soundDisabled, true);
