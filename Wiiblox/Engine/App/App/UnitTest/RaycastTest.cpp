...
BOOST_AUTO_TEST_CASE( CheckInvalidFloatValue ) {
	...
	wbx::DataModel::LegacyLock lock(&dm.dm, wbx::DataModelJob::Write);
	...
	for (unsigned int i = 0; i < 4; ++i) {
		for (unsigned int j = 0; j < 4; ++j) {
			dm.workspace->getRayHit<Instance>(
				WBXRay(vectorPermutations[i], vectorPermutations[j]),
				shared_ptr<Instance>());
		}
	}
}
...
BOOST_AUTO_TEST_CASE( OriginCloseToEntryPoint ) {
	...
	dm.workspace->getRayHit<Instance>(WBXRay(rayStart, Vector3(0,3,0)), shared_ptr<Instance>());
}
...
BOOST_AUTO_TEST_CASE( CheckClusterBoundaryEmptyCluster ) {
	...
	shared_ptr<const Reflection::Tuple> rayHit =
		dm.workspace->getRayHit<Instance>(
			WBXRay(locationOutsideCluster, locationInsideCluster - locationOutsideCluster),
			shared_ptr<Instance>());
	checkRaycastResult("outside => inside", NULL, locationInsideCluster, rayHit);

	// inside => outside
	rayHit = dm.workspace->getRayHit<Instance>(
			WBXRay(locationInsideCluster, locationOutsideCluster - locationInsideCluster),
			shared_ptr<Instance>());
...
BOOST_AUTO_TEST_CASE( HitsWedgeInWaterWedgeAsAppropriate ) {
	...
	shared_ptr<const Reflection::Tuple> rayHit =
		dm.workspace->getRayHit<Instance>(
			WBXRay(hitWedgeStart, direction),
			shared_ptr<Instance>());
	...
	// miss wedge
	rayHit = dm.workspace->getRayHit<Instance>(
			WBXRay(missWedgeStart, direction),
			shared_ptr<Instance>());
...
BOOST_AUTO_TEST_CASE( GetSurfaceReturnsCellSurface ) {
	...
	{
		int tmp = 0;
		wbx::Surface surface = dm.cluster->getSurface(WBXRay(worldCellCenter - Vector3(rayDelta, 0, 0), Vector3(2 * rayDelta, 0, 0)), tmp);;
	...
	{
		int tmp = 0;
		wbx::Surface surface = dm.cluster->getSurface(WBXRay(worldCellCenter - Vector3(0, rayDelta, 0), Vector3(0, 2 * rayDelta, 0)), tmp);
	...
	{
		int tmp = 0;
		wbx::Surface surface = dm.cluster->getSurface(WBXRay(worldCellCenter - Vector3(0, 0, rayDelta), Vector3(0, 0, 2 * rayDelta)), tmp);
...
BOOST_AUTO_TEST_CASE( GetSurfaceDoesNotHitTerrainEdge )
{
	...
	wbx::Surface surface = dm.cluster->getSurface(
		WBXRay(rayStart, Vector3(0, 10 * kCELL_SIZE, 10 * kCELL_SIZE)), tmp);
...
BOOST_AUTO_TEST_CASE( IsRegion3Empty ) {
	...
	// empty terrain
	{
		...
		wbx::DataModel::LegacyLock lock(&dm, wbx::DataModelJob::Write);
		wbx::Workspace* workspace = dm->getWorkspace();
		workspace->createTerrain();
		wbx::MegaClusterInstance* cluster = Instance::fastDynamicCast<MegaClusterInstance>(
			workspace->getTerrain());
