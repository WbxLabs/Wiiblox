...
#define lua_assert(x) WBXASSERT(x)
#else
...
catch (wbx::base_exception const& e)	\
{\
...
// Wiiblox CHANGE: effectively NoOps
...
#pragma pack(push)
#pragma pack(8)
// This object is embedded in every Lua thread to manage Wiiblox-specific information

class WiibloxExtraSpace : public wbx::Intrusive::Set<WiibloxExtraSpace>::Hook
{
	typedef wbx::Intrusive::Set<WiibloxExtraSpace> AllThreads;

	struct Shared
	{
		int threadCount;
		wbx::ScriptContext* context;
	...
	typedef wbx::Lua::WeakThreadRef::Node Node;
	boost::intrusive_ptr<Node> node;
public:
	wbx::Security::Identities identity : 5;
	bool yieldCaptured : 1; 
	boost::weak_ptr<wbx::BaseScript> script;	// The script associated with this thread, if any
	boost::scoped_ptr<wbx::Lua::Continuations> continuations;

	wbx::ScriptContext* context() const { return shared->context; }
	size_t getThreadCount() const { return (size_t) shared->threadCount; }

	void setContext(wbx::ScriptContext* context) { shared->context = context; }
	...
	static WiibloxExtraSpace* get(struct lua_State *L)
	{
		return L ? reinterpret_cast<WiibloxExtraSpace*>(((char*) L) - sizeof(WiibloxExtraSpace)) : 0;
	}
	static void constructRoot(lua_State *L)
	{
		new(get(L)) WiibloxExtraSpace();
	}
	static void destroyRoot(lua_State *L)
	{
		get(L)->~WiibloxExtraSpace();
	}
	static void constructChild(lua_State *L, WiibloxExtraSpace *parent)
	{
		new(get(L)) WiibloxExtraSpace(parent);
	}
...
private:
	WiibloxExtraSpace()
		:shared(new Shared())
		,identity(wbx::Security::Anonymous)
		,node(0)
	{
		shared->threadCount++;
	...
	WiibloxExtraSpace(WiibloxExtraSpace* parent)
		...
		,node(parent->node)
	{
        ...
        {
            wbx::Tokens::apiToken.addFlagSafe(wbx::kScriptContextCopy);
        }
        else
        {
           shared->allThreads.insert(*this);
	...
	~WiibloxExtraSpace()
	{
		...
		shared->threadCount--;
		WBXASSERT(shared->threadCount >= 0);
	}
...
*/
#define LUAI_EXTRASPACE		sizeof(WiibloxExtraSpace)
...
{
	WiibloxExtraSpace::constructRoot(L);
}
inline void luai_userstateclose(lua_State *L)
{
	WiibloxExtraSpace::destroyRoot(L);
}
inline void luai_userstatethread(lua_State *L, lua_State *L1)
{
	WiibloxExtraSpace::constructChild(L1, WiibloxExtraSpace::get(L));
}
inline void luai_userstatefree(lua_State *L)
{
	WiibloxExtraSpace::destroyChild(L);
}
inline void luai_userstateresume(lua_State *L, int nargs)
{
	WiibloxExtraSpace::get(L)->yieldCaptured = false;
}
...
{
    wbx::Tokens::apiToken.addFlagFast(wbx::kLuaHooked);
    wbx::pmcHash.nonce = 0;
...
#define lua_chk_ptr_rblx(ptr, handler, L) (wbx::isWBXTextAddr(ptr) ? ((void)0) : handler(L))
